...
name|'error'
op|'('
string|'"Command \'%(cmdline)s\' with process id \'%(pid)s\' expected "'
nl|'\n'
string|'"return code in \'%(ok)s\' but got \'%(rc)s\': %(err)s"'
op|'%'
nl|'\n'
op|'{'
string|"'cmdline'"
...
nl|'\n'
nl|'\n'
DECL|function|try_kill_process
dedent|''
name|'def'
name|'try_kill_process'
op|'('
name|'proc'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Sends the given process the SIGKILL signal."""'
newline|'\n'
name|'pid'
op|'='
name|'proc'
...
newline|'\n'
nl|'\n'
comment|'# Rename any format of name to 0.vhd when there is only single one'
nl|'\n'
dedent|''
dedent|''
name|'contents'
...
op|'('
nl|'\n'
string|'"VHD %s is marked as hidden without child"'
op|'%'
name|'path'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|_vhd_util_check
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'_vhd_util_check'
op|'('
name|'vdi_path'
op|')'
op|':'
...
name|'warning'
op|'('
string|'"VHD invalid, attempting repair."'
op|')'
newline|'\n'
name|'repair_cmd'
op|'='
...
name|'Exception'
op|'('
string|'"VDI \'%s\' not present which breaks"'
nl|'\n'
...
name|'Exception'
op|'('
string|'"Unexpected output \'%s\' from vhd-util"'
op|'%'
name|'out'
op|')'
newline|'\n'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(tpownall): If we do not poll for the tar process exit'
nl|'\n'
comment|'# code when tar has exited pre maturely there is the chance'
nl|'\n'
comment|'# that tar will become a defunct zombie child under glance plugin'
nl|'\n'
comment|'# and re parented under init forever waiting on the stdin pipe to'
nl|'\n'
comment|'# close.  Polling for the exit code allows us to break the pipe.'
nl|'\n'
name|'returncode'
op|'='
name|'tar_proc'
...
name|'error'
op|'('
string|'"tar extract with process id \'%(pid)s\' "'
nl|'\n'
string|'"exited early with \'%(rc)s\'"'
op|'%'
nl|'\n'
op|'{'
string|"'pid'"
...
op|':'
newline|'\n'
comment|'# no need to kill already dead process'
nl|'\n'
indent|'        '
name|'raise'
newline|'\n'
...
name|'exception'
op|'('
string|'"Failed while sending data to tar pid: %s"'
op|'%'
name|'tar_pid'
op|')'
newline|'\n'
