begin_unit
comment|'#    Copyright 2010 OpenStack Foundation'
nl|'\n'
comment|'#    Copyright 2012 University Of Minho'
nl|'\n'
comment|'#    Copyright 2014-2015 Red Hat, Inc'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
name|'import'
name|'sys'
newline|'\n'
nl|'\n'
name|'import'
name|'mock'
newline|'\n'
name|'from'
name|'oslo_config'
name|'import'
name|'cfg'
newline|'\n'
name|'from'
name|'oslo_utils'
name|'import'
name|'encodeutils'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
name|'import'
name|'context'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'test'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
op|'.'
name|'virt'
op|'.'
name|'libvirt'
name|'import'
name|'fakelibvirt'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'utils'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'virt'
op|'.'
name|'libvirt'
name|'import'
name|'config'
name|'as'
name|'vconfig'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'virt'
op|'.'
name|'libvirt'
name|'import'
name|'guest'
name|'as'
name|'libvirt_guest'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'virt'
op|'.'
name|'libvirt'
name|'import'
name|'host'
newline|'\n'
nl|'\n'
nl|'\n'
name|'host'
op|'.'
name|'libvirt'
op|'='
name|'fakelibvirt'
newline|'\n'
name|'libvirt_guest'
op|'.'
name|'libvirt'
op|'='
name|'fakelibvirt'
newline|'\n'
nl|'\n'
DECL|variable|CONF
name|'CONF'
op|'='
name|'cfg'
op|'.'
name|'CONF'
newline|'\n'
nl|'\n'
name|'if'
name|'sys'
op|'.'
name|'version_info'
op|'>'
op|'('
number|'3'
op|','
op|')'
op|':'
newline|'\n'
DECL|variable|long
indent|'    '
name|'long'
op|'='
name|'int'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|GuestTestCase
dedent|''
name|'class'
name|'GuestTestCase'
op|'('
name|'test'
op|'.'
name|'NoDBTestCase'
op|')'
op|':'
newline|'\n'
nl|'\n'
DECL|member|setUp
indent|'    '
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'super'
op|'('
name|'GuestTestCase'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'useFixture'
op|'('
name|'fakelibvirt'
op|'.'
name|'FakeLibvirtFixture'
op|'('
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'host'
op|'='
name|'host'
op|'.'
name|'Host'
op|'('
string|'"qemu:///system"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'context'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'fakelibvirt'
op|'.'
name|'virDomain'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'='
name|'libvirt_guest'
op|'.'
name|'Guest'
op|'('
name|'self'
op|'.'
name|'domain'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_repr
dedent|''
name|'def'
name|'test_repr'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'ID'
op|'.'
name|'return_value'
op|'='
number|'99'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'UUIDString'
op|'.'
name|'return_value'
op|'='
string|'"UUID"'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'name'
op|'.'
name|'return_value'
op|'='
string|'"foo"'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
string|'"<Guest 99 foo UUID>"'
op|','
name|'repr'
op|'('
name|'self'
op|'.'
name|'guest'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'fakelibvirt'
op|'.'
name|'Connection'
op|','
string|"'defineXML'"
op|')'
newline|'\n'
DECL|member|test_create
name|'def'
name|'test_create'
op|'('
name|'self'
op|','
name|'mock_define'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'libvirt_guest'
op|'.'
name|'Guest'
op|'.'
name|'create'
op|'('
string|'"xml"'
op|','
name|'self'
op|'.'
name|'host'
op|')'
newline|'\n'
name|'mock_define'
op|'.'
name|'assert_called_once_with'
op|'('
string|'"xml"'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'fakelibvirt'
op|'.'
name|'Connection'
op|','
string|"'defineXML'"
op|')'
newline|'\n'
DECL|member|test_create_exception
name|'def'
name|'test_create_exception'
op|'('
name|'self'
op|','
name|'mock_define'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'mock_define'
op|'.'
name|'side_effect'
op|'='
name|'test'
op|'.'
name|'TestingException'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'test'
op|'.'
name|'TestingException'
op|','
nl|'\n'
name|'libvirt_guest'
op|'.'
name|'Guest'
op|'.'
name|'create'
op|','
nl|'\n'
string|'"foo"'
op|','
name|'self'
op|'.'
name|'host'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_launch
dedent|''
name|'def'
name|'test_launch'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'launch'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'createWithFlags'
op|'.'
name|'assert_called_once_with'
op|'('
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_launch_and_pause
dedent|''
name|'def'
name|'test_launch_and_pause'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'launch'
op|'('
name|'pause'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'createWithFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_START_PAUSED'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_shutdown
dedent|''
name|'def'
name|'test_shutdown'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'shutdown'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'shutdown'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'shutdown'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'encodeutils'
op|','
string|"'safe_decode'"
op|')'
newline|'\n'
DECL|member|test_launch_exception
name|'def'
name|'test_launch_exception'
op|'('
name|'self'
op|','
name|'mock_safe_decode'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'createWithFlags'
op|'.'
name|'side_effect'
op|'='
name|'test'
op|'.'
name|'TestingException'
newline|'\n'
name|'mock_safe_decode'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'test'
op|'.'
name|'TestingException'
op|','
name|'self'
op|'.'
name|'guest'
op|'.'
name|'launch'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'mock_safe_decode'
op|'.'
name|'called'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'utils'
op|','
string|"'execute'"
op|')'
newline|'\n'
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'libvirt_guest'
op|'.'
name|'Guest'
op|','
string|"'get_interfaces'"
op|')'
newline|'\n'
DECL|member|test_enable_hairpin
name|'def'
name|'test_enable_hairpin'
op|'('
name|'self'
op|','
name|'mock_get_interfaces'
op|','
name|'mock_execute'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'mock_get_interfaces'
op|'.'
name|'return_value'
op|'='
op|'['
string|'"vnet0"'
op|','
string|'"vnet1"'
op|']'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'enable_hairpin'
op|'('
op|')'
newline|'\n'
name|'mock_execute'
op|'.'
name|'assert_has_calls'
op|'('
op|'['
nl|'\n'
name|'mock'
op|'.'
name|'call'
op|'('
nl|'\n'
string|"'tee'"
op|','
string|"'/sys/class/net/vnet0/brport/hairpin_mode'"
op|','
nl|'\n'
name|'run_as_root'
op|'='
name|'True'
op|','
name|'process_input'
op|'='
string|"'1'"
op|','
name|'check_exit_code'
op|'='
op|'['
number|'0'
op|','
number|'1'
op|']'
op|')'
op|','
nl|'\n'
name|'mock'
op|'.'
name|'call'
op|'('
nl|'\n'
string|"'tee'"
op|','
string|"'/sys/class/net/vnet1/brport/hairpin_mode'"
op|','
nl|'\n'
name|'run_as_root'
op|'='
name|'True'
op|','
name|'process_input'
op|'='
string|"'1'"
op|','
name|'check_exit_code'
op|'='
op|'['
number|'0'
op|','
number|'1'
op|']'
op|')'
op|']'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'encodeutils'
op|','
string|"'safe_decode'"
op|')'
newline|'\n'
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'utils'
op|','
string|"'execute'"
op|')'
newline|'\n'
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'libvirt_guest'
op|'.'
name|'Guest'
op|','
string|"'get_interfaces'"
op|')'
newline|'\n'
DECL|member|test_enable_hairpin_exception
name|'def'
name|'test_enable_hairpin_exception'
op|'('
name|'self'
op|','
name|'mock_get_interfaces'
op|','
nl|'\n'
name|'mock_execute'
op|','
name|'mock_safe_decode'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'mock_get_interfaces'
op|'.'
name|'return_value'
op|'='
op|'['
string|'"foo"'
op|']'
newline|'\n'
name|'mock_execute'
op|'.'
name|'side_effect'
op|'='
name|'test'
op|'.'
name|'TestingException'
op|'('
string|"'oops'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'test'
op|'.'
name|'TestingException'
op|','
name|'self'
op|'.'
name|'guest'
op|'.'
name|'enable_hairpin'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'mock_safe_decode'
op|'.'
name|'called'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_interfaces
dedent|''
name|'def'
name|'test_get_interfaces'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'XMLDesc'
op|'.'
name|'return_value'
op|'='
string|'"""<domain>\n  <devices>\n    <interface type="network">\n      <target dev="vnet0"/>\n    </interface>\n    <interface type="network">\n      <target dev="vnet1"/>\n    </interface>\n  </devices>\n</domain>"""'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
op|'['
string|'"vnet0"'
op|','
string|'"vnet1"'
op|']'
op|','
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_interfaces'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_interfaces_exception
dedent|''
name|'def'
name|'test_get_interfaces_exception'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'XMLDesc'
op|'.'
name|'return_value'
op|'='
string|'"<bad xml>"'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
op|'['
op|']'
op|','
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_interfaces'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_poweroff
dedent|''
name|'def'
name|'test_poweroff'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'poweroff'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'destroy'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_resume
dedent|''
name|'def'
name|'test_resume'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'resume'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'resume'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_vcpus_info
dedent|''
name|'def'
name|'test_get_vcpus_info'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'vcpus'
op|'.'
name|'return_value'
op|'='
op|'('
op|'['
op|'('
number|'0'
op|','
number|'1'
op|','
name|'int'
op|'('
number|'10290000000'
op|')'
op|','
number|'2'
op|')'
op|']'
op|','
nl|'\n'
op|'['
op|'('
name|'True'
op|','
name|'True'
op|')'
op|']'
op|')'
newline|'\n'
name|'vcpus'
op|'='
name|'list'
op|'('
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_vcpus_info'
op|'('
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'0'
op|','
name|'vcpus'
op|'['
number|'0'
op|']'
op|'.'
name|'id'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'2'
op|','
name|'vcpus'
op|'['
number|'0'
op|']'
op|'.'
name|'cpu'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'vcpus'
op|'['
number|'0'
op|']'
op|'.'
name|'state'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'int'
op|'('
number|'10290000000'
op|')'
op|','
name|'vcpus'
op|'['
number|'0'
op|']'
op|'.'
name|'time'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_delete_configuration
dedent|''
name|'def'
name|'test_delete_configuration'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'delete_configuration'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'undefineFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_UNDEFINE_MANAGED_SAVE'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_delete_configuration_exception
dedent|''
name|'def'
name|'test_delete_configuration_exception'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'undefineFlags'
op|'.'
name|'side_effect'
op|'='
name|'fakelibvirt'
op|'.'
name|'libvirtError'
op|'('
nl|'\n'
string|"'oops'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'ID'
op|'.'
name|'return_value'
op|'='
number|'1'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'delete_configuration'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'undefine'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_attach_device
dedent|''
name|'def'
name|'test_attach_device'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'attach_device'
op|'('
name|'conf'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'attachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_attach_device_persistent
dedent|''
name|'def'
name|'test_attach_device_persistent'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'attach_device'
op|'('
name|'conf'
op|','
name|'persistent'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'attachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_CONFIG'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_attach_device_live
dedent|''
name|'def'
name|'test_attach_device_live'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'attach_device'
op|'('
name|'conf'
op|','
name|'live'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'attachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_LIVE'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_attach_device_persistent_live
dedent|''
name|'def'
name|'test_attach_device_persistent_live'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'attach_device'
op|'('
name|'conf'
op|','
name|'persistent'
op|'='
name|'True'
op|','
name|'live'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'attachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
op|'('
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_CONFIG'
op|'|'
nl|'\n'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_LIVE'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_detach_device
dedent|''
name|'def'
name|'test_detach_device'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'detach_device'
op|'('
name|'conf'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_detach_device_persistent
dedent|''
name|'def'
name|'test_detach_device_persistent'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'detach_device'
op|'('
name|'conf'
op|','
name|'persistent'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_CONFIG'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_detach_device_live
dedent|''
name|'def'
name|'test_detach_device_live'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'detach_device'
op|'('
name|'conf'
op|','
name|'live'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_LIVE'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_detach_device_persistent_live
dedent|''
name|'def'
name|'test_detach_device_persistent_live'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'detach_device'
op|'('
name|'conf'
op|','
name|'persistent'
op|'='
name|'True'
op|','
name|'live'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
op|'('
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_CONFIG'
op|'|'
nl|'\n'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_LIVE'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_detach_device_with_retry_detach_success
dedent|''
name|'def'
name|'test_detach_device_with_retry_detach_success'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
name|'get_config'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
op|')'
newline|'\n'
comment|'# Force multiple retries of detach'
nl|'\n'
name|'get_config'
op|'.'
name|'side_effect'
op|'='
op|'['
name|'conf'
op|','
name|'conf'
op|','
name|'conf'
op|','
name|'None'
op|']'
newline|'\n'
name|'dev_path'
op|'='
string|'"/dev/vdb"'
newline|'\n'
nl|'\n'
name|'retry_detach'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'detach_device_with_retry'
op|'('
nl|'\n'
name|'get_config'
op|','
name|'dev_path'
op|','
name|'persistent'
op|'='
name|'True'
op|','
name|'live'
op|'='
name|'True'
op|','
nl|'\n'
name|'inc_sleep_time'
op|'='
number|'.01'
op|')'
newline|'\n'
comment|"# Ensure we've only done the initial detach call"
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
op|'('
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_CONFIG'
op|'|'
nl|'\n'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_LIVE'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'get_config'
op|'.'
name|'assert_called_with'
op|'('
name|'dev_path'
op|')'
newline|'\n'
nl|'\n'
comment|'# Some time later, we can do the wait/retry to ensure detach succeeds'
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'reset_mock'
op|'('
op|')'
newline|'\n'
name|'retry_detach'
op|'('
op|')'
newline|'\n'
comment|'# Should have two retries before we pretend device is detached'
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'2'
op|','
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'call_count'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_detach_device_with_retry_detach_failure
dedent|''
name|'def'
name|'test_detach_device_with_retry_detach_failure'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDevice'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|'"</xml>"'
newline|'\n'
comment|'# Continue to return some value for the disk config'
nl|'\n'
name|'get_config'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'return_value'
op|'='
name|'conf'
op|')'
newline|'\n'
nl|'\n'
name|'retry_detach'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'detach_device_with_retry'
op|'('
nl|'\n'
name|'get_config'
op|','
string|'"/dev/vdb"'
op|','
name|'persistent'
op|'='
name|'True'
op|','
name|'live'
op|'='
name|'True'
op|','
nl|'\n'
name|'inc_sleep_time'
op|'='
number|'.01'
op|','
name|'max_retry_count'
op|'='
number|'3'
op|')'
newline|'\n'
comment|"# Ensure we've only done the initial detach call"
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|'"</xml>"'
op|','
name|'flags'
op|'='
op|'('
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_CONFIG'
op|'|'
nl|'\n'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_AFFECT_LIVE'
op|')'
op|')'
newline|'\n'
nl|'\n'
comment|'# Some time later, we can do the wait/retry to ensure detach'
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'reset_mock'
op|'('
op|')'
newline|'\n'
comment|'# Should hit max # of retries'
nl|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'DeviceDetachFailed'
op|','
name|'retry_detach'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'4'
op|','
name|'self'
op|'.'
name|'domain'
op|'.'
name|'detachDeviceFlags'
op|'.'
name|'call_count'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_detach_device_with_retry_device_not_found
dedent|''
name|'def'
name|'test_detach_device_with_retry_device_not_found'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'get_config'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'return_value'
op|'='
name|'None'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
nl|'\n'
name|'exception'
op|'.'
name|'DeviceNotFound'
op|','
name|'self'
op|'.'
name|'guest'
op|'.'
name|'detach_device_with_retry'
op|','
nl|'\n'
name|'get_config'
op|','
string|'"/dev/vdb"'
op|','
name|'persistent'
op|'='
name|'True'
op|','
name|'live'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_xml_desc
dedent|''
name|'def'
name|'test_get_xml_desc'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_xml_desc'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'XMLDesc'
op|'.'
name|'assert_called_once_with'
op|'('
name|'flags'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_xml_desc_dump_inactive
dedent|''
name|'def'
name|'test_get_xml_desc_dump_inactive'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_xml_desc'
op|'('
name|'dump_inactive'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'XMLDesc'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_XML_INACTIVE'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_xml_desc_dump_sensitive
dedent|''
name|'def'
name|'test_get_xml_desc_dump_sensitive'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_xml_desc'
op|'('
name|'dump_sensitive'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'XMLDesc'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_XML_SECURE'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_xml_desc_dump_inactive_dump_sensitive
dedent|''
name|'def'
name|'test_get_xml_desc_dump_inactive_dump_sensitive'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_xml_desc'
op|'('
name|'dump_inactive'
op|'='
name|'True'
op|','
name|'dump_sensitive'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'XMLDesc'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'flags'
op|'='
op|'('
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_XML_INACTIVE'
op|'|'
nl|'\n'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_XML_SECURE'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_xml_desc_dump_migratable
dedent|''
name|'def'
name|'test_get_xml_desc_dump_migratable'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_xml_desc'
op|'('
name|'dump_migratable'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'XMLDesc'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_XML_MIGRATABLE'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_has_persistent_configuration
dedent|''
name|'def'
name|'test_has_persistent_configuration'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'assertTrue'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'has_persistent_configuration'
op|'('
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'isPersistent'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_save_memory_state
dedent|''
name|'def'
name|'test_save_memory_state'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'save_memory_state'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'managedSave'
op|'.'
name|'assert_called_once_with'
op|'('
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_block_device
dedent|''
name|'def'
name|'test_get_block_device'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'disk'
op|'='
string|"'vda'"
newline|'\n'
name|'gblock'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_block_device'
op|'('
name|'disk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'disk'
op|','
name|'gblock'
op|'.'
name|'_disk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'guest'
op|','
name|'gblock'
op|'.'
name|'_guest'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_set_user_password
dedent|''
name|'def'
name|'test_set_user_password'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'set_user_password'
op|'('
string|'"foo"'
op|','
string|'"123"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'setUserPassword'
op|'.'
name|'assert_called_once_with'
op|'('
string|'"foo"'
op|','
string|'"123"'
op|','
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_devices
dedent|''
name|'def'
name|'test_get_devices'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'xml'
op|'='
string|'"""\n<domain type=\'qemu\'>\n  <name>QEMUGuest1</name>\n  <uuid>c7a5fdbd-edaf-9455-926a-d65c16db1809</uuid>\n  <memory unit=\'KiB\'>219136</memory>\n  <currentMemory unit=\'KiB\'>219136</currentMemory>\n  <vcpu placement=\'static\'>1</vcpu>\n  <os>\n    <type arch=\'i686\' machine=\'pc\'>hvm</type>\n    <boot dev=\'hd\'/>\n  </os>\n  <clock offset=\'utc\'/>\n  <on_poweroff>destroy</on_poweroff>\n  <on_reboot>restart</on_reboot>\n  <on_crash>destroy</on_crash>\n  <devices>\n    <emulator>/usr/bin/qemu</emulator>\n    <disk type=\'block\' device=\'disk\'>\n      <driver name=\'qemu\' type=\'raw\'/>\n      <source dev=\'/dev/HostVG/QEMUGuest2\'/>\n      <target dev=\'hda\' bus=\'ide\'/>\n      <address type=\'drive\' controller=\'0\' bus=\'0\' target=\'0\' unit=\'0\'/>\n    </disk>\n    <disk type=\'network\' device=\'disk\'>\n      <driver name=\'qemu\' type=\'raw\'/>\n      <auth username=\'myname\'>\n        <secret type=\'iscsi\' usage=\'mycluster_myname\'/>\n      </auth>\n      <source protocol=\'iscsi\' name=\'iqn.1992-01.com.example\'>\n        <host name=\'example.org\' port=\'6000\'/>\n      </source>\n      <target dev=\'vda\' bus=\'virtio\'/>\n    </disk>\n    <disk type=\'network\' device=\'disk\'>\n      <driver name=\'qemu\' type=\'raw\'/>\n      <source protocol=\'iscsi\' name=\'iqn.1992-01.com.example/1\'>\n        <host name=\'example.org\' port=\'6000\'/>\n      </source>\n      <target dev=\'vdb\' bus=\'virtio\'/>\n    </disk>\n    <hostdev mode=\'subsystem\' type=\'pci\' managed=\'yes\'>\n      <source>\n        <address domain=\'0x0000\' bus=\'0x06\' slot=\'0x12\' function=\'0x5\'/>\n      </source>\n    </hostdev>\n    <hostdev mode=\'subsystem\' type=\'pci\' managed=\'yes\'>\n      <source>\n        <address domain=\'0x0000\' bus=\'0x06\' slot=\'0x12\' function=\'0x6\'/>\n      </source>\n    </hostdev>\n    <interface type="bridge">\n      <mac address="fa:16:3e:f9:af:ae"/>\n      <model type="virtio"/>\n      <driver name="qemu"/>\n      <source bridge="qbr84008d03-11"/>\n      <target dev="tap84008d03-11"/>\n    </interface>\n    <controller type=\'usb\' index=\'0\'/>\n    <controller type=\'pci\' index=\'0\' model=\'pci-root\'/>\n    <memballoon model=\'none\'/>\n  </devices>\n</domain>\n"""'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'XMLDesc'
op|'.'
name|'return_value'
op|'='
name|'xml'
newline|'\n'
nl|'\n'
name|'devs'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_all_devices'
op|'('
op|')'
newline|'\n'
comment|'# Only currently parse <disk>, <hostdev> and <interface> elements'
nl|'\n'
comment|"# hence we're not counting the controller/memballoon"
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'6'
op|','
name|'len'
op|'('
name|'devs'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'0'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'1'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'2'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'3'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestHostdev'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'4'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestHostdev'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'5'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestInterface'
op|')'
newline|'\n'
nl|'\n'
name|'devs'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_all_devices'
op|'('
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'3'
op|','
name|'len'
op|'('
name|'devs'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'0'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'1'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'2'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
nl|'\n'
name|'devs'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_all_disks'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'3'
op|','
name|'len'
op|'('
name|'devs'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'0'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'1'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'2'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestDisk'
op|')'
newline|'\n'
nl|'\n'
name|'devs'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_all_devices'
op|'('
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestHostdev'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'2'
op|','
name|'len'
op|'('
name|'devs'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'0'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestHostdev'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'1'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestHostdev'
op|')'
newline|'\n'
nl|'\n'
name|'devs'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_all_devices'
op|'('
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestInterface'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'len'
op|'('
name|'devs'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'devs'
op|'['
number|'0'
op|']'
op|','
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestInterface'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertIsNotNone'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_interface_by_mac'
op|'('
string|"'fa:16:3e:f9:af:ae'"
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsNone'
op|'('
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_interface_by_mac'
op|'('
name|'None'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_info
dedent|''
name|'def'
name|'test_get_info'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'info'
op|'.'
name|'return_value'
op|'='
op|'('
number|'1'
op|','
number|'2'
op|','
number|'3'
op|','
number|'4'
op|','
number|'5'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'ID'
op|'.'
name|'return_value'
op|'='
number|'6'
newline|'\n'
name|'info'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_info'
op|'('
name|'self'
op|'.'
name|'host'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'info'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'info'
op|'.'
name|'state'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'2'
op|','
name|'info'
op|'.'
name|'max_mem_kb'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'3'
op|','
name|'info'
op|'.'
name|'mem_kb'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'4'
op|','
name|'info'
op|'.'
name|'num_cpu'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'5'
op|','
name|'info'
op|'.'
name|'cpu_time_ns'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'6'
op|','
name|'info'
op|'.'
name|'id'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_get_power_state
dedent|''
name|'def'
name|'test_get_power_state'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'info'
op|'.'
name|'return_value'
op|'='
op|'('
number|'1'
op|','
number|'2'
op|','
number|'3'
op|','
number|'4'
op|','
number|'5'
op|')'
newline|'\n'
name|'power'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_power_state'
op|'('
name|'self'
op|'.'
name|'host'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'power'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_is_active_when_domain_is_active
dedent|''
name|'def'
name|'test_is_active_when_domain_is_active'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'with'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'self'
op|'.'
name|'domain'
op|','
string|'"isActive"'
op|','
name|'return_value'
op|'='
name|'True'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'self'
op|'.'
name|'guest'
op|'.'
name|'is_active'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_is_active_when_domain_not_active
dedent|''
dedent|''
name|'def'
name|'test_is_active_when_domain_not_active'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'with'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'self'
op|'.'
name|'domain'
op|','
string|'"isActive"'
op|','
name|'return_value'
op|'='
name|'False'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'self'
op|'.'
name|'guest'
op|'.'
name|'is_active'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_freeze_filesystems
dedent|''
dedent|''
name|'def'
name|'test_freeze_filesystems'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'freeze_filesystems'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'fsFreeze'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_thaw_filesystems
dedent|''
name|'def'
name|'test_thaw_filesystems'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'thaw_filesystems'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'fsThaw'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|_conf_snapshot
dedent|''
name|'def'
name|'_conf_snapshot'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'vconfig'
op|'.'
name|'LibvirtConfigGuestSnapshotDisk'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'return_value'
op|'='
string|"'<disk/>'"
newline|'\n'
name|'return'
name|'conf'
newline|'\n'
nl|'\n'
DECL|function|test_snapshot
dedent|''
name|'def'
name|'test_snapshot'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'self'
op|'.'
name|'_conf_snapshot'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'snapshot'
op|'('
name|'conf'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'snapshotCreateXML'
op|'('
string|"'<disk/>'"
op|','
name|'flags'
op|'='
number|'0'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_snapshot_no_metadata
dedent|''
name|'def'
name|'test_snapshot_no_metadata'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'self'
op|'.'
name|'_conf_snapshot'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'snapshot'
op|'('
name|'conf'
op|','
name|'no_metadata'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'snapshotCreateXML'
op|'('
nl|'\n'
string|"'<disk/>'"
op|','
nl|'\n'
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_SNAPSHOT_CREATE_NO_METADATA'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_snapshot_disk_only
dedent|''
name|'def'
name|'test_snapshot_disk_only'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'self'
op|'.'
name|'_conf_snapshot'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'snapshot'
op|'('
name|'conf'
op|','
name|'disk_only'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'snapshotCreateXML'
op|'('
nl|'\n'
string|"'<disk/>'"
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_SNAPSHOT_CREATE_DISK_ONLY'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_snapshot_reuse_ext
dedent|''
name|'def'
name|'test_snapshot_reuse_ext'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'self'
op|'.'
name|'_conf_snapshot'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'snapshot'
op|'('
name|'conf'
op|','
name|'reuse_ext'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'snapshotCreateXML'
op|'('
nl|'\n'
string|"'<disk/>'"
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_SNAPSHOT_CREATE_REUSE_EXT'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_snapshot_quiesce
dedent|''
name|'def'
name|'test_snapshot_quiesce'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'self'
op|'.'
name|'_conf_snapshot'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'snapshot'
op|'('
name|'conf'
op|','
name|'quiesce'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'snapshotCreateXML'
op|'('
nl|'\n'
string|"'<disk/>'"
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_SNAPSHOT_CREATE_QUIESCE'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_snapshot_all
dedent|''
name|'def'
name|'test_snapshot_all'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'conf'
op|'='
name|'self'
op|'.'
name|'_conf_snapshot'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'.'
name|'snapshot'
op|'('
name|'conf'
op|','
name|'no_metadata'
op|'='
name|'True'
op|','
nl|'\n'
name|'disk_only'
op|'='
name|'True'
op|','
name|'reuse_ext'
op|'='
name|'True'
op|','
nl|'\n'
name|'quiesce'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'snapshotCreateXML'
op|'('
nl|'\n'
string|"'<disk/>'"
op|','
name|'flags'
op|'='
op|'('
nl|'\n'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_SNAPSHOT_CREATE_REUSE_EXT'
nl|'\n'
op|'|'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_SNAPSHOT_CREATE_DISK_ONLY'
nl|'\n'
op|'|'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_SNAPSHOT_CREATE_NO_METADATA'
nl|'\n'
op|'|'
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_SNAPSHOT_CREATE_QUIESCE'
op|')'
op|')'
newline|'\n'
name|'conf'
op|'.'
name|'to_xml'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|function|test_pause
dedent|''
name|'def'
name|'test_pause'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'guest'
op|'.'
name|'pause'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'suspend'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|GuestBlockTestCase
dedent|''
dedent|''
name|'class'
name|'GuestBlockTestCase'
op|'('
name|'test'
op|'.'
name|'NoDBTestCase'
op|')'
op|':'
newline|'\n'
nl|'\n'
DECL|member|setUp
indent|'    '
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'super'
op|'('
name|'GuestBlockTestCase'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'useFixture'
op|'('
name|'fakelibvirt'
op|'.'
name|'FakeLibvirtFixture'
op|'('
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'host'
op|'='
name|'host'
op|'.'
name|'Host'
op|'('
string|'"qemu:///system"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'context'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
name|'spec'
op|'='
name|'fakelibvirt'
op|'.'
name|'virDomain'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'guest'
op|'='
name|'libvirt_guest'
op|'.'
name|'Guest'
op|'('
name|'self'
op|'.'
name|'domain'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'gblock'
op|'='
name|'self'
op|'.'
name|'guest'
op|'.'
name|'get_block_device'
op|'('
string|"'vda'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_abort_job
dedent|''
name|'def'
name|'test_abort_job'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'abort_job'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobAbort'
op|'.'
name|'assert_called_once_with'
op|'('
string|"'vda'"
op|','
name|'flags'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_abort_job_async
dedent|''
name|'def'
name|'test_abort_job_async'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'abort_job'
op|'('
name|'async'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobAbort'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_BLOCK_JOB_ABORT_ASYNC'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_abort_job_pivot
dedent|''
name|'def'
name|'test_abort_job_pivot'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'abort_job'
op|'('
name|'pivot'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobAbort'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_BLOCK_JOB_ABORT_PIVOT'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_job_info
dedent|''
name|'def'
name|'test_get_job_info'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobInfo'
op|'.'
name|'return_value'
op|'='
op|'{'
nl|'\n'
string|'"type"'
op|':'
number|'1'
op|','
nl|'\n'
string|'"bandwidth"'
op|':'
number|'18'
op|','
nl|'\n'
string|'"cur"'
op|':'
number|'66'
op|','
nl|'\n'
string|'"end"'
op|':'
number|'100'
op|'}'
newline|'\n'
nl|'\n'
name|'info'
op|'='
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'get_job_info'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'info'
op|'.'
name|'job'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'18'
op|','
name|'info'
op|'.'
name|'bandwidth'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'66'
op|','
name|'info'
op|'.'
name|'cur'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'100'
op|','
name|'info'
op|'.'
name|'end'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobInfo'
op|'.'
name|'assert_called_once_with'
op|'('
string|"'vda'"
op|','
name|'flags'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_resize
dedent|''
name|'def'
name|'test_resize'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'resize'
op|'('
number|'10'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockResize'
op|'.'
name|'assert_called_once_with'
op|'('
string|"'vda'"
op|','
number|'10'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_rebase
dedent|''
name|'def'
name|'test_rebase'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'rebase'
op|'('
string|'"foo"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockRebase'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
string|'"foo"'
op|','
number|'0'
op|','
name|'flags'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_rebase_shallow
dedent|''
name|'def'
name|'test_rebase_shallow'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'rebase'
op|'('
string|'"foo"'
op|','
name|'shallow'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockRebase'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
string|'"foo"'
op|','
number|'0'
op|','
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_BLOCK_REBASE_SHALLOW'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_rebase_reuse_ext
dedent|''
name|'def'
name|'test_rebase_reuse_ext'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'rebase'
op|'('
string|'"foo"'
op|','
name|'reuse_ext'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockRebase'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
string|'"foo"'
op|','
number|'0'
op|','
nl|'\n'
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_BLOCK_REBASE_REUSE_EXT'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_rebase_copy
dedent|''
name|'def'
name|'test_rebase_copy'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'rebase'
op|'('
string|'"foo"'
op|','
name|'copy'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockRebase'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
string|'"foo"'
op|','
number|'0'
op|','
nl|'\n'
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_BLOCK_REBASE_COPY'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_rebase_relative
dedent|''
name|'def'
name|'test_rebase_relative'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'rebase'
op|'('
string|'"foo"'
op|','
name|'relative'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockRebase'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
string|'"foo"'
op|','
number|'0'
op|','
nl|'\n'
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_BLOCK_REBASE_RELATIVE'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_commit
dedent|''
name|'def'
name|'test_commit'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'commit'
op|'('
string|'"foo"'
op|','
string|'"top"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockCommit'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
string|'"foo"'
op|','
string|'"top"'
op|','
number|'0'
op|','
name|'flags'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_commit_relative
dedent|''
name|'def'
name|'test_commit_relative'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'commit'
op|'('
string|'"foo"'
op|','
string|'"top"'
op|','
name|'relative'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockCommit'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
string|"'vda'"
op|','
string|'"foo"'
op|','
string|'"top"'
op|','
number|'0'
op|','
nl|'\n'
name|'flags'
op|'='
name|'fakelibvirt'
op|'.'
name|'VIR_DOMAIN_BLOCK_COMMIT_RELATIVE'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_wait_for_job
dedent|''
name|'def'
name|'test_wait_for_job'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobInfo'
op|'.'
name|'return_value'
op|'='
op|'{'
nl|'\n'
string|'"type"'
op|':'
number|'4'
op|','
nl|'\n'
string|'"bandwidth"'
op|':'
number|'18'
op|','
nl|'\n'
string|'"cur"'
op|':'
number|'95'
op|','
nl|'\n'
string|'"end"'
op|':'
number|'100'
op|'}'
newline|'\n'
name|'in_progress'
op|'='
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'wait_for_job'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'in_progress'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobInfo'
op|'.'
name|'return_value'
op|'='
op|'{'
nl|'\n'
string|'"type"'
op|':'
number|'4'
op|','
nl|'\n'
string|'"bandwidth"'
op|':'
number|'18'
op|','
nl|'\n'
string|'"cur"'
op|':'
number|'100'
op|','
nl|'\n'
string|'"end"'
op|':'
number|'100'
op|'}'
newline|'\n'
name|'in_progress'
op|'='
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'wait_for_job'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'in_progress'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobInfo'
op|'.'
name|'return_value'
op|'='
op|'{'
string|'"type"'
op|':'
number|'0'
op|'}'
newline|'\n'
name|'in_progress'
op|'='
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'wait_for_job'
op|'('
name|'wait_for_job_clean'
op|'='
name|'True'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'in_progress'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_wait_for_job_arbort_on_error
dedent|''
name|'def'
name|'test_wait_for_job_arbort_on_error'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'domain'
op|'.'
name|'blockJobInfo'
op|'.'
name|'return_value'
op|'='
op|'-'
number|'1'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
nl|'\n'
name|'exception'
op|'.'
name|'NovaException'
op|','
nl|'\n'
name|'self'
op|'.'
name|'gblock'
op|'.'
name|'wait_for_job'
op|','
name|'abort_on_error'
op|'='
name|'True'
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
