begin_unit
comment|'#  Copyright 2013 Cloudbase Solutions Srl'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
name|'import'
name|'mock'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
op|'.'
name|'virt'
op|'.'
name|'hyperv'
name|'import'
name|'test_vhdutils'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'virt'
op|'.'
name|'hyperv'
name|'import'
name|'constants'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'virt'
op|'.'
name|'hyperv'
name|'import'
name|'vhdutilsv2'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'virt'
op|'.'
name|'hyperv'
name|'import'
name|'vmutils'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|VHDUtilsV2TestCase
name|'class'
name|'VHDUtilsV2TestCase'
op|'('
name|'test_vhdutils'
op|'.'
name|'VHDUtilsBaseTestCase'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Unit tests for the Hyper-V VHDUtilsV2 class."""'
newline|'\n'
nl|'\n'
DECL|variable|_FAKE_BLOCK_SIZE
name|'_FAKE_BLOCK_SIZE'
op|'='
number|'33554432L'
newline|'\n'
DECL|variable|_FAKE_LOG_SIZE
name|'_FAKE_LOG_SIZE'
op|'='
number|'1048576'
newline|'\n'
DECL|variable|_FAKE_LOGICAL_SECTOR_SIZE
name|'_FAKE_LOGICAL_SECTOR_SIZE'
op|'='
number|'4096'
newline|'\n'
DECL|variable|_FAKE_METADATA_SIZE
name|'_FAKE_METADATA_SIZE'
op|'='
number|'1048576'
newline|'\n'
DECL|variable|_FAKE_PHYSICAL_SECTOR_SIZE
name|'_FAKE_PHYSICAL_SECTOR_SIZE'
op|'='
number|'4096L'
newline|'\n'
nl|'\n'
DECL|member|setUp
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'super'
op|'('
name|'VHDUtilsV2TestCase'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'='
name|'vhdutilsv2'
op|'.'
name|'VHDUtilsV2'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_conn'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_vmutils'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_fake_file_handle'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_fake_vhd_info'
op|'='
op|'{'
nl|'\n'
string|"'Path'"
op|':'
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
nl|'\n'
string|"'ParentPath'"
op|':'
name|'self'
op|'.'
name|'_FAKE_PARENT_PATH'
op|','
nl|'\n'
string|"'Format'"
op|':'
name|'self'
op|'.'
name|'_FAKE_FORMAT'
op|','
nl|'\n'
string|"'MaxInternalSize'"
op|':'
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|','
nl|'\n'
string|"'Type'"
op|':'
name|'self'
op|'.'
name|'_FAKE_TYPE'
op|','
nl|'\n'
string|"'BlockSize'"
op|':'
name|'self'
op|'.'
name|'_FAKE_BLOCK_SIZE'
op|','
nl|'\n'
string|"'LogicalSectorSize'"
op|':'
name|'self'
op|'.'
name|'_FAKE_LOGICAL_SECTOR_SIZE'
op|','
nl|'\n'
string|"'PhysicalSectorSize'"
op|':'
name|'self'
op|'.'
name|'_FAKE_PHYSICAL_SECTOR_SIZE'
op|'}'
newline|'\n'
nl|'\n'
DECL|member|_mock_get_vhd_info
dedent|''
name|'def'
name|'_mock_get_vhd_info'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'mock_img_svc'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_conn'
op|'.'
name|'Msvm_ImageManagementService'
op|'('
op|')'
op|'['
number|'0'
op|']'
newline|'\n'
name|'mock_img_svc'
op|'.'
name|'GetVirtualHardDiskSettingData'
op|'.'
name|'return_value'
op|'='
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_JOB_PATH'
op|','
name|'self'
op|'.'
name|'_FAKE_RET_VAL'
op|','
name|'self'
op|'.'
name|'_FAKE_VHD_INFO_XML'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_vhd_info
dedent|''
name|'def'
name|'test_get_vhd_info'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_mock_get_vhd_info'
op|'('
op|')'
newline|'\n'
name|'vhd_info'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_vhd_info'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
name|'vhd_info'
op|'['
string|"'Path'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_PARENT_PATH'
op|','
name|'vhd_info'
op|'['
string|"'ParentPath'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_FORMAT'
op|','
name|'vhd_info'
op|'['
string|"'Format'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|','
nl|'\n'
name|'vhd_info'
op|'['
string|"'MaxInternalSize'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_TYPE'
op|','
name|'vhd_info'
op|'['
string|"'Type'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_vhd_info_no_parent
dedent|''
name|'def'
name|'test_get_vhd_info_no_parent'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_vhd_xml_no_parent'
op|'='
name|'self'
op|'.'
name|'_FAKE_VHD_INFO_XML'
op|'.'
name|'replace'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_PARENT_PATH'
op|','
string|'""'
op|')'
newline|'\n'
nl|'\n'
name|'mock_img_svc'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_conn'
op|'.'
name|'Msvm_ImageManagementService'
op|'('
op|')'
op|'['
number|'0'
op|']'
newline|'\n'
name|'mock_img_svc'
op|'.'
name|'GetVirtualHardDiskSettingData'
op|'.'
name|'return_value'
op|'='
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_JOB_PATH'
op|','
name|'self'
op|'.'
name|'_FAKE_RET_VAL'
op|','
name|'fake_vhd_xml_no_parent'
op|')'
newline|'\n'
nl|'\n'
name|'vhd_info'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_vhd_info'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
name|'vhd_info'
op|'['
string|"'Path'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsNone'
op|'('
name|'vhd_info'
op|'['
string|"'ParentPath'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_FORMAT'
op|','
name|'vhd_info'
op|'['
string|"'Format'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|','
nl|'\n'
name|'vhd_info'
op|'['
string|"'MaxInternalSize'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_TYPE'
op|','
name|'vhd_info'
op|'['
string|"'Type'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_create_dynamic_vhd
dedent|''
name|'def'
name|'test_create_dynamic_vhd'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_vhd_info'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
op|'{'
string|"'Format'"
op|':'
name|'self'
op|'.'
name|'_FAKE_FORMAT'
op|'}'
op|')'
newline|'\n'
nl|'\n'
name|'mock_img_svc'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_conn'
op|'.'
name|'Msvm_ImageManagementService'
op|'('
op|')'
op|'['
number|'0'
op|']'
newline|'\n'
name|'mock_img_svc'
op|'.'
name|'CreateVirtualHardDisk'
op|'.'
name|'return_value'
op|'='
op|'('
name|'self'
op|'.'
name|'_FAKE_JOB_PATH'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_RET_VAL'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'create_dynamic_vhd'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|','
nl|'\n'
name|'constants'
op|'.'
name|'DISK_FORMAT_VHDX'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'mock_img_svc'
op|'.'
name|'CreateVirtualHardDisk'
op|'.'
name|'called'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_create_differencing_vhd
dedent|''
name|'def'
name|'test_create_differencing_vhd'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_vhd_info'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
op|'{'
string|"'ParentPath'"
op|':'
name|'self'
op|'.'
name|'_FAKE_PARENT_PATH'
op|','
nl|'\n'
string|"'Format'"
op|':'
name|'self'
op|'.'
name|'_FAKE_FORMAT'
op|'}'
op|')'
newline|'\n'
nl|'\n'
name|'mock_img_svc'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_conn'
op|'.'
name|'Msvm_ImageManagementService'
op|'('
op|')'
op|'['
number|'0'
op|']'
newline|'\n'
name|'mock_img_svc'
op|'.'
name|'CreateVirtualHardDisk'
op|'.'
name|'return_value'
op|'='
op|'('
name|'self'
op|'.'
name|'_FAKE_JOB_PATH'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_RET_VAL'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'create_differencing_vhd'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_PARENT_PATH'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'mock_img_svc'
op|'.'
name|'CreateVirtualHardDisk'
op|'.'
name|'called'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_reconnect_parent_vhd
dedent|''
name|'def'
name|'test_reconnect_parent_vhd'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'mock_img_svc'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_conn'
op|'.'
name|'Msvm_ImageManagementService'
op|'('
op|')'
op|'['
number|'0'
op|']'
newline|'\n'
name|'fake_new_parent_path'
op|'='
string|"'fake_new_parent_path'"
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhd_info_xml'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'self'
op|'.'
name|'_FAKE_VHD_INFO_XML'
op|')'
newline|'\n'
nl|'\n'
name|'mock_img_svc'
op|'.'
name|'SetVirtualHardDiskSettingData'
op|'.'
name|'return_value'
op|'='
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_JOB_PATH'
op|','
name|'self'
op|'.'
name|'_FAKE_RET_VAL'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'reconnect_parent_vhd'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
nl|'\n'
name|'fake_new_parent_path'
op|')'
newline|'\n'
nl|'\n'
name|'expected_virt_disk_data'
op|'='
name|'self'
op|'.'
name|'_FAKE_VHD_INFO_XML'
op|'.'
name|'replace'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_PARENT_PATH'
op|','
name|'fake_new_parent_path'
op|')'
newline|'\n'
name|'mock_img_svc'
op|'.'
name|'SetVirtualHardDiskSettingData'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'VirtualDiskSettingData'
op|'='
name|'expected_virt_disk_data'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_reconnect_parent_vhd_exception
dedent|''
name|'def'
name|'test_reconnect_parent_vhd_exception'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# Test that reconnect_parent_vhd raises an exception if the'
nl|'\n'
comment|'# vhd info XML does not contain the ParentPath property.'
nl|'\n'
indent|'        '
name|'fake_vhd_info_xml'
op|'='
name|'self'
op|'.'
name|'_FAKE_VHD_INFO_XML'
op|'.'
name|'replace'
op|'('
string|"'ParentPath'"
op|','
nl|'\n'
string|"'FakeParentPath'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhd_info_xml'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'fake_vhd_info_xml'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'vmutils'
op|'.'
name|'HyperVException'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'reconnect_parent_vhd'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
nl|'\n'
name|'mock'
op|'.'
name|'sentinel'
op|'.'
name|'new_parent_path'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_resize_vhd
dedent|''
name|'def'
name|'test_resize_vhd'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'mock_img_svc'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_conn'
op|'.'
name|'Msvm_ImageManagementService'
op|'('
op|')'
op|'['
number|'0'
op|']'
newline|'\n'
name|'mock_img_svc'
op|'.'
name|'ResizeVirtualHardDisk'
op|'.'
name|'return_value'
op|'='
op|'('
name|'self'
op|'.'
name|'_FAKE_JOB_PATH'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_RET_VAL'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_internal_vhd_size_by_file_size'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'resize_vhd'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|')'
newline|'\n'
nl|'\n'
name|'mock_img_svc'
op|'.'
name|'ResizeVirtualHardDisk'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'Path'
op|'='
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
nl|'\n'
name|'MaxInternalSize'
op|'='
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'mock_get'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_internal_vhd_size_by_file_size'
newline|'\n'
name|'self'
op|'.'
name|'mock_get'
op|'.'
name|'assert_called_once_with'
op|'('
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_test_get_vhdx_internal_size
dedent|''
name|'def'
name|'_test_get_vhdx_internal_size'
op|'('
name|'self'
op|','
name|'vhd_type'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_vhd_info'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_vhd_parent_path'
op|'='
name|'mock'
op|'.'
name|'Mock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'self'
op|'.'
name|'_FAKE_PARENT_PATH'
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'vhd_type'
op|'=='
number|'4'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_vhd_info'
op|'.'
name|'side_effect'
op|'='
op|'['
nl|'\n'
op|'{'
string|"'Type'"
op|':'
name|'vhd_type'
op|'}'
op|','
name|'self'
op|'.'
name|'_fake_vhd_info'
op|']'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_vhd_info'
op|'.'
name|'return_value'
op|'='
name|'self'
op|'.'
name|'_fake_vhd_info'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.virt.hyperv.vhdutils.VHDUtils.get_vhd_format'"
op|')'
newline|'\n'
DECL|member|test_get_vhdx_internal_size
name|'def'
name|'test_get_vhdx_internal_size'
op|'('
name|'self'
op|','
name|'mock_get_vhd_format'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'mock_get_vhd_format'
op|'.'
name|'return_value'
op|'='
name|'constants'
op|'.'
name|'DISK_FORMAT_VHDX'
newline|'\n'
name|'self'
op|'.'
name|'_mock_get_vhd_info'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_log_size'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'self'
op|'.'
name|'_FAKE_LOG_SIZE'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_metadata_size_and_offset'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
op|'('
name|'self'
op|'.'
name|'_FAKE_METADATA_SIZE'
op|','
number|'1024'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_block_size'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'self'
op|'.'
name|'_FAKE_BLOCK_SIZE'
op|')'
newline|'\n'
nl|'\n'
name|'file_mock'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
op|')'
newline|'\n'
name|'with'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'__builtin__.open'"
op|','
name|'file_mock'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'internal_size'
op|'='
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_internal_vhd_size_by_file_size'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_FAKE_VHD_PATH'
op|','
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'_FAKE_MAX_INTERNAL_SIZE'
op|'-'
name|'self'
op|'.'
name|'_FAKE_BLOCK_SIZE'
op|','
nl|'\n'
name|'internal_size'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_vhdx_internal_size_dynamic
dedent|''
name|'def'
name|'test_get_vhdx_internal_size_dynamic'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_get_vhdx_internal_size'
op|'('
number|'3'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_vhdx_internal_size_differencing
dedent|''
name|'def'
name|'test_get_vhdx_internal_size_differencing'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_get_vhdx_internal_size'
op|'('
number|'4'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_vhdx_current_header
dedent|''
name|'def'
name|'test_get_vhdx_current_header'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'VHDX_HEADER_OFFSETS'
op|'='
op|'['
number|'64'
op|'*'
number|'1024'
op|','
number|'128'
op|'*'
number|'1024'
op|']'
newline|'\n'
name|'fake_sequence_numbers'
op|'='
op|'['
string|"'\\x01\\x00\\x00\\x00\\x00\\x00\\x00\\x00'"
op|','
nl|'\n'
string|"'\\x02\\x00\\x00\\x00\\x00\\x00\\x00\\x00'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'_fake_file_handle'
op|'.'
name|'read'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'side_effect'
op|'='
name|'fake_sequence_numbers'
op|')'
newline|'\n'
nl|'\n'
name|'offset'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_current_header_offset'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_fake_file_handle'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'offset'
op|','
name|'VHDX_HEADER_OFFSETS'
op|'['
number|'1'
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_vhdx_metadata_size
dedent|''
name|'def'
name|'test_get_vhdx_metadata_size'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_metadata_offset'
op|'='
string|"'\\x01\\x00\\x00\\x00\\x00\\x00\\x00\\x00'"
newline|'\n'
name|'fake_metadata_size'
op|'='
string|"'\\x01\\x00\\x00\\x00'"
newline|'\n'
name|'self'
op|'.'
name|'_fake_file_handle'
op|'.'
name|'read'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'side_effect'
op|'='
op|'['
name|'fake_metadata_offset'
op|','
name|'fake_metadata_size'
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'metadata_size'
op|','
name|'metadata_offset'
op|'='
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_metadata_size_and_offset'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_fake_file_handle'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'metadata_size'
op|','
number|'1'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'metadata_offset'
op|','
number|'1'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_block_size
dedent|''
name|'def'
name|'test_get_block_size'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_metadata_size_and_offset'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
op|'('
name|'self'
op|'.'
name|'_FAKE_METADATA_SIZE'
op|','
number|'1024'
op|')'
op|')'
newline|'\n'
name|'fake_block_size'
op|'='
string|"'\\x01\\x00\\x00\\x00'"
newline|'\n'
name|'self'
op|'.'
name|'_fake_file_handle'
op|'.'
name|'read'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'fake_block_size'
op|')'
newline|'\n'
nl|'\n'
name|'block_size'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_block_size'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'_fake_file_handle'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'block_size'
op|','
number|'1'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_log_size
dedent|''
name|'def'
name|'test_get_log_size'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_current_header_offset'
op|'='
number|'64'
op|'*'
number|'1024'
newline|'\n'
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_current_header_offset'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'fake_current_header_offset'
op|')'
newline|'\n'
name|'fake_log_size'
op|'='
string|"'\\x01\\x00\\x00\\x00'"
newline|'\n'
name|'self'
op|'.'
name|'_fake_file_handle'
op|'.'
name|'read'
op|'='
name|'mock'
op|'.'
name|'MagicMock'
op|'('
nl|'\n'
name|'return_value'
op|'='
name|'fake_log_size'
op|')'
newline|'\n'
nl|'\n'
name|'log_size'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'_get_vhdx_log_size'
op|'('
name|'self'
op|'.'
name|'_fake_file_handle'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'log_size'
op|','
number|'1'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_supported_vhd_format
dedent|''
name|'def'
name|'test_get_supported_vhd_format'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fmt'
op|'='
name|'self'
op|'.'
name|'_vhdutils'
op|'.'
name|'get_best_supported_vhd_format'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'constants'
op|'.'
name|'DISK_FORMAT_VHDX'
op|','
name|'fmt'
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
