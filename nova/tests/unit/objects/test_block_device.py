begin_unit
comment|'#    Copyright 2013 Red Hat Inc.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
name|'import'
name|'mock'
newline|'\n'
name|'import'
name|'six'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'cells'
name|'import'
name|'rpcapi'
name|'as'
name|'cells_rpcapi'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'context'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'db'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'objects'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'objects'
name|'import'
name|'block_device'
name|'as'
name|'block_device_obj'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'test'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
name|'import'
name|'fake_block_device'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
name|'import'
name|'fake_instance'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
op|'.'
name|'objects'
name|'import'
name|'test_objects'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|_TestBlockDeviceMappingObject
name|'class'
name|'_TestBlockDeviceMappingObject'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
DECL|member|fake_bdm
indent|'    '
name|'def'
name|'fake_bdm'
op|'('
name|'self'
op|','
name|'instance'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance'
op|'='
name|'instance'
name|'or'
op|'{'
op|'}'
newline|'\n'
name|'fake_bdm'
op|'='
name|'fake_block_device'
op|'.'
name|'FakeDbBlockDeviceDict'
op|'('
op|'{'
nl|'\n'
string|"'id'"
op|':'
number|'123'
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
name|'instance'
op|'.'
name|'get'
op|'('
string|"'uuid'"
op|')'
name|'or'
string|"'fake-instance'"
op|','
nl|'\n'
string|"'device_name'"
op|':'
string|"'/dev/sda2'"
op|','
nl|'\n'
string|"'source_type'"
op|':'
string|"'snapshot'"
op|','
nl|'\n'
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
nl|'\n'
string|"'connection_info'"
op|':'
string|'"{\'fake\': \'connection_info\'}"'
op|','
nl|'\n'
string|"'snapshot_id'"
op|':'
string|"'fake-snapshot-id-1'"
op|','
nl|'\n'
string|"'boot_index'"
op|':'
op|'-'
number|'1'
nl|'\n'
op|'}'
op|')'
newline|'\n'
name|'if'
name|'instance'
op|':'
newline|'\n'
indent|'            '
name|'fake_bdm'
op|'['
string|"'instance'"
op|']'
op|'='
name|'instance'
newline|'\n'
dedent|''
name|'return'
name|'fake_bdm'
newline|'\n'
nl|'\n'
DECL|member|_test_save
dedent|''
name|'def'
name|'_test_save'
op|'('
name|'self'
op|','
name|'cell_type'
op|'='
name|'None'
op|','
name|'update_device_name'
op|'='
name|'False'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'cell_type'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'flags'
op|'('
name|'enable'
op|'='
name|'True'
op|','
name|'cell_type'
op|'='
name|'cell_type'
op|','
name|'group'
op|'='
string|"'cells'"
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'flags'
op|'('
name|'enable'
op|'='
name|'False'
op|','
name|'group'
op|'='
string|"'cells'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'create'
op|'='
name|'False'
newline|'\n'
name|'fake_bdm'
op|'='
name|'self'
op|'.'
name|'fake_bdm'
op|'('
op|')'
newline|'\n'
name|'with'
name|'test'
op|'.'
name|'nested'
op|'('
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
nl|'\n'
name|'db'
op|','
string|"'block_device_mapping_update'"
op|','
name|'return_value'
op|'='
name|'fake_bdm'
op|')'
op|','
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
nl|'\n'
name|'cells_rpcapi'
op|'.'
name|'CellsAPI'
op|','
string|"'bdm_update_or_create_at_top'"
op|')'
nl|'\n'
op|')'
name|'as'
op|'('
name|'bdm_update_mock'
op|','
name|'cells_update_mock'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'bdm_object'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|')'
newline|'\n'
name|'bdm_object'
op|'.'
name|'id'
op|'='
number|'123'
newline|'\n'
name|'bdm_object'
op|'.'
name|'volume_id'
op|'='
string|"'fake_volume_id'"
newline|'\n'
name|'if'
name|'update_device_name'
op|':'
newline|'\n'
indent|'                '
name|'bdm_object'
op|'.'
name|'device_name'
op|'='
string|"'/dev/vda'"
newline|'\n'
name|'create'
op|'='
name|'None'
newline|'\n'
dedent|''
name|'bdm_object'
op|'.'
name|'save'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'update_device_name'
op|':'
newline|'\n'
indent|'                '
name|'bdm_update_mock'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
number|'123'
op|','
nl|'\n'
op|'{'
string|"'volume_id'"
op|':'
string|"'fake_volume_id'"
op|','
nl|'\n'
string|"'device_name'"
op|':'
string|"'/dev/vda'"
op|'}'
op|','
nl|'\n'
name|'legacy'
op|'='
name|'False'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'bdm_update_mock'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
number|'123'
op|','
op|'{'
string|"'volume_id'"
op|':'
string|"'fake_volume_id'"
op|'}'
op|','
nl|'\n'
name|'legacy'
op|'='
name|'False'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'cell_type'
op|'!='
string|"'compute'"
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'cells_update_mock'
op|'.'
name|'called'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'cells_update_mock'
op|'.'
name|'call_count'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'len'
op|'('
name|'cells_update_mock'
op|'.'
name|'call_args'
op|'['
number|'0'
op|']'
op|')'
op|'>'
number|'1'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'cells_update_mock'
op|'.'
name|'call_args'
op|'['
number|'0'
op|']'
op|'['
number|'1'
op|']'
op|','
nl|'\n'
name|'block_device_obj'
op|'.'
name|'BlockDeviceMapping'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
op|'{'
string|"'create'"
op|':'
name|'create'
op|'}'
op|','
nl|'\n'
name|'cells_update_mock'
op|'.'
name|'call_args'
op|'['
number|'1'
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_save_nocells
dedent|''
dedent|''
dedent|''
name|'def'
name|'test_save_nocells'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_save'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_save_apicell
dedent|''
name|'def'
name|'test_save_apicell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_save'
op|'('
name|'cell_type'
op|'='
string|"'api'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_save_computecell
dedent|''
name|'def'
name|'test_save_computecell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_save'
op|'('
name|'cell_type'
op|'='
string|"'compute'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_save_computecell_device_name_changed
dedent|''
name|'def'
name|'test_save_computecell_device_name_changed'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_save'
op|'('
name|'cell_type'
op|'='
string|"'compute'"
op|','
name|'update_device_name'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_save_instance_changed
dedent|''
name|'def'
name|'test_save_instance_changed'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bdm_object'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|')'
newline|'\n'
name|'bdm_object'
op|'.'
name|'instance'
op|'='
name|'objects'
op|'.'
name|'Instance'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'ObjectActionError'
op|','
nl|'\n'
name|'bdm_object'
op|'.'
name|'save'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_update'"
op|','
name|'return_value'
op|'='
name|'None'
op|')'
newline|'\n'
DECL|member|test_save_not_found
name|'def'
name|'test_save_not_found'
op|'('
name|'self'
op|','
name|'bdm_update'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bdm_object'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|')'
newline|'\n'
name|'bdm_object'
op|'.'
name|'id'
op|'='
number|'123'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'BDMNotFound'
op|','
name|'bdm_object'
op|'.'
name|'save'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_volume_id'"
op|')'
newline|'\n'
DECL|member|test_get_by_volume_id
name|'def'
name|'test_get_by_volume_id'
op|'('
name|'self'
op|','
name|'get_by_vol_id'
op|')'
op|':'
newline|'\n'
comment|'# NOTE(danms): Include two results to make sure the first was picked.'
nl|'\n'
comment|"# An invalid second item shouldn't be touched -- if it is, it'll"
nl|'\n'
comment|'# fail from_db_object().'
nl|'\n'
indent|'        '
name|'get_by_vol_id'
op|'.'
name|'return_value'
op|'='
op|'['
name|'self'
op|'.'
name|'fake_bdm'
op|'('
op|')'
op|','
nl|'\n'
name|'None'
op|']'
newline|'\n'
nl|'\n'
name|'vol_bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'.'
name|'get_by_volume_id'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|')'
newline|'\n'
name|'for'
name|'attr'
name|'in'
name|'block_device_obj'
op|'.'
name|'BLOCK_DEVICE_OPTIONAL_ATTRS'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'vol_bdm'
op|'.'
name|'obj_attr_is_set'
op|'('
name|'attr'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_volume_id'"
op|')'
newline|'\n'
DECL|member|test_get_by_volume_id_not_found
name|'def'
name|'test_get_by_volume_id_not_found'
op|'('
name|'self'
op|','
name|'get_by_vol_id'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'get_by_vol_id'
op|'.'
name|'return_value'
op|'='
name|'None'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'VolumeBDMNotFound'
op|','
nl|'\n'
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'.'
name|'get_by_volume_id'
op|','
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_volume_id'"
op|')'
newline|'\n'
DECL|member|test_get_by_volume_instance_uuid_missmatch
name|'def'
name|'test_get_by_volume_instance_uuid_missmatch'
op|'('
name|'self'
op|','
name|'get_by_vol_id'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_bdm_vol'
op|'='
name|'self'
op|'.'
name|'fake_bdm'
op|'('
name|'instance'
op|'='
op|'{'
string|"'uuid'"
op|':'
string|"'other-fake-instance'"
op|'}'
op|')'
newline|'\n'
name|'get_by_vol_id'
op|'.'
name|'return_value'
op|'='
op|'['
name|'fake_bdm_vol'
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'InvalidVolume'
op|','
nl|'\n'
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'.'
name|'get_by_volume_id'
op|','
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|','
nl|'\n'
name|'instance_uuid'
op|'='
string|"'fake-instance'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_volume_id'"
op|')'
newline|'\n'
DECL|member|test_get_by_volume_id_with_expected
name|'def'
name|'test_get_by_volume_id_with_expected'
op|'('
name|'self'
op|','
name|'get_by_vol_id'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'get_by_vol_id'
op|'.'
name|'return_value'
op|'='
op|'['
name|'self'
op|'.'
name|'fake_bdm'
op|'('
nl|'\n'
name|'fake_instance'
op|'.'
name|'fake_db_instance'
op|'('
op|')'
op|')'
op|']'
newline|'\n'
nl|'\n'
name|'vol_bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'.'
name|'get_by_volume_id'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|','
name|'expected_attrs'
op|'='
op|'['
string|"'instance'"
op|']'
op|')'
newline|'\n'
name|'for'
name|'attr'
name|'in'
name|'block_device_obj'
op|'.'
name|'BLOCK_DEVICE_OPTIONAL_ATTRS'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'vol_bdm'
op|'.'
name|'obj_attr_is_set'
op|'('
name|'attr'
op|')'
op|')'
newline|'\n'
dedent|''
name|'get_by_vol_id'
op|'.'
name|'assert_called_once_with'
op|'('
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|','
nl|'\n'
op|'['
string|"'instance'"
op|']'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_volume_id'"
op|')'
newline|'\n'
DECL|member|test_get_by_volume_returned_single
name|'def'
name|'test_get_by_volume_returned_single'
op|'('
name|'self'
op|','
name|'get_all'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_bdm_vol'
op|'='
name|'self'
op|'.'
name|'fake_bdm'
op|'('
op|')'
newline|'\n'
name|'get_all'
op|'.'
name|'return_value'
op|'='
op|'['
name|'fake_bdm_vol'
op|']'
newline|'\n'
name|'vol_bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'.'
name|'get_by_volume'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'fake_bdm_vol'
op|'['
string|"'id'"
op|']'
op|','
name|'vol_bdm'
op|'.'
name|'id'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_volume_id'"
op|')'
newline|'\n'
DECL|member|test_get_by_volume_returned_multiple
name|'def'
name|'test_get_by_volume_returned_multiple'
op|'('
name|'self'
op|','
name|'get_all'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_bdm_vol1'
op|'='
name|'self'
op|'.'
name|'fake_bdm'
op|'('
op|')'
newline|'\n'
name|'fake_bdm_vol2'
op|'='
name|'self'
op|'.'
name|'fake_bdm'
op|'('
op|')'
newline|'\n'
name|'get_all'
op|'.'
name|'return_value'
op|'='
op|'['
name|'fake_bdm_vol1'
op|','
name|'fake_bdm_vol2'
op|']'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'VolumeBDMIsMultiAttach'
op|','
nl|'\n'
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'.'
name|'get_by_volume'
op|','
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
nl|'\n'
string|"'block_device_mapping_get_by_instance_and_volume_id'"
op|')'
newline|'\n'
DECL|member|test_get_by_instance_and_volume_id
name|'def'
name|'test_get_by_instance_and_volume_id'
op|'('
name|'self'
op|','
name|'mock_get'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_inst'
op|'='
name|'fake_instance'
op|'.'
name|'fake_db_instance'
op|'('
op|')'
newline|'\n'
name|'mock_get'
op|'.'
name|'return_value'
op|'='
name|'self'
op|'.'
name|'fake_bdm'
op|'('
name|'fake_inst'
op|')'
newline|'\n'
nl|'\n'
name|'obj_bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
newline|'\n'
name|'vol_bdm'
op|'='
name|'obj_bdm'
op|'.'
name|'get_by_volume_and_instance'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|','
string|"'fake-instance-id'"
op|')'
newline|'\n'
name|'for'
name|'attr'
name|'in'
name|'block_device_obj'
op|'.'
name|'BLOCK_DEVICE_OPTIONAL_ATTRS'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'vol_bdm'
op|'.'
name|'obj_attr_is_set'
op|'('
name|'attr'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
nl|'\n'
string|"'block_device_mapping_get_by_instance_and_volume_id'"
op|')'
newline|'\n'
DECL|member|test_test_get_by_instance_and_volume_id_with_expected
name|'def'
name|'test_test_get_by_instance_and_volume_id_with_expected'
op|'('
name|'self'
op|','
name|'mock_get'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_inst'
op|'='
name|'fake_instance'
op|'.'
name|'fake_db_instance'
op|'('
op|')'
newline|'\n'
name|'mock_get'
op|'.'
name|'return_value'
op|'='
name|'self'
op|'.'
name|'fake_bdm'
op|'('
name|'fake_inst'
op|')'
newline|'\n'
nl|'\n'
name|'obj_bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
newline|'\n'
name|'vol_bdm'
op|'='
name|'obj_bdm'
op|'.'
name|'get_by_volume_and_instance'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|','
name|'fake_inst'
op|'['
string|"'uuid'"
op|']'
op|','
nl|'\n'
name|'expected_attrs'
op|'='
op|'['
string|"'instance'"
op|']'
op|')'
newline|'\n'
name|'for'
name|'attr'
name|'in'
name|'block_device_obj'
op|'.'
name|'BLOCK_DEVICE_OPTIONAL_ATTRS'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'vol_bdm'
op|'.'
name|'obj_attr_is_set'
op|'('
name|'attr'
op|')'
op|')'
newline|'\n'
dedent|''
name|'mock_get'
op|'.'
name|'assert_called_once_with'
op|'('
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|','
nl|'\n'
name|'fake_inst'
op|'['
string|"'uuid'"
op|']'
op|','
op|'['
string|"'instance'"
op|']'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
nl|'\n'
string|"'block_device_mapping_get_by_instance_and_volume_id'"
op|')'
newline|'\n'
DECL|member|test_get_by_instance_and_volume_id_not_found
name|'def'
name|'test_get_by_instance_and_volume_id_not_found'
op|'('
name|'self'
op|','
name|'mock_get'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'mock_get'
op|'.'
name|'return_value'
op|'='
name|'None'
newline|'\n'
nl|'\n'
name|'obj_bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'VolumeBDMNotFound'
op|','
nl|'\n'
name|'obj_bdm'
op|'.'
name|'get_by_volume_and_instance'
op|','
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-volume-id'"
op|','
string|"'fake-instance-id'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|_test_create_mocked
dedent|''
name|'def'
name|'_test_create_mocked'
op|'('
name|'self'
op|','
name|'cell_type'
op|'='
name|'None'
op|','
name|'update_or_create'
op|'='
name|'False'
op|','
nl|'\n'
name|'device_name'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'cell_type'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'flags'
op|'('
name|'enable'
op|'='
name|'True'
op|','
name|'cell_type'
op|'='
name|'cell_type'
op|','
name|'group'
op|'='
string|"'cells'"
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'flags'
op|'('
name|'enable'
op|'='
name|'False'
op|','
name|'group'
op|'='
string|"'cells'"
op|')'
newline|'\n'
dedent|''
name|'values'
op|'='
op|'{'
string|"'source_type'"
op|':'
string|"'volume'"
op|','
string|"'volume_id'"
op|':'
string|"'fake-vol-id'"
op|','
nl|'\n'
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
string|"'fake-instance'"
op|'}'
newline|'\n'
name|'if'
name|'device_name'
op|':'
newline|'\n'
indent|'            '
name|'values'
op|'['
string|"'device_name'"
op|']'
op|'='
name|'device_name'
newline|'\n'
dedent|''
name|'fake_bdm'
op|'='
name|'fake_block_device'
op|'.'
name|'FakeDbBlockDeviceDict'
op|'('
name|'values'
op|')'
newline|'\n'
nl|'\n'
name|'with'
name|'test'
op|'.'
name|'nested'
op|'('
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
nl|'\n'
name|'db'
op|','
string|"'block_device_mapping_create'"
op|','
name|'return_value'
op|'='
name|'fake_bdm'
op|')'
op|','
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
nl|'\n'
name|'db'
op|','
string|"'block_device_mapping_update_or_create'"
op|','
nl|'\n'
name|'return_value'
op|'='
name|'fake_bdm'
op|')'
op|','
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'cells_rpcapi'
op|'.'
name|'CellsAPI'
op|','
nl|'\n'
string|"'bdm_update_or_create_at_top'"
op|')'
nl|'\n'
op|')'
name|'as'
op|'('
name|'bdm_create_mock'
op|','
name|'bdm_update_or_create_mock'
op|','
name|'cells_update_mock'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
op|'**'
name|'values'
op|')'
newline|'\n'
name|'if'
name|'update_or_create'
op|':'
newline|'\n'
indent|'                '
name|'method'
op|'='
name|'bdm'
op|'.'
name|'update_or_create'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'method'
op|'='
name|'bdm'
op|'.'
name|'create'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'cell_type'
op|'=='
string|"'api'"
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'ObjectActionError'
op|','
nl|'\n'
name|'method'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'method'
op|'('
op|')'
newline|'\n'
name|'if'
name|'update_or_create'
op|':'
newline|'\n'
indent|'                    '
name|'bdm_update_or_create_mock'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
name|'values'
op|','
name|'legacy'
op|'='
name|'False'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                    '
name|'bdm_create_mock'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
name|'values'
op|','
name|'legacy'
op|'='
name|'False'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'cell_type'
op|'=='
string|"'compute'"
name|'and'
string|"'device_name'"
name|'in'
name|'values'
op|':'
newline|'\n'
indent|'                    '
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'cells_update_mock'
op|'.'
name|'call_count'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'len'
op|'('
name|'cells_update_mock'
op|'.'
name|'call_args'
op|'['
number|'0'
op|']'
op|')'
op|'>'
number|'1'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'cells_update_mock'
op|'.'
name|'call_args'
op|'['
number|'0'
op|']'
op|'['
number|'0'
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'cells_update_mock'
op|'.'
name|'call_args'
op|'['
number|'0'
op|']'
op|'['
number|'1'
op|']'
op|','
nl|'\n'
name|'block_device_obj'
op|'.'
name|'BlockDeviceMapping'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
op|'{'
string|"'create'"
op|':'
name|'update_or_create'
name|'or'
name|'None'
op|'}'
op|','
nl|'\n'
name|'cells_update_mock'
op|'.'
name|'call_args'
op|'['
number|'1'
op|']'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                    '
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'cells_update_mock'
op|'.'
name|'called'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_create_nocells
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'test_create_nocells'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_create_mocked'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_update_or_create
dedent|''
name|'def'
name|'test_update_or_create'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_create_mocked'
op|'('
name|'update_or_create'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_create_apicell
dedent|''
name|'def'
name|'test_create_apicell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_create_mocked'
op|'('
name|'cell_type'
op|'='
string|"'api'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_update_or_create_apicell
dedent|''
name|'def'
name|'test_update_or_create_apicell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_create_mocked'
op|'('
name|'cell_type'
op|'='
string|"'api'"
op|','
name|'update_or_create'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_create_computecell
dedent|''
name|'def'
name|'test_create_computecell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_create_mocked'
op|'('
name|'cell_type'
op|'='
string|"'compute'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_update_or_create_computecell
dedent|''
name|'def'
name|'test_update_or_create_computecell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_create_mocked'
op|'('
name|'cell_type'
op|'='
string|"'compute'"
op|','
name|'update_or_create'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_device_name_compute_cell
dedent|''
name|'def'
name|'test_device_name_compute_cell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_create_mocked'
op|'('
name|'cell_type'
op|'='
string|"'compute'"
op|','
name|'device_name'
op|'='
string|"'/dev/xvdb'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_create
dedent|''
name|'def'
name|'test_create'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'values'
op|'='
op|'{'
string|"'source_type'"
op|':'
string|"'volume'"
op|','
string|"'volume_id'"
op|':'
string|"'fake-vol-id'"
op|','
nl|'\n'
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
string|"'fake-instance'"
op|'}'
newline|'\n'
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
op|'**'
name|'values'
op|')'
newline|'\n'
name|'with'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'cells_rpcapi'
op|'.'
name|'CellsAPI'
op|','
nl|'\n'
string|"'bdm_update_or_create_at_top'"
op|')'
op|':'
newline|'\n'
indent|'            '
name|'bdm'
op|'.'
name|'create'
op|'('
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'for'
name|'k'
op|','
name|'v'
name|'in'
name|'six'
op|'.'
name|'iteritems'
op|'('
name|'values'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'v'
op|','
name|'getattr'
op|'('
name|'bdm'
op|','
name|'k'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_create_fails
dedent|''
dedent|''
name|'def'
name|'test_create_fails'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'values'
op|'='
op|'{'
string|"'source_type'"
op|':'
string|"'volume'"
op|','
string|"'volume_id'"
op|':'
string|"'fake-vol-id'"
op|','
nl|'\n'
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
string|"'fake-instance'"
op|'}'
newline|'\n'
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
op|'**'
name|'values'
op|')'
newline|'\n'
name|'bdm'
op|'.'
name|'create'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'ObjectActionError'
op|','
nl|'\n'
name|'bdm'
op|'.'
name|'create'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_create_fails_instance
dedent|''
name|'def'
name|'test_create_fails_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'values'
op|'='
op|'{'
string|"'source_type'"
op|':'
string|"'volume'"
op|','
string|"'volume_id'"
op|':'
string|"'fake-vol-id'"
op|','
nl|'\n'
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
string|"'fake-instance'"
op|','
nl|'\n'
string|"'instance'"
op|':'
name|'objects'
op|'.'
name|'Instance'
op|'('
op|')'
op|'}'
newline|'\n'
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
op|'**'
name|'values'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'ObjectActionError'
op|','
nl|'\n'
name|'bdm'
op|'.'
name|'create'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_test_destroy_mocked
dedent|''
name|'def'
name|'_test_destroy_mocked'
op|'('
name|'self'
op|','
name|'cell_type'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'values'
op|'='
op|'{'
string|"'source_type'"
op|':'
string|"'volume'"
op|','
string|"'volume_id'"
op|':'
string|"'fake-vol-id'"
op|','
nl|'\n'
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
string|"'id'"
op|':'
number|'1'
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
string|"'fake-instance'"
op|','
string|"'device_name'"
op|':'
string|"'fake'"
op|'}'
newline|'\n'
name|'if'
name|'cell_type'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'flags'
op|'('
name|'enable'
op|'='
name|'True'
op|','
name|'cell_type'
op|'='
name|'cell_type'
op|','
name|'group'
op|'='
string|"'cells'"
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'flags'
op|'('
name|'enable'
op|'='
name|'False'
op|','
name|'group'
op|'='
string|"'cells'"
op|')'
newline|'\n'
dedent|''
name|'with'
name|'test'
op|'.'
name|'nested'
op|'('
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_destroy'"
op|')'
op|','
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'cells_rpcapi'
op|'.'
name|'CellsAPI'
op|','
string|"'bdm_destroy_at_top'"
op|')'
nl|'\n'
op|')'
name|'as'
op|'('
name|'bdm_del'
op|','
name|'cells_destroy'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
op|'**'
name|'values'
op|')'
newline|'\n'
name|'bdm'
op|'.'
name|'destroy'
op|'('
op|')'
newline|'\n'
name|'bdm_del'
op|'.'
name|'assert_called_once_with'
op|'('
name|'self'
op|'.'
name|'context'
op|','
name|'values'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'if'
name|'cell_type'
op|'!='
string|"'compute'"
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'cells_destroy'
op|'.'
name|'called'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'cells_destroy'
op|'.'
name|'assert_called_once_with'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
name|'values'
op|'['
string|"'instance_uuid'"
op|']'
op|','
nl|'\n'
name|'device_name'
op|'='
name|'values'
op|'['
string|"'device_name'"
op|']'
op|','
nl|'\n'
name|'volume_id'
op|'='
name|'values'
op|'['
string|"'volume_id'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_destroy_nocells
dedent|''
dedent|''
dedent|''
name|'def'
name|'test_destroy_nocells'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_destroy_mocked'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_destroy_apicell
dedent|''
name|'def'
name|'test_destroy_apicell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_destroy_mocked'
op|'('
name|'cell_type'
op|'='
string|"'api'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_destroy_computecell
dedent|''
name|'def'
name|'test_destroy_computecell'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_destroy_mocked'
op|'('
name|'cell_type'
op|'='
string|"'compute'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_is_image_true
dedent|''
name|'def'
name|'test_is_image_true'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'source_type'
op|'='
string|"'image'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'bdm'
op|'.'
name|'is_image'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_is_image_false
dedent|''
name|'def'
name|'test_is_image_false'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'source_type'
op|'='
string|"'snapshot'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'bdm'
op|'.'
name|'is_image'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_is_volume_true
dedent|''
name|'def'
name|'test_is_volume_true'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'destination_type'
op|'='
string|"'volume'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'bdm'
op|'.'
name|'is_volume'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_is_volume_false
dedent|''
name|'def'
name|'test_is_volume_false'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'destination_type'
op|'='
string|"'local'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'bdm'
op|'.'
name|'is_volume'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_obj_make_compatible_pre_1_17
dedent|''
name|'def'
name|'test_obj_make_compatible_pre_1_17'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'values'
op|'='
op|'{'
string|"'source_type'"
op|':'
string|"'volume'"
op|','
string|"'volume_id'"
op|':'
string|"'fake-vol-id'"
op|','
nl|'\n'
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
string|"'fake-instance'"
op|'}'
newline|'\n'
name|'bdm'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|'('
name|'context'
op|'='
name|'self'
op|'.'
name|'context'
op|','
op|'**'
name|'values'
op|')'
newline|'\n'
name|'primitive'
op|'='
name|'bdm'
op|'.'
name|'obj_to_primitive'
op|'('
name|'target_version'
op|'='
string|"'1.16'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertNotIn'
op|'('
string|"'tag'"
op|','
name|'primitive'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
dedent|''
name|'class'
name|'TestBlockDeviceMappingObject'
op|'('
name|'test_objects'
op|'.'
name|'_LocalTest'
op|','
nl|'\n'
DECL|class|TestBlockDeviceMappingObject
name|'_TestBlockDeviceMappingObject'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
name|'class'
name|'TestRemoteBlockDeviceMappingObject'
op|'('
name|'test_objects'
op|'.'
name|'_RemoteTest'
op|','
nl|'\n'
DECL|class|TestRemoteBlockDeviceMappingObject
name|'_TestBlockDeviceMappingObject'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|_TestBlockDeviceMappingListObject
dedent|''
name|'class'
name|'_TestBlockDeviceMappingListObject'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
DECL|member|fake_bdm
indent|'    '
name|'def'
name|'fake_bdm'
op|'('
name|'self'
op|','
name|'bdm_id'
op|','
name|'boot_index'
op|'='
op|'-'
number|'1'
op|','
name|'instance_uuid'
op|'='
string|"'fake-instance'"
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_bdm'
op|'='
name|'fake_block_device'
op|'.'
name|'FakeDbBlockDeviceDict'
op|'('
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'bdm_id'
op|','
nl|'\n'
string|"'boot_index'"
op|':'
name|'boot_index'
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
name|'instance_uuid'
op|','
nl|'\n'
string|"'device_name'"
op|':'
string|"'/dev/sda2'"
op|','
nl|'\n'
string|"'source_type'"
op|':'
string|"'snapshot'"
op|','
nl|'\n'
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
nl|'\n'
string|"'connection_info'"
op|':'
string|'"{\'fake\': \'connection_info\'}"'
op|','
nl|'\n'
string|"'snapshot_id'"
op|':'
string|"'fake-snapshot-id-1'"
op|','
nl|'\n'
op|'}'
op|')'
newline|'\n'
name|'return'
name|'fake_bdm'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance_uuids'"
op|')'
newline|'\n'
DECL|member|test_bdms_by_instance_uuid
name|'def'
name|'test_bdms_by_instance_uuid'
op|'('
name|'self'
op|','
name|'get_all_by_inst_uuids'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fakes'
op|'='
op|'['
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'123'
op|')'
op|','
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'456'
op|')'
op|']'
newline|'\n'
name|'get_all_by_inst_uuids'
op|'.'
name|'return_value'
op|'='
name|'fakes'
newline|'\n'
name|'bdms_by_uuid'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'bdms_by_instance_uuid'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
op|'['
string|"'fake-instance'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
op|'['
string|"'fake-instance'"
op|']'
op|','
name|'list'
op|'('
name|'bdms_by_uuid'
op|'.'
name|'keys'
op|'('
op|')'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
nl|'\n'
name|'bdms_by_uuid'
op|'['
string|"'fake-instance'"
op|']'
op|','
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|')'
newline|'\n'
name|'for'
name|'faked'
op|','
name|'got'
name|'in'
name|'zip'
op|'('
name|'fakes'
op|','
name|'bdms_by_uuid'
op|'['
string|"'fake-instance'"
op|']'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'got'
op|','
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'faked'
op|'['
string|"'id'"
op|']'
op|','
name|'got'
op|'.'
name|'id'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance_uuids'"
op|')'
newline|'\n'
DECL|member|test_bdms_by_instance_uuid_no_result
name|'def'
name|'test_bdms_by_instance_uuid_no_result'
op|'('
name|'self'
op|','
name|'get_all_by_inst_uuids'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'get_all_by_inst_uuids'
op|'.'
name|'return_value'
op|'='
name|'None'
newline|'\n'
name|'bdms_by_uuid'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'bdms_by_instance_uuid'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
op|'['
string|"'fake-instance'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
op|'{'
op|'}'
op|','
name|'bdms_by_uuid'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance_uuids'"
op|')'
newline|'\n'
DECL|member|test_get_by_instance_uuids
name|'def'
name|'test_get_by_instance_uuids'
op|'('
name|'self'
op|','
name|'get_all_by_inst_uuids'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fakes'
op|'='
op|'['
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'123'
op|')'
op|','
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'456'
op|')'
op|']'
newline|'\n'
name|'get_all_by_inst_uuids'
op|'.'
name|'return_value'
op|'='
name|'fakes'
newline|'\n'
name|'bdm_list'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'get_by_instance_uuids'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
op|'['
string|"'fake-instance'"
op|']'
op|')'
newline|'\n'
name|'for'
name|'faked'
op|','
name|'got'
name|'in'
name|'zip'
op|'('
name|'fakes'
op|','
name|'bdm_list'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'got'
op|','
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'faked'
op|'['
string|"'id'"
op|']'
op|','
name|'got'
op|'.'
name|'id'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance_uuids'"
op|')'
newline|'\n'
DECL|member|test_get_by_instance_uuids_no_result
name|'def'
name|'test_get_by_instance_uuids_no_result'
op|'('
name|'self'
op|','
name|'get_all_by_inst_uuids'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'get_all_by_inst_uuids'
op|'.'
name|'return_value'
op|'='
name|'None'
newline|'\n'
name|'bdm_list'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'get_by_instance_uuids'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
op|'['
string|"'fake-instance'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'0'
op|','
name|'len'
op|'('
name|'bdm_list'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance'"
op|')'
newline|'\n'
DECL|member|test_get_by_instance_uuid
name|'def'
name|'test_get_by_instance_uuid'
op|'('
name|'self'
op|','
name|'get_all_by_inst'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fakes'
op|'='
op|'['
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'123'
op|')'
op|','
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'456'
op|')'
op|']'
newline|'\n'
name|'get_all_by_inst'
op|'.'
name|'return_value'
op|'='
name|'fakes'
newline|'\n'
name|'bdm_list'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'get_by_instance_uuid'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-instance'"
op|')'
newline|'\n'
name|'for'
name|'faked'
op|','
name|'got'
name|'in'
name|'zip'
op|'('
name|'fakes'
op|','
name|'bdm_list'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'got'
op|','
name|'objects'
op|'.'
name|'BlockDeviceMapping'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'faked'
op|'['
string|"'id'"
op|']'
op|','
name|'got'
op|'.'
name|'id'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance'"
op|')'
newline|'\n'
DECL|member|test_get_by_instance_uuid_no_result
name|'def'
name|'test_get_by_instance_uuid_no_result'
op|'('
name|'self'
op|','
name|'get_all_by_inst'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'get_all_by_inst'
op|'.'
name|'return_value'
op|'='
name|'None'
newline|'\n'
name|'bdm_list'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'get_by_instance_uuid'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-instance'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'0'
op|','
name|'len'
op|'('
name|'bdm_list'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance'"
op|')'
newline|'\n'
DECL|member|test_root_bdm
name|'def'
name|'test_root_bdm'
op|'('
name|'self'
op|','
name|'get_all_by_inst'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fakes'
op|'='
op|'['
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'123'
op|')'
op|','
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'456'
op|','
name|'boot_index'
op|'='
number|'0'
op|')'
op|']'
newline|'\n'
name|'get_all_by_inst'
op|'.'
name|'return_value'
op|'='
name|'fakes'
newline|'\n'
name|'bdm_list'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'get_by_instance_uuid'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-instance'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'456'
op|','
name|'bdm_list'
op|'.'
name|'root_bdm'
op|'('
op|')'
op|'.'
name|'id'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance'"
op|')'
newline|'\n'
DECL|member|test_root_bdm_empty_bdm_list
name|'def'
name|'test_root_bdm_empty_bdm_list'
op|'('
name|'self'
op|','
name|'get_all_by_inst'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'get_all_by_inst'
op|'.'
name|'return_value'
op|'='
name|'None'
newline|'\n'
name|'bdm_list'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'get_by_instance_uuid'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-instance'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsNone'
op|'('
name|'bdm_list'
op|'.'
name|'root_bdm'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'db'
op|','
string|"'block_device_mapping_get_all_by_instance'"
op|')'
newline|'\n'
DECL|member|test_root_bdm_undefined
name|'def'
name|'test_root_bdm_undefined'
op|'('
name|'self'
op|','
name|'get_all_by_inst'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fakes'
op|'='
op|'['
nl|'\n'
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'123'
op|','
name|'instance_uuid'
op|'='
string|"'uuid_1'"
op|')'
op|','
nl|'\n'
name|'self'
op|'.'
name|'fake_bdm'
op|'('
number|'456'
op|','
name|'instance_uuid'
op|'='
string|"'uuid_2'"
op|')'
nl|'\n'
op|']'
newline|'\n'
name|'get_all_by_inst'
op|'.'
name|'return_value'
op|'='
name|'fakes'
newline|'\n'
name|'bdm_list'
op|'='
name|'objects'
op|'.'
name|'BlockDeviceMappingList'
op|'.'
name|'get_by_instance_uuid'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'context'
op|','
string|"'fake-instance'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'UndefinedRootBDM'
op|','
name|'bdm_list'
op|'.'
name|'root_bdm'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
dedent|''
name|'class'
name|'TestBlockDeviceMappingListObject'
op|'('
name|'test_objects'
op|'.'
name|'_LocalTest'
op|','
nl|'\n'
DECL|class|TestBlockDeviceMappingListObject
name|'_TestBlockDeviceMappingListObject'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
name|'class'
name|'TestRemoteBlockDeviceMappingListObject'
op|'('
nl|'\n'
DECL|class|TestRemoteBlockDeviceMappingListObject
name|'test_objects'
op|'.'
name|'_RemoteTest'
op|','
name|'_TestBlockDeviceMappingListObject'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|TestBlockDeviceUtils
dedent|''
name|'class'
name|'TestBlockDeviceUtils'
op|'('
name|'test'
op|'.'
name|'NoDBTestCase'
op|')'
op|':'
newline|'\n'
DECL|member|test_make_list_from_dicts
indent|'    '
name|'def'
name|'test_make_list_from_dicts'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'ctx'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'dicts'
op|'='
op|'['
op|'{'
string|"'id'"
op|':'
number|'1'
op|'}'
op|','
op|'{'
string|"'id'"
op|':'
number|'2'
op|'}'
op|']'
newline|'\n'
name|'objs'
op|'='
name|'block_device_obj'
op|'.'
name|'block_device_make_list_from_dicts'
op|'('
name|'ctx'
op|','
nl|'\n'
name|'dicts'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'objs'
op|','
name|'block_device_obj'
op|'.'
name|'BlockDeviceMappingList'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'2'
op|','
name|'len'
op|'('
name|'objs'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'1'
op|','
name|'objs'
op|'['
number|'0'
op|']'
op|'.'
name|'id'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'2'
op|','
name|'objs'
op|'['
number|'1'
op|']'
op|'.'
name|'id'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_make_list_from_dicts_empty
dedent|''
name|'def'
name|'test_make_list_from_dicts_empty'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'ctx'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'objs'
op|'='
name|'block_device_obj'
op|'.'
name|'block_device_make_list_from_dicts'
op|'('
name|'ctx'
op|','
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsInstance'
op|'('
name|'objs'
op|','
name|'block_device_obj'
op|'.'
name|'BlockDeviceMappingList'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
number|'0'
op|','
name|'len'
op|'('
name|'objs'
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
