begin_unit
comment|'# Copyright (c) 2011 OpenStack Foundation'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
name|'import'
name|'testtools'
newline|'\n'
name|'import'
name|'webob'
op|'.'
name|'exc'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'api'
op|'.'
name|'openstack'
op|'.'
name|'compute'
op|'.'
name|'contrib'
name|'import'
name|'hosts'
name|'as'
name|'os_hosts_v2'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'api'
op|'.'
name|'openstack'
op|'.'
name|'compute'
op|'.'
name|'plugins'
op|'.'
name|'v3'
name|'import'
name|'hosts'
name|'as'
name|'os_hosts_v21'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'compute'
name|'import'
name|'power_state'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'compute'
name|'import'
name|'vm_states'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'context'
name|'as'
name|'context_maker'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'db'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'test'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
op|'.'
name|'api'
op|'.'
name|'openstack'
name|'import'
name|'fakes'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
name|'import'
name|'fake_hosts'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|stub_service_get_all
name|'def'
name|'stub_service_get_all'
op|'('
name|'context'
op|','
name|'disabled'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'return'
name|'fake_hosts'
op|'.'
name|'SERVICES_LIST'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|stub_service_get_by_host_and_binary
dedent|''
name|'def'
name|'stub_service_get_by_host_and_binary'
op|'('
name|'context'
op|','
name|'host_name'
op|','
name|'binary'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'for'
name|'service'
name|'in'
name|'stub_service_get_all'
op|'('
name|'context'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'service'
op|'['
string|"'host'"
op|']'
op|'=='
name|'host_name'
name|'and'
name|'service'
op|'['
string|"'binary'"
op|']'
op|'=='
name|'binary'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'service'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|stub_set_host_enabled
dedent|''
dedent|''
dedent|''
name|'def'
name|'stub_set_host_enabled'
op|'('
name|'context'
op|','
name|'host_name'
op|','
name|'enabled'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Simulates three possible behaviours for VM drivers or compute\n    drivers when enabling or disabling a host.\n\n    \'enabled\' means new instances can go to this host\n    \'disabled\' means they can\'t\n    """'
newline|'\n'
name|'results'
op|'='
op|'{'
name|'True'
op|':'
string|'"enabled"'
op|','
name|'False'
op|':'
string|'"disabled"'
op|'}'
newline|'\n'
name|'if'
name|'host_name'
op|'=='
string|'"notimplemented"'
op|':'
newline|'\n'
comment|"# The vm driver for this host doesn't support this feature"
nl|'\n'
indent|'        '
name|'raise'
name|'NotImplementedError'
op|'('
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'host_name'
op|'=='
string|'"dummydest"'
op|':'
newline|'\n'
comment|'# The host does not exist'
nl|'\n'
indent|'        '
name|'raise'
name|'exception'
op|'.'
name|'ComputeHostNotFound'
op|'('
name|'host'
op|'='
name|'host_name'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'host_name'
op|'=='
string|'"service_not_available"'
op|':'
newline|'\n'
comment|'# The service is not available'
nl|'\n'
indent|'        '
name|'raise'
name|'exception'
op|'.'
name|'ComputeServiceUnavailable'
op|'('
name|'host'
op|'='
name|'host_name'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'host_name'
op|'=='
string|'"host_c2"'
op|':'
newline|'\n'
comment|'# Simulate a failure'
nl|'\n'
indent|'        '
name|'return'
name|'results'
op|'['
name|'not'
name|'enabled'
op|']'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
comment|'# Do the right thing'
nl|'\n'
indent|'        '
name|'return'
name|'results'
op|'['
name|'enabled'
op|']'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|stub_set_host_maintenance
dedent|''
dedent|''
name|'def'
name|'stub_set_host_maintenance'
op|'('
name|'context'
op|','
name|'host_name'
op|','
name|'mode'
op|')'
op|':'
newline|'\n'
comment|"# We'll simulate success and failure by assuming"
nl|'\n'
comment|"# that 'host_c1' always succeeds, and 'host_c2'"
nl|'\n'
comment|'# always fails'
nl|'\n'
indent|'    '
name|'results'
op|'='
op|'{'
name|'True'
op|':'
string|'"on_maintenance"'
op|','
name|'False'
op|':'
string|'"off_maintenance"'
op|'}'
newline|'\n'
name|'if'
name|'host_name'
op|'=='
string|'"notimplemented"'
op|':'
newline|'\n'
comment|"# The vm driver for this host doesn't support this feature"
nl|'\n'
indent|'        '
name|'raise'
name|'NotImplementedError'
op|'('
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'host_name'
op|'=='
string|'"dummydest"'
op|':'
newline|'\n'
comment|'# The host does not exist'
nl|'\n'
indent|'        '
name|'raise'
name|'exception'
op|'.'
name|'ComputeHostNotFound'
op|'('
name|'host'
op|'='
name|'host_name'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'host_name'
op|'=='
string|'"service_not_available"'
op|':'
newline|'\n'
comment|'# The service is not available'
nl|'\n'
indent|'        '
name|'raise'
name|'exception'
op|'.'
name|'ComputeServiceUnavailable'
op|'('
name|'host'
op|'='
name|'host_name'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'host_name'
op|'=='
string|'"host_c2"'
op|':'
newline|'\n'
comment|'# Simulate a failure'
nl|'\n'
indent|'        '
name|'return'
name|'results'
op|'['
name|'not'
name|'mode'
op|']'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
comment|'# Do the right thing'
nl|'\n'
indent|'        '
name|'return'
name|'results'
op|'['
name|'mode'
op|']'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|stub_host_power_action
dedent|''
dedent|''
name|'def'
name|'stub_host_power_action'
op|'('
name|'context'
op|','
name|'host_name'
op|','
name|'action'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'host_name'
op|'=='
string|'"notimplemented"'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'NotImplementedError'
op|'('
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'host_name'
op|'=='
string|'"dummydest"'
op|':'
newline|'\n'
comment|'# The host does not exist'
nl|'\n'
indent|'        '
name|'raise'
name|'exception'
op|'.'
name|'ComputeHostNotFound'
op|'('
name|'host'
op|'='
name|'host_name'
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'host_name'
op|'=='
string|'"service_not_available"'
op|':'
newline|'\n'
comment|'# The service is not available'
nl|'\n'
indent|'        '
name|'raise'
name|'exception'
op|'.'
name|'ComputeServiceUnavailable'
op|'('
name|'host'
op|'='
name|'host_name'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'action'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|_create_instance
dedent|''
name|'def'
name|'_create_instance'
op|'('
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Create a test instance."""'
newline|'\n'
name|'ctxt'
op|'='
name|'context_maker'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'return'
name|'db'
op|'.'
name|'instance_create'
op|'('
name|'ctxt'
op|','
name|'_create_instance_dict'
op|'('
op|'**'
name|'kwargs'
op|')'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|_create_instance_dict
dedent|''
name|'def'
name|'_create_instance_dict'
op|'('
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Create a dictionary for a test instance."""'
newline|'\n'
name|'inst'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'inst'
op|'['
string|"'image_ref'"
op|']'
op|'='
string|"'cedef40a-ed67-4d10-800e-17455edce175'"
newline|'\n'
name|'inst'
op|'['
string|"'reservation_id'"
op|']'
op|'='
string|"'r-fakeres'"
newline|'\n'
name|'inst'
op|'['
string|"'user_id'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'user_id'"
op|','
string|"'admin'"
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'project_id'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'project_id'"
op|','
string|"'fake'"
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'instance_type_id'"
op|']'
op|'='
string|"'1'"
newline|'\n'
name|'if'
string|"'host'"
name|'in'
name|'kwargs'
op|':'
newline|'\n'
indent|'        '
name|'inst'
op|'['
string|"'host'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'host'"
op|')'
newline|'\n'
dedent|''
name|'inst'
op|'['
string|"'vcpus'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'vcpus'"
op|','
number|'1'
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'memory_mb'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'memory_mb'"
op|','
number|'20'
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'root_gb'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'root_gb'"
op|','
number|'30'
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'ephemeral_gb'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'ephemeral_gb'"
op|','
number|'30'
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'vm_state'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'vm_state'"
op|','
name|'vm_states'
op|'.'
name|'ACTIVE'
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'power_state'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'power_state'"
op|','
name|'power_state'
op|'.'
name|'RUNNING'
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'task_state'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'task_state'"
op|','
name|'None'
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'availability_zone'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'availability_zone'"
op|','
name|'None'
op|')'
newline|'\n'
name|'inst'
op|'['
string|"'ami_launch_index'"
op|']'
op|'='
number|'0'
newline|'\n'
name|'inst'
op|'['
string|"'launched_on'"
op|']'
op|'='
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'launched_on'"
op|','
string|"'dummy'"
op|')'
newline|'\n'
name|'return'
name|'inst'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|FakeRequestWithNovaZone
dedent|''
name|'class'
name|'FakeRequestWithNovaZone'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
DECL|variable|environ
indent|'    '
name|'environ'
op|'='
op|'{'
string|'"nova.context"'
op|':'
name|'context_maker'
op|'.'
name|'get_admin_context'
op|'('
op|')'
op|'}'
newline|'\n'
DECL|variable|GET
name|'GET'
op|'='
op|'{'
string|'"zone"'
op|':'
string|'"nova"'
op|'}'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|FakeRequestWithNovaService
dedent|''
name|'class'
name|'FakeRequestWithNovaService'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
DECL|variable|environ
indent|'    '
name|'environ'
op|'='
op|'{'
string|'"nova.context"'
op|':'
name|'context_maker'
op|'.'
name|'get_admin_context'
op|'('
op|')'
op|'}'
newline|'\n'
DECL|variable|GET
name|'GET'
op|'='
op|'{'
string|'"service"'
op|':'
string|'"compute"'
op|'}'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|FakeRequestWithInvalidNovaService
dedent|''
name|'class'
name|'FakeRequestWithInvalidNovaService'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
DECL|variable|environ
indent|'    '
name|'environ'
op|'='
op|'{'
string|'"nova.context"'
op|':'
name|'context_maker'
op|'.'
name|'get_admin_context'
op|'('
op|')'
op|'}'
newline|'\n'
DECL|variable|GET
name|'GET'
op|'='
op|'{'
string|'"service"'
op|':'
string|'"invalid"'
op|'}'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|HostTestCaseV21
dedent|''
name|'class'
name|'HostTestCaseV21'
op|'('
name|'test'
op|'.'
name|'TestCase'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Test Case for hosts."""'
newline|'\n'
DECL|variable|validation_ex
name|'validation_ex'
op|'='
name|'exception'
op|'.'
name|'ValidationError'
newline|'\n'
DECL|variable|Controller
name|'Controller'
op|'='
name|'os_hosts_v21'
op|'.'
name|'HostController'
newline|'\n'
DECL|variable|policy_ex
name|'policy_ex'
op|'='
name|'exception'
op|'.'
name|'PolicyNotAuthorized'
newline|'\n'
nl|'\n'
DECL|member|_setup_stubs
name|'def'
name|'_setup_stubs'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# Pretend we have fake_hosts.HOST_LIST in the DB'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'stubs'
op|'.'
name|'Set'
op|'('
name|'db'
op|','
string|"'service_get_all'"
op|','
nl|'\n'
name|'stub_service_get_all'
op|')'
newline|'\n'
comment|'# Only hosts in our fake DB exist'
nl|'\n'
name|'self'
op|'.'
name|'stubs'
op|'.'
name|'Set'
op|'('
name|'db'
op|','
string|"'service_get_by_host_and_binary'"
op|','
nl|'\n'
name|'stub_service_get_by_host_and_binary'
op|')'
newline|'\n'
comment|"# 'host_c1' always succeeds, and 'host_c2'"
nl|'\n'
name|'self'
op|'.'
name|'stubs'
op|'.'
name|'Set'
op|'('
name|'self'
op|'.'
name|'hosts_api'
op|','
string|"'set_host_enabled'"
op|','
nl|'\n'
name|'stub_set_host_enabled'
op|')'
newline|'\n'
comment|"# 'host_c1' always succeeds, and 'host_c2'"
nl|'\n'
name|'self'
op|'.'
name|'stubs'
op|'.'
name|'Set'
op|'('
name|'self'
op|'.'
name|'hosts_api'
op|','
string|"'set_host_maintenance'"
op|','
nl|'\n'
name|'stub_set_host_maintenance'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'stubs'
op|'.'
name|'Set'
op|'('
name|'self'
op|'.'
name|'hosts_api'
op|','
string|"'host_power_action'"
op|','
nl|'\n'
name|'stub_host_power_action'
op|')'
newline|'\n'
nl|'\n'
DECL|member|setUp
dedent|''
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'super'
op|'('
name|'HostTestCaseV21'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'controller'
op|'='
name|'self'
op|'.'
name|'Controller'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'hosts_api'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'api'
newline|'\n'
name|'self'
op|'.'
name|'req'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequest'
op|'.'
name|'blank'
op|'('
string|"''"
op|','
name|'use_admin_context'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_setup_stubs'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|_test_host_update
dedent|''
name|'def'
name|'_test_host_update'
op|'('
name|'self'
op|','
name|'host'
op|','
name|'key'
op|','
name|'val'
op|','
name|'expected_value'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'body'
op|'='
op|'{'
name|'key'
op|':'
name|'val'
op|'}'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'host'
op|','
name|'body'
op|'='
name|'body'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'result'
op|'['
name|'key'
op|']'
op|','
name|'expected_value'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_list_hosts
dedent|''
name|'def'
name|'test_list_hosts'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Verify that the compute hosts are returned."""'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'index'
op|'('
name|'self'
op|'.'
name|'req'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIn'
op|'('
string|"'hosts'"
op|','
name|'result'
op|')'
newline|'\n'
name|'hosts'
op|'='
name|'result'
op|'['
string|"'hosts'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'fake_hosts'
op|'.'
name|'HOST_LIST'
op|','
name|'hosts'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_disable_host
dedent|''
name|'def'
name|'test_disable_host'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_update'
op|'('
string|"'host_c1'"
op|','
string|"'status'"
op|','
string|"'disable'"
op|','
string|"'disabled'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_host_update'
op|'('
string|"'host_c2'"
op|','
string|"'status'"
op|','
string|"'disable'"
op|','
string|"'enabled'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_enable_host
dedent|''
name|'def'
name|'test_enable_host'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_update'
op|'('
string|"'host_c1'"
op|','
string|"'status'"
op|','
string|"'enable'"
op|','
string|"'enabled'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_host_update'
op|'('
string|"'host_c2'"
op|','
string|"'status'"
op|','
string|"'enable'"
op|','
string|"'disabled'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_enable_maintenance
dedent|''
name|'def'
name|'test_enable_maintenance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_update'
op|'('
string|"'host_c1'"
op|','
string|"'maintenance_mode'"
op|','
nl|'\n'
string|"'enable'"
op|','
string|"'on_maintenance'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_disable_maintenance
dedent|''
name|'def'
name|'test_disable_maintenance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_update'
op|'('
string|"'host_c1'"
op|','
string|"'maintenance_mode'"
op|','
nl|'\n'
string|"'disable'"
op|','
string|"'off_maintenance'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|_test_host_update_notimpl
dedent|''
name|'def'
name|'_test_host_update_notimpl'
op|'('
name|'self'
op|','
name|'key'
op|','
name|'val'
op|')'
op|':'
newline|'\n'
DECL|function|stub_service_get_all_notimpl
indent|'        '
name|'def'
name|'stub_service_get_all_notimpl'
op|'('
name|'self'
op|','
name|'req'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'return'
op|'['
op|'{'
string|"'host'"
op|':'
string|"'notimplemented'"
op|','
string|"'topic'"
op|':'
name|'None'
op|','
nl|'\n'
string|"'availability_zone'"
op|':'
name|'None'
op|'}'
op|']'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'stubs'
op|'.'
name|'Set'
op|'('
name|'db'
op|','
string|"'service_get_all'"
op|','
nl|'\n'
name|'stub_service_get_all_notimpl'
op|')'
newline|'\n'
name|'body'
op|'='
op|'{'
name|'key'
op|':'
name|'val'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotImplemented'
op|','
nl|'\n'
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|','
nl|'\n'
name|'self'
op|'.'
name|'req'
op|','
string|"'notimplemented'"
op|','
name|'body'
op|'='
name|'body'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_disable_host_notimpl
dedent|''
name|'def'
name|'test_disable_host_notimpl'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_update_notimpl'
op|'('
string|"'status'"
op|','
string|"'disable'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_enable_maintenance_notimpl
dedent|''
name|'def'
name|'test_enable_maintenance_notimpl'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_update_notimpl'
op|'('
string|"'maintenance_mode'"
op|','
string|"'enable'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_startup
dedent|''
name|'def'
name|'test_host_startup'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'startup'
op|'('
name|'self'
op|'.'
name|'req'
op|','
string|'"host_c1"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'result'
op|'['
string|'"power_action"'
op|']'
op|','
string|'"startup"'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_shutdown
dedent|''
name|'def'
name|'test_host_shutdown'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'shutdown'
op|'('
name|'self'
op|'.'
name|'req'
op|','
string|'"host_c1"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'result'
op|'['
string|'"power_action"'
op|']'
op|','
string|'"shutdown"'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_reboot
dedent|''
name|'def'
name|'test_host_reboot'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'reboot'
op|'('
name|'self'
op|'.'
name|'req'
op|','
string|'"host_c1"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'result'
op|'['
string|'"power_action"'
op|']'
op|','
string|'"reboot"'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_test_host_power_action_notimpl
dedent|''
name|'def'
name|'_test_host_power_action_notimpl'
op|'('
name|'self'
op|','
name|'method'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotImplemented'
op|','
nl|'\n'
name|'method'
op|','
name|'self'
op|'.'
name|'req'
op|','
string|'"notimplemented"'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_startup_notimpl
dedent|''
name|'def'
name|'test_host_startup_notimpl'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_power_action_notimpl'
op|'('
name|'self'
op|'.'
name|'controller'
op|'.'
name|'startup'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_shutdown_notimpl
dedent|''
name|'def'
name|'test_host_shutdown_notimpl'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_power_action_notimpl'
op|'('
name|'self'
op|'.'
name|'controller'
op|'.'
name|'shutdown'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_reboot_notimpl
dedent|''
name|'def'
name|'test_host_reboot_notimpl'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_host_power_action_notimpl'
op|'('
name|'self'
op|'.'
name|'controller'
op|'.'
name|'reboot'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_status_bad_host
dedent|''
name|'def'
name|'test_host_status_bad_host'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# A host given as an argument does not exist.'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'True'
newline|'\n'
name|'dest'
op|'='
string|"'dummydest'"
newline|'\n'
name|'with'
name|'testtools'
op|'.'
name|'ExpectedException'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|','
nl|'\n'
string|'".*%s.*"'
op|'%'
name|'dest'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'dest'
op|','
name|'body'
op|'='
op|'{'
string|"'status'"
op|':'
string|"'enable'"
op|'}'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_maintenance_bad_host
dedent|''
dedent|''
name|'def'
name|'test_host_maintenance_bad_host'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# A host given as an argument does not exist.'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'True'
newline|'\n'
name|'dest'
op|'='
string|"'dummydest'"
newline|'\n'
name|'with'
name|'testtools'
op|'.'
name|'ExpectedException'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|','
nl|'\n'
string|'".*%s.*"'
op|'%'
name|'dest'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'dest'
op|','
nl|'\n'
name|'body'
op|'='
op|'{'
string|"'maintenance_mode'"
op|':'
string|"'enable'"
op|'}'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_power_action_bad_host
dedent|''
dedent|''
name|'def'
name|'test_host_power_action_bad_host'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# A host given as an argument does not exist.'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'True'
newline|'\n'
name|'dest'
op|'='
string|"'dummydest'"
newline|'\n'
name|'with'
name|'testtools'
op|'.'
name|'ExpectedException'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|','
nl|'\n'
string|'".*%s.*"'
op|'%'
name|'dest'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'controller'
op|'.'
name|'reboot'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'dest'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_status_bad_status
dedent|''
dedent|''
name|'def'
name|'test_host_status_bad_status'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# A host given as an argument does not exist.'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'True'
newline|'\n'
name|'dest'
op|'='
string|"'service_not_available'"
newline|'\n'
name|'with'
name|'testtools'
op|'.'
name|'ExpectedException'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|','
nl|'\n'
string|'".*%s.*"'
op|'%'
name|'dest'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'dest'
op|','
name|'body'
op|'='
op|'{'
string|"'status'"
op|':'
string|"'enable'"
op|'}'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_maintenance_bad_status
dedent|''
dedent|''
name|'def'
name|'test_host_maintenance_bad_status'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# A host given as an argument does not exist.'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'True'
newline|'\n'
name|'dest'
op|'='
string|"'service_not_available'"
newline|'\n'
name|'with'
name|'testtools'
op|'.'
name|'ExpectedException'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|','
nl|'\n'
string|'".*%s.*"'
op|'%'
name|'dest'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'dest'
op|','
nl|'\n'
name|'body'
op|'='
op|'{'
string|"'maintenance_mode'"
op|':'
string|"'enable'"
op|'}'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_power_action_bad_status
dedent|''
dedent|''
name|'def'
name|'test_host_power_action_bad_status'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# A host given as an argument does not exist.'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'True'
newline|'\n'
name|'dest'
op|'='
string|"'service_not_available'"
newline|'\n'
name|'with'
name|'testtools'
op|'.'
name|'ExpectedException'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|','
nl|'\n'
string|'".*%s.*"'
op|'%'
name|'dest'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'controller'
op|'.'
name|'reboot'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'dest'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_bad_status_value
dedent|''
dedent|''
name|'def'
name|'test_bad_status_value'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bad_body'
op|'='
op|'{'
string|'"status"'
op|':'
string|'"bad"'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'self'
op|'.'
name|'validation_ex'
op|','
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|','
nl|'\n'
name|'self'
op|'.'
name|'req'
op|','
string|'"host_c1"'
op|','
name|'body'
op|'='
name|'bad_body'
op|')'
newline|'\n'
name|'bad_body2'
op|'='
op|'{'
string|'"status"'
op|':'
string|'"disablabc"'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'self'
op|'.'
name|'validation_ex'
op|','
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|','
nl|'\n'
name|'self'
op|'.'
name|'req'
op|','
string|'"host_c1"'
op|','
name|'body'
op|'='
name|'bad_body2'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_bad_update_key
dedent|''
name|'def'
name|'test_bad_update_key'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bad_body'
op|'='
op|'{'
string|'"crazy"'
op|':'
string|'"bad"'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'self'
op|'.'
name|'validation_ex'
op|','
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|','
nl|'\n'
name|'self'
op|'.'
name|'req'
op|','
string|'"host_c1"'
op|','
name|'body'
op|'='
name|'bad_body'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_bad_update_key_and_correct_update_key
dedent|''
name|'def'
name|'test_bad_update_key_and_correct_update_key'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'bad_body'
op|'='
op|'{'
string|'"status"'
op|':'
string|'"disable"'
op|','
string|'"crazy"'
op|':'
string|'"bad"'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'self'
op|'.'
name|'validation_ex'
op|','
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|','
nl|'\n'
name|'self'
op|'.'
name|'req'
op|','
string|'"host_c1"'
op|','
name|'body'
op|'='
name|'bad_body'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_good_update_keys
dedent|''
name|'def'
name|'test_good_update_keys'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'body'
op|'='
op|'{'
string|'"status"'
op|':'
string|'"disable"'
op|','
string|'"maintenance_mode"'
op|':'
string|'"enable"'
op|'}'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'update'
op|'('
name|'self'
op|'.'
name|'req'
op|','
string|"'host_c1'"
op|','
name|'body'
op|'='
name|'body'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'result'
op|'['
string|'"host"'
op|']'
op|','
string|'"host_c1"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'result'
op|'['
string|'"status"'
op|']'
op|','
string|'"disabled"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'result'
op|'['
string|'"maintenance_mode"'
op|']'
op|','
string|'"on_maintenance"'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show_forbidden
dedent|''
name|'def'
name|'test_show_forbidden'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'False'
newline|'\n'
name|'dest'
op|'='
string|"'dummydest'"
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'self'
op|'.'
name|'policy_ex'
op|','
nl|'\n'
name|'self'
op|'.'
name|'controller'
op|'.'
name|'show'
op|','
nl|'\n'
name|'self'
op|'.'
name|'req'
op|','
name|'dest'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'True'
newline|'\n'
nl|'\n'
DECL|member|test_show_host_not_exist
dedent|''
name|'def'
name|'test_show_host_not_exist'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# A host given as an argument does not exist.'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'is_admin'
op|'='
name|'True'
newline|'\n'
name|'dest'
op|'='
string|"'dummydest'"
newline|'\n'
name|'with'
name|'testtools'
op|'.'
name|'ExpectedException'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|','
nl|'\n'
string|'".*%s.*"'
op|'%'
name|'dest'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'controller'
op|'.'
name|'show'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'dest'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_create_compute_service
dedent|''
dedent|''
name|'def'
name|'_create_compute_service'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Create compute-manager(ComputeNode and Service record)."""'
newline|'\n'
name|'ctxt'
op|'='
name|'self'
op|'.'
name|'req'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
newline|'\n'
name|'dic'
op|'='
op|'{'
string|"'host'"
op|':'
string|"'dummy'"
op|','
string|"'binary'"
op|':'
string|"'nova-compute'"
op|','
string|"'topic'"
op|':'
string|"'compute'"
op|','
nl|'\n'
string|"'report_count'"
op|':'
number|'0'
op|'}'
newline|'\n'
name|'s_ref'
op|'='
name|'db'
op|'.'
name|'service_create'
op|'('
name|'ctxt'
op|','
name|'dic'
op|')'
newline|'\n'
nl|'\n'
name|'dic'
op|'='
op|'{'
string|"'service_id'"
op|':'
name|'s_ref'
op|'['
string|"'id'"
op|']'
op|','
nl|'\n'
string|"'host'"
op|':'
name|'s_ref'
op|'['
string|"'host'"
op|']'
op|','
nl|'\n'
string|"'vcpus'"
op|':'
number|'16'
op|','
string|"'memory_mb'"
op|':'
number|'32'
op|','
string|"'local_gb'"
op|':'
number|'100'
op|','
nl|'\n'
string|"'vcpus_used'"
op|':'
number|'16'
op|','
string|"'memory_mb_used'"
op|':'
number|'32'
op|','
string|"'local_gb_used'"
op|':'
number|'10'
op|','
nl|'\n'
string|"'hypervisor_type'"
op|':'
string|"'qemu'"
op|','
string|"'hypervisor_version'"
op|':'
number|'12003'
op|','
nl|'\n'
string|"'cpu_info'"
op|':'
string|"''"
op|','
string|"'stats'"
op|':'
string|"''"
op|'}'
newline|'\n'
name|'db'
op|'.'
name|'compute_node_create'
op|'('
name|'ctxt'
op|','
name|'dic'
op|')'
newline|'\n'
nl|'\n'
name|'return'
name|'db'
op|'.'
name|'service_get'
op|'('
name|'ctxt'
op|','
name|'s_ref'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show_no_project
dedent|''
name|'def'
name|'test_show_no_project'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""No instances are running on the given host."""'
newline|'\n'
name|'ctxt'
op|'='
name|'context_maker'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'s_ref'
op|'='
name|'self'
op|'.'
name|'_create_compute_service'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'show'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'s_ref'
op|'['
string|"'host'"
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'proj'
op|'='
op|'['
string|"'(total)'"
op|','
string|"'(used_now)'"
op|','
string|"'(used_max)'"
op|']'
newline|'\n'
name|'column'
op|'='
op|'['
string|"'host'"
op|','
string|"'project'"
op|','
string|"'cpu'"
op|','
string|"'memory_mb'"
op|','
string|"'disk_gb'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'result'
op|'['
string|"'host'"
op|']'
op|')'
op|','
number|'3'
op|')'
newline|'\n'
name|'for'
name|'resource'
name|'in'
name|'result'
op|'['
string|"'host'"
op|']'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertIn'
op|'('
name|'resource'
op|'['
string|"'resource'"
op|']'
op|'['
string|"'project'"
op|']'
op|','
name|'proj'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'resource'
op|'['
string|"'resource'"
op|']'
op|')'
op|','
number|'5'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'set'
op|'('
name|'column'
op|')'
op|','
name|'set'
op|'('
name|'resource'
op|'['
string|"'resource'"
op|']'
op|'.'
name|'keys'
op|'('
op|')'
op|')'
op|')'
newline|'\n'
dedent|''
name|'db'
op|'.'
name|'service_destroy'
op|'('
name|'ctxt'
op|','
name|'s_ref'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show_works_correctly
dedent|''
name|'def'
name|'test_show_works_correctly'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""show() works correctly as expected."""'
newline|'\n'
name|'ctxt'
op|'='
name|'context_maker'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'s_ref'
op|'='
name|'self'
op|'.'
name|'_create_compute_service'
op|'('
op|')'
newline|'\n'
name|'i_ref1'
op|'='
name|'_create_instance'
op|'('
name|'project_id'
op|'='
string|"'p-01'"
op|','
name|'host'
op|'='
name|'s_ref'
op|'['
string|"'host'"
op|']'
op|')'
newline|'\n'
name|'i_ref2'
op|'='
name|'_create_instance'
op|'('
name|'project_id'
op|'='
string|"'p-02'"
op|','
name|'vcpus'
op|'='
number|'3'
op|','
nl|'\n'
name|'host'
op|'='
name|'s_ref'
op|'['
string|"'host'"
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'show'
op|'('
name|'self'
op|'.'
name|'req'
op|','
name|'s_ref'
op|'['
string|"'host'"
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'proj'
op|'='
op|'['
string|"'(total)'"
op|','
string|"'(used_now)'"
op|','
string|"'(used_max)'"
op|','
string|"'p-01'"
op|','
string|"'p-02'"
op|']'
newline|'\n'
name|'column'
op|'='
op|'['
string|"'host'"
op|','
string|"'project'"
op|','
string|"'cpu'"
op|','
string|"'memory_mb'"
op|','
string|"'disk_gb'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'result'
op|'['
string|"'host'"
op|']'
op|')'
op|','
number|'5'
op|')'
newline|'\n'
name|'for'
name|'resource'
name|'in'
name|'result'
op|'['
string|"'host'"
op|']'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertIn'
op|'('
name|'resource'
op|'['
string|"'resource'"
op|']'
op|'['
string|"'project'"
op|']'
op|','
name|'proj'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'resource'
op|'['
string|"'resource'"
op|']'
op|')'
op|','
number|'5'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'set'
op|'('
name|'column'
op|')'
op|','
name|'set'
op|'('
name|'resource'
op|'['
string|"'resource'"
op|']'
op|'.'
name|'keys'
op|'('
op|')'
op|')'
op|')'
newline|'\n'
dedent|''
name|'db'
op|'.'
name|'service_destroy'
op|'('
name|'ctxt'
op|','
name|'s_ref'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'db'
op|'.'
name|'instance_destroy'
op|'('
name|'ctxt'
op|','
name|'i_ref1'
op|'['
string|"'uuid'"
op|']'
op|')'
newline|'\n'
name|'db'
op|'.'
name|'instance_destroy'
op|'('
name|'ctxt'
op|','
name|'i_ref2'
op|'['
string|"'uuid'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_list_hosts_with_zone
dedent|''
name|'def'
name|'test_list_hosts_with_zone'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'index'
op|'('
name|'FakeRequestWithNovaZone'
op|'('
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIn'
op|'('
string|"'hosts'"
op|','
name|'result'
op|')'
newline|'\n'
name|'hosts'
op|'='
name|'result'
op|'['
string|"'hosts'"
op|']'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'fake_hosts'
op|'.'
name|'HOST_LIST_NOVA_ZONE'
op|','
name|'hosts'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_list_hosts_with_service
dedent|''
name|'def'
name|'test_list_hosts_with_service'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'index'
op|'('
name|'FakeRequestWithNovaService'
op|'('
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'fake_hosts'
op|'.'
name|'HOST_LIST_NOVA_ZONE'
op|','
name|'result'
op|'['
string|"'hosts'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_list_hosts_with_invalid_service
dedent|''
name|'def'
name|'test_list_hosts_with_invalid_service'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'result'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'index'
op|'('
name|'FakeRequestWithInvalidNovaService'
op|'('
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
op|'['
op|']'
op|','
name|'result'
op|'['
string|"'hosts'"
op|']'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|HostTestCaseV20
dedent|''
dedent|''
name|'class'
name|'HostTestCaseV20'
op|'('
name|'HostTestCaseV21'
op|')'
op|':'
newline|'\n'
DECL|variable|validation_ex
indent|'    '
name|'validation_ex'
op|'='
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
newline|'\n'
DECL|variable|policy_ex
name|'policy_ex'
op|'='
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPForbidden'
newline|'\n'
DECL|variable|Controller
name|'Controller'
op|'='
name|'os_hosts_v2'
op|'.'
name|'HostController'
newline|'\n'
nl|'\n'
comment|"# Note: V2 api don't support list with services"
nl|'\n'
DECL|member|test_list_hosts_with_service
name|'def'
name|'test_list_hosts_with_service'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'pass'
newline|'\n'
nl|'\n'
DECL|member|test_list_hosts_with_invalid_service
dedent|''
name|'def'
name|'test_list_hosts_with_invalid_service'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'pass'
newline|'\n'
nl|'\n'
DECL|member|test_list_hosts_with_non_admin
dedent|''
name|'def'
name|'test_list_hosts_with_non_admin'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'exception'
op|'.'
name|'AdminRequired'
op|','
nl|'\n'
name|'self'
op|'.'
name|'controller'
op|'.'
name|'index'
op|','
name|'fakes'
op|'.'
name|'HTTPRequest'
op|'.'
name|'blank'
op|'('
string|"''"
op|')'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|HostsPolicyEnforcementV21
dedent|''
dedent|''
name|'class'
name|'HostsPolicyEnforcementV21'
op|'('
name|'test'
op|'.'
name|'NoDBTestCase'
op|')'
op|':'
newline|'\n'
nl|'\n'
DECL|member|setUp
indent|'    '
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'super'
op|'('
name|'HostsPolicyEnforcementV21'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'controller'
op|'='
name|'os_hosts_v21'
op|'.'
name|'HostController'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'req'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequest'
op|'.'
name|'blank'
op|'('
string|"''"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_index_policy_failed
dedent|''
name|'def'
name|'test_index_policy_failed'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'rule_name'
op|'='
string|'"compute_extension:v3:os-hosts"'
newline|'\n'
name|'self'
op|'.'
name|'policy'
op|'.'
name|'set_rules'
op|'('
op|'{'
name|'rule_name'
op|':'
string|'"project_id:non_fake"'
op|'}'
op|')'
newline|'\n'
name|'exc'
op|'='
name|'self'
op|'.'
name|'assertRaises'
op|'('
nl|'\n'
name|'exception'
op|'.'
name|'PolicyNotAuthorized'
op|','
nl|'\n'
name|'self'
op|'.'
name|'controller'
op|'.'
name|'index'
op|','
name|'self'
op|'.'
name|'req'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
nl|'\n'
string|'"Policy doesn\'t allow %s to be performed."'
op|'%'
name|'rule_name'
op|','
nl|'\n'
name|'exc'
op|'.'
name|'format_message'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
