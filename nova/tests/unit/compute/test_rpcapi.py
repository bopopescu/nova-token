begin_unit
comment|'# Copyright 2012, Red Hat, Inc.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
string|'"""\nUnit Tests for nova.compute.rpcapi\n"""'
newline|'\n'
nl|'\n'
name|'import'
name|'contextlib'
newline|'\n'
nl|'\n'
name|'import'
name|'mock'
newline|'\n'
name|'from'
name|'oslo_config'
name|'import'
name|'cfg'
newline|'\n'
name|'from'
name|'oslo_serialization'
name|'import'
name|'jsonutils'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'compute'
name|'import'
name|'rpcapi'
name|'as'
name|'compute_rpcapi'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'context'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'objects'
name|'import'
name|'block_device'
name|'as'
name|'objects_block_dev'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'objects'
name|'import'
name|'compute_node'
name|'as'
name|'objects_compute_node'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'objects'
name|'import'
name|'network_request'
name|'as'
name|'objects_network_request'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'objects'
name|'import'
name|'numa'
name|'as'
name|'objects_numa'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'test'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
name|'import'
name|'fake_block_device'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'unit'
name|'import'
name|'fake_instance'
newline|'\n'
nl|'\n'
DECL|variable|CONF
name|'CONF'
op|'='
name|'cfg'
op|'.'
name|'CONF'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|ComputeRpcAPITestCase
name|'class'
name|'ComputeRpcAPITestCase'
op|'('
name|'test'
op|'.'
name|'NoDBTestCase'
op|')'
op|':'
newline|'\n'
nl|'\n'
DECL|member|setUp
indent|'    '
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'super'
op|'('
name|'ComputeRpcAPITestCase'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'context'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'instance_attr'
op|'='
op|'{'
string|"'host'"
op|':'
string|"'fake_host'"
op|','
nl|'\n'
string|"'instance_type_id'"
op|':'
number|'1'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'fake_instance_obj'
op|'='
name|'fake_instance'
op|'.'
name|'fake_instance_obj'
op|'('
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
op|'**'
name|'instance_attr'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'fake_instance'
op|'='
name|'jsonutils'
op|'.'
name|'to_primitive'
op|'('
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'fake_volume_bdm'
op|'='
name|'jsonutils'
op|'.'
name|'to_primitive'
op|'('
nl|'\n'
name|'fake_block_device'
op|'.'
name|'FakeDbBlockDeviceDict'
op|'('
nl|'\n'
op|'{'
string|"'source_type'"
op|':'
string|"'volume'"
op|','
string|"'destination_type'"
op|':'
string|"'volume'"
op|','
nl|'\n'
string|"'instance_uuid'"
op|':'
name|'self'
op|'.'
name|'fake_instance'
op|'['
string|"'uuid'"
op|']'
op|','
nl|'\n'
string|"'volume_id'"
op|':'
string|"'fake-volume-id'"
op|'}'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_serialized_instance_has_name
dedent|''
name|'def'
name|'test_serialized_instance_has_name'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'assertIn'
op|'('
string|"'name'"
op|','
name|'self'
op|'.'
name|'fake_instance'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_test_compute_api
dedent|''
name|'def'
name|'_test_compute_api'
op|'('
name|'self'
op|','
name|'method'
op|','
name|'rpc_method'
op|','
nl|'\n'
name|'assert_dict'
op|'='
name|'False'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'RequestContext'
op|'('
string|"'fake_user'"
op|','
string|"'fake_project'"
op|')'
newline|'\n'
nl|'\n'
name|'rpcapi'
op|'='
name|'kwargs'
op|'.'
name|'pop'
op|'('
string|"'rpcapi_class'"
op|','
name|'compute_rpcapi'
op|'.'
name|'ComputeAPI'
op|')'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsNotNone'
op|'('
name|'rpcapi'
op|'.'
name|'client'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'rpcapi'
op|'.'
name|'client'
op|'.'
name|'target'
op|'.'
name|'topic'
op|','
name|'CONF'
op|'.'
name|'compute_topic'
op|')'
newline|'\n'
nl|'\n'
name|'orig_prepare'
op|'='
name|'rpcapi'
op|'.'
name|'client'
op|'.'
name|'prepare'
newline|'\n'
comment|'# TODO(danms): Remove this special case when we drop 3.x'
nl|'\n'
name|'if'
name|'CONF'
op|'.'
name|'upgrade_levels'
op|'.'
name|'compute'
op|'=='
string|"'kilo'"
op|':'
newline|'\n'
indent|'            '
name|'base_version'
op|'='
string|"'3.0'"
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'base_version'
op|'='
name|'rpcapi'
op|'.'
name|'client'
op|'.'
name|'target'
op|'.'
name|'version'
newline|'\n'
dedent|''
name|'expected_version'
op|'='
name|'kwargs'
op|'.'
name|'pop'
op|'('
string|"'version'"
op|','
name|'base_version'
op|')'
newline|'\n'
name|'nova_network'
op|'='
name|'kwargs'
op|'.'
name|'pop'
op|'('
string|"'nova_network'"
op|','
name|'False'
op|')'
newline|'\n'
nl|'\n'
name|'expected_kwargs'
op|'='
name|'kwargs'
op|'.'
name|'copy'
op|'('
op|')'
newline|'\n'
name|'if'
op|'('
string|"'requested_networks'"
name|'in'
name|'expected_kwargs'
name|'and'
nl|'\n'
name|'expected_version'
op|'=='
string|"'3.23'"
op|')'
op|':'
newline|'\n'
indent|'            '
name|'expected_kwargs'
op|'['
string|"'requested_networks'"
op|']'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'requested_network'
name|'in'
name|'kwargs'
op|'['
string|"'requested_networks'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'not'
name|'nova_network'
op|':'
newline|'\n'
indent|'                    '
name|'expected_kwargs'
op|'['
string|"'requested_networks'"
op|']'
op|'.'
name|'append'
op|'('
nl|'\n'
op|'('
name|'requested_network'
op|'.'
name|'network_id'
op|','
nl|'\n'
name|'str'
op|'('
name|'requested_network'
op|'.'
name|'address'
op|')'
op|','
nl|'\n'
name|'requested_network'
op|'.'
name|'port_id'
op|')'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                    '
name|'expected_kwargs'
op|'['
string|"'requested_networks'"
op|']'
op|'.'
name|'append'
op|'('
nl|'\n'
op|'('
name|'requested_network'
op|'.'
name|'network_id'
op|','
nl|'\n'
name|'str'
op|'('
name|'requested_network'
op|'.'
name|'address'
op|')'
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'if'
string|"'host_param'"
name|'in'
name|'expected_kwargs'
op|':'
newline|'\n'
indent|'            '
name|'expected_kwargs'
op|'['
string|"'host'"
op|']'
op|'='
name|'expected_kwargs'
op|'.'
name|'pop'
op|'('
string|"'host_param'"
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'expected_kwargs'
op|'.'
name|'pop'
op|'('
string|"'host'"
op|','
name|'None'
op|')'
newline|'\n'
dedent|''
name|'if'
string|"'legacy_limits'"
name|'in'
name|'expected_kwargs'
op|':'
newline|'\n'
indent|'            '
name|'expected_kwargs'
op|'['
string|"'limits'"
op|']'
op|'='
name|'expected_kwargs'
op|'.'
name|'pop'
op|'('
string|"'legacy_limits'"
op|')'
newline|'\n'
name|'kwargs'
op|'.'
name|'pop'
op|'('
string|"'legacy_limits'"
op|','
name|'None'
op|')'
newline|'\n'
dedent|''
name|'expected_kwargs'
op|'.'
name|'pop'
op|'('
string|"'destination'"
op|','
name|'None'
op|')'
newline|'\n'
nl|'\n'
name|'if'
string|"'mountpoint'"
name|'in'
name|'expected_kwargs'
name|'and'
name|'expected_version'
op|'=='
string|"'4.0'"
op|':'
newline|'\n'
comment|'# TODO(danms): Remove me when we drop 3.x'
nl|'\n'
indent|'            '
name|'del'
name|'expected_kwargs'
op|'['
string|"'mountpoint'"
op|']'
newline|'\n'
name|'del'
name|'expected_kwargs'
op|'['
string|"'volume_id'"
op|']'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'assert_dict'
op|':'
newline|'\n'
indent|'            '
name|'expected_kwargs'
op|'['
string|"'instance'"
op|']'
op|'='
name|'jsonutils'
op|'.'
name|'to_primitive'
op|'('
nl|'\n'
name|'expected_kwargs'
op|'['
string|"'instance'"
op|']'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'cast_and_call'
op|'='
op|'['
string|"'confirm_resize'"
op|','
string|"'stop_instance'"
op|']'
newline|'\n'
name|'if'
name|'rpc_method'
op|'=='
string|"'call'"
name|'and'
name|'method'
name|'in'
name|'cast_and_call'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'method'
op|'=='
string|"'confirm_resize'"
op|':'
newline|'\n'
indent|'                '
name|'kwargs'
op|'['
string|"'cast'"
op|']'
op|'='
name|'False'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'kwargs'
op|'['
string|"'do_cast'"
op|']'
op|'='
name|'False'
newline|'\n'
dedent|''
dedent|''
name|'if'
string|"'host'"
name|'in'
name|'kwargs'
op|':'
newline|'\n'
indent|'            '
name|'host'
op|'='
name|'kwargs'
op|'['
string|"'host'"
op|']'
newline|'\n'
dedent|''
name|'elif'
string|"'destination'"
name|'in'
name|'kwargs'
op|':'
newline|'\n'
indent|'            '
name|'host'
op|'='
name|'kwargs'
op|'['
string|"'destination'"
op|']'
newline|'\n'
dedent|''
name|'elif'
string|"'instances'"
name|'in'
name|'kwargs'
op|':'
newline|'\n'
indent|'            '
name|'host'
op|'='
name|'kwargs'
op|'['
string|"'instances'"
op|']'
op|'['
number|'0'
op|']'
op|'['
string|"'host'"
op|']'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'host'
op|'='
name|'kwargs'
op|'['
string|"'instance'"
op|']'
op|'['
string|"'host'"
op|']'
newline|'\n'
nl|'\n'
dedent|''
name|'with'
name|'contextlib'
op|'.'
name|'nested'
op|'('
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'rpcapi'
op|'.'
name|'client'
op|','
name|'rpc_method'
op|')'
op|','
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'rpcapi'
op|'.'
name|'client'
op|','
string|"'prepare'"
op|')'
op|','
nl|'\n'
name|'mock'
op|'.'
name|'patch'
op|'.'
name|'object'
op|'('
name|'rpcapi'
op|'.'
name|'client'
op|','
string|"'can_send_version'"
op|')'
op|','
nl|'\n'
op|')'
name|'as'
op|'('
nl|'\n'
name|'rpc_mock'
op|','
name|'prepare_mock'
op|','
name|'csv_mock'
nl|'\n'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'prepare_mock'
op|'.'
name|'return_value'
op|'='
name|'rpcapi'
op|'.'
name|'client'
newline|'\n'
name|'if'
string|"'_return_value'"
name|'in'
name|'kwargs'
op|':'
newline|'\n'
indent|'                '
name|'rpc_mock'
op|'.'
name|'return_value'
op|'='
name|'kwargs'
op|'.'
name|'pop'
op|'('
string|"'_return_value'"
op|')'
newline|'\n'
name|'del'
name|'expected_kwargs'
op|'['
string|"'_return_value'"
op|']'
newline|'\n'
dedent|''
name|'elif'
string|"'return_bdm_object'"
name|'in'
name|'kwargs'
op|':'
newline|'\n'
indent|'                '
name|'del'
name|'kwargs'
op|'['
string|"'return_bdm_object'"
op|']'
newline|'\n'
name|'rpc_mock'
op|'.'
name|'return_value'
op|'='
name|'objects_block_dev'
op|'.'
name|'BlockDeviceMapping'
op|'('
op|')'
newline|'\n'
dedent|''
name|'elif'
name|'rpc_method'
op|'=='
string|"'call'"
op|':'
newline|'\n'
indent|'                '
name|'rpc_mock'
op|'.'
name|'return_value'
op|'='
string|"'foo'"
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'rpc_mock'
op|'.'
name|'return_value'
op|'='
name|'None'
newline|'\n'
dedent|''
name|'csv_mock'
op|'.'
name|'side_effect'
op|'='
op|'('
nl|'\n'
name|'lambda'
name|'v'
op|':'
name|'orig_prepare'
op|'('
name|'version'
op|'='
name|'v'
op|')'
op|'.'
name|'can_send_version'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'retval'
op|'='
name|'getattr'
op|'('
name|'rpcapi'
op|','
name|'method'
op|')'
op|'('
name|'ctxt'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'retval'
op|','
name|'rpc_mock'
op|'.'
name|'return_value'
op|')'
newline|'\n'
nl|'\n'
name|'prepare_mock'
op|'.'
name|'assert_called_once_with'
op|'('
name|'version'
op|'='
name|'expected_version'
op|','
nl|'\n'
name|'server'
op|'='
name|'host'
op|')'
newline|'\n'
name|'rpc_mock'
op|'.'
name|'assert_called_once_with'
op|'('
name|'ctxt'
op|','
name|'method'
op|','
op|'**'
name|'expected_kwargs'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_add_aggregate_host
dedent|''
dedent|''
name|'def'
name|'test_add_aggregate_host'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'add_aggregate_host'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'aggregate'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
name|'host_param'
op|'='
string|"'host'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'slave_info'
op|'='
op|'{'
op|'}'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'add_aggregate_host'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'aggregate'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
name|'host_param'
op|'='
string|"'host'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'slave_info'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_add_fixed_ip_to_instance
dedent|''
name|'def'
name|'test_add_fixed_ip_to_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'add_fixed_ip_to_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'network_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'add_fixed_ip_to_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'network_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.12'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_attach_interface
dedent|''
name|'def'
name|'test_attach_interface'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'attach_interface'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'network_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'port_id'
op|'='
string|"'id2'"
op|','
name|'version'
op|'='
string|"'4.0'"
op|','
name|'requested_ip'
op|'='
string|"'192.168.1.50'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'attach_interface'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'network_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'port_id'
op|'='
string|"'id2'"
op|','
name|'version'
op|'='
string|"'3.17'"
op|','
name|'requested_ip'
op|'='
string|"'192.168.1.50'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_attach_volume
dedent|''
name|'def'
name|'test_attach_volume'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'attach_volume'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'volume_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'mountpoint'
op|'='
string|"'mp'"
op|','
name|'bdm'
op|'='
name|'self'
op|'.'
name|'fake_volume_bdm'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'attach_volume'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'volume_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'mountpoint'
op|'='
string|"'mp'"
op|','
name|'bdm'
op|'='
name|'self'
op|'.'
name|'fake_volume_bdm'
op|','
name|'version'
op|'='
string|"'3.16'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_change_instance_metadata
dedent|''
name|'def'
name|'test_change_instance_metadata'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'change_instance_metadata'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'diff'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'change_instance_metadata'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'diff'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'3.7'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.compute.rpcapi.ComputeAPI._warn_buggy_live_migrations'"
op|')'
newline|'\n'
DECL|member|test_check_can_live_migrate_destination
name|'def'
name|'test_check_can_live_migrate_destination'
op|'('
name|'self'
op|','
name|'mock_warn'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'check_can_live_migrate_destination'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'destination'
op|'='
string|"'dest'"
op|','
name|'block_migration'
op|'='
name|'True'
op|','
nl|'\n'
name|'disk_over_commit'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'check_can_live_migrate_destination'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'destination'
op|'='
string|"'dest'"
op|','
name|'block_migration'
op|'='
name|'True'
op|','
nl|'\n'
name|'disk_over_commit'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.32'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'mock_warn'
op|'.'
name|'called'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.compute.rpcapi.ComputeAPI._warn_buggy_live_migrations'"
op|')'
newline|'\n'
DECL|member|test_check_can_live_migrate_destination_old_warning
name|'def'
name|'test_check_can_live_migrate_destination_old_warning'
op|'('
name|'self'
op|','
name|'mock_warn'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'check_can_live_migrate_destination'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'destination'
op|'='
string|"'dest'"
op|','
name|'block_migration'
op|'='
name|'True'
op|','
nl|'\n'
name|'disk_over_commit'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'mock_warn'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.compute.rpcapi.ComputeAPI._warn_buggy_live_migrations'"
op|')'
newline|'\n'
DECL|member|test_check_can_live_migrate_source
name|'def'
name|'test_check_can_live_migrate_source'
op|'('
name|'self'
op|','
name|'mock_warn'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'check_can_live_migrate_source'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'dest_check_data'
op|'='
op|'{'
string|'"test"'
op|':'
string|'"data"'
op|'}'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'check_can_live_migrate_source'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'dest_check_data'
op|'='
op|'{'
string|'"test"'
op|':'
string|'"data"'
op|'}'
op|','
name|'version'
op|'='
string|"'3.32'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'mock_warn'
op|'.'
name|'called'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.compute.rpcapi.ComputeAPI._warn_buggy_live_migrations'"
op|')'
newline|'\n'
DECL|member|test_check_can_live_migrate_source_old_warning
name|'def'
name|'test_check_can_live_migrate_source_old_warning'
op|'('
name|'self'
op|','
name|'mock_warn'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'check_can_live_migrate_source'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'dest_check_data'
op|'='
op|'{'
string|'"test"'
op|':'
string|'"data"'
op|'}'
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'mock_warn'
op|'.'
name|'assert_called_once_with'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_check_instance_shared_storage
dedent|''
name|'def'
name|'test_check_instance_shared_storage'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'check_instance_shared_storage'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'data'
op|'='
string|"'foo'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'check_instance_shared_storage'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'data'
op|'='
string|"'foo'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.29'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_confirm_resize_cast
dedent|''
name|'def'
name|'test_confirm_resize_cast'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'confirm_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'foo'"
op|'}'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'confirm_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'foo'"
op|'}'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_confirm_resize_call
dedent|''
name|'def'
name|'test_confirm_resize_call'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'confirm_resize'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'foo'"
op|'}'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'confirm_resize'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'foo'"
op|'}'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_detach_interface
dedent|''
name|'def'
name|'test_detach_interface'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'detach_interface'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'port_id'
op|'='
string|"'fake_id'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'detach_interface'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.17'"
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'port_id'
op|'='
string|"'fake_id'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_detach_volume
dedent|''
name|'def'
name|'test_detach_volume'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'detach_volume'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'volume_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'detach_volume'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'volume_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.25'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_finish_resize
dedent|''
name|'def'
name|'test_finish_resize'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'finish_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'foo'"
op|'}'
op|','
nl|'\n'
name|'image'
op|'='
string|"'image'"
op|','
name|'disk_info'
op|'='
string|"'disk_info'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'finish_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'foo'"
op|'}'
op|','
nl|'\n'
name|'image'
op|'='
string|"'image'"
op|','
name|'disk_info'
op|'='
string|"'disk_info'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_finish_revert_resize
dedent|''
name|'def'
name|'test_finish_revert_resize'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'finish_revert_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'finish_revert_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_console_output
dedent|''
name|'def'
name|'test_get_console_output'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_console_output'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'tail_length'
op|'='
string|"'tl'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_console_output'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'tail_length'
op|'='
string|"'tl'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.28'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_console_pool_info
dedent|''
name|'def'
name|'test_get_console_pool_info'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_console_pool_info'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'console_type'
op|'='
string|"'type'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_console_pool_info'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'console_type'
op|'='
string|"'type'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_console_topic
dedent|''
name|'def'
name|'test_get_console_topic'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_console_topic'"
op|','
string|"'call'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_console_topic'"
op|','
string|"'call'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_diagnostics
dedent|''
name|'def'
name|'test_get_diagnostics'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_diagnostics'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_diagnostics'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'3.18'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_instance_diagnostics
dedent|''
name|'def'
name|'test_get_instance_diagnostics'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_instance_diagnostics'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'assert_dict'
op|'='
name|'True'
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_instance_diagnostics'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'assert_dict'
op|'='
name|'True'
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.31'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_vnc_console
dedent|''
name|'def'
name|'test_get_vnc_console'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_vnc_console'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'console_type'
op|'='
string|"'type'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_vnc_console'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'console_type'
op|'='
string|"'type'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.2'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_spice_console
dedent|''
name|'def'
name|'test_get_spice_console'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_spice_console'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'console_type'
op|'='
string|"'type'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_spice_console'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'console_type'
op|'='
string|"'type'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.1'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_rdp_console
dedent|''
name|'def'
name|'test_get_rdp_console'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_rdp_console'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'console_type'
op|'='
string|"'type'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_rdp_console'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'console_type'
op|'='
string|"'type'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.10'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_serial_console
dedent|''
name|'def'
name|'test_get_serial_console'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_serial_console'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'console_type'
op|'='
string|"'serial'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_serial_console'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'console_type'
op|'='
string|"'serial'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.34'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_validate_console_port
dedent|''
name|'def'
name|'test_validate_console_port'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'validate_console_port'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'port'
op|'='
string|'"5900"'
op|','
nl|'\n'
name|'console_type'
op|'='
string|'"novnc"'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'validate_console_port'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'port'
op|'='
string|'"5900"'
op|','
nl|'\n'
name|'console_type'
op|'='
string|'"novnc"'
op|','
name|'version'
op|'='
string|"'3.3'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_maintenance_mode
dedent|''
name|'def'
name|'test_host_maintenance_mode'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'host_maintenance_mode'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'host_param'
op|'='
string|"'param'"
op|','
name|'mode'
op|'='
string|"'mode'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'host_maintenance_mode'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'host_param'
op|'='
string|"'param'"
op|','
name|'mode'
op|'='
string|"'mode'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_host_power_action
dedent|''
name|'def'
name|'test_host_power_action'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'host_power_action'"
op|','
string|"'call'"
op|','
name|'action'
op|'='
string|"'action'"
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'host_power_action'"
op|','
string|"'call'"
op|','
name|'action'
op|'='
string|"'action'"
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_inject_network_info
dedent|''
name|'def'
name|'test_inject_network_info'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'inject_network_info'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'inject_network_info'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_live_migration
dedent|''
name|'def'
name|'test_live_migration'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'live_migration'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'dest'
op|'='
string|"'dest'"
op|','
nl|'\n'
name|'block_migration'
op|'='
string|"'blockity_block'"
op|','
name|'host'
op|'='
string|"'tsoh'"
op|','
nl|'\n'
name|'migrate_data'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'live_migration'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'dest'
op|'='
string|"'dest'"
op|','
nl|'\n'
name|'block_migration'
op|'='
string|"'blockity_block'"
op|','
name|'host'
op|'='
string|"'tsoh'"
op|','
nl|'\n'
name|'migrate_data'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'3.26'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_post_live_migration_at_destination
dedent|''
name|'def'
name|'test_post_live_migration_at_destination'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'post_live_migration_at_destination'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'block_migration'
op|'='
string|"'block_migration'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'post_live_migration_at_destination'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'block_migration'
op|'='
string|"'block_migration'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'version'
op|'='
string|"'3.14'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_pause_instance
dedent|''
name|'def'
name|'test_pause_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'pause_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'pause_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_soft_delete_instance
dedent|''
name|'def'
name|'test_soft_delete_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'soft_delete_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'reservations'
op|'='
op|'['
string|"'uuid1'"
op|','
string|"'uuid2'"
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'soft_delete_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'reservations'
op|'='
op|'['
string|"'uuid1'"
op|','
string|"'uuid2'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_swap_volume
dedent|''
name|'def'
name|'test_swap_volume'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'swap_volume'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'old_volume_id'
op|'='
string|"'oldid'"
op|','
nl|'\n'
name|'new_volume_id'
op|'='
string|"'newid'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'swap_volume'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'old_volume_id'
op|'='
string|"'oldid'"
op|','
nl|'\n'
name|'new_volume_id'
op|'='
string|"'newid'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_restore_instance
dedent|''
name|'def'
name|'test_restore_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'restore_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'restore_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'3.20'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_pre_live_migration
dedent|''
name|'def'
name|'test_pre_live_migration'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'pre_live_migration'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'block_migration'
op|'='
string|"'block_migration'"
op|','
name|'disk'
op|'='
string|"'disk'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'migrate_data'
op|'='
name|'None'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'pre_live_migration'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'block_migration'
op|'='
string|"'block_migration'"
op|','
name|'disk'
op|'='
string|"'disk'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'migrate_data'
op|'='
name|'None'
op|','
name|'version'
op|'='
string|"'3.19'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_prep_resize
dedent|''
name|'def'
name|'test_prep_resize'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'prep_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'instance_type'
op|'='
string|"'fake_type'"
op|','
nl|'\n'
name|'image'
op|'='
string|"'fake_image'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|','
nl|'\n'
name|'request_spec'
op|'='
string|"'fake_spec'"
op|','
nl|'\n'
name|'filter_properties'
op|'='
op|'{'
string|"'fakeprop'"
op|':'
string|"'fakeval'"
op|'}'
op|','
nl|'\n'
name|'node'
op|'='
string|"'node'"
op|','
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'prep_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'instance_type'
op|'='
string|"'fake_type'"
op|','
nl|'\n'
name|'image'
op|'='
string|"'fake_image'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|','
nl|'\n'
name|'request_spec'
op|'='
string|"'fake_spec'"
op|','
nl|'\n'
name|'filter_properties'
op|'='
op|'{'
string|"'fakeprop'"
op|':'
string|"'fakeval'"
op|'}'
op|','
nl|'\n'
name|'node'
op|'='
string|"'node'"
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.38'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'prep_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'instance_type'
op|'='
string|"'fake_type'"
op|','
nl|'\n'
name|'image'
op|'='
string|"'fake_image'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|','
nl|'\n'
name|'request_spec'
op|'='
string|"'fake_spec'"
op|','
nl|'\n'
name|'filter_properties'
op|'='
op|'{'
string|"'fakeprop'"
op|':'
string|"'fakeval'"
op|'}'
op|','
nl|'\n'
name|'node'
op|'='
string|"'node'"
op|','
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.38'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'prep_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'instance_type'
op|'='
string|"'fake_type'"
op|','
nl|'\n'
name|'image'
op|'='
string|"'fake_image'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|','
nl|'\n'
name|'request_spec'
op|'='
string|"'fake_spec'"
op|','
nl|'\n'
name|'filter_properties'
op|'='
op|'{'
string|"'fakeprop'"
op|':'
string|"'fakeval'"
op|'}'
op|','
nl|'\n'
name|'node'
op|'='
string|"'node'"
op|','
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.38'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_reboot_instance
dedent|''
name|'def'
name|'test_reboot_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'maxDiff'
op|'='
name|'None'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'reboot_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'block_device_info'
op|'='
op|'{'
op|'}'
op|','
nl|'\n'
name|'reboot_type'
op|'='
string|"'type'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'reboot_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'block_device_info'
op|'='
op|'{'
op|'}'
op|','
nl|'\n'
name|'reboot_type'
op|'='
string|"'type'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_rebuild_instance
dedent|''
name|'def'
name|'test_rebuild_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rebuild_instance'"
op|','
string|"'cast'"
op|','
name|'new_pass'
op|'='
string|"'None'"
op|','
nl|'\n'
name|'injected_files'
op|'='
string|"'None'"
op|','
name|'image_ref'
op|'='
string|"'None'"
op|','
name|'orig_image_ref'
op|'='
string|"'None'"
op|','
nl|'\n'
name|'bdms'
op|'='
op|'['
op|']'
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'new_host'"
op|','
nl|'\n'
name|'orig_sys_metadata'
op|'='
name|'None'
op|','
name|'recreate'
op|'='
name|'True'
op|','
name|'on_shared_storage'
op|'='
name|'True'
op|','
nl|'\n'
name|'preserve_ephemeral'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rebuild_instance'"
op|','
string|"'cast'"
op|','
name|'new_pass'
op|'='
string|"'None'"
op|','
nl|'\n'
name|'injected_files'
op|'='
string|"'None'"
op|','
name|'image_ref'
op|'='
string|"'None'"
op|','
name|'orig_image_ref'
op|'='
string|"'None'"
op|','
nl|'\n'
name|'bdms'
op|'='
op|'['
op|']'
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'new_host'"
op|','
nl|'\n'
name|'orig_sys_metadata'
op|'='
name|'None'
op|','
name|'recreate'
op|'='
name|'True'
op|','
name|'on_shared_storage'
op|'='
name|'True'
op|','
nl|'\n'
name|'preserve_ephemeral'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.21'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_reserve_block_device_name
dedent|''
name|'def'
name|'test_reserve_block_device_name'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'reserve_block_device_name'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'device'
op|'='
string|"'device'"
op|','
nl|'\n'
name|'volume_id'
op|'='
string|"'id'"
op|','
name|'disk_bus'
op|'='
string|"'ide'"
op|','
name|'device_type'
op|'='
string|"'cdrom'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|','
nl|'\n'
name|'_return_value'
op|'='
name|'objects_block_dev'
op|'.'
name|'BlockDeviceMapping'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'reserve_block_device_name'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'device'
op|'='
string|"'device'"
op|','
nl|'\n'
name|'volume_id'
op|'='
string|"'id'"
op|','
name|'disk_bus'
op|'='
string|"'ide'"
op|','
name|'device_type'
op|'='
string|"'cdrom'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.35'"
op|','
name|'return_bdm_object'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|member|refresh_provider_fw_rules
dedent|''
name|'def'
name|'refresh_provider_fw_rules'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'refresh_provider_fw_rules'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'refresh_provider_fw_rules'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_refresh_security_group_rules
dedent|''
name|'def'
name|'test_refresh_security_group_rules'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'refresh_security_group_rules'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'security_group_id'
op|'='
string|"'id'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'refresh_security_group_rules'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'security_group_id'
op|'='
string|"'id'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_refresh_security_group_members
dedent|''
name|'def'
name|'test_refresh_security_group_members'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'refresh_security_group_members'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'security_group_id'
op|'='
string|"'id'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'refresh_security_group_members'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'security_group_id'
op|'='
string|"'id'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_refresh_instance_security_rules
dedent|''
name|'def'
name|'test_refresh_instance_security_rules'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'refresh_instance_security_rules'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'host'
op|'='
string|"'fake_host'"
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|','
name|'assert_dict'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'refresh_instance_security_rules'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'host'
op|'='
string|"'fake_host'"
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.0'"
op|','
name|'assert_dict'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_remove_aggregate_host
dedent|''
name|'def'
name|'test_remove_aggregate_host'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'remove_aggregate_host'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'aggregate'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
name|'host_param'
op|'='
string|"'host'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'slave_info'
op|'='
op|'{'
op|'}'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'remove_aggregate_host'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'aggregate'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
name|'host_param'
op|'='
string|"'host'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'slave_info'
op|'='
op|'{'
op|'}'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_remove_fixed_ip_from_instance
dedent|''
name|'def'
name|'test_remove_fixed_ip_from_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'remove_fixed_ip_from_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'address'
op|'='
string|"'addr'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'remove_fixed_ip_from_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'address'
op|'='
string|"'addr'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.13'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_remove_volume_connection
dedent|''
name|'def'
name|'test_remove_volume_connection'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'remove_volume_connection'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance'
op|','
name|'volume_id'
op|'='
string|"'id'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'remove_volume_connection'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance'
op|','
name|'volume_id'
op|'='
string|"'id'"
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.30'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_rescue_instance
dedent|''
name|'def'
name|'test_rescue_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rescue_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'rescue_password'
op|'='
string|"'pw'"
op|','
nl|'\n'
name|'rescue_image_ref'
op|'='
string|"'fake_image_ref'"
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.9'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rescue_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'rescue_password'
op|'='
string|"'pw'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.9'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.24'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rescue_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'rescue_password'
op|'='
string|"'pw'"
op|','
nl|'\n'
name|'rescue_image_ref'
op|'='
string|"'fake_image_ref'"
op|','
name|'version'
op|'='
string|"'3.24'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.37'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rescue_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'rescue_password'
op|'='
string|"'pw'"
op|','
nl|'\n'
name|'rescue_image_ref'
op|'='
string|"'fake_image_ref'"
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rescue_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'rescue_password'
op|'='
string|"'pw'"
op|','
nl|'\n'
name|'rescue_image_ref'
op|'='
string|"'fake_image_ref'"
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_reset_network
dedent|''
name|'def'
name|'test_reset_network'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'reset_network'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'reset_network'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_resize_instance
dedent|''
name|'def'
name|'test_resize_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'resize_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
nl|'\n'
name|'image'
op|'='
string|"'image'"
op|','
name|'instance_type'
op|'='
op|'{'
string|"'id'"
op|':'
number|'1'
op|'}'
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'resize_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
nl|'\n'
name|'image'
op|'='
string|"'image'"
op|','
name|'instance_type'
op|'='
op|'{'
string|"'id'"
op|':'
number|'1'
op|'}'
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.37'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'resize_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
nl|'\n'
name|'image'
op|'='
string|"'image'"
op|','
name|'instance_type'
op|'='
op|'{'
string|"'id'"
op|':'
number|'1'
op|'}'
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'resize_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
nl|'\n'
name|'image'
op|'='
string|"'image'"
op|','
name|'instance_type'
op|'='
op|'{'
string|"'id'"
op|':'
number|'1'
op|'}'
op|','
nl|'\n'
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_resume_instance
dedent|''
name|'def'
name|'test_resume_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'resume_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'resume_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_revert_resize
dedent|''
name|'def'
name|'test_revert_resize'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'revert_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'revert_resize'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'migration'
op|'='
op|'{'
string|"'id'"
op|':'
string|"'fake_id'"
op|'}'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
name|'reservations'
op|'='
name|'list'
op|'('
string|"'fake_res'"
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.compute.rpcapi.ComputeAPI._warn_buggy_live_migrations'"
op|')'
newline|'\n'
DECL|member|test_rollback_live_migration_at_destination
name|'def'
name|'test_rollback_live_migration_at_destination'
op|'('
name|'self'
op|','
name|'mock_warn'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rollback_live_migration_at_destination'"
op|','
nl|'\n'
string|"'cast'"
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'destroy_disks'
op|'='
name|'True'
op|','
name|'migrate_data'
op|'='
name|'None'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rollback_live_migration_at_destination'"
op|','
nl|'\n'
string|"'cast'"
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'destroy_disks'
op|'='
name|'True'
op|','
name|'migrate_data'
op|'='
name|'None'
op|','
name|'version'
op|'='
string|"'3.32'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'mock_warn'
op|'.'
name|'called'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.compute.rpcapi.ComputeAPI._warn_buggy_live_migrations'"
op|')'
newline|'\n'
DECL|member|test_rollback_live_migration_at_destination_old_warning
name|'def'
name|'test_rollback_live_migration_at_destination_old_warning'
op|'('
name|'self'
op|','
nl|'\n'
name|'mock_warn'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'rollback_live_migration_at_destination'"
op|','
nl|'\n'
string|"'cast'"
op|','
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'mock_warn'
op|'.'
name|'assert_called_once_with'
op|'('
name|'None'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_set_admin_password
dedent|''
name|'def'
name|'test_set_admin_password'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'set_admin_password'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'new_pass'
op|'='
string|"'pw'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'set_admin_password'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'new_pass'
op|'='
string|"'pw'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.8'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_set_host_enabled
dedent|''
name|'def'
name|'test_set_host_enabled'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'set_host_enabled'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'enabled'
op|'='
string|"'enabled'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'set_host_enabled'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'enabled'
op|'='
string|"'enabled'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_host_uptime
dedent|''
name|'def'
name|'test_get_host_uptime'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_host_uptime'"
op|','
string|"'call'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'get_host_uptime'"
op|','
string|"'call'"
op|','
name|'host'
op|'='
string|"'host'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_backup_instance
dedent|''
name|'def'
name|'test_backup_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'backup_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'image_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'backup_type'
op|'='
string|"'type'"
op|','
name|'rotation'
op|'='
string|"'rotation'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'backup_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'image_id'
op|'='
string|"'id'"
op|','
nl|'\n'
name|'backup_type'
op|'='
string|"'type'"
op|','
name|'rotation'
op|'='
string|"'rotation'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_snapshot_instance
dedent|''
name|'def'
name|'test_snapshot_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'snapshot_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'image_id'
op|'='
string|"'id'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'snapshot_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'image_id'
op|'='
string|"'id'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_start_instance
dedent|''
name|'def'
name|'test_start_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'start_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'start_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_stop_instance_cast
dedent|''
name|'def'
name|'test_stop_instance_cast'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'stop_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'stop_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.37'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'stop_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'stop_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_stop_instance_call
dedent|''
name|'def'
name|'test_stop_instance_call'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'stop_instance'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'stop_instance'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.37'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'stop_instance'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'stop_instance'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_suspend_instance
dedent|''
name|'def'
name|'test_suspend_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'suspend_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'suspend_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_terminate_instance
dedent|''
name|'def'
name|'test_terminate_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'terminate_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'bdms'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'reservations'
op|'='
op|'['
string|"'uuid1'"
op|','
string|"'uuid2'"
op|']'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'terminate_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'bdms'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'reservations'
op|'='
op|'['
string|"'uuid1'"
op|','
string|"'uuid2'"
op|']'
op|','
name|'version'
op|'='
string|"'3.22'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_unpause_instance
dedent|''
name|'def'
name|'test_unpause_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'unpause_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'unpause_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_unrescue_instance
dedent|''
name|'def'
name|'test_unrescue_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'unrescue_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'unrescue_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'3.11'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_shelve_instance
dedent|''
name|'def'
name|'test_shelve_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'shelve_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'image_id'
op|'='
string|"'image_id'"
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'shelve_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'image_id'
op|'='
string|"'image_id'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.37'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'shelve_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'image_id'
op|'='
string|"'image_id'"
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'shelve_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'image_id'
op|'='
string|"'image_id'"
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_shelve_offload_instance
dedent|''
name|'def'
name|'test_shelve_offload_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'shelve_offload_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.0'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'shelve_offload_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.0'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'3.37'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'shelve_offload_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'shelve_offload_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'clean_shutdown'
op|'='
name|'True'
op|','
name|'version'
op|'='
string|"'3.37'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_unshelve_instance
dedent|''
name|'def'
name|'test_unshelve_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'unshelve_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'image'
op|'='
string|"'image'"
op|','
nl|'\n'
name|'filter_properties'
op|'='
op|'{'
string|"'fakeprop'"
op|':'
string|"'fakeval'"
op|'}'
op|','
name|'node'
op|'='
string|"'node'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'unshelve_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'image'
op|'='
string|"'image'"
op|','
nl|'\n'
name|'filter_properties'
op|'='
op|'{'
string|"'fakeprop'"
op|':'
string|"'fakeval'"
op|'}'
op|','
name|'node'
op|'='
string|"'node'"
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.15'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_volume_snapshot_create
dedent|''
name|'def'
name|'test_volume_snapshot_create'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'volume_snapshot_create'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'volume_id'
op|'='
string|"'fake_id'"
op|','
nl|'\n'
name|'create_info'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'volume_snapshot_create'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'volume_id'
op|'='
string|"'fake_id'"
op|','
nl|'\n'
name|'create_info'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'3.6'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_volume_snapshot_delete
dedent|''
name|'def'
name|'test_volume_snapshot_delete'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'volume_snapshot_delete'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'volume_id'
op|'='
string|"'fake_id'"
op|','
nl|'\n'
name|'snapshot_id'
op|'='
string|"'fake_id2'"
op|','
name|'delete_info'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'volume_snapshot_delete'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'volume_id'
op|'='
string|"'fake_id'"
op|','
nl|'\n'
name|'snapshot_id'
op|'='
string|"'fake_id2'"
op|','
name|'delete_info'
op|'='
op|'{'
op|'}'
op|','
name|'version'
op|'='
string|"'3.6'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_external_instance_event
dedent|''
name|'def'
name|'test_external_instance_event'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'external_instance_event'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instances'
op|'='
op|'['
name|'self'
op|'.'
name|'fake_instance_obj'
op|']'
op|','
nl|'\n'
name|'events'
op|'='
op|'['
string|"'event'"
op|']'
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'external_instance_event'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instances'
op|'='
op|'['
name|'self'
op|'.'
name|'fake_instance_obj'
op|']'
op|','
nl|'\n'
name|'events'
op|'='
op|'['
string|"'event'"
op|']'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.23'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_build_and_run_instance
dedent|''
name|'def'
name|'test_build_and_run_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'build_and_run_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'image'
op|'='
string|"'image'"
op|','
nl|'\n'
name|'request_spec'
op|'='
op|'{'
string|"'request'"
op|':'
string|"'spec'"
op|'}'
op|','
name|'filter_properties'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'admin_password'
op|'='
string|"'passwd'"
op|','
name|'injected_files'
op|'='
name|'None'
op|','
nl|'\n'
name|'requested_networks'
op|'='
op|'['
string|"'network1'"
op|']'
op|','
name|'security_groups'
op|'='
name|'None'
op|','
nl|'\n'
name|'block_device_mapping'
op|'='
name|'None'
op|','
name|'node'
op|'='
string|"'node'"
op|','
name|'limits'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'build_and_run_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'image'
op|'='
string|"'image'"
op|','
nl|'\n'
name|'request_spec'
op|'='
op|'{'
string|"'request'"
op|':'
string|"'spec'"
op|'}'
op|','
name|'filter_properties'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'admin_password'
op|'='
string|"'passwd'"
op|','
name|'injected_files'
op|'='
name|'None'
op|','
nl|'\n'
name|'requested_networks'
op|'='
op|'['
string|"'network1'"
op|']'
op|','
name|'security_groups'
op|'='
name|'None'
op|','
nl|'\n'
name|'block_device_mapping'
op|'='
name|'None'
op|','
name|'node'
op|'='
string|"'node'"
op|','
name|'limits'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.40'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.utils.is_neutron'"
op|','
name|'return_value'
op|'='
name|'True'
op|')'
newline|'\n'
DECL|member|test_build_and_run_instance_icehouse_compat
name|'def'
name|'test_build_and_run_instance_icehouse_compat'
op|'('
name|'self'
op|','
name|'is_neutron'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'icehouse'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'build_and_run_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'image'
op|'='
string|"'image'"
op|','
nl|'\n'
name|'request_spec'
op|'='
op|'{'
string|"'request'"
op|':'
string|"'spec'"
op|'}'
op|','
name|'filter_properties'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'admin_password'
op|'='
string|"'passwd'"
op|','
name|'injected_files'
op|'='
name|'None'
op|','
nl|'\n'
name|'requested_networks'
op|'='
name|'objects_network_request'
op|'.'
name|'NetworkRequestList'
op|'('
nl|'\n'
name|'objects'
op|'='
op|'['
name|'objects_network_request'
op|'.'
name|'NetworkRequest'
op|'('
nl|'\n'
name|'network_id'
op|'='
string|'"fake_network_id"'
op|','
name|'address'
op|'='
string|'"10.0.0.1"'
op|','
nl|'\n'
name|'port_id'
op|'='
string|'"fake_port_id"'
op|')'
op|']'
op|')'
op|','
nl|'\n'
name|'security_groups'
op|'='
name|'None'
op|','
nl|'\n'
name|'block_device_mapping'
op|'='
name|'None'
op|','
name|'node'
op|'='
string|"'node'"
op|','
name|'limits'
op|'='
op|'{'
op|'}'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.23'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.utils.is_neutron'"
op|','
name|'return_value'
op|'='
name|'False'
op|')'
newline|'\n'
DECL|member|test_build_and_run_instance_icehouse_compat_nova_net
name|'def'
name|'test_build_and_run_instance_icehouse_compat_nova_net'
op|'('
name|'self'
op|','
name|'is_neutron'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'icehouse'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'build_and_run_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'image'
op|'='
string|"'image'"
op|','
nl|'\n'
name|'request_spec'
op|'='
op|'{'
string|"'request'"
op|':'
string|"'spec'"
op|'}'
op|','
name|'filter_properties'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'admin_password'
op|'='
string|"'passwd'"
op|','
name|'injected_files'
op|'='
name|'None'
op|','
nl|'\n'
name|'requested_networks'
op|'='
name|'objects_network_request'
op|'.'
name|'NetworkRequestList'
op|'('
nl|'\n'
name|'objects'
op|'='
op|'['
name|'objects_network_request'
op|'.'
name|'NetworkRequest'
op|'('
nl|'\n'
name|'network_id'
op|'='
string|"'fake_network_id'"
op|','
name|'address'
op|'='
string|"'10.0.0.1'"
op|')'
op|']'
op|')'
op|','
nl|'\n'
name|'security_groups'
op|'='
name|'None'
op|','
nl|'\n'
name|'block_device_mapping'
op|'='
name|'None'
op|','
name|'node'
op|'='
string|"'node'"
op|','
name|'limits'
op|'='
op|'{'
op|'}'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.23'"
op|','
name|'nova_network'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_quiesce_instance
dedent|''
name|'def'
name|'test_quiesce_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'quiesce_instance'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'quiesce_instance'"
op|','
string|"'call'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'version'
op|'='
string|"'3.39'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_unquiesce_instance
dedent|''
name|'def'
name|'test_unquiesce_instance'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'unquiesce_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'mapping'
op|'='
name|'None'
op|','
name|'version'
op|'='
string|"'4.0'"
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'kilo'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'unquiesce_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'mapping'
op|'='
name|'None'
op|','
name|'version'
op|'='
string|"'3.39'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.utils.is_neutron'"
op|','
name|'return_value'
op|'='
name|'True'
op|')'
newline|'\n'
DECL|member|test_build_and_run_instance_juno_compat
name|'def'
name|'test_build_and_run_instance_juno_compat'
op|'('
name|'self'
op|','
name|'is_neutron'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'juno'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'build_and_run_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
name|'host'
op|'='
string|"'host'"
op|','
name|'image'
op|'='
string|"'image'"
op|','
nl|'\n'
name|'request_spec'
op|'='
op|'{'
string|"'request'"
op|':'
string|"'spec'"
op|'}'
op|','
name|'filter_properties'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'admin_password'
op|'='
string|"'passwd'"
op|','
name|'injected_files'
op|'='
name|'None'
op|','
nl|'\n'
name|'requested_networks'
op|'='
name|'objects_network_request'
op|'.'
name|'NetworkRequestList'
op|'('
nl|'\n'
name|'objects'
op|'='
op|'['
name|'objects_network_request'
op|'.'
name|'NetworkRequest'
op|'('
nl|'\n'
name|'network_id'
op|'='
string|'"fake_network_id"'
op|','
name|'address'
op|'='
string|'"10.0.0.1"'
op|','
nl|'\n'
name|'port_id'
op|'='
string|'"fake_port_id"'
op|')'
op|']'
op|')'
op|','
nl|'\n'
name|'security_groups'
op|'='
name|'None'
op|','
nl|'\n'
name|'block_device_mapping'
op|'='
name|'None'
op|','
name|'node'
op|'='
string|"'node'"
op|','
name|'limits'
op|'='
op|'{'
op|'}'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.33'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.objects.ComputeNode.get_by_host_and_nodename'"
op|')'
newline|'\n'
op|'@'
name|'mock'
op|'.'
name|'patch'
op|'('
string|"'nova.utils.is_neutron'"
op|','
name|'return_value'
op|'='
name|'True'
op|')'
newline|'\n'
DECL|member|test_build_and_run_instance_limits_juno_compat
name|'def'
name|'test_build_and_run_instance_limits_juno_compat'
op|'('
nl|'\n'
name|'self'
op|','
name|'is_neutron'
op|','
name|'get_by_host_and_nodename'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'host_topology'
op|'='
name|'objects_numa'
op|'.'
name|'NUMATopology'
op|'('
name|'cells'
op|'='
op|'['
nl|'\n'
name|'objects_numa'
op|'.'
name|'NUMACell'
op|'('
nl|'\n'
name|'id'
op|'='
number|'0'
op|','
name|'cpuset'
op|'='
name|'set'
op|'('
op|'['
number|'1'
op|','
number|'2'
op|']'
op|')'
op|','
name|'memory'
op|'='
number|'512'
op|','
nl|'\n'
name|'cpu_usage'
op|'='
number|'2'
op|','
name|'memory_usage'
op|'='
number|'256'
op|','
nl|'\n'
name|'pinned_cpus'
op|'='
name|'set'
op|'('
op|'['
number|'1'
op|']'
op|')'
op|')'
op|','
nl|'\n'
name|'objects_numa'
op|'.'
name|'NUMACell'
op|'('
nl|'\n'
name|'id'
op|'='
number|'1'
op|','
name|'cpuset'
op|'='
name|'set'
op|'('
op|'['
number|'3'
op|','
number|'4'
op|']'
op|')'
op|','
name|'memory'
op|'='
number|'512'
op|','
nl|'\n'
name|'cpu_usage'
op|'='
number|'1'
op|','
name|'memory_usage'
op|'='
number|'128'
op|','
nl|'\n'
name|'pinned_cpus'
op|'='
name|'set'
op|'('
op|'['
op|']'
op|')'
op|')'
nl|'\n'
op|']'
op|')'
newline|'\n'
name|'limits'
op|'='
name|'objects_numa'
op|'.'
name|'NUMATopologyLimits'
op|'('
nl|'\n'
name|'cpu_allocation_ratio'
op|'='
number|'16'
op|','
nl|'\n'
name|'ram_allocation_ratio'
op|'='
number|'2'
op|')'
newline|'\n'
name|'cnode'
op|'='
name|'objects_compute_node'
op|'.'
name|'ComputeNode'
op|'('
nl|'\n'
name|'numa_topology'
op|'='
name|'jsonutils'
op|'.'
name|'dumps'
op|'('
nl|'\n'
name|'host_topology'
op|'.'
name|'obj_to_primitive'
op|'('
op|')'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'get_by_host_and_nodename'
op|'.'
name|'return_value'
op|'='
name|'cnode'
newline|'\n'
name|'legacy_limits'
op|'='
name|'jsonutils'
op|'.'
name|'dumps'
op|'('
nl|'\n'
name|'limits'
op|'.'
name|'to_dict_legacy'
op|'('
name|'host_topology'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'flags'
op|'('
name|'compute'
op|'='
string|"'juno'"
op|','
name|'group'
op|'='
string|"'upgrade_levels'"
op|')'
newline|'\n'
name|'netreqs'
op|'='
name|'objects_network_request'
op|'.'
name|'NetworkRequestList'
op|'('
name|'objects'
op|'='
op|'['
nl|'\n'
name|'objects_network_request'
op|'.'
name|'NetworkRequest'
op|'('
nl|'\n'
name|'network_id'
op|'='
string|'"fake_network_id"'
op|','
nl|'\n'
name|'address'
op|'='
string|'"10.0.0.1"'
op|','
nl|'\n'
name|'port_id'
op|'='
string|'"fake_port_id"'
op|')'
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_test_compute_api'
op|'('
string|"'build_and_run_instance'"
op|','
string|"'cast'"
op|','
nl|'\n'
name|'instance'
op|'='
name|'self'
op|'.'
name|'fake_instance_obj'
op|','
nl|'\n'
name|'host'
op|'='
string|"'host'"
op|','
nl|'\n'
name|'image'
op|'='
string|"'image'"
op|','
nl|'\n'
name|'request_spec'
op|'='
op|'{'
string|"'request'"
op|':'
string|"'spec'"
op|'}'
op|','
nl|'\n'
name|'filter_properties'
op|'='
op|'['
op|']'
op|','
nl|'\n'
name|'admin_password'
op|'='
string|"'passwd'"
op|','
nl|'\n'
name|'injected_files'
op|'='
name|'None'
op|','
nl|'\n'
name|'requested_networks'
op|'='
name|'netreqs'
op|','
nl|'\n'
name|'security_groups'
op|'='
name|'None'
op|','
nl|'\n'
name|'block_device_mapping'
op|'='
name|'None'
op|','
nl|'\n'
name|'node'
op|'='
string|"'node'"
op|','
nl|'\n'
name|'limits'
op|'='
op|'{'
string|"'numa_topology'"
op|':'
name|'limits'
op|'}'
op|','
nl|'\n'
name|'legacy_limits'
op|'='
op|'{'
string|"'numa_topology'"
op|':'
name|'legacy_limits'
op|'}'
op|','
nl|'\n'
name|'version'
op|'='
string|"'3.33'"
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
