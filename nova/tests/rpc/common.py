begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright 2010 United States Government as represented by the'
nl|'\n'
comment|'# Administrator of the National Aeronautics and Space Administration.'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
string|'"""\nUnit Tests for remote procedure calls shared between all implementations\n"""'
newline|'\n'
nl|'\n'
name|'import'
name|'time'
newline|'\n'
nl|'\n'
name|'import'
name|'eventlet'
newline|'\n'
name|'from'
name|'eventlet'
name|'import'
name|'greenthread'
newline|'\n'
name|'import'
name|'nose'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
name|'import'
name|'context'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'flags'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'log'
name|'as'
name|'logging'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'rpc'
name|'import'
name|'amqp'
name|'as'
name|'rpc_amqp'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'rpc'
name|'import'
name|'common'
name|'as'
name|'rpc_common'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'rpc'
name|'import'
name|'dispatcher'
name|'as'
name|'rpc_dispatcher'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'test'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|FLAGS
name|'FLAGS'
op|'='
name|'flags'
op|'.'
name|'FLAGS'
newline|'\n'
DECL|variable|LOG
name|'LOG'
op|'='
name|'logging'
op|'.'
name|'getLogger'
op|'('
name|'__name__'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|BaseRpcTestCase
name|'class'
name|'BaseRpcTestCase'
op|'('
name|'test'
op|'.'
name|'TestCase'
op|')'
op|':'
newline|'\n'
DECL|member|setUp
indent|'    '
name|'def'
name|'setUp'
op|'('
name|'self'
op|','
name|'supports_timeouts'
op|'='
name|'True'
op|','
name|'topic'
op|'='
string|"'test'"
op|','
nl|'\n'
name|'topic_nested'
op|'='
string|"'nested'"
op|')'
op|':'
newline|'\n'
indent|'        '
name|'super'
op|'('
name|'BaseRpcTestCase'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'topic'
op|'='
name|'topic'
name|'or'
name|'self'
op|'.'
name|'topic'
newline|'\n'
name|'self'
op|'.'
name|'topic_nested'
op|'='
name|'topic_nested'
name|'or'
name|'self'
op|'.'
name|'topic_nested'
newline|'\n'
name|'self'
op|'.'
name|'supports_timeouts'
op|'='
name|'supports_timeouts'
newline|'\n'
name|'self'
op|'.'
name|'context'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'receiver'
op|'='
name|'TestReceiver'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'conn'
op|'='
name|'self'
op|'.'
name|'_create_consumer'
op|'('
name|'receiver'
op|','
name|'self'
op|'.'
name|'topic'
op|')'
newline|'\n'
nl|'\n'
DECL|member|tearDown
dedent|''
dedent|''
name|'def'
name|'tearDown'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'conn'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
dedent|''
name|'super'
op|'('
name|'BaseRpcTestCase'
op|','
name|'self'
op|')'
op|'.'
name|'tearDown'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|_create_consumer
dedent|''
name|'def'
name|'_create_consumer'
op|'('
name|'self'
op|','
name|'proxy'
op|','
name|'topic'
op|','
name|'fanout'
op|'='
name|'False'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'dispatcher'
op|'='
name|'rpc_dispatcher'
op|'.'
name|'RpcDispatcher'
op|'('
op|'['
name|'proxy'
op|']'
op|')'
newline|'\n'
name|'conn'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'create_connection'
op|'('
name|'FLAGS'
op|','
name|'True'
op|')'
newline|'\n'
name|'conn'
op|'.'
name|'create_consumer'
op|'('
name|'topic'
op|','
name|'dispatcher'
op|','
name|'fanout'
op|')'
newline|'\n'
name|'conn'
op|'.'
name|'consume_in_thread'
op|'('
op|')'
newline|'\n'
name|'return'
name|'conn'
newline|'\n'
nl|'\n'
DECL|member|test_call_succeed
dedent|''
name|'def'
name|'test_call_succeed'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'value'
op|'='
number|'42'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'call'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"echo"'
op|','
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'value'
op|','
name|'result'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_call_succeed_despite_multiple_returns_yield
dedent|''
name|'def'
name|'test_call_succeed_despite_multiple_returns_yield'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'value'
op|'='
number|'42'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'call'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"echo_three_times_yield"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'value'
op|'+'
number|'2'
op|','
name|'result'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_multicall_succeed_once
dedent|''
name|'def'
name|'test_multicall_succeed_once'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'value'
op|'='
number|'42'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'multicall'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"echo"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'x'
name|'in'
name|'enumerate'
op|'('
name|'result'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'i'
op|'>'
number|'0'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'fail'
op|'('
string|"'should only receive one response'"
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'value'
op|'+'
name|'i'
op|','
name|'x'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_multicall_three_nones
dedent|''
dedent|''
name|'def'
name|'test_multicall_three_nones'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'value'
op|'='
number|'42'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'multicall'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"multicall_three_nones"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'x'
name|'in'
name|'enumerate'
op|'('
name|'result'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'x'
op|','
name|'None'
op|')'
newline|'\n'
comment|'# i should have been 0, 1, and finally 2:'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'i'
op|','
number|'2'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_multicall_succeed_three_times_yield
dedent|''
name|'def'
name|'test_multicall_succeed_three_times_yield'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'value'
op|'='
number|'42'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'multicall'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"echo_three_times_yield"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'x'
name|'in'
name|'enumerate'
op|'('
name|'result'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'value'
op|'+'
name|'i'
op|','
name|'x'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_context_passed
dedent|''
dedent|''
name|'def'
name|'test_context_passed'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
string|'"""Makes sure a context is passed through rpc call."""'
newline|'\n'
name|'value'
op|'='
number|'42'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'call'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'self'
op|'.'
name|'topic'
op|','
op|'{'
string|'"method"'
op|':'
string|'"context"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'self'
op|'.'
name|'context'
op|'.'
name|'to_dict'
op|'('
op|')'
op|','
name|'result'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_test_cast
dedent|''
name|'def'
name|'_test_cast'
op|'('
name|'self'
op|','
name|'fanout'
op|'='
name|'False'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Test casts by pushing items through a channeled queue."""'
newline|'\n'
nl|'\n'
comment|'# Not a true global, but capitalized so'
nl|'\n'
comment|'# it is clear it is leaking scope into Nested()'
nl|'\n'
name|'QUEUE'
op|'='
name|'eventlet'
op|'.'
name|'queue'
op|'.'
name|'Queue'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
comment|"# We use the nested topic so we don't need QUEUE to be a proper"
nl|'\n'
comment|'# global, and do not keep state outside this test.'
nl|'\n'
DECL|class|Nested
dedent|''
name|'class'
name|'Nested'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
indent|'            '
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|put_queue
name|'def'
name|'put_queue'
op|'('
name|'context'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'LOG'
op|'.'
name|'debug'
op|'('
string|'"Got value in put_queue: %s"'
op|','
name|'value'
op|')'
newline|'\n'
name|'QUEUE'
op|'.'
name|'put'
op|'('
name|'value'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'nested'
op|'='
name|'Nested'
op|'('
op|')'
newline|'\n'
name|'conn'
op|'='
name|'self'
op|'.'
name|'_create_consumer'
op|'('
name|'nested'
op|','
name|'self'
op|'.'
name|'topic_nested'
op|','
name|'fanout'
op|')'
newline|'\n'
name|'value'
op|'='
number|'42'
newline|'\n'
nl|'\n'
name|'method'
op|'='
op|'('
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'cast'
op|','
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'fanout_cast'
op|')'
op|'['
name|'fanout'
op|']'
newline|'\n'
name|'method'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'self'
op|'.'
name|'topic_nested'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"put_queue"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
nl|'\n'
name|'try'
op|':'
newline|'\n'
comment|'# If it does not succeed in 2 seconds, give up and assume'
nl|'\n'
comment|'# failure.'
nl|'\n'
DECL|variable|result
indent|'            '
name|'result'
op|'='
name|'QUEUE'
op|'.'
name|'get'
op|'('
name|'True'
op|','
number|'2'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'Exception'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'value'
op|','
name|'None'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'conn'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'value'
op|','
name|'result'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_cast_success
dedent|''
name|'def'
name|'test_cast_success'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_cast'
op|'('
name|'False'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_fanout_success
dedent|''
name|'def'
name|'test_fanout_success'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_test_cast'
op|'('
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_nested_calls
dedent|''
name|'def'
name|'test_nested_calls'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
string|'"""Test that we can do an rpc.call inside another call."""'
newline|'\n'
DECL|class|Nested
name|'class'
name|'Nested'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
indent|'            '
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|echo
name|'def'
name|'echo'
op|'('
name|'context'
op|','
name|'queue'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'                '
string|'"""Calls echo in the passed queue."""'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|'"Nested received %(queue)s, %(value)s"'
op|')'
nl|'\n'
op|'%'
name|'locals'
op|'('
op|')'
op|')'
newline|'\n'
comment|'# TODO(comstud):'
nl|'\n'
comment|'# so, it will replay the context and use the same REQID?'
nl|'\n'
comment|"# that's bizarre."
nl|'\n'
name|'ret'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'call'
op|'('
name|'FLAGS'
op|','
name|'context'
op|','
nl|'\n'
name|'queue'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"echo"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|'"Nested return %s"'
op|')'
op|','
name|'ret'
op|')'
newline|'\n'
name|'return'
name|'value'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'nested'
op|'='
name|'Nested'
op|'('
op|')'
newline|'\n'
name|'conn'
op|'='
name|'self'
op|'.'
name|'_create_consumer'
op|'('
name|'nested'
op|','
name|'self'
op|'.'
name|'topic_nested'
op|')'
newline|'\n'
nl|'\n'
name|'value'
op|'='
number|'42'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'call'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'self'
op|'.'
name|'topic_nested'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"echo"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"queue"'
op|':'
string|'"test"'
op|','
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'conn'
op|'.'
name|'close'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'value'
op|','
name|'result'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_call_timeout
dedent|''
name|'def'
name|'test_call_timeout'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
string|'"""Make sure rpc.call will time out."""'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'supports_timeouts'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
name|'_'
op|'('
string|'"RPC backend does not support timeouts"'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'value'
op|'='
number|'42'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'rpc_common'
op|'.'
name|'Timeout'
op|','
nl|'\n'
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'call'
op|','
nl|'\n'
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"block"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|','
name|'timeout'
op|'='
number|'1'
op|')'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'call'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
nl|'\n'
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"block"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|','
nl|'\n'
name|'timeout'
op|'='
number|'1'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'fail'
op|'('
string|'"should have thrown Timeout"'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'rpc_common'
op|'.'
name|'Timeout'
name|'as'
name|'exc'
op|':'
newline|'\n'
indent|'            '
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|BaseRpcAMQPTestCase
dedent|''
dedent|''
dedent|''
name|'class'
name|'BaseRpcAMQPTestCase'
op|'('
name|'BaseRpcTestCase'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Base test class for all AMQP-based RPC tests."""'
newline|'\n'
DECL|member|test_proxycallback_handles_exceptions
name|'def'
name|'test_proxycallback_handles_exceptions'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Make sure exceptions unpacking messages don\'t cause hangs."""'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'rpc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'nose'
op|'.'
name|'SkipTest'
op|'('
string|"'rpc driver not available.'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'orig_unpack'
op|'='
name|'rpc_amqp'
op|'.'
name|'unpack_context'
newline|'\n'
nl|'\n'
name|'info'
op|'='
op|'{'
string|"'unpacked'"
op|':'
name|'False'
op|'}'
newline|'\n'
nl|'\n'
DECL|function|fake_unpack_context
name|'def'
name|'fake_unpack_context'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'info'
op|'['
string|"'unpacked'"
op|']'
op|'='
name|'True'
newline|'\n'
name|'raise'
name|'test'
op|'.'
name|'TestingException'
op|'('
string|"'moo'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'stubs'
op|'.'
name|'Set'
op|'('
name|'rpc_amqp'
op|','
string|"'unpack_context'"
op|','
name|'fake_unpack_context'
op|')'
newline|'\n'
nl|'\n'
name|'value'
op|'='
number|'41'
newline|'\n'
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'cast'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"echo"'
op|','
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
nl|'\n'
comment|'# Wait for the cast to complete.'
nl|'\n'
name|'for'
name|'x'
name|'in'
name|'xrange'
op|'('
number|'50'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'info'
op|'['
string|"'unpacked'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'break'
newline|'\n'
dedent|''
name|'greenthread'
op|'.'
name|'sleep'
op|'('
number|'0.1'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'fail'
op|'('
string|'"Timeout waiting for message to be consumed"'
op|')'
newline|'\n'
nl|'\n'
comment|'# Now see if we get a response even though we raised an'
nl|'\n'
comment|'# exception for the cast above.'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'stubs'
op|'.'
name|'Set'
op|'('
name|'rpc_amqp'
op|','
string|"'unpack_context'"
op|','
name|'orig_unpack'
op|')'
newline|'\n'
nl|'\n'
name|'value'
op|'='
number|'42'
newline|'\n'
name|'result'
op|'='
name|'self'
op|'.'
name|'rpc'
op|'.'
name|'call'
op|'('
name|'FLAGS'
op|','
name|'self'
op|'.'
name|'context'
op|','
name|'self'
op|'.'
name|'topic'
op|','
nl|'\n'
op|'{'
string|'"method"'
op|':'
string|'"echo"'
op|','
nl|'\n'
string|'"args"'
op|':'
op|'{'
string|'"value"'
op|':'
name|'value'
op|'}'
op|'}'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'value'
op|','
name|'result'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|TestReceiver
dedent|''
dedent|''
name|'class'
name|'TestReceiver'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Simple Proxy class so the consumer has methods to call.\n\n    Uses static methods because we aren\'t actually storing any state.\n\n    """'
newline|'\n'
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|echo
name|'def'
name|'echo'
op|'('
name|'context'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Simply returns whatever value is sent in."""'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|'"Received %s"'
op|')'
op|','
name|'value'
op|')'
newline|'\n'
name|'return'
name|'value'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|context
name|'def'
name|'context'
op|'('
name|'context'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Returns dictionary version of context."""'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|'"Received %s"'
op|')'
op|','
name|'context'
op|')'
newline|'\n'
name|'return'
name|'context'
op|'.'
name|'to_dict'
op|'('
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|multicall_three_nones
name|'def'
name|'multicall_three_nones'
op|'('
name|'context'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'yield'
name|'None'
newline|'\n'
name|'yield'
name|'None'
newline|'\n'
name|'yield'
name|'None'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|echo_three_times_yield
name|'def'
name|'echo_three_times_yield'
op|'('
name|'context'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'yield'
name|'value'
newline|'\n'
name|'yield'
name|'value'
op|'+'
number|'1'
newline|'\n'
name|'yield'
name|'value'
op|'+'
number|'2'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|fail
name|'def'
name|'fail'
op|'('
name|'context'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Raises an exception with the value sent in."""'
newline|'\n'
name|'raise'
name|'NotImplementedError'
op|'('
name|'value'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|fail_converted
name|'def'
name|'fail_converted'
op|'('
name|'context'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Raises an exception with the value sent in."""'
newline|'\n'
name|'raise'
name|'exception'
op|'.'
name|'ConvertedException'
op|'('
name|'explanation'
op|'='
name|'value'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|block
name|'def'
name|'block'
op|'('
name|'context'
op|','
name|'value'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'time'
op|'.'
name|'sleep'
op|'('
number|'2'
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
