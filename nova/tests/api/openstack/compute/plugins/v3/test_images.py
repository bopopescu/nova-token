begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright 2010 OpenStack Foundation'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
string|'"""\nTests of the new image services, both as a service layer,\nand as a WSGI layer\n"""'
newline|'\n'
nl|'\n'
name|'import'
name|'urlparse'
newline|'\n'
nl|'\n'
name|'from'
name|'lxml'
name|'import'
name|'etree'
newline|'\n'
name|'import'
name|'webob'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'api'
op|'.'
name|'openstack'
op|'.'
name|'compute'
op|'.'
name|'plugins'
op|'.'
name|'v3'
name|'import'
name|'images'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'api'
op|'.'
name|'openstack'
op|'.'
name|'compute'
op|'.'
name|'views'
name|'import'
name|'images'
name|'as'
name|'images_view'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'api'
op|'.'
name|'openstack'
name|'import'
name|'xmlutil'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'image'
name|'import'
name|'glance'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'test'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
op|'.'
name|'api'
op|'.'
name|'openstack'
name|'import'
name|'fakes'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'tests'
name|'import'
name|'matchers'
newline|'\n'
nl|'\n'
DECL|variable|NS
name|'NS'
op|'='
string|'"{http://docs.openstack.org/compute/api/v1.1}"'
newline|'\n'
DECL|variable|ATOMNS
name|'ATOMNS'
op|'='
string|'"{http://www.w3.org/2005/Atom}"'
newline|'\n'
DECL|variable|NOW_API_FORMAT
name|'NOW_API_FORMAT'
op|'='
string|'"2010-10-11T10:30:22Z"'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|ImagesControllerTest
name|'class'
name|'ImagesControllerTest'
op|'('
name|'test'
op|'.'
name|'TestCase'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""\n    Test of the OpenStack API /images application controller w/Glance.\n    """'
newline|'\n'
nl|'\n'
DECL|member|setUp
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Run before each test."""'
newline|'\n'
name|'super'
op|'('
name|'ImagesControllerTest'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
name|'fakes'
op|'.'
name|'stub_out_networking'
op|'('
name|'self'
op|'.'
name|'stubs'
op|')'
newline|'\n'
name|'fakes'
op|'.'
name|'stub_out_rate_limiting'
op|'('
name|'self'
op|'.'
name|'stubs'
op|')'
newline|'\n'
name|'fakes'
op|'.'
name|'stub_out_key_pair_funcs'
op|'('
name|'self'
op|'.'
name|'stubs'
op|')'
newline|'\n'
name|'fakes'
op|'.'
name|'stub_out_compute_api_snapshot'
op|'('
name|'self'
op|'.'
name|'stubs'
op|')'
newline|'\n'
name|'fakes'
op|'.'
name|'stub_out_compute_api_backup'
op|'('
name|'self'
op|'.'
name|'stubs'
op|')'
newline|'\n'
name|'fakes'
op|'.'
name|'stub_out_glance'
op|'('
name|'self'
op|'.'
name|'stubs'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_image
dedent|''
name|'def'
name|'test_get_image'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_req'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/os-images/123'"
op|')'
newline|'\n'
name|'actual_image'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'show'
op|'('
name|'fake_req'
op|','
string|"'124'"
op|')'
newline|'\n'
nl|'\n'
name|'href'
op|'='
string|'"http://localhost/v3/images/124"'
newline|'\n'
name|'bookmark'
op|'='
string|'"http://localhost/images/124"'
newline|'\n'
name|'alternate'
op|'='
string|'"%s/images/124"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
newline|'\n'
name|'server_uuid'
op|'='
string|'"aa640691-d1a7-4a67-9d3c-d35ee6b3cc74"'
newline|'\n'
name|'server_href'
op|'='
string|'"http://localhost/v3/servers/"'
op|'+'
name|'server_uuid'
newline|'\n'
name|'server_bookmark'
op|'='
string|'"http://localhost/servers/"'
op|'+'
name|'server_uuid'
newline|'\n'
nl|'\n'
name|'expected_image'
op|'='
op|'{'
nl|'\n'
string|'"image"'
op|':'
op|'{'
nl|'\n'
string|'"id"'
op|':'
string|'"124"'
op|','
nl|'\n'
string|'"name"'
op|':'
string|'"queued snapshot"'
op|','
nl|'\n'
string|'"updated"'
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|'"created"'
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|'"status"'
op|':'
string|'"SAVING"'
op|','
nl|'\n'
string|'"progress"'
op|':'
number|'25'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|'"minDisk"'
op|':'
number|'0'
op|','
nl|'\n'
string|'"minRam"'
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"metadata"'
op|':'
op|'{'
nl|'\n'
string|'"instance_uuid"'
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"user_id"'
op|':'
string|'"fake"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'bookmark'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'alternate'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'}'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertThat'
op|'('
name|'actual_image'
op|','
name|'matchers'
op|'.'
name|'DictMatches'
op|'('
name|'expected_image'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_image_with_custom_prefix
dedent|''
name|'def'
name|'test_get_image_with_custom_prefix'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'flags'
op|'('
name|'osapi_compute_link_prefix'
op|'='
string|"'https://zoo.com:42'"
op|','
nl|'\n'
name|'osapi_glance_link_prefix'
op|'='
string|"'http://circus.com:34'"
op|')'
newline|'\n'
name|'fake_req'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/124'"
op|')'
newline|'\n'
name|'actual_image'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'show'
op|'('
name|'fake_req'
op|','
string|"'124'"
op|')'
newline|'\n'
name|'href'
op|'='
string|'"https://zoo.com:42/v3/images/124"'
newline|'\n'
name|'bookmark'
op|'='
string|'"https://zoo.com:42/images/124"'
newline|'\n'
name|'alternate'
op|'='
string|'"http://circus.com:34/images/124"'
newline|'\n'
name|'server_uuid'
op|'='
string|'"aa640691-d1a7-4a67-9d3c-d35ee6b3cc74"'
newline|'\n'
name|'server_href'
op|'='
string|'"https://zoo.com:42/v3/servers/"'
op|'+'
name|'server_uuid'
newline|'\n'
name|'server_bookmark'
op|'='
string|'"https://zoo.com:42/servers/"'
op|'+'
name|'server_uuid'
newline|'\n'
nl|'\n'
name|'expected_image'
op|'='
op|'{'
nl|'\n'
string|'"image"'
op|':'
op|'{'
nl|'\n'
string|'"id"'
op|':'
string|'"124"'
op|','
nl|'\n'
string|'"name"'
op|':'
string|'"queued snapshot"'
op|','
nl|'\n'
string|'"updated"'
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|'"created"'
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|'"status"'
op|':'
string|'"SAVING"'
op|','
nl|'\n'
string|'"progress"'
op|':'
number|'25'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|'"minDisk"'
op|':'
number|'0'
op|','
nl|'\n'
string|'"minRam"'
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"metadata"'
op|':'
op|'{'
nl|'\n'
string|'"instance_uuid"'
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"user_id"'
op|':'
string|'"fake"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'bookmark'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'alternate'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'assertThat'
op|'('
name|'actual_image'
op|','
name|'matchers'
op|'.'
name|'DictMatches'
op|'('
name|'expected_image'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_image_404
dedent|''
name|'def'
name|'test_get_image_404'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'fake_req'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/os-images/unknown'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|','
nl|'\n'
name|'self'
op|'.'
name|'controller'
op|'.'
name|'show'
op|','
name|'fake_req'
op|','
string|"'unknown'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_image_details
dedent|''
name|'def'
name|'test_get_image_details'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/os-images/detail'"
op|')'
newline|'\n'
name|'response'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
name|'response_list'
op|'='
name|'response'
op|'['
string|'"images"'
op|']'
newline|'\n'
nl|'\n'
name|'server_uuid'
op|'='
string|'"aa640691-d1a7-4a67-9d3c-d35ee6b3cc74"'
newline|'\n'
name|'server_href'
op|'='
string|'"http://localhost/v3/servers/"'
op|'+'
name|'server_uuid'
newline|'\n'
name|'server_bookmark'
op|'='
string|'"http://localhost/servers/"'
op|'+'
name|'server_uuid'
newline|'\n'
name|'alternate'
op|'='
string|'"%s/images/%s"'
newline|'\n'
nl|'\n'
name|'expected'
op|'='
op|'['
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'123'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'public image'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
string|"'key1'"
op|':'
string|"'value1'"
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'ACTIVE'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'100'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'10'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'128'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/123"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/123"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'alternate'
op|'%'
op|'('
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
op|','
number|'123'
op|')'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'124'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'queued snapshot'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
nl|'\n'
string|"u'instance_uuid'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|"u'user_id'"
op|':'
string|"u'fake'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'SAVING'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'25'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/124"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/124"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'alternate'
op|'%'
op|'('
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
op|','
number|'124'
op|')'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'125'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'saving snapshot'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
nl|'\n'
string|"u'instance_uuid'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|"u'user_id'"
op|':'
string|"u'fake'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'SAVING'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'50'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/125"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/125"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"%s/images/125"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'126'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'active snapshot'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
nl|'\n'
string|"u'instance_uuid'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|"u'user_id'"
op|':'
string|"u'fake'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'ACTIVE'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'100'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/126"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/126"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"%s/images/126"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'127'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'killed snapshot'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
nl|'\n'
string|"u'instance_uuid'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|"u'user_id'"
op|':'
string|"u'fake'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'ERROR'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'0'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/127"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/127"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"%s/images/127"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'128'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'deleted snapshot'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
nl|'\n'
string|"u'instance_uuid'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|"u'user_id'"
op|':'
string|"u'fake'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'DELETED'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'0'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/128"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/128"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"%s/images/128"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'129'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'pending_delete snapshot'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
nl|'\n'
string|"u'instance_uuid'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|"u'user_id'"
op|':'
string|"u'fake'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'DELETED'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'0'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/129"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/129"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"%s/images/129"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'130'"
op|','
nl|'\n'
string|"'name'"
op|':'
name|'None'
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'ACTIVE'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'100'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'0'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/130"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/130"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"%s/images/130"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'131'"
op|','
nl|'\n'
string|"'name'"
op|':'
name|'None'
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'ACTIVE'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'100'
op|','
nl|'\n'
string|'"size"'
op|':'
number|'0'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/131"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/131"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"%s/images/131"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertThat'
op|'('
name|'expected'
op|','
name|'matchers'
op|'.'
name|'DictListMatches'
op|'('
name|'response_list'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_get_image_details_with_limit
dedent|''
name|'def'
name|'test_get_image_details_with_limit'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/os-images/detail?limit=2'"
op|')'
newline|'\n'
name|'response'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
name|'response_list'
op|'='
name|'response'
op|'['
string|'"images"'
op|']'
newline|'\n'
name|'response_links'
op|'='
name|'response'
op|'['
string|'"images_links"'
op|']'
newline|'\n'
nl|'\n'
name|'server_uuid'
op|'='
string|'"aa640691-d1a7-4a67-9d3c-d35ee6b3cc74"'
newline|'\n'
name|'server_href'
op|'='
string|'"http://localhost/v3/servers/"'
op|'+'
name|'server_uuid'
newline|'\n'
name|'server_bookmark'
op|'='
string|'"http://localhost/servers/"'
op|'+'
name|'server_uuid'
newline|'\n'
name|'alternate'
op|'='
string|'"%s/images/%s"'
newline|'\n'
nl|'\n'
name|'expected'
op|'='
op|'['
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'123'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'public image'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
string|"'key1'"
op|':'
string|"'value1'"
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'ACTIVE'"
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'10'
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'100'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'128'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/123"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/123"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'alternate'
op|'%'
op|'('
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
op|','
number|'123'
op|')'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'id'"
op|':'
string|"'124'"
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'queued snapshot'"
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
nl|'\n'
string|"u'instance_uuid'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|"u'user_id'"
op|':'
string|"u'fake'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'created'"
op|':'
name|'NOW_API_FORMAT'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'SAVING'"
op|','
nl|'\n'
string|'"size"'
op|':'
number|'25165824'
op|','
nl|'\n'
string|"'minDisk'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'25'
op|','
nl|'\n'
string|"'minRam'"
op|':'
number|'0'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'server_uuid'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_href'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'server_bookmark'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|'"links"'
op|':'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/v3/images/124"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
string|'"http://localhost/images/124"'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"alternate"'
op|','
nl|'\n'
string|'"type"'
op|':'
string|'"application/vnd.openstack.image"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'alternate'
op|'%'
op|'('
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
op|','
number|'124'
op|')'
op|','
nl|'\n'
op|'}'
op|']'
op|','
nl|'\n'
op|'}'
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertThat'
op|'('
name|'expected'
op|','
name|'matchers'
op|'.'
name|'DictListMatches'
op|'('
name|'response_list'
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'href_parts'
op|'='
name|'urlparse'
op|'.'
name|'urlparse'
op|'('
name|'response_links'
op|'['
number|'0'
op|']'
op|'['
string|"'href'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
string|"'/v3/images'"
op|','
name|'href_parts'
op|'.'
name|'path'
op|')'
newline|'\n'
name|'params'
op|'='
name|'urlparse'
op|'.'
name|'parse_qs'
op|'('
name|'href_parts'
op|'.'
name|'query'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'assertThat'
op|'('
op|'{'
string|"'limit'"
op|':'
op|'['
string|"'2'"
op|']'
op|','
string|"'marker'"
op|':'
op|'['
string|"'124'"
op|']'
op|'}'
op|','
nl|'\n'
name|'matchers'
op|'.'
name|'DictMatches'
op|'('
name|'params'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_filter_with_name
dedent|''
name|'def'
name|'test_image_detail_filter_with_name'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'filters'
op|'='
op|'{'
string|"'name'"
op|':'
string|"'testname'"
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/detail'"
nl|'\n'
string|"'?name=testname'"
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_filter_with_status
dedent|''
name|'def'
name|'test_image_detail_filter_with_status'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'filters'
op|'='
op|'{'
string|"'status'"
op|':'
string|"'active'"
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/detail'"
nl|'\n'
string|"'?status=ACTIVE'"
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_filter_with_property
dedent|''
name|'def'
name|'test_image_detail_filter_with_property'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'filters'
op|'='
op|'{'
string|"'property-test'"
op|':'
string|"'3'"
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/detail'"
nl|'\n'
string|"'?property-test=3'"
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_filter_server_href
dedent|''
name|'def'
name|'test_image_detail_filter_server_href'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'uuid'
op|'='
string|"'fa95aaf5-ab3b-4cd8-88c0-2be7dd051aaf'"
newline|'\n'
name|'ref'
op|'='
string|"'http://localhost:8774/servers/'"
op|'+'
name|'uuid'
newline|'\n'
name|'url'
op|'='
string|"'/v3/os-images/detail?server='"
op|'+'
name|'ref'
newline|'\n'
name|'filters'
op|'='
op|'{'
string|"'property-instance_uuid'"
op|':'
name|'uuid'
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
name|'url'
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_filter_server_uuid
dedent|''
name|'def'
name|'test_image_detail_filter_server_uuid'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'uuid'
op|'='
string|"'fa95aaf5-ab3b-4cd8-88c0-2be7dd051aaf'"
newline|'\n'
name|'url'
op|'='
string|"'/v3/os-images/detail?server='"
op|'+'
name|'uuid'
newline|'\n'
name|'filters'
op|'='
op|'{'
string|"'property-instance_uuid'"
op|':'
name|'uuid'
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
name|'url'
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_filter_changes_since
dedent|''
name|'def'
name|'test_image_detail_filter_changes_since'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'filters'
op|'='
op|'{'
string|"'changes-since'"
op|':'
string|"'2011-01-24T17:08Z'"
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/detail'"
nl|'\n'
string|"'?changes-since=2011-01-24T17:08Z'"
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_filter_with_type
dedent|''
name|'def'
name|'test_image_detail_filter_with_type'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'filters'
op|'='
op|'{'
string|"'property-image_type'"
op|':'
string|"'BASE'"
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/detail?type=BASE'"
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_filter_not_supported
dedent|''
name|'def'
name|'test_image_detail_filter_not_supported'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'filters'
op|'='
op|'{'
string|"'status'"
op|':'
string|"'active'"
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/detail?status='"
nl|'\n'
string|"'ACTIVE&UNSUPPORTEDFILTER=testname'"
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_no_filters
dedent|''
name|'def'
name|'test_image_detail_no_filters'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'image_service'
op|'='
name|'self'
op|'.'
name|'mox'
op|'.'
name|'CreateMockAnything'
op|'('
op|')'
newline|'\n'
name|'filters'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/detail'"
op|')'
newline|'\n'
name|'context'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|"'nova.context'"
op|']'
newline|'\n'
name|'image_service'
op|'.'
name|'detail'
op|'('
name|'context'
op|','
name|'filters'
op|'='
name|'filters'
op|')'
op|'.'
name|'AndReturn'
op|'('
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'mox'
op|'.'
name|'ReplayAll'
op|'('
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
name|'image_service'
op|'='
name|'image_service'
op|')'
newline|'\n'
name|'controller'
op|'.'
name|'detail'
op|'('
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_image_detail_invalid_marker
dedent|''
name|'def'
name|'test_image_detail_invalid_marker'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
DECL|class|InvalidImageService
indent|'        '
name|'class'
name|'InvalidImageService'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
nl|'\n'
DECL|member|detail
indent|'            '
name|'def'
name|'detail'
op|'('
name|'self'
op|','
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exception'
op|'.'
name|'Invalid'
op|'('
string|"'meow'"
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images?marker=invalid'"
op|')'
newline|'\n'
name|'controller'
op|'='
name|'images'
op|'.'
name|'ImagesController'
op|'('
nl|'\n'
name|'image_service'
op|'='
name|'InvalidImageService'
op|'('
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|','
name|'controller'
op|'.'
name|'detail'
op|','
nl|'\n'
name|'request'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_generate_alternate_link
dedent|''
name|'def'
name|'test_generate_alternate_link'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'view'
op|'='
name|'images_view'
op|'.'
name|'ViewBuilderV3'
op|'('
op|')'
newline|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/1'"
op|')'
newline|'\n'
name|'generated_url'
op|'='
name|'view'
op|'.'
name|'_get_alternate_link'
op|'('
name|'request'
op|','
number|'1'
op|')'
newline|'\n'
name|'actual_url'
op|'='
string|'"%s/images/1"'
op|'%'
name|'glance'
op|'.'
name|'generate_glance_url'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'generated_url'
op|','
name|'actual_url'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_delete_image
dedent|''
name|'def'
name|'test_delete_image'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/124'"
op|')'
newline|'\n'
name|'request'
op|'.'
name|'method'
op|'='
string|"'DELETE'"
newline|'\n'
name|'response'
op|'='
name|'self'
op|'.'
name|'controller'
op|'.'
name|'delete'
op|'('
name|'request'
op|','
string|"'124'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'response'
op|'.'
name|'status_int'
op|','
number|'204'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_delete_deleted_image
dedent|''
name|'def'
name|'test_delete_deleted_image'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""If you try to delete a deleted image, you get back 403 Forbidden."""'
newline|'\n'
nl|'\n'
name|'deleted_image_id'
op|'='
number|'128'
newline|'\n'
comment|'# see nova.tests.api.openstack.fakes:_make_image_fixtures'
nl|'\n'
nl|'\n'
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
nl|'\n'
string|"'/v3/os-images/%s'"
op|'%'
name|'deleted_image_id'
op|')'
newline|'\n'
name|'request'
op|'.'
name|'method'
op|'='
string|"'DELETE'"
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPForbidden'
op|','
name|'self'
op|'.'
name|'controller'
op|'.'
name|'delete'
op|','
nl|'\n'
name|'request'
op|','
string|"'%s'"
op|'%'
name|'deleted_image_id'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_delete_image_not_found
dedent|''
name|'def'
name|'test_delete_image_not_found'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'request'
op|'='
name|'fakes'
op|'.'
name|'HTTPRequestV3'
op|'.'
name|'blank'
op|'('
string|"'/v3/os-images/300'"
op|')'
newline|'\n'
name|'request'
op|'.'
name|'method'
op|'='
string|"'DELETE'"
newline|'\n'
name|'self'
op|'.'
name|'assertRaises'
op|'('
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|','
nl|'\n'
name|'self'
op|'.'
name|'controller'
op|'.'
name|'delete'
op|','
name|'request'
op|','
string|"'300'"
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|ImageXMLSerializationTest
dedent|''
dedent|''
name|'class'
name|'ImageXMLSerializationTest'
op|'('
name|'test'
op|'.'
name|'TestCase'
op|')'
op|':'
newline|'\n'
nl|'\n'
DECL|variable|TIMESTAMP
indent|'    '
name|'TIMESTAMP'
op|'='
string|'"2010-10-11T10:30:22Z"'
newline|'\n'
DECL|variable|SERVER_UUID
name|'SERVER_UUID'
op|'='
string|"'aa640691-d1a7-4a67-9d3c-d35ee6b3cc74'"
newline|'\n'
DECL|variable|SERVER_HREF
name|'SERVER_HREF'
op|'='
string|"'http://localhost/v3/servers/'"
op|'+'
name|'SERVER_UUID'
newline|'\n'
DECL|variable|SERVER_BOOKMARK
name|'SERVER_BOOKMARK'
op|'='
string|"'http://localhost/servers/'"
op|'+'
name|'SERVER_UUID'
newline|'\n'
DECL|variable|IMAGE_HREF
name|'IMAGE_HREF'
op|'='
string|"'http://localhost/v3/os-images/%s'"
newline|'\n'
name|'IMAGE_NEXT'
op|'='
string|"'http://localhost/v3/os-images?limit=%s&marker=%s'"
newline|'\n'
DECL|variable|IMAGE_BOOKMARK
name|'IMAGE_BOOKMARK'
op|'='
string|"'http://localhost/os-images/%s'"
newline|'\n'
nl|'\n'
DECL|member|setUp
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'super'
op|'('
name|'ImageXMLSerializationTest'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'fixture'
op|'='
op|'{'
nl|'\n'
string|"'image'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
number|'1'
op|','
nl|'\n'
string|"'name'"
op|':'
string|"'Image1'"
op|','
nl|'\n'
string|"'created'"
op|':'
name|'self'
op|'.'
name|'TIMESTAMP'
op|','
nl|'\n'
string|"'updated'"
op|':'
name|'self'
op|'.'
name|'TIMESTAMP'
op|','
nl|'\n'
string|"'status'"
op|':'
string|"'ACTIVE'"
op|','
nl|'\n'
string|"'progress'"
op|':'
number|'80'
op|','
nl|'\n'
string|"'server'"
op|':'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'self'
op|'.'
name|'SERVER_UUID'
op|','
nl|'\n'
string|"'links'"
op|':'
op|'['
nl|'\n'
op|'{'
nl|'\n'
string|"'href'"
op|':'
name|'self'
op|'.'
name|'SERVER_HREF'
op|','
nl|'\n'
string|"'rel'"
op|':'
string|"'self'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'href'"
op|':'
name|'self'
op|'.'
name|'SERVER_BOOKMARK'
op|','
nl|'\n'
string|"'rel'"
op|':'
string|"'bookmark'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'metadata'"
op|':'
op|'{'
nl|'\n'
string|"'key1'"
op|':'
string|"'value1'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
string|"'links'"
op|':'
op|'['
nl|'\n'
op|'{'
nl|'\n'
string|"'href'"
op|':'
name|'self'
op|'.'
name|'IMAGE_HREF'
op|'%'
number|'1'
op|','
nl|'\n'
string|"'rel'"
op|':'
string|"'self'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|"'href'"
op|':'
name|'self'
op|'.'
name|'IMAGE_BOOKMARK'
op|'%'
number|'1'
op|','
nl|'\n'
string|"'rel'"
op|':'
string|"'bookmark'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|']'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'}'
newline|'\n'
name|'image'
op|'='
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
newline|'\n'
name|'image'
op|'['
string|"'id'"
op|']'
op|'='
string|"'2'"
newline|'\n'
name|'image'
op|'['
string|"'name'"
op|']'
op|'='
string|"'Image2'"
newline|'\n'
name|'image'
op|'['
string|"'status'"
op|']'
op|'='
string|"'SAVING'"
newline|'\n'
name|'image'
op|'['
string|"'links'"
op|']'
op|'['
number|'0'
op|']'
op|'['
string|"'href'"
op|']'
op|'='
name|'self'
op|'.'
name|'IMAGE_HREF'
op|'%'
number|'2'
newline|'\n'
name|'image'
op|'['
string|"'links'"
op|']'
op|'['
number|'1'
op|']'
op|'['
string|"'href'"
op|']'
op|'='
name|'self'
op|'.'
name|'IMAGE_BOOKMARK'
op|'%'
number|'2'
newline|'\n'
name|'self'
op|'.'
name|'fixture_dict'
op|'='
op|'{'
nl|'\n'
string|"'images'"
op|':'
op|'['
nl|'\n'
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
op|','
nl|'\n'
name|'image'
op|','
nl|'\n'
op|']'
op|','
nl|'\n'
string|"'images_links'"
op|':'
op|'['
nl|'\n'
op|'{'
nl|'\n'
string|"'rel'"
op|':'
string|"'next'"
op|','
nl|'\n'
string|"'href'"
op|':'
name|'self'
op|'.'
name|'IMAGE_NEXT'
op|'%'
op|'('
number|'2'
op|','
number|'2'
op|')'
op|','
nl|'\n'
op|'}'
nl|'\n'
op|']'
op|','
nl|'\n'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'serializer'
op|'='
name|'images'
op|'.'
name|'ImageTemplate'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_xml_declaration
dedent|''
name|'def'
name|'test_xml_declaration'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'output'
op|'='
name|'self'
op|'.'
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture'
op|')'
newline|'\n'
name|'has_dec'
op|'='
name|'output'
op|'.'
name|'startswith'
op|'('
string|'"<?xml version=\'1.0\' encoding=\'UTF-8\'?>"'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'has_dec'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show
dedent|''
name|'def'
name|'test_show'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
op|'['
string|"'minRam'"
op|']'
op|'='
number|'10'
newline|'\n'
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
op|'['
string|"'minDisk'"
op|']'
op|'='
number|'100'
newline|'\n'
name|'output'
op|'='
name|'self'
op|'.'
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'image'"
op|')'
newline|'\n'
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
string|"'id'"
op|','
nl|'\n'
string|"'updated'"
op|','
string|"'created'"
op|','
string|"'status'"
op|','
nl|'\n'
string|"'progress'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertMetadataEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertServerIdAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show_zero_metadata
dedent|''
name|'def'
name|'test_show_zero_metadata'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
op|'['
string|"'metadata'"
op|']'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'output'
op|'='
name|'self'
op|'.'
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'image'"
op|')'
newline|'\n'
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
newline|'\n'
nl|'\n'
name|'meta_nodes'
op|'='
name|'root'
op|'.'
name|'findall'
op|'('
string|"'{0}meta'"
op|'.'
name|'format'
op|'('
name|'ATOMNS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'meta_nodes'
op|')'
op|','
number|'0'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
string|"'id'"
op|','
nl|'\n'
string|"'updated'"
op|','
string|"'created'"
op|','
string|"'status'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertServerIdAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show_image_no_metadata_key
dedent|''
name|'def'
name|'test_show_image_no_metadata_key'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'del'
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
op|'['
string|"'metadata'"
op|']'
newline|'\n'
name|'output'
op|'='
name|'self'
op|'.'
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'image'"
op|')'
newline|'\n'
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
newline|'\n'
nl|'\n'
name|'meta_nodes'
op|'='
name|'root'
op|'.'
name|'findall'
op|'('
string|"'{0}meta'"
op|'.'
name|'format'
op|'('
name|'ATOMNS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'meta_nodes'
op|')'
op|','
number|'0'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
string|"'id'"
op|','
nl|'\n'
string|"'updated'"
op|','
string|"'created'"
op|','
string|"'status'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertServerIdAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show_no_server
dedent|''
name|'def'
name|'test_show_no_server'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'del'
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
op|'['
string|"'server'"
op|']'
newline|'\n'
name|'output'
op|'='
name|'self'
op|'.'
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'image'"
op|')'
newline|'\n'
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
newline|'\n'
nl|'\n'
name|'server_root'
op|'='
name|'root'
op|'.'
name|'find'
op|'('
string|"'{0}server'"
op|'.'
name|'format'
op|'('
name|'NS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertIsNone'
op|'('
name|'server_root'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
string|"'id'"
op|','
nl|'\n'
string|"'updated'"
op|','
string|"'created'"
op|','
string|"'status'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertMetadataEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show_with_min_ram
dedent|''
name|'def'
name|'test_show_with_min_ram'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
op|'['
string|"'minRam'"
op|']'
op|'='
number|'256'
newline|'\n'
name|'output'
op|'='
name|'self'
op|'.'
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'image'"
op|')'
newline|'\n'
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
string|"'id'"
op|','
nl|'\n'
string|"'updated'"
op|','
string|"'created'"
op|','
string|"'status'"
op|','
nl|'\n'
string|"'progress'"
op|','
string|"'minRam'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertMetadataEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertServerIdAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_show_with_min_disk
dedent|''
name|'def'
name|'test_show_with_min_disk'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
op|'['
string|"'minDisk'"
op|']'
op|'='
number|'5'
newline|'\n'
name|'output'
op|'='
name|'self'
op|'.'
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'image'"
op|')'
newline|'\n'
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture'
op|'['
string|"'image'"
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
string|"'id'"
op|','
nl|'\n'
string|"'updated'"
op|','
string|"'created'"
op|','
string|"'status'"
op|','
nl|'\n'
string|"'progress'"
op|','
string|"'minDisk'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertMetadataEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertServerIdAndLinksEquals'
op|'('
name|'root'
op|','
name|'image_dict'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_index
dedent|''
name|'def'
name|'test_index'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'serializer'
op|'='
name|'images'
op|'.'
name|'MinimalImagesTemplate'
op|'('
op|')'
newline|'\n'
name|'output'
op|'='
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture_dict'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'images_index'"
op|')'
newline|'\n'
name|'image_elems'
op|'='
name|'root'
op|'.'
name|'findall'
op|'('
string|"'{0}image'"
op|'.'
name|'format'
op|'('
name|'NS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'image_elems'
op|')'
op|','
number|'2'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'image_elem'
name|'in'
name|'enumerate'
op|'('
name|'image_elems'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture_dict'
op|'['
string|"'images'"
op|']'
op|'['
name|'i'
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'image_elem'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
nl|'\n'
string|"'id'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_index_with_links
dedent|''
dedent|''
name|'def'
name|'test_index_with_links'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'serializer'
op|'='
name|'images'
op|'.'
name|'MinimalImagesTemplate'
op|'('
op|')'
newline|'\n'
name|'output'
op|'='
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture_dict'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'images_index'"
op|')'
newline|'\n'
name|'image_elems'
op|'='
name|'root'
op|'.'
name|'findall'
op|'('
string|"'{0}image'"
op|'.'
name|'format'
op|'('
name|'NS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'image_elems'
op|')'
op|','
number|'2'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'image_elem'
name|'in'
name|'enumerate'
op|'('
name|'image_elems'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture_dict'
op|'['
string|"'images'"
op|']'
op|'['
name|'i'
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'image_elem'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
nl|'\n'
string|"'id'"
op|']'
op|')'
newline|'\n'
nl|'\n'
name|'images_links'
op|'='
name|'root'
op|'.'
name|'findall'
op|'('
string|"'{0}link'"
op|'.'
name|'format'
op|'('
name|'ATOMNS'
op|')'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'link'
name|'in'
name|'enumerate'
op|'('
name|'self'
op|'.'
name|'fixture_dict'
op|'['
string|"'images_links'"
op|']'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'for'
name|'key'
op|','
name|'value'
name|'in'
name|'link'
op|'.'
name|'items'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'images_links'
op|'['
name|'i'
op|']'
op|'.'
name|'get'
op|'('
name|'key'
op|')'
op|','
name|'value'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_index_zero_images
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'test_index_zero_images'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'serializer'
op|'='
name|'images'
op|'.'
name|'MinimalImagesTemplate'
op|'('
op|')'
newline|'\n'
name|'del'
name|'self'
op|'.'
name|'fixture_dict'
op|'['
string|"'images'"
op|']'
newline|'\n'
name|'output'
op|'='
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture_dict'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'images_index'"
op|')'
newline|'\n'
name|'image_elems'
op|'='
name|'root'
op|'.'
name|'findall'
op|'('
string|"'{0}image'"
op|'.'
name|'format'
op|'('
name|'NS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'image_elems'
op|')'
op|','
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_detail
dedent|''
name|'def'
name|'test_detail'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'serializer'
op|'='
name|'images'
op|'.'
name|'ImagesTemplate'
op|'('
op|')'
newline|'\n'
name|'output'
op|'='
name|'serializer'
op|'.'
name|'serialize'
op|'('
name|'self'
op|'.'
name|'fixture_dict'
op|')'
newline|'\n'
name|'root'
op|'='
name|'etree'
op|'.'
name|'XML'
op|'('
name|'output'
op|')'
newline|'\n'
name|'xmlutil'
op|'.'
name|'validate_schema'
op|'('
name|'root'
op|','
string|"'images'"
op|')'
newline|'\n'
name|'image_elems'
op|'='
name|'root'
op|'.'
name|'findall'
op|'('
string|"'{0}image'"
op|'.'
name|'format'
op|'('
name|'NS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'image_elems'
op|')'
op|','
number|'2'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'image_elem'
name|'in'
name|'enumerate'
op|'('
name|'image_elems'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'image_dict'
op|'='
name|'self'
op|'.'
name|'fixture_dict'
op|'['
string|"'images'"
op|']'
op|'['
name|'i'
op|']'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_assertElementAndLinksEquals'
op|'('
name|'image_elem'
op|','
name|'image_dict'
op|','
op|'['
string|"'name'"
op|','
nl|'\n'
string|"'id'"
op|','
string|"'updated'"
op|','
string|"'created'"
op|','
nl|'\n'
string|"'status'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_assertElementAndLinksEquals
dedent|''
dedent|''
name|'def'
name|'_assertElementAndLinksEquals'
op|'('
name|'self'
op|','
name|'elem'
op|','
name|'dict'
op|','
name|'keys'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'for'
name|'key'
name|'in'
name|'keys'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'assertEquals'
op|'('
name|'elem'
op|'.'
name|'get'
op|'('
name|'key'
op|')'
op|','
name|'str'
op|'('
name|'dict'
op|'['
name|'key'
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'_assertLinksEquals'
op|'('
name|'elem'
op|','
name|'dict'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_assertLinksEquals
dedent|''
name|'def'
name|'_assertLinksEquals'
op|'('
name|'self'
op|','
name|'root'
op|','
name|'dict'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'link_nodes'
op|'='
name|'root'
op|'.'
name|'findall'
op|'('
string|"'{0}link'"
op|'.'
name|'format'
op|'('
name|'ATOMNS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'link_nodes'
op|')'
op|','
number|'2'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'link'
name|'in'
name|'enumerate'
op|'('
name|'dict'
op|'['
string|"'links'"
op|']'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'for'
name|'key'
op|','
name|'value'
name|'in'
name|'link'
op|'.'
name|'items'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'link_nodes'
op|'['
name|'i'
op|']'
op|'.'
name|'get'
op|'('
name|'key'
op|')'
op|','
name|'value'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_assertServerIdAndLinksEquals
dedent|''
dedent|''
dedent|''
name|'def'
name|'_assertServerIdAndLinksEquals'
op|'('
name|'self'
op|','
name|'root'
op|','
name|'dict'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'server_root'
op|'='
name|'root'
op|'.'
name|'find'
op|'('
string|"'{0}server'"
op|'.'
name|'format'
op|'('
name|'NS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'server_root'
op|'.'
name|'get'
op|'('
string|"'id'"
op|')'
op|','
name|'dict'
op|'['
string|"'server'"
op|']'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_assertLinksEquals'
op|'('
name|'server_root'
op|','
name|'dict'
op|'['
string|"'server'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_assertMetadataEquals
dedent|''
name|'def'
name|'_assertMetadataEquals'
op|'('
name|'self'
op|','
name|'root'
op|','
name|'dict'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'metadata_root'
op|'='
name|'root'
op|'.'
name|'find'
op|'('
string|"'{0}metadata'"
op|'.'
name|'format'
op|'('
name|'NS'
op|')'
op|')'
newline|'\n'
name|'metadata_elems'
op|'='
name|'metadata_root'
op|'.'
name|'findall'
op|'('
string|"'{0}meta'"
op|'.'
name|'format'
op|'('
name|'NS'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'len'
op|'('
name|'metadata_elems'
op|')'
op|','
number|'1'
op|')'
newline|'\n'
name|'for'
name|'i'
op|','
name|'metadata_elem'
name|'in'
name|'enumerate'
op|'('
name|'metadata_elems'
op|')'
op|':'
newline|'\n'
indent|'            '
op|'('
name|'meta_key'
op|','
name|'meta_value'
op|')'
op|'='
name|'dict'
op|'['
string|"'metadata'"
op|']'
op|'.'
name|'items'
op|'('
op|')'
op|'['
name|'i'
op|']'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'str'
op|'('
name|'metadata_elem'
op|'.'
name|'get'
op|'('
string|"'key'"
op|')'
op|')'
op|','
name|'str'
op|'('
name|'meta_key'
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'str'
op|'('
name|'metadata_elem'
op|'.'
name|'text'
op|')'
op|'.'
name|'strip'
op|'('
op|')'
op|','
name|'str'
op|'('
name|'meta_value'
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
endmarker|''
end_unit
