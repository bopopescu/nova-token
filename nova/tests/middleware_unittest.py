begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright 2010 United States Government as represented by the'
nl|'\n'
comment|'# Administrator of the National Aeronautics and Space Administration.'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
name|'import'
name|'webob'
newline|'\n'
name|'import'
name|'webob'
op|'.'
name|'dec'
newline|'\n'
name|'import'
name|'webob'
op|'.'
name|'exc'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'api'
name|'import'
name|'ec2'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'flags'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'test'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|FLAGS
name|'FLAGS'
op|'='
name|'flags'
op|'.'
name|'FLAGS'
newline|'\n'
nl|'\n'
nl|'\n'
op|'@'
name|'webob'
op|'.'
name|'dec'
op|'.'
name|'wsgify'
newline|'\n'
DECL|function|conditional_forbid
name|'def'
name|'conditional_forbid'
op|'('
name|'req'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Helper wsgi app returns 403 if param \'die\' is 1."""'
newline|'\n'
name|'if'
string|"'die'"
name|'in'
name|'req'
op|'.'
name|'params'
name|'and'
name|'req'
op|'.'
name|'params'
op|'['
string|"'die'"
op|']'
op|'=='
string|"'1'"
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPForbidden'
op|'('
op|')'
newline|'\n'
dedent|''
name|'return'
string|"'OK'"
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|LockoutTestCase
dedent|''
name|'class'
name|'LockoutTestCase'
op|'('
name|'test'
op|'.'
name|'TrialTestCase'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Test case for the Lockout middleware."""'
newline|'\n'
DECL|member|setUp
name|'def'
name|'setUp'
op|'('
name|'self'
op|')'
op|':'
comment|'# pylint: disable-msg=C0103'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'local_time'
op|'='
number|'0'
newline|'\n'
name|'self'
op|'.'
name|'lockout'
op|'='
name|'ec2'
op|'.'
name|'Lockout'
op|'('
name|'conditional_forbid'
op|','
nl|'\n'
name|'time_fn'
op|'='
name|'self'
op|'.'
name|'_constant_time'
op|')'
newline|'\n'
name|'super'
op|'('
name|'LockoutTestCase'
op|','
name|'self'
op|')'
op|'.'
name|'setUp'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|_constant_time
dedent|''
name|'def'
name|'_constant_time'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Helper method to force timeouts."""'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'local_time'
newline|'\n'
nl|'\n'
DECL|member|_trigger_lockout
dedent|''
name|'def'
name|'_trigger_lockout'
op|'('
name|'self'
op|','
name|'access_key'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Send x failed requests where x = lockout_attempts."""'
newline|'\n'
name|'for'
name|'i'
name|'in'
name|'xrange'
op|'('
name|'FLAGS'
op|'.'
name|'lockout_attempts'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'req'
op|'='
name|'webob'
op|'.'
name|'Request'
op|'.'
name|'blank'
op|'('
string|"'/?AWSAccessKeyId=%s&die=1'"
op|'%'
name|'access_key'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertEqual'
op|'('
name|'req'
op|'.'
name|'get_response'
op|'('
name|'self'
op|'.'
name|'lockout'
op|')'
op|'.'
name|'status_int'
op|','
number|'403'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_is_locked_out
dedent|''
dedent|''
name|'def'
name|'_is_locked_out'
op|'('
name|'self'
op|','
name|'access_key'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Sends a test request to see if key is locked out."""'
newline|'\n'
name|'req'
op|'='
name|'webob'
op|'.'
name|'Request'
op|'.'
name|'blank'
op|'('
string|"'/?AWSAccessKeyId=%s'"
op|'%'
name|'access_key'
op|')'
newline|'\n'
name|'return'
op|'('
name|'req'
op|'.'
name|'get_response'
op|'('
name|'self'
op|'.'
name|'lockout'
op|')'
op|'.'
name|'status_int'
op|'=='
number|'403'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_timeout
dedent|''
name|'def'
name|'_timeout'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Increment time to 1 second past the lockout."""'
newline|'\n'
name|'self'
op|'.'
name|'local_time'
op|'='
number|'1'
op|'+'
name|'self'
op|'.'
name|'local_time'
op|'+'
name|'FLAGS'
op|'.'
name|'lockout_minutes'
op|'*'
number|'60'
newline|'\n'
nl|'\n'
DECL|member|test_lockout
dedent|''
name|'def'
name|'test_lockout'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_trigger_lockout'
op|'('
string|"'test'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'self'
op|'.'
name|'_is_locked_out'
op|'('
string|"'test'"
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_timeout
dedent|''
name|'def'
name|'test_timeout'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_trigger_lockout'
op|'('
string|"'test'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'self'
op|'.'
name|'_is_locked_out'
op|'('
string|"'test'"
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_timeout'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'self'
op|'.'
name|'_is_locked_out'
op|'('
string|"'test'"
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|test_multiple_keys
dedent|''
name|'def'
name|'test_multiple_keys'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_trigger_lockout'
op|'('
string|"'test1'"
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertTrue'
op|'('
name|'self'
op|'.'
name|'_is_locked_out'
op|'('
string|"'test1'"
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'self'
op|'.'
name|'_is_locked_out'
op|'('
string|"'test2'"
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_timeout'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'self'
op|'.'
name|'_is_locked_out'
op|'('
string|"'test1'"
op|')'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'assertFalse'
op|'('
name|'self'
op|'.'
name|'_is_locked_out'
op|'('
string|"'test2'"
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
