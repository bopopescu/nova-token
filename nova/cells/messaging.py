...
name|'_LE'
op|'('
string|'"Error processing message locally"'
op|')'
op|')'
newline|'\n'
dedent|''
...
op|')'
newline|'\n'
comment|"# NOTE(comstud): 'method_kwargs' needs special serialization"
nl|'\n'
comment|'# because it may contain objects.'
nl|'\n'
name|'method_kwargs'
op|'='
name|'_dict'
...
op|')'
newline|'\n'
comment|'# NOTE(alaski): This occurs when hosts are specified with no cells'
nl|'\n'
...
name|'_'
op|'('
string|"'No cell given in routing path.'"
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
name|'_LE'
op|'('
string|'"Error locating next hop for message"'
op|')'
op|')'
newline|'\n'
name|'return'
...
name|'_LE'
op|'('
string|'"Error locating next hops for message"'
op|')'
op|')'
newline|'\n'
name|'return'
...
name|'_LE'
op|'('
string|'"Error sending message to next hops."'
op|')'
op|')'
newline|'\n'
name|'self'
...
name|'_LE'
op|'('
string|'"Error waiting for responses from"'
nl|'\n'
string|'" neighbor cells"'
op|')'
op|')'
newline|'\n'
name|'return'
...
op|']'
newline|'\n'
comment|'# NOTE: compute/api.py loads these when retrieving an instance for an'
nl|'\n'
comment|"# API request, so there's a good chance that this is what was loaded."
nl|'\n'
name|'expected_attrs'
op|'='
op|'['
...
newline|'\n'
indent|'        '
string|'"""Update an instance in this cell."""'
newline|'\n'
name|'if'
name|'not'
name|'admin_state_reset'
op|':'
newline|'\n'
comment|"# NOTE(comstud): We don't want to nuke this cell's view"
nl|'\n'
comment|"# of vm_state and task_state unless it's a forced reset"
nl|'\n'
comment|'# via admin API.'
nl|'\n'
indent|'            '
name|'instance'
op|'.'
...
newline|'\n'
nl|'\n'
DECL|member|_call_compute_api_with_obj
dedent|''
dedent|''
name|'def'
...
op|':'
newline|'\n'
comment|'# NOTE(comstud): We need to refresh the instance from this'
nl|'\n'
comment|"# cell's view in the DB."
nl|'\n'
indent|'            '
name|'instance'
op|'.'
...
newline|'\n'
indent|'        '
string|'"""Start an instance via compute_api.start()."""'
newline|'\n'
name|'self'
op|'.'
name|'_call_compute_api_with_obj'
...
newline|'\n'
indent|'        '
string|'"""Stop an instance via compute_api.stop()."""'
newline|'\n'
name|'do_cast'
op|'='
name|'not'
...
newline|'\n'
indent|'        '
string|'"""Reboot an instance via compute_api.reboot()."""'
newline|'\n'
name|'self'
op|'.'
name|'_call_compute_api_with_obj'
...
newline|'\n'
indent|'        '
string|'"""Suspend an instance via compute_api.suspend()."""'
newline|'\n'
name|'self'
op|'.'
name|'_call_compute_api_with_obj'
...
newline|'\n'
indent|'        '
string|'"""Resume an instance via compute_api.suspend()."""'
newline|'\n'
name|'self'
op|'.'
name|'_call_compute_api_with_obj'
...
newline|'\n'
indent|'        '
string|'"""Pause an instance via compute_api.pause()."""'
newline|'\n'
name|'self'
op|'.'
name|'_call_compute_api_with_obj'
...
newline|'\n'
indent|'        '
string|'"""Unpause an instance via compute_api.pause()."""'
newline|'\n'
name|'self'
op|'.'
name|'_call_compute_api_with_obj'
...
newline|'\n'
indent|'        '
string|'"""Resize an instance via compute_api.resize()."""'
newline|'\n'
name|'self'
op|'.'
name|'_call_compute_api_with_obj'
...
newline|'\n'
indent|'        '
string|'"""Live migrate an instance via compute_api.live_migrate()."""'
newline|'\n'
name|'self'
op|'.'
name|'_call_compute_api_with_obj'
...
newline|'\n'
nl|'\n'
DECL|member|_get_expected_vm_state
dedent|''
name|'def'
name|'_get_expected_vm_state'
op|'('
name|'self'
op|','
name|'instance'
...
newline|'\n'
nl|'\n'
DECL|member|_get_expected_task_state
dedent|''
dedent|''
name|'def'
name|'_get_expected_task_state'
op|'('
name|'self'
op|','
name|'instance'
...
newline|'\n'
name|'expected_task_state_map'
op|'='
op|'{'
nl|'\n'
comment|"# Always allow updates when task_state doesn't change,"
nl|'\n'
comment|"# but also make sure we don't set resize/rebuild task"
nl|'\n'
comment|"# states for old messages when we've potentially already"
nl|'\n'
comment|'# processed the ACTIVE/None messages.  Ie, these checks'
nl|'\n'
comment|'# will prevent stomping on any ACTIVE/None messages'
nl|'\n'
comment|'# we already processed.'
nl|'\n'
name|'task_states'
op|'.'
...
nl|'\n'
comment|'# network information, for example.'
nl|'\n'
indent|'                '
name|'pass'
newline|'\n'
...
op|':'
newline|'\n'
comment|'# NOTE(alaski): instance_destroy_at_top will sometimes be called'
nl|'\n'
comment|'# when an instance does not exist in a cell but does in the parent.'
nl|'\n'
comment|'# In that case instance.id is not set which causes instance.destroy'
nl|'\n'
comment|'# to fail thinking that the object has already been destroyed.'
nl|'\n'
comment|"# That's the right assumption for it to make because without cells"
nl|'\n'
comment|"# that would be true.  But for cells we'll try to pull the actual"
nl|'\n'
comment|'# instance and try to delete it again.'
nl|'\n'
indent|'            '
name|'try'
op|':'
...
name|'debug'
op|'('
string|'"Got message to create instance fault: %s"'
op|','
name|'instance_fault'
op|')'
newline|'\n'
...
name|'debug'
op|'('
string|"'Calling db.block_device_mapping_update_or_create from '"
nl|'\n'
string|"'API cell with values: %s'"
op|','
name|'bdm'
op|')'
newline|'\n'
...
name|'debug'
op|'('
string|"'Calling db.block_device_mapping_create from API '"
nl|'\n'
string|"'cell with values: %s'"
op|','
name|'bdm'
op|')'
newline|'\n'
...
name|'return'
newline|'\n'
comment|"# Unfortunately this update call wants BDM ID... but we don't know"
nl|'\n'
comment|'# what it is in this cell.  Search for it.. try matching either'
nl|'\n'
comment|'# device_name or volume_id.'
nl|'\n'
dedent|''
name|'dev_name'
op|'='
...
name|'_LW'
op|'('
string|'"No match when trying to update BDM: %(bdm)s"'
op|')'
op|','
nl|'\n'
name|'dict'
...
name|'debug'
op|'('
string|"'Calling db.block_device_mapping_update from API cell with '"
nl|'\n'
string|"'bdm id %s and values: %s'"
op|','
name|'instance_bdm'
op|'['
string|"'id'"
...
newline|'\n'
indent|'        '
string|'"""Destroy a block device mapping in API cells by device name\n        or volume_id.  device_name or volume_id can be None, but not both.\n        """'
newline|'\n'
name|'if'
name|'not'
name|'self'
...
newline|'\n'
indent|'        '
string|'"""Get keypair in API cells by name. Just return None if there is\n        no match keypair.\n        """'
newline|'\n'
name|'if'
name|'not'
name|'self'
...
newline|'\n'
nl|'\n'
DECL|member|_get_migrations_for_cell
dedent|''
name|'def'
name|'_get_migrations_for_cell'
op|'('
name|'self'
op|','
name|'ctxt'
...
op|')'
newline|'\n'
comment|'# NOTE(comstud): We also need to re-serialize any objects that'
nl|'\n'
comment|"# exist in 'method_kwargs'."
nl|'\n'
name|'method_kwargs'
op|'='
name|'message_dict'
...
name|'debug'
op|'('
string|'"Updating parents [%(parent_cell_names)s] with "'
nl|'\n'
string|'"our capabilities: %(capabs)s"'
op|','
nl|'\n'
op|'{'
string|"'parent_cell_names'"
op|':'
name|'parent_cell_names'
op|','
nl|'\n'
...
string|'"Updating parents [%(parent_cell_names)s] with "'
nl|'\n'
string|'"our capacities: %(capacities)s"'
op|','
nl|'\n'
op|'{'
string|"'parent_cell_names'"
...
newline|'\n'
indent|'        '
string|'"""Called by the cell scheduler to tell a child cell to build\n        instance(s).\n        """'
newline|'\n'
name|'method_kwargs'
op|'='
name|'dict'
...
newline|'\n'
indent|'        '
string|'"""Update/Create a BDM at top level cell."""'
newline|'\n'
name|'message'
op|'='
name|'_BroadcastMessage'
...
newline|'\n'
indent|'        '
string|'"""Destroy a BDM at top level cell."""'
newline|'\n'
name|'method_kwargs'
op|'='
name|'dict'
...
newline|'\n'
indent|'        '
string|'"""Fetch all migrations applying the filters for a given cell or all\n        cells.\n        """'
newline|'\n'
name|'method_kwargs'
op|'='
name|'dict'
...
newline|'\n'
nl|'\n'
DECL|member|_instance_action
dedent|''
name|'def'
name|'_instance_action'
op|'('
name|'self'
op|','
name|'ctxt'
...
newline|'\n'
indent|'        '
string|'"""Call instance_<method> in correct cell for instance."""'
newline|'\n'
name|'cell_name'
op|'='
name|'instance'
...
name|'_LW'
op|'('
string|'"No cell_name for %(method)s() from API"'
op|')'
op|','
nl|'\n'
name|'dict'
...
newline|'\n'
indent|'        '
string|'"""Update an instance object in its cell."""'
newline|'\n'
name|'cell_name'
op|'='
name|'instance'
...
name|'_LW'
op|'('
string|'"No cell_name for instance update from API"'
op|')'
op|','
nl|'\n'
name|'instance'
...
newline|'\n'
indent|'        '
string|'"""Get Key Pair by name at top level cell."""'
newline|'\n'
name|'message'
op|'='
name|'_BroadcastMessage'
