...
newline|'\n'
nl|'\n'
DECL|variable|AGGREGATE_ACTION_UPDATE
name|'AGGREGATE_ACTION_UPDATE'
op|'='
string|"'Update'"
newline|'\n'
DECL|variable|AGGREGATE_ACTION_UPDATE_META
name|'AGGREGATE_ACTION_UPDATE_META'
op|'='
string|"'UpdateMeta'"
newline|'\n'
DECL|variable|AGGREGATE_ACTION_DELETE
name|'AGGREGATE_ACTION_DELETE'
op|'='
string|"'Delete'"
newline|'\n'
DECL|variable|AGGREGATE_ACTION_ADD
name|'AGGREGATE_ACTION_ADD'
op|'='
string|"'Add'"
newline|'\n'
nl|'\n'
...
newline|'\n'
indent|'    '
string|'"""Return a dict describing how to change orig to new.  The keys\n    correspond to values that have changed; the value will be a list\n    of one or two elements.  The first element of the list will be\n    either \'+\' or \'-\', indicating whether the key was updated or\n    deleted; if the key was updated, the list will contain a second\n    element, giving the updated value.\n    """'
newline|'\n'
...
nl|'\n'
string|'" tried to run %(min_count)d instances. "'
nl|'\n'
string|'"%(msg)s"'
op|')'
op|','
...
string|'"%(overs)s quota exceeded for %(pid)s,"'
nl|'\n'
string|'" tried to run between %(min_count)d and"'
nl|'\n'
string|'" %(max_count)d instances. %(msg)s"'
op|')'
op|','
nl|'\n'
name|'params'
...
newline|'\n'
nl|'\n'
DECL|member|_get_over_quota_detail
dedent|''
name|'def'
name|'_get_over_quota_detail'
op|'('
...
name|'_'
op|'('
string|'"Metadata type should be dict."'
op|')'
op|')'
newline|'\n'
name|'raise'
...
nl|'\n'
comment|'# Those are only used in V2 API, from V2.1 API, those checks are'
nl|'\n'
comment|'# validated at API layer schema validation.'
nl|'\n'
dedent|''
name|'for'
name|'k'
...
newline|'\n'
indent|'        '
string|'"""Check if the security group requested exists and belongs to\n        the project.\n        """'
newline|'\n'
name|'for'
name|'secgroup'
name|'in'
...
op|':'
newline|'\n'
comment|'# NOTE(danms): Temporary transition'
nl|'\n'
indent|'            '
name|'requested_networks'
op|'='
...
op|')'
newline|'\n'
comment|'# kernel_id could have been a URI, not a UUID, so to keep behaviour'
nl|'\n'
comment|'# from before, which leaked that implementation detail out to the'
nl|'\n'
comment|'# caller, we return the image UUID of the kernel image and ramdisk'
nl|'\n'
comment|'# image (below) and not any image URIs that might have been'
nl|'\n'
comment|'# supplied.'
nl|'\n'
comment|'# TODO(jaypipes): Get rid of this silliness once we move to a real'
nl|'\n'
comment|'# Image object and hide all of that stuff within nova.image.api.'
nl|'\n'
name|'kernel_id'
op|'='
name|'kernel_image'
...
name|'staticmethod'
newline|'\n'
DECL|member|parse_availability_zone
name|'def'
...
newline|'\n'
nl|'\n'
DECL|member|_ensure_auto_disk_config_is_valid
dedent|''
name|'def'
name|'_ensure_auto_disk_config_is_valid'
op|'('
name|'self'
op|','
name|'auto_disk_config_img'
op|','
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_new_instance_name_from_template
dedent|''
name|'def'
name|'_new_instance_name_from_template'
op|'('
name|'self'
op|','
name|'uuid'
...
newline|'\n'
nl|'\n'
comment|'# Image min_disk is in gb, size is in bytes. For sanity, have them both'
nl|'\n'
comment|'# in bytes.'
nl|'\n'
dedent|''
...
newline|'\n'
nl|'\n'
comment|"# Target disk is a volume. Don't check flavor disk size because it"
nl|'\n'
comment|"# doesn't make sense, and check min_disk against the volume size."
nl|'\n'
name|'if'
op|'('
name|'root_bdm'
...
op|':'
newline|'\n'
comment|'# There are 2 possibilities here: either the target volume already'
nl|'\n'
comment|"# exists, or it doesn't, in which case the bdm will contain the"
nl|'\n'
comment|'# intended volume size.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# Cinder does its own check against min_disk, so if the target'
nl|'\n'
comment|"# volume already exists this has already been done and we don't"
nl|'\n'
comment|'# need to check it again here. In this case, volume_size may not be'
nl|'\n'
comment|'# set on the bdm.'
nl|'\n'
comment|'#'
nl|'\n'
comment|"# If we're going to create the volume, the bdm will contain"
nl|'\n'
comment|'# volume_size. Therefore we should check it if it exists. This will'
nl|'\n'
comment|'# still be checked again by cinder when the volume is created, but'
nl|'\n'
comment|'# that will not happen until the request reaches a host. By'
nl|'\n'
comment|'# checking it here, the user gets an immediate and useful failure'
nl|'\n'
comment|'# indication.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# The third possibility is that we have failed to consider'
nl|'\n'
comment|'# something, and there are actually more than 2 possibilities. In'
nl|'\n'
comment|'# this case cinder will still do the check at volume creation time.'
nl|'\n'
comment|'# The behaviour will still be correct, but the user will not get an'
nl|'\n'
comment|'# immediate failure from the api, and will instead have to'
nl|'\n'
comment|'# determine why the instance is in an error state with a task of'
nl|'\n'
comment|'# block_device_mapping.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# We could reasonably refactor this check into _validate_bdm at'
nl|'\n'
comment|'# some future date, as the various size logic is already split out'
nl|'\n'
comment|'# in there.'
nl|'\n'
indent|'            '
...
newline|'\n'
nl|'\n'
comment|'# Target disk is a local disk whose size is taken from the flavor'
nl|'\n'
dedent|''
dedent|''
dedent|''
...
newline|'\n'
nl|'\n'
DECL|member|_get_image_defined_bdms
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'_get_image_defined_bdms'
op|'('
name|'self'
op|','
name|'base_options'
...
newline|'\n'
nl|'\n'
comment|'# Get the block device mappings defined by the image.'
nl|'\n'
name|'image_defined_bdms'
op|'='
name|'image_properties'
op|'.'
name|'get'
...
newline|'\n'
nl|'\n'
DECL|member|_get_flavor_defined_bdms
dedent|''
name|'def'
name|'_get_flavor_defined_bdms'
op|'('
name|'self'
op|','
name|'instance_type'
...
newline|'\n'
indent|'        '
name|'flavor_defined_bdms'
op|'='
op|'['
op|']'
newline|'\n'
nl|'\n'
name|'have_ephemeral_bdms'
op|'='
name|'any'
op|'('
name|'filter'
...
newline|'\n'
nl|'\n'
DECL|member|_check_and_transform_bdm
dedent|''
name|'def'
...
op|':'
newline|'\n'
comment|"# NOTE (ndipanov): Assume root dev name is 'vda' if not supplied."
nl|'\n'
comment|"#                  It's needed for legacy conversion to work."
nl|'\n'
indent|'        '
name|'root_device_name'
op|'='
...
op|')'
newline|'\n'
comment|'# If the instance is booted by image and has a volume attached,'
nl|'\n'
comment|'# the volume cannot have the same device name as root_device_name'
nl|'\n'
name|'if'
name|'image_ref'
op|':'
...
name|'_'
op|'('
string|"'The volume cannot be assigned the same device'"
nl|'\n'
string|"' name as the root device %s'"
op|')'
op|'%'
name|'root_device_name'
newline|'\n'
...
op|':'
newline|'\n'
comment|'# NOTE (ndipanov): client will insert an image mapping into the v2'
nl|'\n'
comment|'# block_device_mapping, but if there is a bootable device in image'
nl|'\n'
comment|'# mappings - we need to get rid of the inserted image'
nl|'\n'
comment|'# NOTE (gibi): another case is when a server is booted with an'
nl|'\n'
comment|'# image to bdm mapping where the image only contains a bdm to a'
nl|'\n'
comment|'# snapshot. In this case the other image to bdm mapping'
nl|'\n'
comment|'# contains an unnecessary device with boot_index == 0.'
nl|'\n'
comment|'# Also in this case the image_ref is None as we are booting from'
nl|'\n'
comment|'# an image to volume bdm.'
nl|'\n'
DECL|function|not_image_and_root_bdm
indent|'            '
name|'def'
name|'not_image_and_root_bdm'
op|'('
name|'bdm'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
comment|'# Note:  max_count is the number of instances requested by the user,'
nl|'\n'
comment|'# max_network_count is the maximum number of instances taking into'
nl|'\n'
comment|'# account any network quotas'
nl|'\n'
...
op|':'
newline|'\n'
comment|'# there must be invalid values in the image meta properties so'
nl|'\n'
comment|'# consider this an invalid request'
nl|'\n'
indent|'            '
name|'msg'
op|'='
name|'_'
op|'('
string|"'Invalid image metadata. Error: %s'"
op|')'
op|'%'
name|'six'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# PCI requests come from two sources: instance flavor and'
nl|'\n'
comment|'# requested_networks. The first call in below returns an'
nl|'\n'
comment|'# InstancePCIRequests object which is a list of InstancePCIRequest'
nl|'\n'
comment|'# objects. The second call in below creates an InstancePCIRequest'
nl|'\n'
comment|'# object for each SR-IOV port, and append it to the list in the'
nl|'\n'
comment|'# InstancePCIRequests object'
nl|'\n'
name|'pci_request_info'
op|'='
name|'pci_request'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# return the validated options and maximum number of instances allowed'
nl|'\n'
comment|'# by the network quotas'
nl|'\n'
name|'return'
name|'base_options'
op|','
...
newline|'\n'
nl|'\n'
DECL|member|_create_build_request
dedent|''
name|'def'
name|'_create_build_request'
op|'('
name|'self'
op|','
name|'context'
...
op|':'
newline|'\n'
comment|'# Store the BuildRequest that will help populate an instance'
nl|'\n'
comment|'# object for a list/show request'
nl|'\n'
indent|'        '
name|'info_cache'
op|'='
...
op|')'
newline|'\n'
comment|"# NOTE: base_options['config_drive'] is either True or '' due"
nl|'\n'
comment|"# to how it's represented in the instances table in the db."
nl|'\n'
comment|'# BuildRequest needs a boolean.'
nl|'\n'
name|'bool_config_drive'
op|'='
name|'strutils'
op|'.'
name|'bool_from_string'
...
op|')'
newline|'\n'
comment|'# display_name follows a whole set of rules'
nl|'\n'
name|'display_name'
op|'='
name|'base_options'
...
op|':'
newline|'\n'
comment|'# Create a uuid for the instance so we can store the'
nl|'\n'
comment|'# RequestSpec before the instance is created.'
nl|'\n'
indent|'                '
name|'instance_uuid'
op|'='
...
op|')'
newline|'\n'
comment|'# Store the RequestSpec that will be used for scheduling.'
nl|'\n'
name|'req_spec'
op|'='
name|'objects'
...
newline|'\n'
nl|'\n'
comment|'# Create an instance object, but do not store in db yet.'
nl|'\n'
name|'instance'
op|'='
name|'objects'
...
newline|'\n'
nl|'\n'
comment|'# Create an instance_mapping.  The null cell_mapping indicates'
nl|'\n'
comment|"# that the instance doesn't yet exist in a cell, and lookups"
nl|'\n'
comment|'# for it need to instead look for the RequestSpec.'
nl|'\n'
comment|'# cell_mapping will be populated after scheduling, with a'
nl|'\n'
comment|'# scheduling failure using the cell_mapping for the special'
nl|'\n'
comment|'# cell0.'
nl|'\n'
name|'inst_mapping'
...
op|')'
newline|'\n'
comment|'# TODO(alaski): Cast to conductor here which will call the'
nl|'\n'
comment|'# scheduler and defer instance creation until the scheduler'
nl|'\n'
comment|'# has picked a cell/host. Set the instance_mapping to the cell'
nl|'\n'
comment|'# that the instance is scheduled to.'
nl|'\n'
comment|'# NOTE(alaski): Instance and block device creation are going'
nl|'\n'
comment|'# to move to the conductor.'
nl|'\n'
name|'instance'
op|'.'
name|'create'
...
op|')'
newline|'\n'
comment|'# The BuildRequest needs to be stored until the instance is in'
nl|'\n'
comment|'# an instance table. At that point it will never be used again'
nl|'\n'
comment|'# and should be deleted. As instance creation is pushed back,'
nl|'\n'
comment|'# as noted above, this will move with it.'
nl|'\n'
comment|'# TODO(alaski): Sync API updates to the build_request to the'
nl|'\n'
comment|'# instance before it is destroyed. Right now only locked_by can'
nl|'\n'
comment|'# be updated before this is destroyed.'
nl|'\n'
name|'build_request'
op|'.'
name|'destroy'
...
name|'_'
op|'('
string|'"Quota exceeded, too many servers in "'
nl|'\n'
string|'"group"'
op|')'
newline|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_get_bdm_image_metadata
dedent|''
name|'def'
...
op|':'
newline|'\n'
comment|'# NOTE(alaski): A volume snapshot inherits metadata from the'
nl|'\n'
comment|'# originating volume, but the API does not expose metadata'
nl|'\n'
comment|'# on the snapshot itself.  So we query the volume for it below.'
nl|'\n'
indent|'                '
name|'snapshot'
op|'='
...
name|'staticmethod'
newline|'\n'
DECL|member|_get_requested_instance_group
name|'def'
name|'_get_requested_instance_group'
op|'('
name|'context'
op|','
name|'filter_properties'
...
newline|'\n'
nl|'\n'
comment|'# TODO(gibi): We need to remove the following validation code when'
nl|'\n'
comment|'# removing legacy v2 code.'
nl|'\n'
dedent|''
name|'if'
name|'not'
...
name|'_'
op|'('
string|"'Server group scheduler hint must be a UUID.'"
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
newline|'\n'
nl|'\n'
comment|'# max_net_count is the maximum number of instances requested by the'
nl|'\n'
comment|'# user adjusted for any network quota constraints, including'
nl|'\n'
comment|'# consideration of connections to each requested network'
nl|'\n'
name|'if'
name|'max_net_count'
op|'<'
...
name|'_LI'
op|'('
string|'"max count reduced from %(max_count)d to "'
nl|'\n'
string|'"%(max_net_count)d due to network port quota"'
op|')'
op|','
nl|'\n'
op|'{'
...
newline|'\n'
nl|'\n'
comment|"# We can't do this check earlier because we need bdms from all sources"
nl|'\n'
comment|'# to have been merged in order to get the root bdm.'
nl|'\n'
name|'self'
op|'.'
name|'_checks_for_create_and_rebuild'
...
newline|'\n'
nl|'\n'
DECL|member|_prepare_image_mapping
dedent|''
name|'def'
name|'_prepare_image_mapping'
op|'('
...
newline|'\n'
indent|'        '
string|'"""Extract and format blank devices from image mappings."""'
newline|'\n'
nl|'\n'
name|'prepared_mappings'
op|'='
op|'['
op|']'
newline|'\n'
...
op|')'
newline|'\n'
name|'instance_block_device_mapping'
op|'='
name|'copy'
op|'.'
name|'deepcopy'
...
op|':'
newline|'\n'
DECL|function|_subsequent_list
indent|'        '
name|'def'
name|'_subsequent_list'
op|'('
name|'l'
op|')'
op|':'
newline|'\n'
comment|'# Each device which is capable of being used as boot device should'
nl|'\n'
comment|'# be given a unique boot index, starting from 0 in ascending order.'
nl|'\n'
indent|'            '
name|'return'
name|'all'
...
newline|'\n'
nl|'\n'
comment|'# Make sure that the boot indexes make sense.'
nl|'\n'
comment|'# Setting a negative value or None indicates that the device should not'
nl|'\n'
comment|'# be used for booting.'
nl|'\n'
dedent|''
name|'boot_indexes'
op|'='
...
op|':'
newline|'\n'
comment|'# Convert the BlockDeviceMappingList to a list for repr details.'
nl|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'debug'
op|'('
string|"'Invalid block device mapping boot sequence for '"
nl|'\n'
string|"'instance: %s'"
op|','
name|'list'
...
string|'"Images with "'
nl|'\n'
string|'"destination_type \'volume\' need to have a non-zero "'
nl|'\n'
...
name|'_'
op|'('
string|'"Blank volumes "'
nl|'\n'
string|'"(source: \'blank\', dest: \'volume\') need to have non-zero "'
nl|'\n'
string|'"size"'
op|')'
op|')'
...
newline|'\n'
nl|'\n'
comment|'# There should be only one swap'
nl|'\n'
dedent|''
name|'swap_list'
op|'='
...
name|'_'
op|'('
string|'"More than one swap drive requested."'
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
newline|'\n'
nl|'\n'
DECL|member|_default_host_name
dedent|''
name|'def'
name|'_default_host_name'
op|'('
...
newline|'\n'
nl|'\n'
comment|"# In case we couldn't find any suitable base_image"
nl|'\n'
name|'system_meta'
op|'.'
name|'setdefault'
...
newline|'\n'
nl|'\n'
comment|'# This method remains because cellsv1 uses it in the scheduler'
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# NOTE (ndipanov): This can now raise exceptions but the instance'
nl|'\n'
comment|'#                  has been created, so delete it and re-raise so'
nl|'\n'
comment|'#                  that other cleanup can happen.'
nl|'\n'
dedent|''
name|'try'
op|':'
...
op|':'
newline|'\n'
comment|"# Because of the foreign key the bdm can't be created unless the"
nl|'\n'
comment|'# instance is.'
nl|'\n'
indent|'            '
...
newline|'\n'
nl|'\n'
DECL|member|_check_multiple_instances_neutron_ports
dedent|''
dedent|''
dedent|''
name|'def'
name|'_check_multiple_instances_neutron_ports'
op|'('
name|'self'
op|','
name|'requested_networks'
...
newline|'\n'
indent|'        '
string|'"""Check whether multiple instances are created from port id(s)."""'
newline|'\n'
name|'for'
name|'requested_net'
name|'in'
...
newline|'\n'
nl|'\n'
DECL|member|_check_multiple_instances_and_specified_ip
dedent|''
dedent|''
dedent|''
name|'def'
name|'_check_multiple_instances_and_specified_ip'
op|'('
name|'self'
op|','
name|'requested_networks'
...
newline|'\n'
indent|'        '
string|'"""Check whether multiple instances are created with specified ip."""'
newline|'\n'
nl|'\n'
name|'for'
name|'requested_net'
...
name|'_'
op|'('
string|'"max_count cannot be greater than 1 if an fixed_ip "'
nl|'\n'
string|'"is specified."'
op|')'
newline|'\n'
...
newline|'\n'
indent|'        '
string|'"""Provision instances, sending instance information to the\n        scheduler.  The scheduler will determine where the instance(s)\n        go and will handle creating the DB entries.\n\n        Returns a tuple of (instances, reservation_id)\n        """'
newline|'\n'
comment|'# Check policies up front to fail before performing more expensive work'
nl|'\n'
name|'self'
op|'.'
name|'_check_create_policies'
...
newline|'\n'
nl|'\n'
DECL|member|_check_auto_disk_config
dedent|''
name|'def'
name|'_check_auto_disk_config'
op|'('
...
newline|'\n'
nl|'\n'
comment|'# At these states an instance has a snapshot associate.'
nl|'\n'
name|'if'
name|'instance'
op|'.'
...
name|'_LI'
op|'('
string|'"Working on deleting snapshot %s "'
nl|'\n'
string|'"from shelved instance..."'
op|')'
op|','
nl|'\n'
name|'snapshot_id'
...
op|':'
newline|'\n'
comment|"# NOTE(comstud): If we're in the API cell, we need to"
nl|'\n'
comment|'# skip all remaining logic and just call the callback,'
nl|'\n'
comment|'# which will cause a cast to the child cell.  Also,'
nl|'\n'
comment|'# commit reservations here early until we have a better'
nl|'\n'
comment|'# way to deal with quotas with cells.'
nl|'\n'
indent|'                '
name|'cb'
op|'('
...
name|'self'
op|'.'
name|'_confirm_resize_on_deleting'
op|'('
name|'context'
op|','
name|'instance'
...
name|'_LI'
op|'('
string|"'Instance is already in deleting state, '"
nl|'\n'
string|"'ignoring this request'"
op|')'
op|','
nl|'\n'
name|'instance'
...
newline|'\n'
nl|'\n'
comment|"# NOTE(snikitin): If instance's vm_state is 'soft-delete',"
nl|'\n'
comment|'# we should not count reservations here, because instance'
nl|'\n'
comment|'# in soft-delete vm_state have already had quotas'
nl|'\n'
comment|'# decremented. More details:'
nl|'\n'
comment|'# https://bugs.launchpad.net/nova/+bug/1333145'
nl|'\n'
name|'if'
name|'instance'
op|'.'
...
op|':'
newline|'\n'
comment|'# If instance is in shelved_offloaded state or compute node'
nl|'\n'
comment|"# isn't up, delete instance from db and clean bdms info and"
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_confirm_resize_on_deleting
dedent|''
dedent|''
dedent|''
dedent|''
...
name|'_LI'
op|'('
string|"'Found an unconfirmed migration during delete, '"
nl|'\n'
string|"'id: %(id)s, status: %(status)s'"
op|')'
op|','
nl|'\n'
op|'{'
...
name|'_LI'
op|'('
string|"'Instance may have been confirmed during delete'"
op|')'
op|','
nl|'\n'
name|'context'
...
name|'_LI'
op|'('
string|"'Migration %s may have been confirmed during '"
nl|'\n'
string|"'delete'"
op|')'
op|','
...
newline|'\n'
nl|'\n'
DECL|member|_local_cleanup_bdm_volumes
dedent|''
name|'def'
...
name|'_LI'
op|'('
string|'"instance is in SHELVED_OFFLOADED state, cleanup"'
nl|'\n'
string|'" the instance\'s info from database."'
op|')'
op|','
nl|'\n'
name|'instance'
...
op|':'
newline|'\n'
comment|'# NOTE(liusheng): In nova-network multi_host scenario,deleting'
nl|'\n'
comment|"# network info of the instance may need instance['host'] as"
nl|'\n'
comment|'# destination host of RPC call. If instance in SHELVED_OFFLOADED'
nl|'\n'
comment|"# state, instance['host'] is None, here, use shelved_host as host"
nl|'\n'
comment|"# to deallocate network info and reset instance['host'] after that."
nl|'\n'
comment|"# Here we shouldn't use instance.save(), because this will mislead"
nl|'\n'
comment|"# user who may think the instance's host has been changed, and"
nl|'\n'
comment|'# actually, the instance.host is always None.'
nl|'\n'
indent|'            '
name|'orig_host'
op|'='
...
newline|'\n'
nl|'\n'
DECL|member|_do_force_delete
dedent|''
dedent|''
name|'def'
name|'_do_force_delete'
op|'('
name|'self'
op|','
name|'context'
...
newline|'\n'
nl|'\n'
DECL|member|_do_soft_delete
dedent|''
dedent|''
name|'def'
name|'_do_soft_delete'
op|'('
name|'self'
op|','
name|'context'
...
newline|'\n'
indent|'        '
string|'"""Force delete an instance in any vm_state/task_state."""'
newline|'\n'
name|'self'
op|'.'
name|'_delete'
...
name|'debug'
op|'('
string|'"Try to trigger crash dump"'
op|','
name|'instance'
op|'='
name|'instance'
...
name|'debug'
op|'('
string|'"Fetching instance by UUID"'
op|','
nl|'\n'
name|'instance_uuid'
op|'='
...
name|'debug'
op|'('
string|'"Failed to fetch instance by id %s"'
op|','
name|'instance_id'
op|')'
newline|'\n'
...
newline|'\n'
nl|'\n'
DECL|function|_remap_metadata_filter
dedent|''
name|'def'
name|'_remap_metadata_filter'
op|'('
name|'metadata'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
DECL|function|_remap_system_metadata_filter
dedent|''
name|'def'
name|'_remap_system_metadata_filter'
op|'('
name|'metadata'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
comment|'# IP address filtering cannot be applied at the DB layer, remove any DB'
nl|'\n'
comment|'# limit so that it can be applied after the IP filter.'
nl|'\n'
dedent|''
dedent|''
dedent|''
...
name|'debug'
op|'('
string|"'Removing limit for DB query due to IP filter'"
op|')'
newline|'\n'
name|'limit'
op|'='
...
newline|'\n'
nl|'\n'
DECL|function|_match_instance
name|'def'
name|'_match_instance'
op|'('
name|'instance'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
comment|"# NOTE(melwitt): We don't check instance lock for backup because lock is"
nl|'\n'
comment|'#                intended to prevent accidental change/delete of instances'
nl|'\n'
dedent|''
op|'@'
name|'wrap_check_policy'
...
name|'_LI'
op|'('
string|'"It\'s not supported to backup volume backed "'
nl|'\n'
string|'"instance."'
op|')'
op|','
...
newline|'\n'
nl|'\n'
comment|'# NOTE(comstud): Any changes to this method should also be made'
nl|'\n'
comment|'# to the backup_instance() method in nova/cells/messaging.py'
nl|'\n'
nl|'\n'
dedent|''
name|'instance'
...
newline|'\n'
nl|'\n'
comment|"# NOTE(melwitt): We don't check instance lock for snapshot because lock is"
nl|'\n'
comment|'#                intended to prevent accidental change/delete of instances'
nl|'\n'
dedent|''
...
comment|'# NOTE(comstud): Any changes to this method should also be made'
nl|'\n'
comment|'# to the snapshot_instance() method in nova/cells/messaging.py'
nl|'\n'
name|'instance'
op|'.'
name|'task_state'
...
name|'self'
op|'.'
name|'_initialize_instance_snapshot_metadata'
op|'('
nl|'\n'
name|'instance'
op|','
...
op|')'
newline|'\n'
comment|"# if we're making a snapshot, omit the disk and container formats,"
nl|'\n'
comment|'# since the image may have been converted to another format, and the'
nl|'\n'
comment|"# original values won't be accurate.  The driver will populate these"
nl|'\n'
comment|'# with the correct values later, on image upload.'
nl|'\n'
name|'if'
name|'image_type'
op|'=='
...
newline|'\n'
nl|'\n'
DECL|member|_initialize_instance_snapshot_metadata
dedent|''
name|'def'
name|'_initialize_instance_snapshot_metadata'
op|'('
...
newline|'\n'
nl|'\n'
comment|'# Delete properties that are non-inheritable'
nl|'\n'
name|'properties'
op|'='
name|'image_meta'
...
newline|'\n'
nl|'\n'
comment|'# The properties in extra_properties have precedence'
nl|'\n'
dedent|''
name|'properties'
op|'.'
...
op|']'
newline|'\n'
comment|'# clean properties before filling'
nl|'\n'
name|'for'
name|'key'
name|'in'
...
name|'_LI'
op|'('
string|"'Skipping quiescing instance: '"
nl|'\n'
string|"'%(reason)s.'"
op|')'
op|','
...
name|'debug'
op|'('
string|"'Creating snapshot from volume %s.'"
op|','
name|'volume'
op|'['
string|"'id'"
...
newline|'\n'
indent|'            '
string|'"""Remove old image properties that we\'re storing as instance\n            system metadata.  These properties start with \'image_\'.\n            Then add the properties for the new image.\n            """'
newline|'\n'
comment|"# FIXME(comstud): There's a race condition here in that if"
nl|'\n'
comment|'# the system_metadata for this instance is updated after'
nl|'\n'
comment|'# we do the previous save() and before we update.. those'
nl|'\n'
comment|'# other updates will be lost. Since this problem exists in'
nl|'\n'
comment|'# a lot of other places, I think it should be addressed in'
nl|'\n'
comment|'# a DB layer overhaul.'
nl|'\n'
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# Since image might have changed, we may have new values for'
nl|'\n'
comment|'# os_type, vm_mode, etc'
nl|'\n'
dedent|''
name|'options_from_image'
op|'='
...
name|'staticmethod'
newline|'\n'
DECL|member|_resize_cells_support
name|'def'
name|'_resize_cells_support'
op|'('
name|'context'
op|','
name|'quotas'
...
newline|'\n'
indent|'        '
string|'"""Special API cell logic for resize."""'
newline|'\n'
comment|'# With cells, the best we can do right now is commit the'
nl|'\n'
comment|'# reservations immediately...'
nl|'\n'
name|'quotas'
op|'.'
name|'commit'
...
op|')'
newline|'\n'
comment|'# NOTE(johannes/comstud): The API cell needs a local migration'
nl|'\n'
comment|'# record for later resize_confirm and resize_reverts to deal'
nl|'\n'
comment|"# with quotas.  We don't need source and/or destination"
nl|'\n'
comment|'# information, just the old and new flavors. Status is set to'
nl|'\n'
comment|"# 'finished' since nothing else will update the status along"
nl|'\n'
comment|'# the way.'
nl|'\n'
name|'mig'
...
name|'_'
op|'('
string|"'Resize to zero disk flavor is not allowed.'"
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
op|':'
newline|'\n'
comment|'# Commit reservations early and create migration record.'
nl|'\n'
indent|'            '
name|'self'
op|'.'
...
op|':'
newline|'\n'
comment|'# Some old instances can still have no RequestSpec object attached'
nl|'\n'
comment|'# to them, we need to support the old way'
nl|'\n'
indent|'            '
name|'request_spec'
op|'='
...
name|'check_instance_host'
newline|'\n'
DECL|member|get_rdp_connect_info
name|'def'
name|'get_rdp_connect_info'
op|'('
name|'self'
op|','
name|'context'
...
name|'check_instance_host'
newline|'\n'
DECL|member|get_serial_console_connect_info
name|'def'
name|'get_serial_console_connect_info'
op|'('
name|'self'
op|','
name|'context'
...
newline|'\n'
indent|'        '
string|'"""Used in a child cell to get serial console."""'
newline|'\n'
name|'connect_info'
op|'='
name|'self'
...
newline|'\n'
indent|'        '
string|'"""Get a url to a MKS console."""'
newline|'\n'
name|'connect_info'
op|'='
name|'self'
...
newline|'\n'
nl|'\n'
DECL|member|is_expected_locked_by
dedent|''
name|'def'
...
newline|'\n'
nl|'\n'
DECL|member|_create_volume_bdm
dedent|''
name|'def'
...
op|':'
newline|'\n'
comment|"# when the creation is done locally we can't specify the device"
nl|'\n'
comment|'# name as we do not have a way to check that the name specified is'
nl|'\n'
comment|'# a valid one.'
nl|'\n'
comment|'# We leave the setting of that value when the actual attach'
nl|'\n'
comment|'# happens on the compute manager'
nl|'\n'
indent|'            '
name|'volume_bdm'
op|'='
...
newline|'\n'
nl|'\n'
DECL|member|_check_attach_and_reserve_volume
dedent|''
name|'def'
name|'_check_attach_and_reserve_volume'
op|'('
name|'self'
op|','
name|'context'
...
newline|'\n'
nl|'\n'
DECL|member|_attach_volume_shelved_offloaded
dedent|''
name|'def'
...
newline|'\n'
nl|'\n'
DECL|member|_check_and_begin_detach
dedent|''
name|'def'
name|'_check_and_begin_detach'
op|'('
name|'self'
op|','
name|'context'
...
newline|'\n'
nl|'\n'
DECL|member|_detach_volume_shelved_offloaded
dedent|''
name|'def'
...
op|')'
newline|'\n'
comment|"# The caller likely got the instance from volume['attachments']"
nl|'\n'
...
name|'_'
op|'('
string|'"Old volume is attached to a different instance."'
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
name|'is_volume'
newline|'\n'
comment|'# in case we hit a very old instance without root bdm, we _assume_ that'
nl|'\n'
comment|'# instance is backed by a volume, if and only if image_ref is not set'
nl|'\n'
dedent|''
name|'return'
name|'not'
...
op|':'
newline|'\n'
comment|'# NOTE(pkoniszewski): It is possible that MessagingTimeout'
nl|'\n'
comment|'# occurs, but LM will still be in progress, so write'
nl|'\n'
comment|'# instance fault to database'
nl|'\n'
indent|'                '
name|'compute_utils'
op|'.'
...
op|')'
newline|'\n'
DECL|member|live_migrate_force_complete
name|'def'
...
name|'debug'
op|'('
string|'"Going to try to force live migration to complete"'
op|','
nl|'\n'
name|'instance'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# NOTE(pkoniszewski): Get migration object to check if there is ongoing'
nl|'\n'
comment|'# live migration for particular instance. Also pass migration id to'
nl|'\n'
comment|'# compute to double check and avoid possible race condition.'
nl|'\n'
name|'migration'
op|'='
name|'objects'
...
op|')'
newline|'\n'
DECL|member|live_migrate_abort
name|'def'
...
name|'debug'
op|'('
string|'"Going to cancel live migration %s"'
op|','
nl|'\n'
name|'migration'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(danms): Create this as a tombstone for the source compute'
nl|'\n'
comment|'# to find and cleanup. No need to pass it anywhere else.'
nl|'\n'
name|'migration'
op|'='
name|'objects'
...
newline|'\n'
nl|'\n'
DECL|member|get_migrations_in_progress_by_instance
dedent|''
name|'def'
...
newline|'\n'
indent|'        '
string|'"""Get all migrations of an instance in progress."""'
newline|'\n'
name|'return'
name|'objects'
op|'.'
...
newline|'\n'
nl|'\n'
DECL|member|get_migration_by_id_and_instance
dedent|''
name|'def'
...
newline|'\n'
indent|'        '
string|'"""Get the migration of an instance by id."""'
newline|'\n'
name|'return'
name|'objects'
op|'.'
...
op|':'
newline|'\n'
comment|'# NOTE(danms): The external API consumer just provides events,'
nl|'\n'
comment|"# but doesn't know where they go. We need to collate lists"
nl|'\n'
comment|'# by the host the affected instance is on and dispatch them'
nl|'\n'
comment|'# according to host'
nl|'\n'
indent|'        '
name|'instances_by_host'
op|'='
op|'{'
op|'}'
newline|'\n'
...
op|':'
newline|'\n'
comment|'# TODO(salv-orlando): Handle exceptions raised by the rpc api layer'
nl|'\n'
comment|'# in order to ensure that a failure in processing events on a host'
nl|'\n'
comment|'# will not prevent processing events on other hosts'
nl|'\n'
indent|'            '
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
DECL|member|get_instance_host_status
dedent|''
dedent|''
name|'def'
...
newline|'\n'
nl|'\n'
DECL|member|get_instances_host_statuses
dedent|''
name|'def'
...
newline|'\n'
indent|'        '
string|'"""Performs the actual service update operation."""'
newline|'\n'
name|'service'
op|'='
name|'objects'
...
newline|'\n'
indent|'        '
string|'"""Performs the actual Service deletion operation."""'
newline|'\n'
name|'objects'
op|'.'
name|'Service'
...
op|')'
newline|'\n'
comment|'# If updated values include availability_zones, then the cache'
nl|'\n'
comment|'# which stored availability_zones and host need to be reset'
nl|'\n'
name|'if'
name|'values'
op|'.'
...
op|')'
newline|'\n'
comment|'# If updated metadata include availability_zones, then the cache'
nl|'\n'
comment|'# which stored availability_zones and host need to be reset'
nl|'\n'
name|'if'
...
name|'_'
op|'('
string|'"Host aggregate is not empty"'
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
newline|'\n'
nl|'\n'
DECL|member|is_safe_to_update_az
dedent|''
name|'def'
name|'is_safe_to_update_az'
op|'('
...
name|'_'
op|'('
string|'"Aggregate %s does not support empty named "'
nl|'\n'
...
name|'self'
op|'.'
name|'_raise_invalid_aggregate_exc'
op|'('
name|'action_name'
op|','
name|'aggregate'
...
name|'_'
op|'('
string|'"One or more hosts already in availability zone(s) "'
nl|'\n'
string|'"%s"'
op|')'
op|'%'
...
newline|'\n'
nl|'\n'
DECL|member|_raise_invalid_aggregate_exc
dedent|''
dedent|''
dedent|''
name|'def'
...
name|'_'
op|'('
string|'"Unexpected aggregate action %s"'
op|')'
op|'%'
name|'action_name'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_update_az_cache_for_host
dedent|''
name|'def'
...
op|':'
newline|'\n'
comment|'# Update the availability_zone cache to avoid getting wrong'
nl|'\n'
comment|'# availability_zone in cache retention time when add/remove'
nl|'\n'
comment|'# host to/from aggregate.'
nl|'\n'
indent|'        '
name|'if'
name|'aggregate_meta'
...
name|'_'
op|'('
string|"'Keypair name must be string and between '"
nl|'\n'
string|"'1 and 255 characters long'"
op|')'
op|')'
newline|'\n'
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_generate_fingerprint
dedent|''
name|'def'
name|'_generate_fingerprint'
op|'('
...
newline|'\n'
nl|'\n'
DECL|member|_generate_key_pair
dedent|''
dedent|''
name|'def'
name|'_generate_key_pair'
...
newline|'\n'
indent|'    '
string|'"""Sub-set of the Compute API related to managing security groups\n    and security group rules\n    """'
newline|'\n'
nl|'\n'
comment|'# The nova security group api does not use a uuid for the id.'
nl|'\n'
...
name|'_'
op|'('
string|'"Security group %(name)s added %(protocol)s ingress "'
nl|'\n'
string|'"(%(from_port)s:%(to_port)s)"'
op|')'
newline|'\n'
name|'rules'
op|'='
...
name|'_'
op|'('
string|'"Security group %(name)s removed %(protocol)s ingress "'
nl|'\n'
string|'"(%(from_port)s:%(to_port)s)"'
op|')'
newline|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_refresh_instance_security_rules
dedent|''
dedent|''
name|'def'
...
op|':'
newline|'\n'
comment|"# Make sure it's an empty SecurityGroupList and not None"
nl|'\n'
indent|'            '
name|'return'
name|'objects'
