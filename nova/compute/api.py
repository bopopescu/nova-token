...
nl|'\n'
comment|'# snapshot. In this case the outher image to bdm mapping'
nl|'\n'
comment|'# contains an unnecessasry device with boot_index == 0.'
nl|'\n'
...
nl|'\n'
comment|'# considertaion of connections to each requested network'
nl|'\n'
name|'if'
name|'max_net_count'
op|'=='
...
op|':'
newline|'\n'
comment|"# NOTE(sbauza): Host can only be in one AZ, so let's take only"
nl|'\n'
...
name|'_LW'
op|'('
string|'"More than 1 AZ for host %s"'
op|')'
op|','
name|'host'
op|')'
...
op|':'
newline|'\n'
comment|'# NOTE(sbauza): Aggregate with AZ set to default AZ can'
nl|'\n'
comment|'#               exist, we need to check'
nl|'\n'
indent|'                    '
name|'host_aggs'
op|'='
...
newline|'\n'
nl|'\n'
DECL|member|_check_az_for_host
dedent|''
name|'def'
name|'_check_az_for_host'
op|'('
...
op|':'
newline|'\n'
comment|'# NOTE(mtreinish) The availability_zone key returns a set of'
nl|'\n'
comment|'# zones so loop over each zone. However there should only'
nl|'\n'
comment|'# ever be one zone in the set because an aggregate can only'
nl|'\n'
comment|'# have a single availability zone set at one time.'
nl|'\n'
indent|'        '
name|'if'
name|'isinstance'
...
op|':'
newline|'\n'
comment|'# NOTE(mtreinish) Ensure that the aggregate_az is not none'
nl|'\n'
comment|'# if it is none then that is just a regular aggregate and'
nl|'\n'
comment|'# it is valid to have a host in multiple aggregates.'
nl|'\n'
indent|'            '
name|'if'
name|'aggregate_az'
...
name|'_'
op|'('
string|'"Host already in availability zone "'
nl|'\n'
string|'"%s"'
op|')'
op|'%'
...
name|'_'
op|'('
string|'\'Specified Keypair type "%s" is invalid\''
op|')'
op|'%'
name|'key_type'
op|')'
