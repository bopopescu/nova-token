...
nl|'\n'
comment|'# signal to a instance during power off.  The overall'
nl|'\n'
...
name|'debug'
op|'('
string|"'Will not delete instance as its host ('"
nl|'\n'
string|"'%(instance_host)s) is not equal to our '"
nl|'\n'
string|"'host (%(our_host)s) but its task state is '"
nl|'\n'
string|"'(%(task_state)s) and vm state is '"
nl|'\n'
string|"'(%(vm_state)s)'"
op|','
nl|'\n'
...
name|'_LW'
op|'('
string|"'Instance %(uuid)s appears to have been '"
nl|'\n'
string|"'evacuated from this host to %(host)s. '"
nl|'\n'
string|"'Not destroying it locally due to '"
nl|'\n'
string|"'config setting '"
nl|'\n'
string|'\'"workarounds.destroy_after_evacuate". \''
nl|'\n'
string|"'If this is not correct, enable that '"
nl|'\n'
string|"'option and restart nova-compute.'"
op|')'
op|','
nl|'\n'
op|'{'
...
op|')'
newline|'\n'
comment|'# FIXME(comstud): This needs fixed. We should be creating'
nl|'\n'
comment|'# reservations and updating quotas, because quotas'
nl|'\n'
comment|"# wouldn't have been updated for this instance since it is"
nl|'\n'
comment|'# still in DELETING.  See bug 1296414.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# Create a dummy quota object for now.'
nl|'\n'
name|'quotas'
op|'='
name|'objects'
...
name|'_LW'
op|'('
string|'"Volume id: %s finished being created but "'
nl|'\n'
string|'"was not set as \'available\'"'
op|')'
op|','
name|'vol_id'
op|')'
...
nl|'\n'
comment|'# anti-affinity.  Since more than one instance may be scheduled at the'
nl|'\n'
comment|"# same time, it's possible that more than one instance with an"
nl|'\n'
comment|'# anti-affinity policy may end up here.  This is a validation step to'
nl|'\n'
comment|"# make sure that starting the instance here doesn't violate the policy."
nl|'\n'
nl|'\n'
indent|'        '
...
name|'_'
op|'('
string|'"Anti-affinity instance group policy was violated."'
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(danms): This method is deprecated, but could be called,'
nl|'\n'
comment|'# and if it is, it will have an old megatuple for requested_networks.'
nl|'\n'
name|'if'
name|'requested_networks'
name|'is'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(danms): Remove this in v4.0 of the RPC API'
nl|'\n'
indent|'        '
name|'if'
op|'('
...
op|')'
newline|'\n'
comment|'# NOTE(melwitt): Remove this in v4.0 of the RPC API'
nl|'\n'
dedent|''
name|'flavor'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# NOTE(sahid): Remove this in v4.0 of the RPC API'
nl|'\n'
dedent|''
name|'if'
op|'('
...
nl|'\n'
comment|'# cleanup those network resource setup on this host before'
nl|'\n'
...
op|':'
newline|'\n'
comment|'# NOTE(alaski) This method should be deprecated when the scheduler and'
nl|'\n'
comment|'# compute rpc interfaces are bumped to 4.x, and slated for removal in'
nl|'\n'
comment|'# 5.x as it is no longer used.'
nl|'\n'
nl|'\n'
indent|'        '
name|'if'
...
newline|'\n'
comment|'# NOTE (ndipanov): If we get non-object BDMs, just get them from the'
nl|'\n'
comment|'# db again, as this means they are sent in the old format and we want'
nl|'\n'
comment|'# to avoid converting them back when we can just get them.'
nl|'\n'
comment|'# Remove this when we bump the RPC major version to 4.0'
nl|'\n'
name|'if'
op|'('
name|'bdms'
...
comment|'# to avoid converting them back when we can just get them.'
nl|'\n'
comment|'# Remove this on the next major RPC version bump'
nl|'\n'
name|'if'
op|'('
name|'bdms'
...
op|':'
newline|'\n'
comment|'# NOTE(ndipanov): disk_bus and device_type will be set to None if not'
nl|'\n'
comment|'# passed (by older clients) and defaulted by the virt driver. Remove'
nl|'\n'
comment|'# default values on the next major RPC version bump.'
nl|'\n'
nl|'\n'
indent|'        '
op|'@'
...
nl|'\n'
nl|'\n'
comment|"# NOTE(PhilDay): Can't use object_compat decorator here as"
nl|'\n'
comment|'#                instance is not the second parameter'
nl|'\n'
name|'if'
name|'isinstance'
op|'('
...
newline|'\n'
nl|'\n'
comment|"# NOTE(danms): since instance is not the first parameter, we can't"
nl|'\n'
comment|'# use @object_compat on this method. Since this is the only example,'
nl|'\n'
comment|'# we do this manually instead of complicating the decorator'
nl|'\n'
name|'if'
name|'not'
name|'isinstance'
...
nl|'\n'
nl|'\n'
comment|'# TODO(danms): This goes away immediately in Lemming and is just'
nl|'\n'
comment|'# present in Kilo so that we can receive v3.x and v4.0 messages'
nl|'\n'
DECL|class|_ComputeV4Proxy
dedent|''
dedent|''
name|'class'
name|'_ComputeV4Proxy'
...
op|':'
newline|'\n'
comment|'# NOTE(danms): In 3.x, attach_volume had mountpoint and volume_id'
nl|'\n'
comment|'# parameters, which are gone from 4.x. Provide None for each to'
nl|'\n'
comment|'# the 3.x manager above and remove in Lemming.'
nl|'\n'
indent|'        '
name|'return'
name|'self'
...
op|':'
newline|'\n'
comment|'# NOTE(danms): Pass instance by kwarg to help the object_compat'
nl|'\n'
comment|'# decorator, as real RPC dispatch does.'
nl|'\n'
indent|'        '
name|'return'
name|'self'
