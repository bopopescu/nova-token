...
newline|'\n'
nl|'\n'
comment|'# NOTE(mriedem): If we are deleting the instance while it was booting'
nl|'\n'
comment|'# from volume, we could be racing with a database update of the BDM'
nl|'\n'
comment|'# volume_id. Since the compute API passes the BDMs over RPC to compute'
nl|'\n'
comment|'# here, the BDMs may be stale at this point. So check for any volume'
nl|'\n'
comment|"# BDMs that don't have volume_id set and if we detect that, we need to"
nl|'\n'
comment|'# refresh the BDM list before proceeding with detach.'
nl|'\n'
dedent|''
name|'for'
name|'bdm'
...
name|'debug'
op|'('
string|"'There are potentially stale BDMs during delete, '"
nl|'\n'
string|"'refreshing the BlockDeviceMappingList.'"
op|','
nl|'\n'
name|'instance'
op|'='
...
op|')'
newline|'\n'
comment|'# now filter the list again'
nl|'\n'
name|'vol_bdms'
op|'='
op|'['
...
newline|'\n'
nl|'\n'
DECL|function|_fail_migration
dedent|''
dedent|''
name|'def'
name|'_fail_migration'
op|'('
name|'migration'
op|')'
op|':'
...
op|':'
newline|'\n'
comment|'# NOTE(danms): Remove these guards in v5.0 of the RPC API'
nl|'\n'
indent|'        '
name|'if'
name|'migration'
