begin_unit
comment|'# Copyright 2014 OpenStack Foundation'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
string|'"""Possible results from instance build\n\nResults represent the ultimate result of an attempt to build an instance.\n\nResults describe whether an instance was actually built, failed to build, or\nwas rescheduled.\n"""'
newline|'\n'
nl|'\n'
DECL|variable|ACTIVE
name|'ACTIVE'
op|'='
string|"'active'"
comment|'# Instance is running'
newline|'\n'
DECL|variable|FAILED
name|'FAILED'
op|'='
string|"'failed'"
comment|'# Instance failed to build and was not rescheduled'
newline|'\n'
DECL|variable|RESCHEDULED
name|'RESCHEDULED'
op|'='
string|"'rescheduled'"
comment|'# Instance failed to build, but was rescheduled'
newline|'\n'
endmarker|''
end_unit
