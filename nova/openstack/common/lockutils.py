...
op|':'
newline|'\n'
comment|"# Hash the name because it's not valid to have POSIX semaphore"
nl|'\n'
comment|'# names with things like / in them. Then use base64 to encode'
nl|'\n'
comment|'# the digest() instead taking the hexdigest() because the'
nl|'\n'
comment|"# result is shorter and most systems can't have shm sempahore"
nl|'\n'
comment|'# names longer than 31 characters.'
nl|'\n'
indent|'        '
name|'h'
op|'='
...
op|':'
newline|'\n'
comment|"# NOTE(bnemec): Create a fake lock path for posix locks so we don't"
nl|'\n'
comment|'# unnecessarily raise the RequiredOptError below.'
nl|'\n'
indent|'        '
name|'if'
name|'InterProcessLock'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(bnemec): If an explicit lock_path was passed to us then it'
nl|'\n'
comment|'# means the caller is relying on file-based locking behavior, so'
nl|'\n'
comment|"# we can't use posix locks for those calls."
nl|'\n'
name|'if'
name|'lock_path'
op|':'
...
newline|'\n'
indent|'    '
string|'"""Remove a external lock file when it\'s not used anymore\n    This will be helpful when we have a lot of lock files\n    """'
newline|'\n'
name|'with'
name|'internal_lock'
op|'('
...
name|'debug'
op|'('
string|'\'Released semaphore "%(lock)s"\''
op|','
op|'{'
string|"'lock'"
op|':'
