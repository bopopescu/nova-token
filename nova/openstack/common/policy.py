begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright (c) 2011 OpenStack, LLC.'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
string|'"""Common Policy Engine Implementation"""'
newline|'\n'
nl|'\n'
name|'import'
name|'logging'
newline|'\n'
name|'import'
name|'urllib'
newline|'\n'
name|'import'
name|'urllib2'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'openstack'
op|'.'
name|'common'
op|'.'
name|'gettextutils'
name|'import'
name|'_'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'openstack'
op|'.'
name|'common'
name|'import'
name|'jsonutils'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|LOG
name|'LOG'
op|'='
name|'logging'
op|'.'
name|'getLogger'
op|'('
name|'__name__'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|_BRAIN
name|'_BRAIN'
op|'='
name|'None'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|set_brain
name|'def'
name|'set_brain'
op|'('
name|'brain'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Set the brain used by enforce().\n\n    Defaults use Brain() if not set.\n\n    """'
newline|'\n'
name|'global'
name|'_BRAIN'
newline|'\n'
name|'_BRAIN'
op|'='
name|'brain'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|reset
dedent|''
name|'def'
name|'reset'
op|'('
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Clear the brain used by enforce()."""'
newline|'\n'
name|'global'
name|'_BRAIN'
newline|'\n'
name|'_BRAIN'
op|'='
name|'None'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|enforce
dedent|''
name|'def'
name|'enforce'
op|'('
name|'match_list'
op|','
name|'target_dict'
op|','
name|'credentials_dict'
op|','
name|'exc'
op|'='
name|'None'
op|','
nl|'\n'
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Enforces authorization of some rules against credentials.\n\n    :param match_list: nested tuples of data to match against\n\n        The basic brain supports three types of match lists:\n\n            1) rules\n\n                looks like: ``(\'rule:compute:get_instance\',)``\n\n                Retrieves the named rule from the rules dict and recursively\n                checks against the contents of the rule.\n\n            2) roles\n\n                looks like: ``(\'role:compute:admin\',)``\n\n                Matches if the specified role is in credentials_dict[\'roles\'].\n\n            3) generic\n\n                looks like: ``(\'tenant_id:%(tenant_id)s\',)``\n\n                Substitutes values from the target dict into the match using\n                the % operator and matches them against the creds dict.\n\n        Combining rules:\n\n            The brain returns True if any of the outer tuple of rules\n            match and also True if all of the inner tuples match. You\n            can use this to perform simple boolean logic.  For\n            example, the following rule would return True if the creds\n            contain the role \'admin\' OR the if the tenant_id matches\n            the target dict AND the the creds contains the role\n            \'compute_sysadmin\':\n\n            ::\n\n                {\n                    "rule:combined": (\n                        \'role:admin\',\n                        (\'tenant_id:%(tenant_id)s\', \'role:compute_sysadmin\')\n                    )\n                }\n\n        Note that rule and role are reserved words in the credentials match, so\n        you can\'t match against properties with those names. Custom brains may\n        also add new reserved words. For example, the HttpBrain adds http as a\n        reserved word.\n\n    :param target_dict: dict of object properties\n\n      Target dicts contain as much information as we can about the object being\n      operated on.\n\n    :param credentials_dict: dict of actor properties\n\n      Credentials dicts contain as much information as we can about the user\n      performing the action.\n\n    :param exc: exception to raise\n\n      Class of the exception to raise if the check fails.  Any remaining\n      arguments passed to enforce() (both positional and keyword arguments)\n      will be passed to the exception class.  If exc is not provided, returns\n      False.\n\n    :return: True if the policy allows the action\n    :return: False if the policy does not allow the action and exc is not set\n    """'
newline|'\n'
name|'global'
name|'_BRAIN'
newline|'\n'
name|'if'
name|'not'
name|'_BRAIN'
op|':'
newline|'\n'
indent|'        '
name|'_BRAIN'
op|'='
name|'Brain'
op|'('
op|')'
newline|'\n'
dedent|''
name|'if'
name|'not'
name|'_BRAIN'
op|'.'
name|'check'
op|'('
name|'match_list'
op|','
name|'target_dict'
op|','
name|'credentials_dict'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'exc'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exc'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'False'
newline|'\n'
dedent|''
name|'return'
name|'True'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|Brain
dedent|''
name|'class'
name|'Brain'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Implements policy checking."""'
newline|'\n'
nl|'\n'
DECL|variable|_checks
name|'_checks'
op|'='
op|'{'
op|'}'
newline|'\n'
nl|'\n'
op|'@'
name|'classmethod'
newline|'\n'
DECL|member|_register
name|'def'
name|'_register'
op|'('
name|'cls'
op|','
name|'name'
op|','
name|'func'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'cls'
op|'.'
name|'_checks'
op|'['
name|'name'
op|']'
op|'='
name|'func'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'classmethod'
newline|'\n'
DECL|member|load_json
name|'def'
name|'load_json'
op|'('
name|'cls'
op|','
name|'data'
op|','
name|'default_rule'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Init a brain using json instead of a rules dictionary."""'
newline|'\n'
name|'rules_dict'
op|'='
name|'jsonutils'
op|'.'
name|'loads'
op|'('
name|'data'
op|')'
newline|'\n'
name|'return'
name|'cls'
op|'('
name|'rules'
op|'='
name|'rules_dict'
op|','
name|'default_rule'
op|'='
name|'default_rule'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__init__
dedent|''
name|'def'
name|'__init__'
op|'('
name|'self'
op|','
name|'rules'
op|'='
name|'None'
op|','
name|'default_rule'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'self'
op|'.'
name|'__class__'
op|'!='
name|'Brain'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'warning'
op|'('
name|'_'
op|'('
string|'"Inheritance-based rules are deprecated; use "'
nl|'\n'
string|'"the default brain instead of %s."'
op|')'
op|'%'
nl|'\n'
name|'self'
op|'.'
name|'__class__'
op|'.'
name|'__name__'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'self'
op|'.'
name|'rules'
op|'='
name|'rules'
name|'or'
op|'{'
op|'}'
newline|'\n'
name|'self'
op|'.'
name|'default_rule'
op|'='
name|'default_rule'
newline|'\n'
nl|'\n'
DECL|member|add_rule
dedent|''
name|'def'
name|'add_rule'
op|'('
name|'self'
op|','
name|'key'
op|','
name|'match'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'rules'
op|'['
name|'key'
op|']'
op|'='
name|'match'
newline|'\n'
nl|'\n'
DECL|member|_check
dedent|''
name|'def'
name|'_check'
op|'('
name|'self'
op|','
name|'match'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'match_kind'
op|','
name|'match_value'
op|'='
name|'match'
op|'.'
name|'split'
op|'('
string|"':'"
op|','
number|'1'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'Exception'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'exception'
op|'('
name|'_'
op|'('
string|'"Failed to understand rule %(match)r"'
op|')'
op|'%'
name|'locals'
op|'('
op|')'
op|')'
newline|'\n'
comment|'# If the rule is invalid, fail closed'
nl|'\n'
name|'return'
name|'False'
newline|'\n'
nl|'\n'
dedent|''
name|'func'
op|'='
name|'None'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'old_func'
op|'='
name|'getattr'
op|'('
name|'self'
op|','
string|"'_check_%s'"
op|'%'
name|'match_kind'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'AttributeError'
op|':'
newline|'\n'
indent|'            '
name|'func'
op|'='
name|'self'
op|'.'
name|'_checks'
op|'.'
name|'get'
op|'('
name|'match_kind'
op|','
name|'self'
op|'.'
name|'_checks'
op|'.'
name|'get'
op|'('
name|'None'
op|','
name|'None'
op|')'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'warning'
op|'('
name|'_'
op|'('
string|'"Inheritance-based rules are deprecated; update "'
nl|'\n'
string|'"_check_%s"'
op|')'
op|'%'
name|'match_kind'
op|')'
newline|'\n'
name|'func'
op|'='
op|'('
name|'lambda'
name|'brain'
op|','
name|'kind'
op|','
name|'value'
op|','
name|'target'
op|','
name|'cred'
op|':'
nl|'\n'
name|'old_func'
op|'('
name|'value'
op|','
name|'target'
op|','
name|'cred'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'not'
name|'func'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'error'
op|'('
name|'_'
op|'('
string|'"No handler for matches of kind %s"'
op|')'
op|'%'
name|'match_kind'
op|')'
newline|'\n'
comment|'# Fail closed'
nl|'\n'
name|'return'
name|'False'
newline|'\n'
nl|'\n'
dedent|''
name|'return'
name|'func'
op|'('
name|'self'
op|','
name|'match_kind'
op|','
name|'match_value'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
newline|'\n'
nl|'\n'
DECL|member|check
dedent|''
name|'def'
name|'check'
op|'('
name|'self'
op|','
name|'match_list'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Checks authorization of some rules against credentials.\n\n        Detailed description of the check with examples in policy.enforce().\n\n        :param match_list: nested tuples of data to match against\n        :param target_dict: dict of object properties\n        :param credentials_dict: dict of actor properties\n\n        :returns: True if the check passes\n\n        """'
newline|'\n'
name|'if'
name|'not'
name|'match_list'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'True'
newline|'\n'
dedent|''
name|'for'
name|'and_list'
name|'in'
name|'match_list'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'isinstance'
op|'('
name|'and_list'
op|','
name|'basestring'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'and_list'
op|'='
op|'('
name|'and_list'
op|','
op|')'
newline|'\n'
dedent|''
name|'if'
name|'all'
op|'('
op|'['
name|'self'
op|'.'
name|'_check'
op|'('
name|'item'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
nl|'\n'
name|'for'
name|'item'
name|'in'
name|'and_list'
op|']'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'return'
name|'True'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'False'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|HttpBrain
dedent|''
dedent|''
name|'class'
name|'HttpBrain'
op|'('
name|'Brain'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""A brain that can check external urls for policy.\n\n    Posts json blobs for target and credentials.\n\n    Note that this brain is deprecated; the http check is registered\n    by default.\n    """'
newline|'\n'
nl|'\n'
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|register
dedent|''
name|'def'
name|'register'
op|'('
name|'name'
op|','
name|'func'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""\n    Register a function as a policy check.\n\n    :param name: Gives the name of the check type, e.g., \'rule\',\n                 \'role\', etc.  If name is None, a default function\n                 will be registered.\n    :param func: If given, provides the function to register.  If not\n                 given, returns a function taking one argument to\n                 specify the function to register, allowing use as a\n                 decorator.\n    """'
newline|'\n'
nl|'\n'
comment|'# Perform the actual decoration by registering the function.'
nl|'\n'
comment|'# Returns the function for compliance with the decorator'
nl|'\n'
comment|'# interface.'
nl|'\n'
DECL|function|decorator
name|'def'
name|'decorator'
op|'('
name|'func'
op|')'
op|':'
newline|'\n'
comment|'# Register the function'
nl|'\n'
indent|'        '
name|'Brain'
op|'.'
name|'_register'
op|'('
name|'name'
op|','
name|'func'
op|')'
newline|'\n'
name|'return'
name|'func'
newline|'\n'
nl|'\n'
comment|'# If the function is given, do the registration'
nl|'\n'
dedent|''
name|'if'
name|'func'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'decorator'
op|'('
name|'func'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'return'
name|'decorator'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
op|'@'
name|'register'
op|'('
string|'"rule"'
op|')'
newline|'\n'
DECL|function|_check_rule
name|'def'
name|'_check_rule'
op|'('
name|'brain'
op|','
name|'match_kind'
op|','
name|'match'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Recursively checks credentials based on the brains rules."""'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'        '
name|'new_match_list'
op|'='
name|'brain'
op|'.'
name|'rules'
op|'['
name|'match'
op|']'
newline|'\n'
dedent|''
name|'except'
name|'KeyError'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'brain'
op|'.'
name|'default_rule'
name|'and'
name|'match'
op|'!='
name|'brain'
op|'.'
name|'default_rule'
op|':'
newline|'\n'
indent|'            '
name|'new_match_list'
op|'='
op|'('
string|"'rule:%s'"
op|'%'
name|'brain'
op|'.'
name|'default_rule'
op|','
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'False'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'return'
name|'brain'
op|'.'
name|'check'
op|'('
name|'new_match_list'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
op|'@'
name|'register'
op|'('
string|'"role"'
op|')'
newline|'\n'
DECL|function|_check_role
name|'def'
name|'_check_role'
op|'('
name|'brain'
op|','
name|'match_kind'
op|','
name|'match'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Check that there is a matching role in the cred dict."""'
newline|'\n'
name|'return'
name|'match'
op|'.'
name|'lower'
op|'('
op|')'
name|'in'
op|'['
name|'x'
op|'.'
name|'lower'
op|'('
op|')'
name|'for'
name|'x'
name|'in'
name|'cred_dict'
op|'['
string|"'roles'"
op|']'
op|']'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
op|'@'
name|'register'
op|'('
string|"'http'"
op|')'
newline|'\n'
DECL|function|_check_http
name|'def'
name|'_check_http'
op|'('
name|'brain'
op|','
name|'match_kind'
op|','
name|'match'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Check http: rules by calling to a remote server.\n\n    This example implementation simply verifies that the response is\n    exactly \'True\'. A custom brain using response codes could easily\n    be implemented.\n\n    """'
newline|'\n'
name|'url'
op|'='
string|"'http:'"
op|'+'
op|'('
name|'match'
op|'%'
name|'target_dict'
op|')'
newline|'\n'
name|'data'
op|'='
op|'{'
string|"'target'"
op|':'
name|'jsonutils'
op|'.'
name|'dumps'
op|'('
name|'target_dict'
op|')'
op|','
nl|'\n'
string|"'credentials'"
op|':'
name|'jsonutils'
op|'.'
name|'dumps'
op|'('
name|'cred_dict'
op|')'
op|'}'
newline|'\n'
name|'post_data'
op|'='
name|'urllib'
op|'.'
name|'urlencode'
op|'('
name|'data'
op|')'
newline|'\n'
name|'f'
op|'='
name|'urllib2'
op|'.'
name|'urlopen'
op|'('
name|'url'
op|','
name|'post_data'
op|')'
newline|'\n'
name|'return'
name|'f'
op|'.'
name|'read'
op|'('
op|')'
op|'=='
string|'"True"'
newline|'\n'
nl|'\n'
nl|'\n'
dedent|''
op|'@'
name|'register'
op|'('
name|'None'
op|')'
newline|'\n'
DECL|function|_check_generic
name|'def'
name|'_check_generic'
op|'('
name|'brain'
op|','
name|'match_kind'
op|','
name|'match'
op|','
name|'target_dict'
op|','
name|'cred_dict'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Check an individual match.\n\n    Matches look like:\n\n        tenant:%(tenant_id)s\n        role:compute:admin\n\n    """'
newline|'\n'
nl|'\n'
comment|'# TODO(termie): do dict inspection via dot syntax'
nl|'\n'
name|'match'
op|'='
name|'match'
op|'%'
name|'target_dict'
newline|'\n'
name|'if'
name|'match_kind'
name|'in'
name|'cred_dict'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'match'
op|'=='
name|'unicode'
op|'('
name|'cred_dict'
op|'['
name|'match_kind'
op|']'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'False'
newline|'\n'
dedent|''
endmarker|''
end_unit
