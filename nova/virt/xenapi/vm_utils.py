...
op|')'
newline|'\n'
comment|'# TODO(sirp): set name-label on VDI'
nl|'\n'
name|'scan_sr'
op|'('
name|'session'
...
name|'_'
op|'('
string|'"xapi \'download_image\' returned VDI UUID %(vdi_uuid)s"'
op|')'
op|'%'
name|'locals'
op|'('
...
newline|'\n'
comment|'# FIXME(sirp): Since the Glance plugin seems to be required for the VHD disk,'
nl|'\n'
comment|'# it may be worth using the plugin for both VHD and RAW and DISK'
nl|'\n'
comment|'# restores'
nl|'\n'
name|'sr_ref'
...
op|':'
newline|'\n'
comment|'# if using glance, then we could be VHD format'
nl|'\n'
indent|'            '
name|'client'
op|'='
...
op|')'
newline|'\n'
comment|'# TODO(sirp): When Glance treats disk_format as a first class'
nl|'\n'
comment|'# attribute, we should start using that rather than an image-property'
nl|'\n'
name|'if'
name|'disk_format'
op|'=='
