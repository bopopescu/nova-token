...
op|')'
newline|'\n'
comment|'#if fetch image fails the exception will be handled in spawn'
nl|'\n'
name|'vdi_uuid'
op|'='
name|'VMHelper'
...
name|'vdi_uuid'
newline|'\n'
comment|'#extract VDI uuid from spawn error'
nl|'\n'
dedent|''
name|'if'
name|'len'
...
name|'_'
op|'('
string|'"VDIS to remove:%s"'
op|')'
op|','
name|'vdis'
op|')'
newline|'\n'
name|'remove_from_dom0'
op|'='
name|'False'
newline|'\n'
name|'for'
...
op|':'
newline|'\n'
comment|'# TODO(sirp): Assuming PV for now; this will need to be'
nl|'\n'
comment|'# configurable as Windows will use HVM.'
nl|'\n'
indent|'            '
name|'pv_kernel'
op|'='
...
op|':'
newline|'\n'
comment|'# if the spwan process fails here it will be necessary to'
nl|'\n'
comment|'# clean up kernel and ramdisk (VDIs and files in dom0)'
nl|'\n'
nl|'\n'
DECL|function|_vdi_uuid
indent|'            '
name|'def'
name|'_vdi_uuid'
op|'('
...
op|':'
newline|'\n'
comment|'#Note: we assume the file name is the same as'
nl|'\n'
comment|'#the uuid of the VDI. If this changes in'
nl|'\n'
comment|'#_copy_kernel_vdi (glance plugin)'
nl|'\n'
comment|'#this function must be updated accordingly'
nl|'\n'
indent|'                '
name|'splits'
op|'='
...
newline|'\n'
nl|'\n'
DECL|function|_inject_onset_files
name|'def'
name|'_inject_onset_files'
op|'('
op|')'
op|':'
newline|'\n'
...
name|'_'
op|'('
string|'"Invalid value for onset_files: \'%s\'"'
op|')'
nl|'\n'
op|'%'
name|'onset_files'
...
newline|'\n'
comment|'# TODO(tr3buchet) - remove comment in multi-nic'
nl|'\n'
comment|"# I've decided to go ahead and consider multiple IPs and networks"
nl|'\n'
comment|"# at this stage even though they aren't implemented because these will"
nl|'\n'
comment|'# be needed for multi-nic and there was no sense writing it for single'
nl|'\n'
comment|'# network/single IP and then having to turn around and re-write it'
nl|'\n'
name|'vm_opaque_ref'
op|'='
name|'self'
...
name|'_'
op|'('
string|'"injecting network info to xenstore for vm: |%s|"'
op|')'
op|','
nl|'\n'
name|'vm_opaque_ref'
...
comment|'# TODO(tr3buchet) - remove comment in multi-nic'
nl|'\n'
comment|'# this bit here about creating the vifs will be updated'
nl|'\n'
comment|'# in multi-nic to handle multiple IPs on the same network'
nl|'\n'
comment|'# and multiple networks'
nl|'\n'
comment|'# for now it works as there is only one of each'
nl|'\n'
dedent|''
name|'for'
name|'network'
