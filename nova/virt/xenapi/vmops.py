...
number|'5'
newline|'\n'
DECL|variable|BUILD_TOTAL_STEPS
name|'BUILD_TOTAL_STEPS'
op|'='
number|'4'
newline|'\n'
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# 2. Fetch the Image over the Network'
nl|'\n'
name|'vdis'
op|'='
name|'self'
...
newline|'\n'
nl|'\n'
comment|'# 3. Create the VM records'
nl|'\n'
name|'vm_ref'
op|'='
name|'self'
...
op|')'
newline|'\n'
comment|'# 4. Prepare security group filters'
nl|'\n'
comment|'# NOTE(salvatore-orlando): setup_basic_filtering might be empty or'
nl|'\n'
comment|'# not implemented at all, as basic filter could be implemented'
nl|'\n'
comment|'# with VIF rules created by xapi plugin'
nl|'\n'
name|'try'
op|':'
newline|'\n'
...
op|')'
newline|'\n'
comment|"# The VM has started, let's ensure the security groups are enforced"
nl|'\n'
name|'self'
op|'.'
name|'firewall_driver'
...
name|'_'
op|'('
string|'"Auto configuring disk for instance"'
nl|'\n'
string|'" %(instance_uuid)s, attempting to"'
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_configure_instance
dedent|''
dedent|''
name|'def'
name|'_configure_instance'
op|'('
name|'self'
op|','
name|'ctx'
...
name|'_'
op|'('
string|"'Spawning VM %(instance_uuid)s created %(vm_ref)s.'"
op|')'
nl|'\n'
op|'%'
name|'locals'
...
name|'_'
op|'('
string|"'Instance %s: waiting for running'"
op|')'
op|','
name|'instance_uuid'
op|')'
...
name|'_'
op|'('
string|'"Unable to Snapshot instance %(instance_uuid)s: "'
nl|'\n'
string|'"%(exc)s"'
op|')'
op|'%'
...
newline|'\n'
indent|'        '
string|'"""Resize a running instance by changing its disk size."""'
newline|'\n'
...
name|'_'
op|'('
string|'"VM %(instance_uuid)s already halted,"'
nl|'\n'
...
name|'_'
op|'('
string|'"Shutting down VM for Instance %(instance_uuid)s"'
op|')'
nl|'\n'
op|'%'
name|'locals'
...
name|'_'
op|'('
string|'"Instance %(instance_uuid)s using RAW or VHD, "'
nl|'\n'
...
name|'_'
op|'('
string|'"Instance %(instance_uuid)s VM destroyed"'
op|')'
op|'%'
name|'locals'
op|'('
...
name|'_'
op|'('
string|'"Destroying VM for Instance %(instance_uuid)s"'
op|')'
op|'%'
name|'locals'
op|'('
...
name|'_'
op|'('
string|'"Instance for migration %d not found, skipping"'
op|')'
op|','
nl|'\n'
name|'migration'
...
newline|'\n'
nl|'\n'
comment|"# NOTE(sirp): setting to error so we don't keep trying to auto"
nl|'\n'
comment|'# confirm this resize'
nl|'\n'
name|'db'
op|'.'
...
nl|'\n'
string|"' response: %(ret)r. VM id=%(instance_uuid)s;'"
nl|'\n'
...
nl|'\n'
string|"'VM id=%(instance_uuid)s; args=%(args)r'"
op|')'
op|'%'
name|'locals'
op|'('
...
nl|'\n'
string|"' supported by the agent. VM id=%(instance_uuid)s;'"
nl|'\n'
string|"' args=%(args)r'"
op|')'
op|'%'
