...
nl|'\n'
comment|'# userdevice 1 is reserved for rescue'
nl|'\n'
name|'userdevice'
op|'='
number|'2'
...
name|'_'
op|'('
string|"'No agent version returned by instance'"
op|')'
op|')'
newline|'\n'
name|'return'
...
name|'_'
op|'('
string|'"Resizing VDI %(vdi_uuid)s for instance %(instance_name)s."'
nl|'\n'
string|'" Expanding to %(instance_local_gb)d GB"'
op|')'
op|'%'
name|'locals'
op|'('
...
name|'ret'
newline|'\n'
comment|'#    def set_host_enabled(self, host, enabled):'
nl|'\n'
comment|'#        """Sets the specified host\'s ability to accept new instances."""'
nl|'\n'
comment|'#        return self._call_xenhost("set_host_enabled", {"enabled": enabled})'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    def _call_xenhost(self, method, arg_dict):'
nl|'\n'
comment|"#        # Create a task ID as something that won't match any instance ID"
nl|'\n'
comment|'#        task_id = random.randint(-80000, -70000)'
nl|'\n'
comment|'#        try:'
nl|'\n'
comment|'#            task = self._session.async_call_plugin("xenhost", method,'
nl|'\n'
comment|'#                    args=arg_dict)'
nl|'\n'
comment|'#            ret = self._session.wait_for_task(task, task_id)'
nl|'\n'
comment|'#        except self.XenAPI.Failure as e:'
nl|'\n'
comment|'#            ret = None'
nl|'\n'
comment|'#            LOG.error(_("The call to %(method)s returned an error: %(e)s.")'
nl|'\n'
comment|'#                    % locals())'
nl|'\n'
comment|'#        return ret'
