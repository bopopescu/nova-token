...
name|'shutil'
newline|'\n'
comment|'# appended by masumotok'
nl|'\n'
comment|'#import libvirt'
nl|'\n'
nl|'\n'
name|'import'
name|'IPy'
...
name|'execute'
op|'('
string|'"sudo virsh migrate --live %s %s"'
nl|'\n'
op|'%'
op|'('
name|'instance_ref'
...
newline|'\n'
nl|'\n'
comment|'# wait for completion of live_migration'
nl|'\n'
name|'d'
op|'='
name|'defer'
...
newline|'\n'
nl|'\n'
DECL|function|_wait_for_live_migration
name|'def'
name|'_wait_for_live_migration'
op|'('
op|')'
op|':'
newline|'\n'
...
op|']'
newline|'\n'
comment|'#except libvirt.libvirtError, e:'
nl|'\n'
dedent|''
name|'except'
name|'exception'
...
newline|'\n'
nl|'\n'
comment|'# 1. detaching volumes'
nl|'\n'
comment|'# (not necessary in current version )'
nl|'\n'
comment|'#try :'
nl|'\n'
comment|"#    ec2_id = instance_ref['ec2_id']"
nl|'\n'
comment|'#    volumes = db.volume_get_by_ec2_id(context, ec2_id)'
nl|'\n'
comment|'#    for volume in volumes :'
nl|'\n'
comment|'#        self.detach_volume(context, instance_id, volume.id)'
nl|'\n'
comment|'#except exception.NotFound:'
nl|'\n'
comment|"#    logging.debug('%s doesnt mount any volumes.. ' % ec2_id)"
nl|'\n'
nl|'\n'
comment|'# 2. releasing vlan'
nl|'\n'
comment|'#   (not necessary in current implementation?)'
nl|'\n'
nl|'\n'
comment|'# 3. releasing security group ingress rule'
nl|'\n'
comment|'#   (not necessary in current implementation?)'
nl|'\n'
nl|'\n'
comment|'# 4. database updating'
nl|'\n'
indent|'        '
name|'ec2_id'
op|'='
...
op|')'
newline|'\n'
comment|'# not return if fixed_ip is not found, otherwise,'
nl|'\n'
...
name|'error'
op|'('
string|"'fixed_ip is not found for %s '"
op|'%'
name|'ec2_id'
op|')'
newline|'\n'
...
name|'debug'
op|'('
string|"'%s doesnt have floating_ip.. '"
op|'%'
name|'ec2_id'
op|')'
newline|'\n'
...
name|'info'
op|'('
string|"'Live migrating %s to %s finishes successfully'"
nl|'\n'
op|'%'
op|'('
name|'ec2_id'
...
name|'retval'
op|'='
string|'"<filter name=\'nova-project-%s\' chain=\'ipv4\'>"'
op|'%'
name|'project'
newline|'\n'
name|'for'
...
name|'nwfilter_xml'
op|'+='
string|'"  <filterref filter=\'nova-project-%s\' />\\n"'
op|'%'
name|'instance'
op|'['
string|"'project_id'"
