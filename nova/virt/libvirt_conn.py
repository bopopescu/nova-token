...
name|'qemu_img_cmd'
op|'='
string|"'%s convert -f qcow2 -O raw -s %s %s %s'"
op|'%'
op|'('
nl|'\n'
name|'FLAGS'
...
op|':'
newline|'\n'
comment|'# NOTE(itoumsn): self.shutdown() and wait instead of destroy would be'
nl|'\n'
comment|'# better because we cannot ensure flushing dirty buffers'
nl|'\n'
comment|'# in the guest OS. But, in case of KVM, shutdown often fails...'
nl|'\n'
comment|"#        instance_id = instance['id']"
nl|'\n'
comment|'#        volume_list = []'
nl|'\n'
comment|'#        dev_path_list = {}'
nl|'\n'
comment|'#        vols = db.volume_get_all_by_instance(context.get_admin_context(),'
nl|'\n'
comment|'#                                             instance_id)'
nl|'\n'
comment|'#        LOG.debug(_("DEBUG: vols %s") % vols)'
nl|'\n'
comment|'#        for vol in vols:'
nl|'\n'
comment|'#            LOG.debug(_("DEBUG: reboot: %s %s") % (vol[\'mountpoint\'],'
nl|'\n'
comment|'#                      vol[\'mountpoint\'].rpartition("/")[2]))'
nl|'\n'
comment|'#            volume_list.append(vol[\'mountpoint\'].partition("/")[2])'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#        if len(volume_list) != 0:'
nl|'\n'
comment|'#            LOG.debug(_("DEBUG: volume_list is not empty"))'
nl|'\n'
comment|"#            virt_dom = self._conn.lookupByName(instance['name'])"
nl|'\n'
comment|'#            xml = virt_dom.XMLDesc(0)'
nl|'\n'
comment|'#            try:'
nl|'\n'
comment|'#                doc = libxml2.parseDoc(xml)'
nl|'\n'
comment|'#            except:'
nl|'\n'
comment|"#                LOG.exception(_('Failed to get xml description %s'),"
nl|'\n'
comment|'#                              instance_id)'
nl|'\n'
comment|'#            ctx = doc.xpathNewContext()'
nl|'\n'
comment|'#            try:'
nl|'\n'
comment|"#                ret = ctx.xpathEval('/domain/devices/disk')"
nl|'\n'
comment|'#                for node in ret:'
nl|'\n'
comment|'#                    LOG.debug(_("DEBUG: node.name %s") % node.name)'
nl|'\n'
comment|"#                    target_dev = ''"
nl|'\n'
comment|"#                    source_dev = ''"
...
comment|'#                        LOG.debug('
nl|'\n'
comment|'#                            _("child.name: %s prop.dev %s") %'
nl|'\n'
comment|"#                            (child.name, child.prop('dev')))"
nl|'\n'
comment|'#'
nl|'\n'
comment|"#                        if child.name == 'source':"
nl|'\n'
comment|"#                            source_dev = child.prop('dev')"
nl|'\n'
comment|"#                        elif child.name == 'target':"
nl|'\n'
comment|"#                            target_dev = child.prop('dev')"
nl|'\n'
comment|'#                    LOG.debug('
nl|'\n'
comment|'#                        _("source: %s target %s") %'
nl|'\n'
comment|'#                        (source_dev, target_dev))'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#                    if target_dev in volume_list:'
nl|'\n'
comment|'#                        dev_path_list[target_dev] = source_dev'
nl|'\n'
comment|'#                        LOG.debug('
nl|'\n'
comment|'#                            _("append to dev_path_list source: %s target: %s") %'
nl|'\n'
comment|'#                            (source_dev, target_dev))'
nl|'\n'
comment|'#                    else:'
nl|'\n'
comment|'#                        LOG.debug(_("DEBUG: %s not found in volume_list") %'
nl|'\n'
comment|'#                                  (target_dev))'
nl|'\n'
comment|'#            finally:'
nl|'\n'
comment|'#                LOG.debug(_("DEBUG: finally block"))'
nl|'\n'
comment|'#                if ctx != None:'
nl|'\n'
comment|'#                    ctx.xpathFreeContext()'
nl|'\n'
comment|'#                    if doc != None:'
nl|'\n'
comment|'#                        doc.freeDoc()'
...
op|')'
newline|'\n'
comment|'#        xml = self.to_xml(instance)'
nl|'\n'
name|'self'
op|'.'
name|'firewall_driver'
...
op|')'
newline|'\n'
comment|'# Re-attach volumes'
nl|'\n'
comment|'#                    for mp in dev_path_list.iterkeys():'
nl|'\n'
comment|'#                        LOG.debug('
nl|'\n'
comment|'#                            _("Re-attaching %s to %s") %'
nl|'\n'
comment|'#                            (dev_path_list[mp], mp))'
nl|'\n'
comment|"#                        self.attach_volume(instance['name'],"
nl|'\n'
comment|'#                                           dev_path_list[mp], mp)'
nl|'\n'
comment|'#'
...
comment|"#                    instance_id = instance['id']"
nl|'\n'
comment|'#                    for vol in db.volume_get_all_by_instance('
nl|'\n'
comment|'#                        context.get_admin_context(), instance_id):'
nl|'\n'
comment|'# NOTE(itoumsn): dev_path is not stored anywhere,'
nl|'\n'
comment|"# and it has driver specific format. Furthermore, it's"
nl|'\n'
comment|'# also compute node specific in general.'
nl|'\n'
comment|'# Therefore, no way other than calling '
nl|'\n'
comment|'# undiscover/discover_driver here at this moment.'
nl|'\n'
comment|'#                        self.volume_manager.driver.undiscover_volume(vol)'
nl|'\n'
comment|'#                        dev_path = self.volume_manager.driver.discover_volume('
nl|'\n'
comment|'#                            context, vol)'
nl|'\n'
comment|'#                        LOG.debug('
...
comment|'#                            _("Re-attaching %s to %s") %'
nl|'\n'
comment|"#                            (dev_path, vol['mountpoint']))"
nl|'\n'
comment|"#                        self.attach_volume(instance['name'],"
nl|'\n'
comment|"#                                           dev_path, vol['mountpoint'])"
...
op|':'
newline|'\n'
comment|"# NOTE(justinsb): When libvirt isn't running / can't connect, we get:"
nl|'\n'
comment|'# libvir: Remote error : unable to connect to'
nl|'\n'
comment|"#  '/var/run/libvirt/libvirt-sock', libvirtd may need to be started:"
nl|'\n'
...
name|'_'
op|'('
string|'"Error from libvirt during lookup. "'
nl|'\n'
