...
newline|'\n'
nl|'\n'
DECL|variable|DEFAULT_UEFI_LOADER_PATH
name|'DEFAULT_UEFI_LOADER_PATH'
op|'='
op|'{'
nl|'\n'
string|'"x86_64"'
op|':'
string|'"/usr/share/OVMF/OVMF_CODE.fd"'
op|','
nl|'\n'
string|'"aarch64"'
op|':'
string|'"/usr/share/AAVMF/AAVMF_CODE.fd"'
nl|'\n'
op|'}'
newline|'\n'
nl|'\n'
...
nl|'\n'
string|"'nova.virt.libvirt.volume.fibrechannel.'"
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# For information about when MIN_LIBVIRT_VERSION and'
nl|'\n'
comment|'# NEXT_MIN_LIBVIRT_VERSION can be changed, consult'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#   https://wiki.openstack.org/wiki/LibvirtDistroSupportMatrix'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# Currently this is effectively the min version for i686/x86_64'
nl|'\n'
comment|'# + KVM/QEMU, as other architectures/hypervisors require newer'
nl|'\n'
comment|'# versions. Over time, this will become a common min version'
nl|'\n'
comment|'# for all architectures/hypervisors, as this value rises to'
nl|'\n'
comment|'# meet them.'
nl|'\n'
...
op|')'
newline|'\n'
DECL|variable|MIN_QEMU_VERSION
name|'MIN_QEMU_VERSION'
op|'='
op|'('
number|'1'
op|','
...
op|')'
newline|'\n'
comment|'# TODO(berrange): Re-evaluate this at start of each release cycle'
nl|'\n'
comment|'# to decide if we want to plan a future min version bump.'
nl|'\n'
comment|'# MIN_LIBVIRT_VERSION can be updated to match this after'
nl|'\n'
comment|'# NEXT_MIN_LIBVIRT_VERSION  has been at a higher value for'
nl|'\n'
comment|'# one cycle'
nl|'\n'
...
op|')'
newline|'\n'
DECL|variable|NEXT_MIN_QEMU_VERSION
name|'NEXT_MIN_QEMU_VERSION'
op|'='
op|'('
number|'1'
op|','
...
op|')'
newline|'\n'
comment|'# Libvirt version 1.2.17 is required for successfull block live migration'
nl|'\n'
comment|'# of vm booted from image with attached devices'
nl|'\n'
DECL|variable|MIN_LIBVIRT_BLOCK_LM_WITH_VOLUMES_VERSION
name|'MIN_LIBVIRT_BLOCK_LM_WITH_VOLUMES_VERSION'
op|'='
op|'('
number|'1'
op|','
...
op|')'
newline|'\n'
comment|'# PowerPC based hosts that support NUMA using libvirt'
nl|'\n'
DECL|variable|MIN_LIBVIRT_NUMA_VERSION_PPC
name|'MIN_LIBVIRT_NUMA_VERSION_PPC'
op|'='
op|'('
number|'1'
op|','
...
newline|'\n'
nl|'\n'
comment|'# UEFI booting support'
nl|'\n'
DECL|variable|MIN_LIBVIRT_UEFI_VERSION
name|'MIN_LIBVIRT_UEFI_VERSION'
op|'='
op|'('
number|'1'
op|','
...
newline|'\n'
nl|'\n'
comment|'# Ability to set the user guest password with Qemu'
nl|'\n'
DECL|variable|MIN_LIBVIRT_SET_ADMIN_PASSWD
name|'MIN_LIBVIRT_SET_ADMIN_PASSWD'
op|'='
op|'('
number|'1'
op|','
...
newline|'\n'
nl|'\n'
comment|'# s/390 & s/390x architectures with KVM'
nl|'\n'
DECL|variable|MIN_LIBVIRT_KVM_S390_VERSION
name|'MIN_LIBVIRT_KVM_S390_VERSION'
op|'='
op|'('
number|'1'
op|','
...
op|')'
newline|'\n'
DECL|variable|MIN_QEMU_S390_VERSION
name|'MIN_QEMU_S390_VERSION'
op|'='
op|'('
number|'2'
op|','
...
newline|'\n'
nl|'\n'
comment|'# libvirt < 1.3 reported virt_functions capability'
nl|'\n'
comment|'# only when VFs are enabled.'
nl|'\n'
comment|'# libvirt 1.3 fix f391889f4e942e22b9ef8ecca492de05106ce41e'
nl|'\n'
DECL|variable|MIN_LIBVIRT_PF_WITH_NO_VFS_CAP_VERSION
name|'MIN_LIBVIRT_PF_WITH_NO_VFS_CAP_VERSION'
op|'='
op|'('
number|'1'
op|','
...
newline|'\n'
nl|'\n'
comment|'# ppc64/ppc64le architectures with KVM'
nl|'\n'
comment|'# NOTE(rfolco): Same levels for Libvirt/Qemu on Big Endian and Little'
nl|'\n'
comment|'# Endian giving the nuance around guest vs host architectures'
nl|'\n'
DECL|variable|MIN_LIBVIRT_KVM_PPC64_VERSION
name|'MIN_LIBVIRT_KVM_PPC64_VERSION'
op|'='
op|'('
number|'1'
op|','
...
op|')'
newline|'\n'
DECL|variable|MIN_QEMU_PPC64_VERSION
name|'MIN_QEMU_PPC64_VERSION'
op|'='
op|'('
number|'2'
op|','
...
newline|'\n'
nl|'\n'
comment|'# Names of the types that do not get compressed during migration'
nl|'\n'
DECL|variable|NO_COMPRESSION_TYPES
name|'NO_COMPRESSION_TYPES'
op|'='
op|'('
string|"'qcow2'"
op|','
...
nl|'\n'
nl|'\n'
comment|'# number of serial console limit'
nl|'\n'
DECL|variable|QEMU_MAX_SERIAL_PORTS
name|'QEMU_MAX_SERIAL_PORTS'
op|'='
number|'4'
newline|'\n'
comment|'# Qemu supports 4 serial consoles, we remove 1 because of the PTY one defined'
nl|'\n'
DECL|variable|ALLOWED_QEMU_SERIAL_PORTS
name|'ALLOWED_QEMU_SERIAL_PORTS'
op|'='
name|'QEMU_MAX_SERIAL_PORTS'
op|'-'
number|'1'
newline|'\n'
nl|'\n'
comment|'# realtime suppport'
nl|'\n'
DECL|variable|MIN_LIBVIRT_REALTIME_VERSION
name|'MIN_LIBVIRT_REALTIME_VERSION'
op|'='
op|'('
number|'1'
op|','
...
newline|'\n'
nl|'\n'
DECL|variable|MIN_LIBVIRT_OTHER_ARCH
name|'MIN_LIBVIRT_OTHER_ARCH'
op|'='
op|'{'
name|'arch'
op|'.'
...
op|'}'
newline|'\n'
DECL|variable|MIN_QEMU_OTHER_ARCH
name|'MIN_QEMU_OTHER_ARCH'
op|'='
op|'{'
name|'arch'
op|'.'
...
name|'_'
op|'('
string|"'Nova requires QEMU version %s or greater.'"
op|')'
op|'%'
nl|'\n'
name|'self'
...
newline|'\n'
nl|'\n'
comment|'# Give the cloud admin a heads up if we are intending to'
nl|'\n'
comment|'# change the MIN_LIBVIRT_VERSION in the next release.'
nl|'\n'
dedent|''
name|'if'
name|'not'
...
name|'_LW'
op|'('
string|"'Running Nova with a QEMU version less than '"
nl|'\n'
string|"'%(version)s is deprecated. The required minimum '"
nl|'\n'
string|"'version of QEMU will be raised to %(version)s '"
nl|'\n'
string|"'in the next release.'"
op|')'
op|','
...
name|'_'
op|'('
string|"'Running Nova with qemu/kvm virt_type on %(arch)s '"
nl|'\n'
string|"'requires libvirt version %(libvirt_ver)s and '"
nl|'\n'
string|"'qemu version %(qemu_ver)s, or greater'"
op|')'
op|'%'
nl|'\n'
op|'{'
...
newline|'\n'
nl|'\n'
DECL|member|_check_required_migration_flags
dedent|''
dedent|''
name|'def'
name|'_check_required_migration_flags'
op|'('
name|'self'
op|','
...
name|'_LW'
op|'('
string|"'Removing the VIR_MIGRATE_PEER2PEER flag from '"
nl|'\n'
string|"'%(config_name)s because peer-to-peer '"
nl|'\n'
string|'\'migrations are not supported by the "xen" \''
nl|'\n'
string|"'virt type'"
op|')'
op|','
...
name|'_LW'
op|'('
string|"'Adding the VIR_MIGRATE_PEER2PEER flag to '"
nl|'\n'
string|"'%(config_name)s because direct migrations '"
nl|'\n'
string|"'are not supported by the %(virt_type)s '"
nl|'\n'
string|"'virt type'"
op|')'
op|','
...
name|'_LW'
op|'('
string|"'Adding the VIR_MIGRATE_UNDEFINE_SOURCE flag to '"
nl|'\n'
string|"'%(config_name)s because, without it, migrated '"
nl|'\n'
string|"'VMs will remain defined on the source host'"
op|')'
op|','
nl|'\n'
op|'{'
...
name|'_LW'
op|'('
string|"'Removing the VIR_MIGRATE_PERSIST_DEST flag from '"
nl|'\n'
string|"'%(config_name)s as Nova ensures the VM is '"
nl|'\n'
string|"'persisted on the destination host'"
op|')'
op|','
nl|'\n'
op|'{'
...
newline|'\n'
nl|'\n'
DECL|member|_check_block_migration_flags
dedent|''
name|'def'
...
newline|'\n'
nl|'\n'
DECL|member|_handle_live_migration_tunnelled
dedent|''
name|'def'
name|'_handle_live_migration_tunnelled'
op|'('
name|'self'
op|','
name|'migration_flags'
...
name|'_LW'
op|'('
string|"'The %(config_name)s config option does not '"
nl|'\n'
string|"'contain the VIR_MIGRATE_TUNNELLED flag but '"
nl|'\n'
string|"'the live_migration_tunnelled is set to True '"
nl|'\n'
string|"'which causes VIR_MIGRATE_TUNNELLED to be '"
nl|'\n'
string|"'set'"
op|')'
op|','
...
name|'_LW'
op|'('
string|"'The %(config_name)s config option contains '"
nl|'\n'
string|"'the VIR_MIGRATE_TUNNELLED flag but the '"
nl|'\n'
string|"'live_migration_tunnelled is set to False '"
nl|'\n'
string|"'which causes VIR_MIGRATE_TUNNELLED to be '"
nl|'\n'
string|"'unset'"
...
name|'_LW'
op|'('
string|'"Ignoring unknown libvirt live migration "'
nl|'\n'
string|'"flag \'%(flag)s\'"'
op|')'
op|','
...
newline|'\n'
nl|'\n'
name|'live_config_name'
op|'='
string|"'live_migration_flag'"
newline|'\n'
name|'block_config_name'
op|'='
...
name|'staticmethod'
newline|'\n'
DECL|member|_live_migration_uri
name|'def'
name|'_live_migration_uri'
op|'('
name|'dest'
op|')'
op|':'
newline|'\n'
comment|'# Only Xen and QEMU support live migration, see'
nl|'\n'
comment|'# https://libvirt.org/migration.html#scenarios for reference'
nl|'\n'
indent|'        '
name|'uris'
op|'='
...
name|'debug'
op|'('
string|"'Attempting to teardown container at path %(dir)s with '"
nl|'\n'
string|"'root device: %(rootfs_dev)s'"
op|','
nl|'\n'
op|'{'
string|"'dir'"
...
op|':'
newline|'\n'
comment|'# This method is called for several events: destroy,'
nl|'\n'
comment|'# rebuild, hard-reboot, power-off - For all of these'
nl|'\n'
comment|'# events we want to release the serial ports acquired'
nl|'\n'
comment|'# for the guest before destroying it.'
nl|'\n'
indent|'                '
name|'serials'
op|'='
name|'self'
op|'.'
name|'_get_serial_ports_from_guest'
op|'('
name|'guest'
op|')'
newline|'\n'
...
nl|'\n'
comment|'#                this may be an endless loop, so check the id of'
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_get_serial_ports_from_guest
dedent|''
dedent|''
name|'def'
name|'_get_serial_ports_from_guest'
...
newline|'\n'
indent|'        '
string|'"""Returns an iterator over serial port(s) configured on guest.\n\n        :param mode: Should be a value in (None, bind, connect)\n        """'
newline|'\n'
name|'xml'
op|'='
name|'guest'
...
op|')'
newline|'\n'
comment|'# TODO(nic): Set ignore_errors=False in a future release.'
nl|'\n'
comment|'# It is set to True here to avoid any upgrade issues surrounding'
nl|'\n'
comment|'# instances being in pending resize state when the software is updated;'
nl|'\n'
comment|'# in that case there will be no snapshot to remove.  Once it can be'
nl|'\n'
comment|'# reasonably assumed that no such instances exist in the wild'
nl|'\n'
comment|'# anymore, it should be set back to False (the default) so it will'
nl|'\n'
comment|'# throw errors, like it should.'
nl|'\n'
name|'backend'
op|'.'
name|'remove_snap'
...
newline|'\n'
nl|'\n'
DECL|member|_check_discard_for_attach_volume
dedent|''
name|'def'
name|'_check_discard_for_attach_volume'
op|'('
name|'self'
op|','
name|'conf'
...
name|'debug'
op|'('
string|"'Attempting to attach volume %(id)s with discard '"
nl|'\n'
string|"'support enabled to an instance using an '"
nl|'\n'
string|"'unsupported configuration. target_bus = '"
nl|'\n'
string|"'%(bus)s. Trim commands will not be issued to '"
nl|'\n'
string|"'the storage device.'"
op|','
nl|'\n'
op|'{'
string|"'bus'"
...
op|':'
newline|'\n'
comment|'# NOTE(mriedem): When deleting an instance and using Neutron,'
nl|'\n'
comment|'# we can be racing against Neutron deleting the port and'
nl|'\n'
comment|'# sending the vif-deleted event which then triggers a call to'
nl|'\n'
comment|'# detach the interface, so we might have failed because the'
nl|'\n'
comment|'# network device no longer exists. Libvirt will fail with'
nl|'\n'
comment|'# "operation failed: no matching network device was found"'
nl|'\n'
comment|'# which unfortunately does not have a unique error code so we'
nl|'\n'
comment|"# need to look up the interface by MAC and if it's not found"
nl|'\n'
comment|'# then we can just log it as a warning rather than tracing an'
nl|'\n'
comment|'# error.'
nl|'\n'
indent|'                '
...
newline|'\n'
nl|'\n'
comment|'# The interface is gone so just log it as a warning.'
nl|'\n'
dedent|''
name|'LOG'
op|'.'
...
name|'_LW'
op|'('
string|"'Detaching interface %(mac)s failed  because '"
nl|'\n'
string|"'the device is no longer found on the guest.'"
op|')'
op|','
nl|'\n'
op|'{'
...
newline|'\n'
nl|'\n'
comment|'# source_format is an on-disk format'
nl|'\n'
comment|'# source_type is a backend type'
nl|'\n'
name|'disk_path'
op|','
name|'source_format'
...
newline|'\n'
nl|'\n'
comment|"# We won't have source_type for raw or qcow2 disks, because we can't"
nl|'\n'
comment|'# determine that from the path. We should have it from the libvirt'
nl|'\n'
comment|'# xml, though.'
nl|'\n'
name|'if'
...
name|'source_format'
newline|'\n'
comment|"# For lxc instances we won't have it either from libvirt xml"
nl|'\n'
comment|'# (because we just gave libvirt the mounted filesystem), or the path,'
nl|'\n'
comment|'# so source_type is still going to be None. In this case,'
nl|'\n'
comment|'# snapshot_backend is going to default to CONF.libvirt.images_type'
nl|'\n'
comment|'# below, which is still safe.'
nl|'\n'
nl|'\n'
dedent|''
name|'image_format'
...
name|'self'
op|'.'
name|'_prepare_domain_for_snapshot'
op|'('
name|'context'
op|','
name|'live_snapshot'
...
name|'_LW'
op|'('
string|"'Performing standard snapshot because direct '"
nl|'\n'
string|"'snapshot failed: %(error)s'"
op|')'
op|','
op|'{'
string|"'error'"
...
newline|'\n'
nl|'\n'
comment|'# TODO(nic): possibly abstract this out to the snapshot_backend'
nl|'\n'
name|'if'
name|'source_type'
op|'=='
...
op|':'
newline|'\n'
comment|'# Standard snapshot uses qemu-img convert from RBD which is'
nl|'\n'
comment|'# not safe to run with live_snapshot.'
nl|'\n'
indent|'                '
name|'live_snapshot'
op|'='
name|'False'
newline|'\n'
comment|'# Suspend the guest, so this is no longer a live snapshot'
nl|'\n'
name|'self'
op|'.'
name|'_prepare_domain_for_snapshot'
...
op|':'
newline|'\n'
comment|'# NOTE(xqueralt): libvirt needs o+x in the tempdir'
nl|'\n'
indent|'                        '
name|'os'
op|'.'
...
name|'_LE'
op|'('
string|'"Failed to snapshot image"'
op|')'
op|')'
newline|'\n'
name|'failed_snap'
...
newline|'\n'
nl|'\n'
DECL|member|_prepare_domain_for_snapshot
dedent|''
name|'def'
name|'_prepare_domain_for_snapshot'
op|'('
...
newline|'\n'
nl|'\n'
DECL|member|_can_set_admin_password
dedent|''
dedent|''
dedent|''
name|'def'
name|'_can_set_admin_password'
op|'('
name|'self'
op|','
name|'image_meta'
...
name|'_'
op|'('
string|"'Error from libvirt while set password for username '"
nl|'\n'
string|'\'"%(user)s": [Error Code %(error_code)s] %(ex)s\''
op|')'
nl|'\n'
op|'%'
op|'{'
...
name|'_LI'
op|'('
string|"'Skipping quiescing instance: %(reason)s.'"
op|')'
op|','
nl|'\n'
op|'{'
...
name|'staticmethod'
newline|'\n'
DECL|member|_rebase_with_qemu_img
name|'def'
name|'_rebase_with_qemu_img'
op|'('
name|'guest'
op|','
name|'device'
...
name|'_'
op|'('
string|'"Something went wrong when deleting a volume snapshot: "'
nl|'\n'
string|'"rebasing a %(protocol)s network disk using qemu-img "'
nl|'\n'
string|'"has not been fully tested"'
op|')'
op|'%'
op|'{'
string|"'protocol'"
...
op|':'
newline|'\n'
comment|'# If backing_file is specified as "" (the empty string), then'
nl|'\n'
comment|'# the image is rebased onto no backing file (i.e. it will exist'
nl|'\n'
comment|'# independently of any backing file).'
nl|'\n'
indent|'            '
name|'backing_file'
op|'='
...
op|':'
newline|'\n'
comment|'# If the rebased image is going to have a backing file then'
nl|'\n'
comment|'# explicitly set the backing file format to avoid any security'
nl|'\n'
comment|'# concerns related to file format auto detection.'
nl|'\n'
indent|'            '
name|'backing_file'
op|'='
...
newline|'\n'
nl|'\n'
comment|"# If the guest is not running libvirt won't do a blockRebase."
nl|'\n'
comment|"# In that case, let's ask qemu-img to rebase the disk."
nl|'\n'
dedent|''
dedent|''
name|'else'
...
name|'debug'
op|'('
string|"'Guest is not running so doing a block rebase '"
nl|'\n'
string|'\'using "qemu-img rebase"\''
op|','
name|'instance'
op|'='
name|'instance'
...
nl|'\n'
indent|'        '
string|'"""Trigger crash dump by injecting an NMI to the specified instance."""'
newline|'\n'
name|'try'
op|':'
newline|'\n'
...
name|'_LE'
op|'('
string|"'Error from libvirt while injecting an NMI to '"
nl|'\n'
...
op|':'
newline|'\n'
comment|'# ensure unlink(filepath) will actually remove the file by deleting'
nl|'\n'
comment|'# the remaining link to it in close(fd)'
nl|'\n'
indent|'            '
name|'if'
name|'fd'
...
name|'staticmethod'
newline|'\n'
DECL|member|_get_disk_config_image_type
name|'def'
name|'_get_disk_config_image_type'
op|'('
op|')'
op|':'
newline|'\n'
comment|'# TODO(mikal): there is a bug here if images_type has'
nl|'\n'
comment|'# changed since creation of the instance, but I am pretty'
nl|'\n'
comment|'# sure that this bug already exists.'
nl|'\n'
indent|'        '
name|'return'
string|"'rbd'"
...
name|'staticmethod'
newline|'\n'
DECL|member|_has_local_disk
name|'def'
name|'_has_local_disk'
op|'('
name|'instance'
op|','
name|'disk_mapping'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(sileht): many callers of this method assume that this'
nl|'\n'
comment|"# method doesn't fail if an image already exists but instead"
nl|'\n'
comment|'# think that it will be reused (ie: (live)-migration/resize)'
nl|'\n'
...
op|':'
newline|'\n'
comment|'# Tell the storage backend about the config drive'
nl|'\n'
indent|'                '
name|'config_drive_image'
op|'='
name|'self'
op|'.'
name|'image_backend'
...
op|':'
newline|'\n'
comment|'# NOTE(mikal): if the config drive was imported into RBD, then'
nl|'\n'
comment|'# we no longer need the local copy'
nl|'\n'
indent|'                '
name|'if'
name|'CONF'
...
op|':'
newline|'\n'
comment|'# In case of SR-IOV vif types we create pci request per SR-IOV port'
nl|'\n'
comment|'# Therefore we can trust that pci_slot value in the vif is correct.'
nl|'\n'
indent|'            '
name|'sriov_pci_addresses'
op|'='
op|'['
nl|'\n'
name|'vif'
...
newline|'\n'
nl|'\n'
comment|'# use detach_pci_devices to avoid failure in case of'
nl|'\n'
comment|'# multiple guest SRIOV ports with the same MAC'
nl|'\n'
comment|'# (protection use-case, ports are on different physical'
nl|'\n'
comment|'# interfaces)'
nl|'\n'
name|'pci_devs'
...
name|'_'
op|'('
string|"'Volume sets discard option, qemu %(qemu)s'"
nl|'\n'
...
name|'_'
op|'('
string|'"Unable to get host UUID: /etc/machine-id does not exist"'
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
name|'_'
op|'('
string|'"Unable to get host UUID: /etc/machine-id is empty"'
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
newline|'\n'
nl|'\n'
comment|'# The vhost-user network backend requires file backed'
nl|'\n'
comment|'# guest memory (ie huge pages) to be marked as shared'
nl|'\n'
comment|'# access, not private, so an external process can read'
nl|'\n'
comment|'# and write the pages.'
nl|'\n'
comment|'#'
nl|'\n'
comment|"# You can't change the shared vs private flag for an"
nl|'\n'
comment|"# already running guest, and since we can't predict what"
nl|'\n'
comment|'# types of NIC may be hotplugged, we have no choice but'
nl|'\n'
comment|'# to unconditionally turn on the shared flag. This has'
nl|'\n'
comment|'# no real negative functional effect on the guest, so'
nl|'\n'
comment|'# is a reasonable approach to take'
nl|'\n'
name|'if'
name|'wants_hugepages'
op|':'
...
newline|'\n'
nl|'\n'
DECL|member|_wants_hugepages
dedent|''
name|'def'
name|'_wants_hugepages'
op|'('
name|'self'
op|','
name|'host_topology'
...
newline|'\n'
indent|'        '
string|'"""Determine if the guest / host topology implies the\n           use of huge pages for guest RAM backing\n        """'
newline|'\n'
nl|'\n'
name|'if'
name|'host_topology'
...
op|')'
newline|'\n'
comment|"# Remove smallest page size as that's not classed as a largepage"
nl|'\n'
name|'avail_pagesize'
op|'='
name|'avail_pagesize'
...
newline|'\n'
nl|'\n'
comment|'# See if we have page size set'
nl|'\n'
name|'for'
name|'cell'
name|'in'
...
name|'libvirt'
op|'.'
name|'realtime_scheduler_priority'
op|')'
newline|'\n'
name|'guest_cpu_tune'
op|'.'
...
newline|'\n'
nl|'\n'
DECL|member|_check_number_of_serial_console
dedent|''
dedent|''
name|'def'
name|'_check_number_of_serial_console'
op|'('
name|'self'
op|','
name|'num_ports'
...
name|'self'
op|'.'
name|'_get_memory_backing_hugepages_support'
op|'('
nl|'\n'
name|'inst_topology'
op|','
...
newline|'\n'
nl|'\n'
DECL|member|_get_memory_backing_hugepages_support
dedent|''
name|'def'
name|'_get_memory_backing_hugepages_support'
op|'('
...
newline|'\n'
nl|'\n'
DECL|member|_has_uefi_support
dedent|''
name|'def'
name|'_has_uefi_support'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# This means that the host can support uefi booting for guests'
nl|'\n'
indent|'        '
...
name|'_LW'
op|'('
string|'"uefi support is without some kind of "'
nl|'\n'
string|'"functional testing and therefore "'
nl|'\n'
string|'"considered experimental."'
op|')'
op|')'
newline|'\n'
name|'uefi_logged'
...
newline|'\n'
nl|'\n'
DECL|member|_get_guest_pointer_model
dedent|''
name|'def'
name|'_get_guest_pointer_model'
op|'('
...
op|':'
newline|'\n'
comment|'# TODO(sahid): We set pointer_model to keep compatibility'
nl|'\n'
comment|'# until the next release O*. It means operators can continue'
nl|'\n'
comment|'# to use the depecrated option "use_usb_tablet" or set a'
nl|'\n'
comment|'# specific device to use'
nl|'\n'
indent|'            '
name|'pointer_model'
op|'='
...
name|'_LW'
op|'('
string|'\'The option "use_usb_tablet" has been \''
nl|'\n'
string|"'deprecated for Newton in favor of the more '"
nl|'\n'
string|'\'generic "pointer_model". Please update \''
nl|'\n'
string|"'nova.conf to address this change.'"
op|')'
op|')'
newline|'\n'
nl|'\n'
...
op|':'
newline|'\n'
comment|"# For backward compatibility We don't want to break"
nl|'\n'
comment|'# process of booting an instance if host is configured'
nl|'\n'
comment|'# to use USB tablet without VNC or SPICE and SPICE'
nl|'\n'
comment|'# agent disable.'
nl|'\n'
indent|'                '
name|'LOG'
op|'.'
...
name|'_LW'
op|'('
string|"'USB tablet requested for guests by host '"
nl|'\n'
string|"'configuration. In order to accept this '"
nl|'\n'
string|"'request VNC should be enabled or SPICE '"
nl|'\n'
string|"'and SPICE agent disabled on host.'"
op|')'
op|')'
newline|'\n'
nl|'\n'
...
comment|"# For backward compatibility We don't want to break"
nl|'\n'
comment|'# process of booting an instance if virtual machine mode'
nl|'\n'
comment|'# is not configured as HVM.'
nl|'\n'
indent|'            '
name|'LOG'
op|'.'
...
string|"'USB tablet requested for guests by host '"
nl|'\n'
string|"'configuration. In order to accept this request '"
nl|'\n'
string|"'the machine mode should be configured as HVM.'"
op|')'
op|')'
newline|'\n'
dedent|''
...
op|')'
newline|'\n'
comment|'# Kind of ugly but we need to pass host to get_info as for a'
nl|'\n'
comment|'# workaround, see libvirt/compat.py'
nl|'\n'
name|'return'
name|'guest'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(apmelton) - Even though the instance is being booted from a'
nl|'\n'
comment|'# cinder volume, it is still presented as a local block device.'
nl|'\n'
comment|"# LocalBlockImage is used here to indicate that the instance's"
nl|'\n'
comment|'# disk is backed by a local block device.'
nl|'\n'
name|'image_model'
op|'='
name|'imgmodel'
...
name|'debug'
op|'('
string|"'Attempting to unmount container filesystem: %s'"
op|','
nl|'\n'
name|'container_dir'
op|','
...
name|'_LW'
op|'('
string|'"couldn\'t obtain the vcpu count from domain id:"'
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# Note(moshele): libvirt < 1.3 reported virt_functions capability'
nl|'\n'
comment|'# only when VFs are enabled. The check below is a workaround'
nl|'\n'
comment|'# to get the correct report regardless of whether or not any'
nl|'\n'
comment|'# VFs are enabled for the device.'
nl|'\n'
dedent|''
dedent|''
name|'if'
...
name|'self'
op|','
string|"'_bad_libvirt_numa_version_warn'"
op|','
name|'False'
op|')'
op|':'
...
name|'_LW'
op|'('
string|"'You are running with libvirt version %s '"
nl|'\n'
string|"'which is known to have broken NUMA support. '"
nl|'\n'
string|"'Consider patching or updating libvirt on '"
nl|'\n'
string|"'this host if you need NUMA support.'"
op|')'
op|','
nl|'\n'
name|'self'
...
name|'self'
op|'.'
name|'_bad_libvirt_numa_version_warn'
op|'='
name|'True'
newline|'\n'
dedent|''
...
name|'images_type'
newline|'\n'
comment|'# Notes(eliqiao): block_migration and disk_over_commit are not'
nl|'\n'
comment|"# nullable, so just don't set them if they are None"
nl|'\n'
name|'if'
name|'block_migration'
name|'is'
...
name|'None'
newline|'\n'
comment|'# We are building listen_addrs of vnc/spice from'
nl|'\n'
comment|'# LibvirtLiveMigrateData; in some certains (e.g. an old-code'
nl|'\n'
comment|'# destination host) those fields may have not been set and we'
nl|'\n'
comment|'# want to avoid any unfortunates exceptions raised.'
nl|'\n'
comment|'# TODO(sahid): The method'
nl|'\n'
comment|'# _check_graphics_addresses_can_live_migrate_should to take an'
nl|'\n'
comment|'# object LibvirtLiveMigrate itself.'
nl|'\n'
name|'if'
op|'('
name|'dest_check_data'
...
op|':'
newline|'\n'
comment|'# In this context want to ensure we do not have to migrate'
nl|'\n'
comment|"# graphic or serial consoles since we can't update guest's"
nl|'\n'
comment|'# domain XML to make it handle destination host.'
nl|'\n'
indent|'            '
name|'self'
op|'.'
...
name|'self'
op|'.'
name|'_verify_serial_console_is_disabled'
op|'('
op|')'
newline|'\n'
nl|'\n'
...
op|':'
newline|'\n'
comment|'# TODO(eliqiao): Once block_migration flag is removed from the API'
nl|'\n'
comment|'# we can safely remove the if condition'
nl|'\n'
indent|'            '
name|'if'
name|'dest_check_data'
...
op|')'
newline|'\n'
comment|'# NOTE(pkoniszewski): libvirt from version 1.2.17 upwards'
nl|'\n'
comment|'# supports selective block device migration. It means that it'
nl|'\n'
comment|'# is possible to define subset of block devices to be copied'
nl|'\n'
comment|'# during migration. If they are not specified - block devices'
nl|'\n'
comment|"# won't be migrated. However, it does not work when live"
nl|'\n'
comment|'# migration is tunnelled through libvirt.'
nl|'\n'
name|'if'
name|'bdm'
name|'and'
...
nl|'\n'
comment|"# we can't do a block migration, since that will result"
nl|'\n'
comment|'# in volumes being copied from themselves to themselves,'
nl|'\n'
comment|'# which is a recipe for disaster.'
nl|'\n'
indent|'                    '
name|'ver'
op|'='
...
name|'_'
op|'('
string|"'Cannot block migrate instance %(uuid)s with'"
nl|'\n'
string|"' mapped volumes. Selective block device'"
nl|'\n'
string|"' migration feature requires libvirt version'"
nl|'\n'
string|"' %(libvirt_ver)s'"
op|')'
op|'%'
nl|'\n'
op|'{'
...
nl|'\n'
string|'"without shared storage except "'
nl|'\n'
string|'"a booted from volume VM which "'
nl|'\n'
string|'"does not have a local disk."'
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
op|'='
op|'('
string|'"http://libvirt.org/html/libvirt-libvirt-host.html#"'
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|live_migration_abort
dedent|''
name|'def'
...
name|'_LE'
op|'('
string|'"Failed to cancel migration %s"'
op|')'
op|','
nl|'\n'
name|'e'
...
newline|'\n'
nl|'\n'
DECL|member|_update_serial_xml
dedent|''
name|'def'
name|'_update_serial_xml'
op|'('
...
newline|'\n'
nl|'\n'
DECL|member|_verify_serial_console_is_disabled
dedent|''
dedent|''
dedent|''
name|'def'
...
string|"' for live migration to work properly you'"
nl|'\n'
string|"' must either disable serial console or'"
nl|'\n'
string|"' upgrade your libvirt version.'"
op|')'
newline|'\n'
name|'raise'
name|'exception'
...
op|','
nl|'\n'
string|"'destination_xml'"
op|':'
name|'new_xml_str'
op|','
nl|'\n'
string|"'migrate_disks'"
op|':'
name|'device_names'
op|','
nl|'\n'
...
name|'staticmethod'
newline|'\n'
DECL|member|_migration_downtime_steps
name|'def'
name|'_migration_downtime_steps'
op|'('
name|'data_gb'
op|')'
op|':'
...
name|'libvirt'
op|'.'
name|'live_migration_downtime_steps'
newline|'\n'
name|'delay'
op|'='
name|'CONF'
...
name|'libvirt'
op|'.'
name|'live_migration_downtime_delay'
newline|'\n'
nl|'\n'
comment|'# TODO(hieulq): Need to move min/max value into the config option,'
nl|'\n'
comment|'# currently oslo_config will raise ValueError instead of setting'
nl|'\n'
comment|'# option value to its min/max.'
nl|'\n'
name|'if'
name|'downtime'
op|'<'
...
newline|'\n'
nl|'\n'
DECL|member|_live_migration_copy_disk_paths
dedent|''
dedent|''
name|'def'
name|'_live_migration_copy_disk_paths'
op|'('
name|'self'
op|','
name|'context'
...
newline|'\n'
nl|'\n'
comment|'# TODO(pkoniszewski): Remove this if-statement when we bump min libvirt'
nl|'\n'
comment|'# version to >= 1.2.17'
nl|'\n'
name|'if'
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
DECL|member|_live_migration_data_gb
dedent|''
name|'def'
name|'_live_migration_data_gb'
op|'('
name|'self'
op|','
name|'instance'
...
name|'_LW'
op|'('
string|'"Unable to stat %(disk)s: %(ex)s"'
op|')'
op|','
nl|'\n'
op|'{'
...
op|')'
newline|'\n'
comment|"# Ignore error since we don't want to break"
nl|'\n'
comment|'# the migration monitoring thread operation'
nl|'\n'
nl|'\n'
dedent|''
dedent|''
...
name|'libvirt'
op|'.'
name|'live_migration_completion_timeout'
op|'*'
name|'data_gb'
op|')'
newline|'\n'
...
name|'libvirt'
op|'.'
name|'live_migration_progress_timeout'
newline|'\n'
name|'migration'
op|'='
name|'migrate_data'
...
newline|'\n'
nl|'\n'
DECL|function|_check_scheduled_migration_task
name|'def'
name|'_check_scheduled_migration_task'
op|'('
op|')'
op|':'
newline|'\n'
...
name|'_LW'
op|'('
string|'"Failed to pause instance during "'
nl|'\n'
string|'"live-migration %s"'
op|')'
op|','
nl|'\n'
name|'e'
...
newline|'\n'
nl|'\n'
DECL|function|_recover_scheduled_migration_task
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'_recover_scheduled_migration_task'
op|'('
op|')'
op|':'
newline|'\n'
...
op|')'
newline|'\n'
comment|'# NOTE(tdurakov): there is still possibility to leave'
nl|'\n'
comment|'# instance paused in case of live-migration failure.'
nl|'\n'
comment|'# This check guarantee that instance will be resumed'
nl|'\n'
...
name|'_LW'
op|'('
string|'"Failed to resume paused instance "'
nl|'\n'
string|'"before live-migration rollback %s"'
op|')'
op|','
nl|'\n'
name|'e'
...
comment|'#'
nl|'\n'
comment|'# This is where we wire up calls to change live'
nl|'\n'
...
name|'_LW'
op|'('
string|'"Live migration stuck for %d sec"'
op|')'
op|','
nl|'\n'
op|'('
...
name|'_LW'
op|'('
string|'"Live migration not completed after %d sec"'
op|')'
op|','
nl|'\n'
name|'completion_timeout'
...
name|'_LW'
op|'('
string|'"Failed to abort migration %s"'
op|')'
op|','
nl|'\n'
name|'e'
...
newline|'\n'
nl|'\n'
comment|'# See if we need to increase the max downtime. We'
nl|'\n'
comment|"# ignore failures, since we'd rather continue trying"
nl|'\n'
comment|'# to migrate'
nl|'\n'
dedent|''
...
name|'_LI'
op|'('
string|'"Increasing downtime to %(downtime)d ms "'
nl|'\n'
string|'"after %(waittime)d sec elapsed time"'
op|')'
op|','
nl|'\n'
op|'{'
...
name|'_LW'
op|'('
string|'"Unable to increase max downtime to %(time)d"'
nl|'\n'
string|'"ms: %(e)s"'
op|')'
op|','
...
op|':'
newline|'\n'
comment|'# Note(Shaohe Feng) every 5 secs to update the migration'
nl|'\n'
comment|'# db, that keeps updates to the instance and migration'
nl|'\n'
comment|'# objects in sync.'
nl|'\n'
indent|'                    '
name|'migration'
op|'.'
...
name|'_LI'
op|'('
string|'"Data remaining %(remaining)d bytes, "'
nl|'\n'
string|'"low watermark %(watermark)d bytes "'
nl|'\n'
string|'"%(last)d seconds ago"'
op|')'
op|','
nl|'\n'
op|'{'
...
newline|'\n'
nl|'\n'
DECL|member|_clear_empty_migration
dedent|''
name|'def'
name|'_clear_empty_migration'
op|'('
...
name|'_LW'
op|'('
string|'"There are no records in active migrations "'
nl|'\n'
string|'"for instance"'
op|')'
op|','
...
op|':'
newline|'\n'
comment|'# NOTE(pkoniszewski): currently only pause during live migration is'
nl|'\n'
comment|'# supported to force live migration to complete, so just try to pause'
nl|'\n'
...
op|':'
newline|'\n'
comment|'# NOTE(sileht): configdrive is stored into the block storage'
nl|'\n'
comment|'# kvm is a block device, live migration will work'
nl|'\n'
comment|'# NOTE(sileht): the configdrive is stored into a shared path'
nl|'\n'
comment|"# kvm don't need to migrate it, live migration will work"
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# Recreate the disk.info file and in doing so stop the'
nl|'\n'
comment|'# imagebackend from recreating it incorrectly by inspecting the'
nl|'\n'
comment|'# contents of each file when using the Raw backend.'
nl|'\n'
name|'if'
name|'disk_info'
op|':'
...
name|'debug'
op|'('
string|"'Creating disk.info with the contents: %s'"
op|','
nl|'\n'
name|'image_disk_info'
op|','
...
op|':'
newline|'\n'
comment|'# NOTE(angdraug): when block storage is shared between source'
nl|'\n'
comment|"# and destination and instance path isn't (e.g. volume backed"
nl|'\n'
comment|'# or rbd backed instance), instance path on destination has to'
nl|'\n'
comment|'# be prepared'
nl|'\n'
nl|'\n'
...
name|'proxyclient_address'
newline|'\n'
comment|'# Store live_migration_inbound_addr'
nl|'\n'
name|'migrate_data'
op|'.'
name|'target_connect_addr'
...
name|'libvirt'
op|'.'
name|'live_migration_inbound_addr'
newline|'\n'
nl|'\n'
name|'for'
name|'vol'
...
op|':'
newline|'\n'
comment|"# Retrieve connection info from Cinder's initialize_connection API."
nl|'\n'
comment|'# The info returned will be accurate for the source server.'
nl|'\n'
indent|'            '
name|'volume_id'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# TODO(leeantho) The following multipath_id logic is temporary'
nl|'\n'
comment|'# and will be removed in the future once os-brick is updated'
nl|'\n'
comment|'# to handle multipath for drivers in a more efficient way.'
nl|'\n'
comment|'# For now this logic is needed to ensure the connection info'
nl|'\n'
comment|'# data is correct.'
nl|'\n'
nl|'\n'
comment|'# Pull out multipath_id from the bdm information. The'
nl|'\n'
comment|'# multipath_id can be placed into the connection info'
nl|'\n'
comment|'# because it is based off of the volume and will be the'
nl|'\n'
comment|'# same on the source and destination hosts.'
nl|'\n'
name|'if'
string|"'multipath_id'"
name|'in'
...
newline|'\n'
nl|'\n'
comment|'# Get all instance uuids'
nl|'\n'
dedent|''
name|'instance_uuids'
op|'='
...
op|')'
newline|'\n'
comment|'# Get instance object list by uuid filter'
nl|'\n'
name|'filters'
op|'='
op|'{'
...
op|'}'
newline|'\n'
comment|'# NOTE(ankit): objects.InstanceList.get_by_filters method is'
nl|'\n'
comment|'# getting called twice one is here and another in the'
nl|'\n'
comment|'# _update_available_resource method of resource_tracker. Since'
nl|'\n'
comment|'# _update_available_resource method is synchronized, there is a'
nl|'\n'
comment|'# possibility the instances list retrieved here to calculate'
nl|'\n'
comment|'# disk_over_committed_size would differ to the list you would get'
nl|'\n'
comment|'# in _update_available_resource method for calculating usages based'
nl|'\n'
comment|'# on instance utilization.'
nl|'\n'
name|'local_instance_list'
op|'='
name|'objects'
op|'.'
name|'InstanceList'
...
op|')'
newline|'\n'
comment|'# Convert instance list to dictionary with instace uuid as key.'
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# Get bdms by instance uuids'
nl|'\n'
name|'bdms'
op|'='
name|'objects'
...
op|':'
newline|'\n'
comment|'# Get block device info for instance'
nl|'\n'
indent|'                    '
name|'block_device_info'
op|'='
...
newline|'\n'
nl|'\n'
comment|'# Ensure disk.info is written to the new path to avoid disks being'
nl|'\n'
comment|'# reinspected and potentially changing format.'
nl|'\n'
dedent|''
dedent|''
name|'src_disk_info_path'
op|'='
name|'os'
op|'.'
name|'path'
...
name|'ephemeral_gb'
newline|'\n'
comment|"# N.B. We don't handle ephemeral disks named disk.ephN here,"
nl|'\n'
comment|"# which is almost certainly a bug. It's not clear what this function"
nl|'\n'
comment|'# should return if an instance has multiple ephemeral disks.'
nl|'\n'
dedent|''
name|'else'
op|':'
...
op|')'
newline|'\n'
comment|'# assume _create_image does nothing if a target file exists.'
nl|'\n'
comment|'# NOTE: This has the intended side-effect of fetching a missing'
nl|'\n'
comment|'# backing file.'
nl|'\n'
name|'self'
op|'.'
name|'_create_image'
...
newline|'\n'
nl|'\n'
comment|'# Resize root disk and a single ephemeral disk called disk.local'
nl|'\n'
comment|'# Also convert raw disks to qcow2 if migrating to host which uses'
nl|'\n'
comment|'# qcow2 from host which uses raw.'
nl|'\n'
comment|'# TODO(mbooth): Handle resize of multiple ephemeral disks, and'
nl|'\n'
comment|'#               ephemeral disks not called disk.local.'
nl|'\n'
name|'disk_info'
op|'='
name|'jsonutils'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(mdbooth): The code below looks wrong, but is actually'
nl|'\n'
comment|'# required to prevent a security hole when migrating from a host'
nl|'\n'
comment|'# with use_cow_images=False to one with use_cow_images=True.'
nl|'\n'
comment|'# Imagebackend uses use_cow_images to select between the'
nl|'\n'
comment|'# atrociously-named-Raw and Qcow2 backends. The Qcow2 backend'
nl|'\n'
comment|'# writes to disk.info, but does not read it as it assumes qcow2.'
nl|'\n'
comment|"# Therefore if we don't convert raw to qcow2 here, a raw disk will"
nl|'\n'
comment|'# be incorrectly assumed to be qcow2, which is a severe security'
nl|'\n'
comment|'# flaw. The reverse is not true, because the atrociously-named-Raw'
nl|'\n'
comment|'# backend supports both qcow2 and raw disks, and will choose'
nl|'\n'
comment|'# appropriately between them as long as disk.info exists and is'
nl|'\n'
comment|'# correctly populated, which it is because Qcow2 writes to'
nl|'\n'
comment|'# disk.info.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# In general, we do not yet support format conversion during'
nl|'\n'
comment|'# migration. For example:'
nl|'\n'
comment|'#   * Converting from use_cow_images=True to use_cow_images=False'
nl|'\n'
comment|"#     isn't handled. This isn't a security bug, but is almost"
nl|'\n'
comment|"#     certainly buggy in other cases, as the 'Raw' backend doesn't"
nl|'\n'
comment|'#     expect a backing file.'
nl|'\n'
comment|'#   * Converting to/from lvm and rbd backends is not supported.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# This behaviour is inconsistent, and therefore undesirable for'
nl|'\n'
comment|'# users. It is tightly-coupled to implementation quirks of 2'
nl|'\n'
comment|'# out of 5 backends in imagebackend and defends against a severe'
nl|'\n'
comment|'# security flaw which is not at all obvious without deep analysis,'
nl|'\n'
comment|'# and is therefore undesirable to developers. We should aim to'
nl|'\n'
comment|'# remove it. This will not be possible, though, until we can'
nl|'\n'
comment|'# represent the storage layout of a specific instance'
nl|'\n'
comment|'# independent of the default configuration of the local compute'
nl|'\n'
comment|'# host.'
nl|'\n'
nl|'\n'
comment|'# Config disks are hard-coded to be raw even when'
nl|'\n'
comment|"# use_cow_images=True (see _get_disk_config_image_type),so don't"
nl|'\n'
comment|'# need to be converted.'
nl|'\n'
dedent|''
name|'if'
op|'('
...
name|'debug'
op|'('
string|'"finish_migration finished successfully."'
op|','
name|'instance'
op|'='
name|'instance'
...
op|')'
newline|'\n'
comment|'# Once we rollback, the snapshot is no longer needed, so remove it'
nl|'\n'
comment|'# TODO(nic): Remove the try/except/finally in a future release'
nl|'\n'
comment|'# To avoid any upgrade issues surrounding instances being in pending'
nl|'\n'
comment|'# resize state when the software is updated, this portion of the'
nl|'\n'
comment|'# method logs exceptions rather than failing on them.  Once it can be'
nl|'\n'
comment|'# reasonably assumed that no such instances exist in the wild'
nl|'\n'
comment|'# anymore, the try/except/finally should be removed,'
nl|'\n'
comment|'# and ignore_errors should be set back to False (the default) so'
nl|'\n'
comment|'# that problems throw errors, like they should.'
nl|'\n'
name|'try'
op|':'
newline|'\n'
...
name|'_LW'
op|'('
string|'"Failed to rollback snapshot (%s)"'
op|')'
op|','
nl|'\n'
name|'libvirt_utils'
...
newline|'\n'
nl|'\n'
comment|'# TODO(sahid): Needs to use get_info but more changes have to'
nl|'\n'
comment|'# be done since a mapping STATE_MAP LIBVIRT_POWER_STATE is'
nl|'\n'
comment|'# needed.'
nl|'\n'
op|'('
