begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright 2010 United States Government as represented by the'
nl|'\n'
comment|'# Administrator of the National Aeronautics and Space Administration.'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
string|'"""Metadata request handler."""'
newline|'\n'
nl|'\n'
name|'import'
name|'base64'
newline|'\n'
name|'import'
name|'collections'
newline|'\n'
nl|'\n'
name|'import'
name|'webob'
op|'.'
name|'dec'
newline|'\n'
name|'import'
name|'webob'
op|'.'
name|'exc'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'api'
op|'.'
name|'ec2'
name|'import'
name|'ec2utils'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'block_device'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'compute'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'context'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'db'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'flags'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'log'
name|'as'
name|'logging'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'network'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'rpc'
name|'import'
name|'common'
name|'as'
name|'rpc_common'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'utils'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'volume'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'wsgi'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|LOG
name|'LOG'
op|'='
name|'logging'
op|'.'
name|'getLogger'
op|'('
name|'__name__'
op|')'
newline|'\n'
DECL|variable|FLAGS
name|'FLAGS'
op|'='
name|'flags'
op|'.'
name|'FLAGS'
newline|'\n'
name|'flags'
op|'.'
name|'DECLARE'
op|'('
string|"'use_forwarded_for'"
op|','
string|"'nova.api.auth'"
op|')'
newline|'\n'
name|'flags'
op|'.'
name|'DECLARE'
op|'('
string|"'dhcp_domain'"
op|','
string|"'nova.network.manager'"
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'FLAGS'
op|'.'
name|'memcached_servers'
op|':'
newline|'\n'
indent|'    '
name|'import'
name|'memcache'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'from'
name|'nova'
op|'.'
name|'common'
name|'import'
name|'memorycache'
name|'as'
name|'memcache'
newline|'\n'
nl|'\n'
DECL|variable|_DEFAULT_MAPPINGS
dedent|''
name|'_DEFAULT_MAPPINGS'
op|'='
op|'{'
string|"'ami'"
op|':'
string|"'sda1'"
op|','
nl|'\n'
string|"'ephemeral0'"
op|':'
string|"'sda2'"
op|','
nl|'\n'
string|"'root'"
op|':'
name|'block_device'
op|'.'
name|'DEFAULT_ROOT_DEV_NAME'
op|','
nl|'\n'
string|"'swap'"
op|':'
string|"'sda3'"
op|'}'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|Versions
name|'class'
name|'Versions'
op|'('
name|'wsgi'
op|'.'
name|'Application'
op|')'
op|':'
newline|'\n'
nl|'\n'
indent|'    '
op|'@'
name|'webob'
op|'.'
name|'dec'
op|'.'
name|'wsgify'
op|'('
name|'RequestClass'
op|'='
name|'wsgi'
op|'.'
name|'Request'
op|')'
newline|'\n'
DECL|member|__call__
name|'def'
name|'__call__'
op|'('
name|'self'
op|','
name|'req'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Respond to a request for all versions."""'
newline|'\n'
comment|'# available api versions'
nl|'\n'
name|'versions'
op|'='
op|'['
nl|'\n'
string|"'1.0'"
op|','
nl|'\n'
string|"'2007-01-19'"
op|','
nl|'\n'
string|"'2007-03-01'"
op|','
nl|'\n'
string|"'2007-08-29'"
op|','
nl|'\n'
string|"'2007-10-10'"
op|','
nl|'\n'
string|"'2007-12-15'"
op|','
nl|'\n'
string|"'2008-02-01'"
op|','
nl|'\n'
string|"'2008-09-01'"
op|','
nl|'\n'
string|"'2009-04-04'"
op|','
nl|'\n'
op|']'
newline|'\n'
name|'return'
string|"''"
op|'.'
name|'join'
op|'('
string|"'%s\\n'"
op|'%'
name|'v'
name|'for'
name|'v'
name|'in'
name|'versions'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|MetadataRequestHandler
dedent|''
dedent|''
name|'class'
name|'MetadataRequestHandler'
op|'('
name|'wsgi'
op|'.'
name|'Application'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Serve metadata."""'
newline|'\n'
nl|'\n'
DECL|member|__init__
name|'def'
name|'__init__'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'network_api'
op|'='
name|'network'
op|'.'
name|'API'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'compute_api'
op|'='
name|'compute'
op|'.'
name|'API'
op|'('
nl|'\n'
name|'network_api'
op|'='
name|'self'
op|'.'
name|'network_api'
op|','
nl|'\n'
name|'volume_api'
op|'='
name|'volume'
op|'.'
name|'API'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'metadata_mapper'
op|'='
op|'{'
nl|'\n'
string|"'user-data'"
op|':'
name|'self'
op|'.'
name|'user_data'
op|','
nl|'\n'
string|"'meta-data'"
op|':'
op|'{'
nl|'\n'
string|"'instance-id'"
op|':'
name|'self'
op|'.'
name|'instance_id'
op|','
nl|'\n'
string|"'instance-type'"
op|':'
name|'self'
op|'.'
name|'instance_type'
op|','
nl|'\n'
string|"'ami-id'"
op|':'
name|'self'
op|'.'
name|'ami_id'
op|','
nl|'\n'
string|"'kernel-id'"
op|':'
name|'self'
op|'.'
name|'kernel_id'
op|','
nl|'\n'
string|"'ramdisk-id'"
op|':'
name|'self'
op|'.'
name|'ramdisk_id'
op|','
nl|'\n'
string|"'block-device-mapping'"
op|':'
name|'self'
op|'.'
name|'block_device_mapping'
op|','
nl|'\n'
string|"'hostname'"
op|':'
name|'self'
op|'.'
name|'hostname'
op|','
nl|'\n'
string|"'public-hostname'"
op|':'
name|'self'
op|'.'
name|'hostname'
op|','
nl|'\n'
string|"'local-hostname'"
op|':'
name|'self'
op|'.'
name|'hostname'
op|','
nl|'\n'
string|"'local-ipv4'"
op|':'
name|'self'
op|'.'
name|'local_ipv4'
op|','
nl|'\n'
string|"'public-ipv4'"
op|':'
name|'self'
op|'.'
name|'public_ipv4'
op|','
nl|'\n'
string|"'security-groups'"
op|':'
name|'self'
op|'.'
name|'security_groups'
op|','
nl|'\n'
string|"'public-keys'"
op|':'
name|'self'
op|'.'
name|'public_keys'
op|','
nl|'\n'
string|"'ami-launch-index'"
op|':'
name|'self'
op|'.'
name|'ami_launch_index'
op|','
nl|'\n'
string|"'reservation-id'"
op|':'
name|'self'
op|'.'
name|'reservation_id'
op|','
nl|'\n'
string|"'placement'"
op|':'
name|'self'
op|'.'
name|'placement'
op|','
nl|'\n'
string|"'instance-action'"
op|':'
name|'self'
op|'.'
name|'instance_action'
op|','
nl|'\n'
string|"'ami-manifest-path'"
op|':'
name|'self'
op|'.'
name|'ami_manifest_path'
op|','
nl|'\n'
op|'}'
nl|'\n'
op|'}'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_cache'
op|'='
name|'memcache'
op|'.'
name|'Client'
op|'('
name|'FLAGS'
op|'.'
name|'memcached_servers'
op|','
name|'debug'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_format_instance_mapping
dedent|''
name|'def'
name|'_format_instance_mapping'
op|'('
name|'self'
op|','
name|'instance_ref'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'root_device_name'
op|'='
name|'instance_ref'
op|'['
string|"'root_device_name'"
op|']'
newline|'\n'
name|'if'
name|'root_device_name'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'_DEFAULT_MAPPINGS'
newline|'\n'
nl|'\n'
dedent|''
name|'mappings'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'mappings'
op|'['
string|"'ami'"
op|']'
op|'='
name|'block_device'
op|'.'
name|'strip_dev'
op|'('
name|'root_device_name'
op|')'
newline|'\n'
name|'mappings'
op|'['
string|"'root'"
op|']'
op|'='
name|'root_device_name'
newline|'\n'
name|'default_ephemeral_device'
op|'='
name|'instance_ref'
op|'.'
name|'get'
op|'('
string|"'default_ephemeral_device'"
op|')'
newline|'\n'
name|'if'
name|'default_ephemeral_device'
op|':'
newline|'\n'
indent|'            '
name|'mappings'
op|'['
string|"'ephemeral0'"
op|']'
op|'='
name|'default_ephemeral_device'
newline|'\n'
dedent|''
name|'default_swap_device'
op|'='
name|'instance_ref'
op|'.'
name|'get'
op|'('
string|"'default_swap_device'"
op|')'
newline|'\n'
name|'if'
name|'default_swap_device'
op|':'
newline|'\n'
indent|'            '
name|'mappings'
op|'['
string|"'swap'"
op|']'
op|'='
name|'default_swap_device'
newline|'\n'
dedent|''
name|'ebs_devices'
op|'='
op|'['
op|']'
newline|'\n'
nl|'\n'
comment|"# 'ephemeralN', 'swap' and ebs"
nl|'\n'
name|'for'
name|'bdm'
name|'in'
name|'db'
op|'.'
name|'block_device_mapping_get_all_by_instance'
op|'('
nl|'\n'
name|'ctxt'
op|','
name|'instance_ref'
op|'['
string|"'id'"
op|']'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'bdm'
op|'['
string|"'no_device'"
op|']'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
nl|'\n'
comment|'# ebs volume case'
nl|'\n'
dedent|''
name|'if'
op|'('
name|'bdm'
op|'['
string|"'volume_id'"
op|']'
name|'or'
name|'bdm'
op|'['
string|"'snapshot_id'"
op|']'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'ebs_devices'
op|'.'
name|'append'
op|'('
name|'bdm'
op|'['
string|"'device_name'"
op|']'
op|')'
newline|'\n'
name|'continue'
newline|'\n'
nl|'\n'
dedent|''
name|'virtual_name'
op|'='
name|'bdm'
op|'['
string|"'virtual_name'"
op|']'
newline|'\n'
name|'if'
name|'not'
name|'virtual_name'
op|':'
newline|'\n'
indent|'                '
name|'continue'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'block_device'
op|'.'
name|'is_swap_or_ephemeral'
op|'('
name|'virtual_name'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'mappings'
op|'['
name|'virtual_name'
op|']'
op|'='
name|'bdm'
op|'['
string|"'device_name'"
op|']'
newline|'\n'
nl|'\n'
comment|"# NOTE(yamahata): I'm not sure how ebs device should be numbered."
nl|'\n'
comment|'#                 Right now sort by device name for deterministic'
nl|'\n'
comment|'#                 result.'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'ebs_devices'
op|':'
newline|'\n'
indent|'            '
name|'nebs'
op|'='
number|'0'
newline|'\n'
name|'ebs_devices'
op|'.'
name|'sort'
op|'('
op|')'
newline|'\n'
name|'for'
name|'ebs'
name|'in'
name|'ebs_devices'
op|':'
newline|'\n'
indent|'                '
name|'mappings'
op|'['
string|"'ebs%d'"
op|'%'
name|'nebs'
op|']'
op|'='
name|'ebs'
newline|'\n'
name|'nebs'
op|'+='
number|'1'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
name|'return'
name|'mappings'
newline|'\n'
nl|'\n'
DECL|member|get_instance
dedent|''
name|'def'
name|'get_instance'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""get instance_ref for a given fixed_ip, raising\n        exception.NotFound if unable to find instance\n\n        this will attempt to use memcache or fake memcache (an in-memory\n        cache) to remove the DB query + RPC query for batched calls (eg\n        cloud-init making dozens of queries on boot)\n        """'
newline|'\n'
nl|'\n'
name|'cache_key'
op|'='
string|"'metadata-%s'"
op|'%'
name|'address'
newline|'\n'
name|'instance_dict'
op|'='
name|'self'
op|'.'
name|'_cache'
op|'.'
name|'get'
op|'('
name|'cache_key'
op|')'
newline|'\n'
name|'if'
name|'instance_dict'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'instance_dict'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'not'
name|'address'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'FixedIpNotFoundForAddress'
op|'('
name|'address'
op|'='
name|'address'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'fixed_ip'
op|'='
name|'self'
op|'.'
name|'network_api'
op|'.'
name|'get_fixed_ip_by_address'
op|'('
name|'ctxt'
op|','
name|'address'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'rpc_common'
op|'.'
name|'RemoteError'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'FixedIpNotFoundForAddress'
op|'('
name|'address'
op|'='
name|'address'
op|')'
newline|'\n'
dedent|''
name|'instance_ref'
op|'='
name|'db'
op|'.'
name|'instance_get'
op|'('
name|'ctxt'
op|','
name|'fixed_ip'
op|'['
string|"'instance_id'"
op|']'
op|')'
newline|'\n'
name|'instance_dict'
op|'='
name|'utils'
op|'.'
name|'to_primitive'
op|'('
name|'instance_ref'
op|')'
newline|'\n'
nl|'\n'
name|'self'
op|'.'
name|'_cache'
op|'.'
name|'set'
op|'('
name|'cache_key'
op|','
name|'instance_dict'
op|','
number|'15'
op|')'
newline|'\n'
nl|'\n'
name|'return'
name|'instance_dict'
newline|'\n'
nl|'\n'
DECL|member|user_data
dedent|''
name|'def'
name|'user_data'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'return'
name|'base64'
op|'.'
name|'b64decode'
op|'('
name|'instance_ref'
op|'['
string|"'user_data'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|instance_id
dedent|''
name|'def'
name|'instance_id'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'return'
name|'ec2utils'
op|'.'
name|'id_to_ec2_id'
op|'('
name|'instance_ref'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
nl|'\n'
DECL|member|instance_type
dedent|''
name|'def'
name|'instance_type'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'return'
name|'instance_ref'
op|'['
string|"'instance_type'"
op|']'
op|'['
string|"'name'"
op|']'
newline|'\n'
nl|'\n'
DECL|member|ami_id
dedent|''
name|'def'
name|'ami_id'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'image_id'
op|'='
name|'instance_ref'
op|'['
string|"'image_ref'"
op|']'
newline|'\n'
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'return'
name|'ec2utils'
op|'.'
name|'glance_id_to_ec2_id'
op|'('
name|'ctxt'
op|','
name|'image_id'
op|')'
newline|'\n'
nl|'\n'
DECL|member|kernel_id
dedent|''
name|'def'
name|'kernel_id'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'kernel_id'
op|'='
name|'instance_ref'
op|'.'
name|'get'
op|'('
string|"'kernel_id'"
op|')'
newline|'\n'
name|'if'
name|'kernel_id'
op|':'
newline|'\n'
indent|'            '
name|'image_type'
op|'='
name|'ec2utils'
op|'.'
name|'image_type'
op|'('
string|"'kernel'"
op|')'
newline|'\n'
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'return'
name|'ec2utils'
op|'.'
name|'glance_id_to_ec2_id'
op|'('
name|'ctxt'
op|','
name|'kernel_id'
op|','
name|'image_type'
op|')'
newline|'\n'
nl|'\n'
DECL|member|ramdisk_id
dedent|''
dedent|''
name|'def'
name|'ramdisk_id'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'ramdisk_id'
op|'='
name|'instance_ref'
op|'.'
name|'get'
op|'('
string|"'ramdisk_id'"
op|')'
newline|'\n'
name|'if'
name|'ramdisk_id'
op|':'
newline|'\n'
indent|'            '
name|'image_type'
op|'='
name|'ec2utils'
op|'.'
name|'image_type'
op|'('
string|"'ramdisk'"
op|')'
newline|'\n'
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'return'
name|'ec2utils'
op|'.'
name|'image_ec2_id'
op|'('
name|'ctxt'
op|','
name|'ramdisk_id'
op|','
name|'image_type'
op|')'
newline|'\n'
nl|'\n'
DECL|member|ami_launch_index
dedent|''
dedent|''
name|'def'
name|'ami_launch_index'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'return'
name|'instance_ref'
op|'['
string|"'launch_index'"
op|']'
newline|'\n'
nl|'\n'
DECL|member|block_device_mapping
dedent|''
name|'def'
name|'block_device_mapping'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'_format_instance_mapping'
op|'('
name|'instance_ref'
op|')'
newline|'\n'
nl|'\n'
DECL|member|hostname
dedent|''
name|'def'
name|'hostname'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'return'
string|'"%s.%s"'
op|'%'
op|'('
name|'instance_ref'
op|'['
string|"'hostname'"
op|']'
op|','
name|'FLAGS'
op|'.'
name|'dhcp_domain'
op|')'
newline|'\n'
nl|'\n'
DECL|member|local_ipv4
dedent|''
name|'def'
name|'local_ipv4'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'address'
newline|'\n'
nl|'\n'
DECL|member|public_ipv4
dedent|''
name|'def'
name|'public_ipv4'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'ip_info'
op|'='
name|'ec2utils'
op|'.'
name|'get_ip_info_for_instance'
op|'('
name|'ctxt'
op|','
name|'instance_ref'
op|')'
newline|'\n'
name|'floating_ips'
op|'='
name|'ip_info'
op|'['
string|"'floating_ips'"
op|']'
newline|'\n'
name|'floating_ip'
op|'='
name|'floating_ips'
name|'and'
name|'floating_ips'
op|'['
number|'0'
op|']'
name|'or'
string|"''"
newline|'\n'
name|'return'
name|'floating_ip'
newline|'\n'
nl|'\n'
DECL|member|reservation_id
dedent|''
name|'def'
name|'reservation_id'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'return'
name|'instance_ref'
op|'['
string|"'reservation_id'"
op|']'
newline|'\n'
nl|'\n'
DECL|member|placement
dedent|''
name|'def'
name|'placement'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'host'
op|'='
name|'instance_ref'
op|'['
string|"'host'"
op|']'
newline|'\n'
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
comment|'# note(ja): original code had ctx.elevated?'
nl|'\n'
name|'services'
op|'='
name|'db'
op|'.'
name|'service_get_all_by_host'
op|'('
name|'ctxt'
op|','
name|'host'
op|')'
newline|'\n'
name|'zone'
op|'='
name|'ec2utils'
op|'.'
name|'get_availability_zone_by_host'
op|'('
name|'services'
op|','
name|'host'
op|')'
newline|'\n'
name|'return'
op|'{'
string|"'availability-zone'"
op|':'
name|'zone'
op|'}'
newline|'\n'
nl|'\n'
DECL|member|security_groups
dedent|''
name|'def'
name|'security_groups'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
name|'ctxt'
op|'='
name|'context'
op|'.'
name|'get_admin_context'
op|'('
op|')'
newline|'\n'
name|'groups'
op|'='
name|'db'
op|'.'
name|'security_group_get_by_instance'
op|'('
name|'ctxt'
op|','
nl|'\n'
name|'instance_ref'
op|'['
string|"'id'"
op|']'
op|')'
newline|'\n'
name|'return'
op|'['
name|'g'
op|'['
string|"'name'"
op|']'
name|'for'
name|'g'
name|'in'
name|'groups'
op|']'
newline|'\n'
nl|'\n'
DECL|member|public_keys
dedent|''
name|'def'
name|'public_keys'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'instance_ref'
op|'='
name|'self'
op|'.'
name|'get_instance'
op|'('
name|'address'
op|')'
newline|'\n'
comment|'# public-keys should be in meta-data only if user specified one'
nl|'\n'
name|'if'
name|'instance_ref'
op|'['
string|"'key_name'"
op|']'
op|':'
newline|'\n'
indent|'            '
name|'return'
op|'{'
string|"'0'"
op|':'
op|'{'
string|"'_name'"
op|':'
name|'instance_ref'
op|'['
string|"'key_name'"
op|']'
op|','
nl|'\n'
string|"'openssh-key'"
op|':'
name|'instance_ref'
op|'['
string|"'key_data'"
op|']'
op|'}'
op|'}'
newline|'\n'
nl|'\n'
DECL|member|ami_manifest_path
dedent|''
dedent|''
name|'def'
name|'ami_manifest_path'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
string|"'Not Implemented'"
newline|'\n'
nl|'\n'
DECL|member|instance_action
dedent|''
name|'def'
name|'instance_action'
op|'('
name|'self'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
string|"'none'"
newline|'\n'
nl|'\n'
DECL|member|format_data
dedent|''
name|'def'
name|'format_data'
op|'('
name|'self'
op|','
name|'data'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'isinstance'
op|'('
name|'data'
op|','
name|'dict'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'output'
op|'='
string|"''"
newline|'\n'
name|'for'
name|'key'
name|'in'
name|'data'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'key'
op|'=='
string|"'_name'"
op|':'
newline|'\n'
indent|'                    '
name|'continue'
newline|'\n'
dedent|''
name|'output'
op|'+='
name|'key'
newline|'\n'
name|'if'
name|'isinstance'
op|'('
name|'data'
op|'['
name|'key'
op|']'
op|','
name|'dict'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'if'
string|"'_name'"
name|'in'
name|'data'
op|'['
name|'key'
op|']'
op|':'
newline|'\n'
indent|'                        '
name|'output'
op|'+='
string|"'='"
op|'+'
name|'str'
op|'('
name|'data'
op|'['
name|'key'
op|']'
op|'['
string|"'_name'"
op|']'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                        '
name|'output'
op|'+='
string|"'/'"
newline|'\n'
dedent|''
dedent|''
name|'output'
op|'+='
string|"'\\n'"
newline|'\n'
comment|'# Cut off last \\n'
nl|'\n'
dedent|''
name|'return'
name|'output'
op|'['
op|':'
op|'-'
number|'1'
op|']'
newline|'\n'
dedent|''
name|'elif'
name|'isinstance'
op|'('
name|'data'
op|','
name|'list'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'return'
string|"'\\n'"
op|'.'
name|'join'
op|'('
name|'data'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'str'
op|'('
name|'data'
op|')'
newline|'\n'
nl|'\n'
DECL|member|lookup
dedent|''
dedent|''
name|'def'
name|'lookup'
op|'('
name|'self'
op|','
name|'path'
op|','
name|'address'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'items'
op|'='
name|'path'
op|'.'
name|'split'
op|'('
string|"'/'"
op|')'
newline|'\n'
name|'data'
op|'='
name|'self'
op|'.'
name|'metadata_mapper'
newline|'\n'
name|'for'
name|'item'
name|'in'
name|'items'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'item'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'not'
name|'isinstance'
op|'('
name|'data'
op|','
name|'dict'
op|')'
op|':'
newline|'\n'
comment|'# FIXME(ja): should we check that we are at the end'
nl|'\n'
comment|'# of the path as well before we just return?'
nl|'\n'
indent|'                    '
name|'return'
name|'data'
newline|'\n'
dedent|''
name|'if'
name|'not'
name|'item'
name|'in'
name|'data'
op|':'
newline|'\n'
indent|'                    '
name|'return'
name|'None'
newline|'\n'
dedent|''
name|'data'
op|'='
name|'data'
op|'['
name|'item'
op|']'
newline|'\n'
name|'if'
name|'isinstance'
op|'('
name|'data'
op|','
name|'collections'
op|'.'
name|'Callable'
op|')'
op|':'
newline|'\n'
comment|'# lazy evaluation'
nl|'\n'
indent|'                    '
name|'data'
op|'='
name|'data'
op|'('
name|'address'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'return'
name|'data'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'webob'
op|'.'
name|'dec'
op|'.'
name|'wsgify'
op|'('
name|'RequestClass'
op|'='
name|'wsgi'
op|'.'
name|'Request'
op|')'
newline|'\n'
DECL|member|__call__
name|'def'
name|'__call__'
op|'('
name|'self'
op|','
name|'req'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'remote_address'
op|'='
name|'req'
op|'.'
name|'remote_addr'
newline|'\n'
name|'if'
name|'FLAGS'
op|'.'
name|'use_forwarded_for'
op|':'
newline|'\n'
indent|'            '
name|'remote_address'
op|'='
name|'req'
op|'.'
name|'headers'
op|'.'
name|'get'
op|'('
string|"'X-Forwarded-For'"
op|','
name|'remote_address'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'data'
op|'='
name|'self'
op|'.'
name|'lookup'
op|'('
name|'req'
op|'.'
name|'path_info'
op|','
nl|'\n'
name|'remote_address'
op|')'
newline|'\n'
dedent|''
name|'except'
op|'('
name|'exception'
op|'.'
name|'NotFound'
op|','
name|'exception'
op|'.'
name|'FixedIpNotFoundForAddress'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'error'
op|'('
name|'_'
op|'('
string|"'Failed to get metadata for ip: %s'"
op|')'
op|','
name|'remote_address'
op|')'
newline|'\n'
name|'return'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|'('
op|')'
newline|'\n'
dedent|''
name|'except'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'exception'
op|'('
name|'_'
op|'('
string|"'Failed to get metadata for ip: %s'"
op|')'
op|','
nl|'\n'
name|'remote_address'
op|')'
newline|'\n'
name|'msg'
op|'='
name|'_'
op|'('
string|"'An unknown error has occurred. '"
nl|'\n'
string|"'Please try your request again.'"
op|')'
newline|'\n'
name|'return'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPInternalServerError'
op|'('
name|'explanation'
op|'='
name|'unicode'
op|'('
name|'msg'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'if'
name|'data'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|'('
op|')'
newline|'\n'
dedent|''
name|'return'
name|'self'
op|'.'
name|'format_data'
op|'('
name|'data'
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
