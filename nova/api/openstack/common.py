begin_unit
comment|'# Copyright 2010 OpenStack Foundation'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
name|'import'
name|'collections'
newline|'\n'
name|'import'
name|'functools'
newline|'\n'
name|'import'
name|'itertools'
newline|'\n'
name|'import'
name|'re'
newline|'\n'
nl|'\n'
name|'from'
name|'oslo_log'
name|'import'
name|'log'
name|'as'
name|'logging'
newline|'\n'
name|'from'
name|'oslo_utils'
name|'import'
name|'strutils'
newline|'\n'
name|'from'
name|'oslo_utils'
name|'import'
name|'uuidutils'
newline|'\n'
name|'import'
name|'six'
newline|'\n'
name|'import'
name|'six'
op|'.'
name|'moves'
op|'.'
name|'urllib'
op|'.'
name|'parse'
name|'as'
name|'urlparse'
newline|'\n'
name|'import'
name|'webob'
newline|'\n'
name|'from'
name|'webob'
name|'import'
name|'exc'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
op|'.'
name|'compute'
name|'import'
name|'task_states'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'compute'
name|'import'
name|'utils'
name|'as'
name|'compute_utils'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'compute'
name|'import'
name|'vm_states'
newline|'\n'
name|'import'
name|'nova'
op|'.'
name|'conf'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'i18n'
name|'import'
name|'_'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'i18n'
name|'import'
name|'_LE'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'i18n'
name|'import'
name|'_LW'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'objects'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'quota'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'utils'
newline|'\n'
nl|'\n'
DECL|variable|CONF
name|'CONF'
op|'='
name|'nova'
op|'.'
name|'conf'
op|'.'
name|'CONF'
newline|'\n'
nl|'\n'
DECL|variable|LOG
name|'LOG'
op|'='
name|'logging'
op|'.'
name|'getLogger'
op|'('
name|'__name__'
op|')'
newline|'\n'
DECL|variable|QUOTAS
name|'QUOTAS'
op|'='
name|'quota'
op|'.'
name|'QUOTAS'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|_STATE_MAP
name|'_STATE_MAP'
op|'='
op|'{'
nl|'\n'
name|'vm_states'
op|'.'
name|'ACTIVE'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'ACTIVE'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBOOTING'
op|':'
string|"'REBOOT'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBOOT_PENDING'
op|':'
string|"'REBOOT'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBOOT_STARTED'
op|':'
string|"'REBOOT'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBOOTING_HARD'
op|':'
string|"'HARD_REBOOT'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBOOT_PENDING_HARD'
op|':'
string|"'HARD_REBOOT'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBOOT_STARTED_HARD'
op|':'
string|"'HARD_REBOOT'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'UPDATING_PASSWORD'
op|':'
string|"'PASSWORD'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILDING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILD_BLOCK_DEVICE_MAPPING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILD_SPAWNING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'MIGRATING'
op|':'
string|"'MIGRATING'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_PREP'
op|':'
string|"'RESIZE'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_MIGRATING'
op|':'
string|"'RESIZE'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_MIGRATED'
op|':'
string|"'RESIZE'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_FINISH'
op|':'
string|"'RESIZE'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'BUILDING'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'BUILD'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'STOPPED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'SHUTOFF'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_PREP'
op|':'
string|"'RESIZE'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_MIGRATING'
op|':'
string|"'RESIZE'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_MIGRATED'
op|':'
string|"'RESIZE'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_FINISH'
op|':'
string|"'RESIZE'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILDING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILD_BLOCK_DEVICE_MAPPING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILD_SPAWNING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'RESIZED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'VERIFY_RESIZE'"
op|','
nl|'\n'
comment|"# Note(maoy): the OS API spec 1.1 doesn't have CONFIRMING_RESIZE"
nl|'\n'
comment|'# state so we comment that out for future reference only.'
nl|'\n'
comment|"# task_states.RESIZE_CONFIRMING: 'CONFIRMING_RESIZE',"
nl|'\n'
name|'task_states'
op|'.'
name|'RESIZE_REVERTING'
op|':'
string|"'REVERT_RESIZE'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'PAUSED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'PAUSED'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'MIGRATING'
op|':'
string|"'MIGRATING'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'SUSPENDED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'SUSPENDED'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'RESCUED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'RESCUE'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'ERROR'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'ERROR'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILDING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILD_BLOCK_DEVICE_MAPPING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
name|'task_states'
op|'.'
name|'REBUILD_SPAWNING'
op|':'
string|"'REBUILD'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'DELETED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'DELETED'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'SOFT_DELETED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'SOFT_DELETED'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'SHELVED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'SHELVED'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
name|'vm_states'
op|'.'
name|'SHELVED_OFFLOADED'
op|':'
op|'{'
nl|'\n'
string|"'default'"
op|':'
string|"'SHELVED_OFFLOADED'"
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'}'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|status_from_state
name|'def'
name|'status_from_state'
op|'('
name|'vm_state'
op|','
name|'task_state'
op|'='
string|"'default'"
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Given vm_state and task_state, return a status string."""'
newline|'\n'
name|'task_map'
op|'='
name|'_STATE_MAP'
op|'.'
name|'get'
op|'('
name|'vm_state'
op|','
name|'dict'
op|'('
name|'default'
op|'='
string|"'UNKNOWN'"
op|')'
op|')'
newline|'\n'
name|'status'
op|'='
name|'task_map'
op|'.'
name|'get'
op|'('
name|'task_state'
op|','
name|'task_map'
op|'['
string|"'default'"
op|']'
op|')'
newline|'\n'
name|'if'
name|'status'
op|'=='
string|'"UNKNOWN"'
op|':'
newline|'\n'
indent|'        '
name|'LOG'
op|'.'
name|'error'
op|'('
name|'_LE'
op|'('
string|'"status is UNKNOWN from vm_state=%(vm_state)s "'
nl|'\n'
string|'"task_state=%(task_state)s. Bad upgrade or db "'
nl|'\n'
string|'"corrupted?"'
op|')'
op|','
nl|'\n'
op|'{'
string|"'vm_state'"
op|':'
name|'vm_state'
op|','
string|"'task_state'"
op|':'
name|'task_state'
op|'}'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'status'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|task_and_vm_state_from_status
dedent|''
name|'def'
name|'task_and_vm_state_from_status'
op|'('
name|'statuses'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Map the server\'s multiple status strings to list of vm states and\n    list of task states.\n    """'
newline|'\n'
name|'vm_states'
op|'='
name|'set'
op|'('
op|')'
newline|'\n'
name|'task_states'
op|'='
name|'set'
op|'('
op|')'
newline|'\n'
name|'lower_statuses'
op|'='
op|'['
name|'status'
op|'.'
name|'lower'
op|'('
op|')'
name|'for'
name|'status'
name|'in'
name|'statuses'
op|']'
newline|'\n'
name|'for'
name|'state'
op|','
name|'task_map'
name|'in'
name|'six'
op|'.'
name|'iteritems'
op|'('
name|'_STATE_MAP'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'for'
name|'task_state'
op|','
name|'mapped_state'
name|'in'
name|'six'
op|'.'
name|'iteritems'
op|'('
name|'task_map'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'status_string'
op|'='
name|'mapped_state'
newline|'\n'
name|'if'
name|'status_string'
op|'.'
name|'lower'
op|'('
op|')'
name|'in'
name|'lower_statuses'
op|':'
newline|'\n'
indent|'                '
name|'vm_states'
op|'.'
name|'add'
op|'('
name|'state'
op|')'
newline|'\n'
name|'task_states'
op|'.'
name|'add'
op|'('
name|'task_state'
op|')'
newline|'\n'
comment|'# Add sort to avoid different order on set in Python 3'
nl|'\n'
dedent|''
dedent|''
dedent|''
name|'return'
name|'sorted'
op|'('
name|'vm_states'
op|')'
op|','
name|'sorted'
op|'('
name|'task_states'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|get_sort_params
dedent|''
name|'def'
name|'get_sort_params'
op|'('
name|'input_params'
op|','
name|'default_key'
op|'='
string|"'created_at'"
op|','
nl|'\n'
name|'default_dir'
op|'='
string|"'desc'"
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Retrieves sort keys/directions parameters.\n\n    Processes the parameters to create a list of sort keys and sort directions\n    that correspond to the \'sort_key\' and \'sort_dir\' parameter values. These\n    sorting parameters can be specified multiple times in order to generate\n    the list of sort keys and directions.\n\n    The input parameters are not modified.\n\n    :param input_params: webob.multidict of request parameters (from\n                         nova.wsgi.Request.params)\n    :param default_key: default sort key value, added to the list if no\n                        \'sort_key\' parameters are supplied\n    :param default_dir: default sort dir value, added to the list if no\n                        \'sort_dir\' parameters are supplied\n    :returns: list of sort keys, list of sort dirs\n    """'
newline|'\n'
name|'params'
op|'='
name|'input_params'
op|'.'
name|'copy'
op|'('
op|')'
newline|'\n'
name|'sort_keys'
op|'='
op|'['
op|']'
newline|'\n'
name|'sort_dirs'
op|'='
op|'['
op|']'
newline|'\n'
name|'while'
string|"'sort_key'"
name|'in'
name|'params'
op|':'
newline|'\n'
indent|'        '
name|'sort_keys'
op|'.'
name|'append'
op|'('
name|'params'
op|'.'
name|'pop'
op|'('
string|"'sort_key'"
op|')'
op|'.'
name|'strip'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
name|'while'
string|"'sort_dir'"
name|'in'
name|'params'
op|':'
newline|'\n'
indent|'        '
name|'sort_dirs'
op|'.'
name|'append'
op|'('
name|'params'
op|'.'
name|'pop'
op|'('
string|"'sort_dir'"
op|')'
op|'.'
name|'strip'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'len'
op|'('
name|'sort_keys'
op|')'
op|'=='
number|'0'
name|'and'
name|'default_key'
op|':'
newline|'\n'
indent|'        '
name|'sort_keys'
op|'.'
name|'append'
op|'('
name|'default_key'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'len'
op|'('
name|'sort_dirs'
op|')'
op|'=='
number|'0'
name|'and'
name|'default_dir'
op|':'
newline|'\n'
indent|'        '
name|'sort_dirs'
op|'.'
name|'append'
op|'('
name|'default_dir'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'sort_keys'
op|','
name|'sort_dirs'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|get_pagination_params
dedent|''
name|'def'
name|'get_pagination_params'
op|'('
name|'request'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Return marker, limit tuple from request.\n\n    :param request: `wsgi.Request` possibly containing \'marker\' and \'limit\'\n                    GET variables. \'marker\' is the id of the last element\n                    the client has seen, and \'limit\' is the maximum number\n                    of items to return. If \'limit\' is not specified, 0, or\n                    > max_limit, we default to max_limit. Negative values\n                    for either marker or limit will cause\n                    exc.HTTPBadRequest() exceptions to be raised.\n\n    """'
newline|'\n'
name|'params'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'if'
string|"'limit'"
name|'in'
name|'request'
op|'.'
name|'GET'
op|':'
newline|'\n'
indent|'        '
name|'params'
op|'['
string|"'limit'"
op|']'
op|'='
name|'_get_int_param'
op|'('
name|'request'
op|','
string|"'limit'"
op|')'
newline|'\n'
dedent|''
name|'if'
string|"'page_size'"
name|'in'
name|'request'
op|'.'
name|'GET'
op|':'
newline|'\n'
indent|'        '
name|'params'
op|'['
string|"'page_size'"
op|']'
op|'='
name|'_get_int_param'
op|'('
name|'request'
op|','
string|"'page_size'"
op|')'
newline|'\n'
dedent|''
name|'if'
string|"'marker'"
name|'in'
name|'request'
op|'.'
name|'GET'
op|':'
newline|'\n'
indent|'        '
name|'params'
op|'['
string|"'marker'"
op|']'
op|'='
name|'_get_marker_param'
op|'('
name|'request'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'params'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|_get_int_param
dedent|''
name|'def'
name|'_get_int_param'
op|'('
name|'request'
op|','
name|'param'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Extract integer param from request or fail."""'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'        '
name|'int_param'
op|'='
name|'utils'
op|'.'
name|'validate_integer'
op|'('
name|'request'
op|'.'
name|'GET'
op|'['
name|'param'
op|']'
op|','
name|'param'
op|','
nl|'\n'
name|'min_value'
op|'='
number|'0'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'exception'
op|'.'
name|'InvalidInput'
name|'as'
name|'e'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|'('
name|'explanation'
op|'='
name|'e'
op|'.'
name|'format_message'
op|'('
op|')'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'int_param'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|_get_marker_param
dedent|''
name|'def'
name|'_get_marker_param'
op|'('
name|'request'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Extract marker id from request or fail."""'
newline|'\n'
name|'return'
name|'request'
op|'.'
name|'GET'
op|'['
string|"'marker'"
op|']'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|limited
dedent|''
name|'def'
name|'limited'
op|'('
name|'items'
op|','
name|'request'
op|','
name|'max_limit'
op|'='
name|'CONF'
op|'.'
name|'osapi_max_limit'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Return a slice of items according to requested offset and limit.\n\n    :param items: A sliceable entity\n    :param request: ``wsgi.Request`` possibly containing \'offset\' and \'limit\'\n                    GET variables. \'offset\' is where to start in the list,\n                    and \'limit\' is the maximum number of items to return. If\n                    \'limit\' is not specified, 0, or > max_limit, we default\n                    to max_limit. Negative values for either offset or limit\n                    will cause exc.HTTPBadRequest() exceptions to be raised.\n    :kwarg max_limit: The maximum number of items to return from \'items\'\n    """'
newline|'\n'
name|'offset'
op|'='
name|'request'
op|'.'
name|'GET'
op|'.'
name|'get'
op|'('
string|'"offset"'
op|','
number|'0'
op|')'
newline|'\n'
name|'limit'
op|'='
name|'request'
op|'.'
name|'GET'
op|'.'
name|'get'
op|'('
string|"'limit'"
op|','
name|'max_limit'
op|')'
newline|'\n'
nl|'\n'
name|'try'
op|':'
newline|'\n'
indent|'        '
name|'offset'
op|'='
name|'utils'
op|'.'
name|'validate_integer'
op|'('
name|'offset'
op|','
string|'"offset"'
op|','
name|'min_value'
op|'='
number|'0'
op|')'
newline|'\n'
name|'limit'
op|'='
name|'utils'
op|'.'
name|'validate_integer'
op|'('
name|'limit'
op|','
string|'"limit"'
op|','
name|'min_value'
op|'='
number|'0'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'exception'
op|'.'
name|'InvalidInput'
name|'as'
name|'e'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|'('
name|'explanation'
op|'='
name|'e'
op|'.'
name|'format_message'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'limit'
op|'='
name|'min'
op|'('
name|'max_limit'
op|','
name|'limit'
name|'or'
name|'max_limit'
op|')'
newline|'\n'
name|'range_end'
op|'='
name|'offset'
op|'+'
name|'limit'
newline|'\n'
name|'return'
name|'items'
op|'['
name|'offset'
op|':'
name|'range_end'
op|']'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|get_limit_and_marker
dedent|''
name|'def'
name|'get_limit_and_marker'
op|'('
name|'request'
op|','
name|'max_limit'
op|'='
name|'CONF'
op|'.'
name|'osapi_max_limit'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""get limited parameter from request."""'
newline|'\n'
name|'params'
op|'='
name|'get_pagination_params'
op|'('
name|'request'
op|')'
newline|'\n'
name|'limit'
op|'='
name|'params'
op|'.'
name|'get'
op|'('
string|"'limit'"
op|','
name|'max_limit'
op|')'
newline|'\n'
name|'limit'
op|'='
name|'min'
op|'('
name|'max_limit'
op|','
name|'limit'
op|')'
newline|'\n'
name|'marker'
op|'='
name|'params'
op|'.'
name|'get'
op|'('
string|"'marker'"
op|')'
newline|'\n'
nl|'\n'
name|'return'
name|'limit'
op|','
name|'marker'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|get_id_from_href
dedent|''
name|'def'
name|'get_id_from_href'
op|'('
name|'href'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Return the id or uuid portion of a url.\n\n    Given: \'http://www.foo.com/bar/123?q=4\'\n    Returns: \'123\'\n\n    Given: \'http://www.foo.com/bar/abc123?q=4\'\n    Returns: \'abc123\'\n\n    """'
newline|'\n'
name|'return'
name|'urlparse'
op|'.'
name|'urlsplit'
op|'('
string|'"%s"'
op|'%'
name|'href'
op|')'
op|'.'
name|'path'
op|'.'
name|'split'
op|'('
string|"'/'"
op|')'
op|'['
op|'-'
number|'1'
op|']'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|remove_trailing_version_from_href
dedent|''
name|'def'
name|'remove_trailing_version_from_href'
op|'('
name|'href'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Removes the api version from the href.\n\n    Given: \'http://www.nova.com/compute/v1.1\'\n    Returns: \'http://www.nova.com/compute\'\n\n    Given: \'http://www.nova.com/v1.1\'\n    Returns: \'http://www.nova.com\'\n\n    """'
newline|'\n'
name|'parsed_url'
op|'='
name|'urlparse'
op|'.'
name|'urlsplit'
op|'('
name|'href'
op|')'
newline|'\n'
name|'url_parts'
op|'='
name|'parsed_url'
op|'.'
name|'path'
op|'.'
name|'rsplit'
op|'('
string|"'/'"
op|','
number|'1'
op|')'
newline|'\n'
nl|'\n'
comment|'# NOTE: this should match vX.X or vX'
nl|'\n'
name|'expression'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
string|"r'^v([0-9]+|[0-9]+\\.[0-9]+)(/.*|$)'"
op|')'
newline|'\n'
name|'if'
name|'not'
name|'expression'
op|'.'
name|'match'
op|'('
name|'url_parts'
op|'.'
name|'pop'
op|'('
op|')'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'LOG'
op|'.'
name|'debug'
op|'('
string|"'href %s does not contain version'"
op|','
name|'href'
op|')'
newline|'\n'
name|'raise'
name|'ValueError'
op|'('
name|'_'
op|'('
string|"'href %s does not contain version'"
op|')'
op|'%'
name|'href'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'new_path'
op|'='
name|'url_join'
op|'('
op|'*'
name|'url_parts'
op|')'
newline|'\n'
name|'parsed_url'
op|'='
name|'list'
op|'('
name|'parsed_url'
op|')'
newline|'\n'
name|'parsed_url'
op|'['
number|'2'
op|']'
op|'='
name|'new_path'
newline|'\n'
name|'return'
name|'urlparse'
op|'.'
name|'urlunsplit'
op|'('
name|'parsed_url'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|check_img_metadata_properties_quota
dedent|''
name|'def'
name|'check_img_metadata_properties_quota'
op|'('
name|'context'
op|','
name|'metadata'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'not'
name|'metadata'
op|':'
newline|'\n'
indent|'        '
name|'return'
newline|'\n'
dedent|''
name|'try'
op|':'
newline|'\n'
indent|'        '
name|'QUOTAS'
op|'.'
name|'limit_check'
op|'('
name|'context'
op|','
name|'metadata_items'
op|'='
name|'len'
op|'('
name|'metadata'
op|')'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'exception'
op|'.'
name|'OverQuota'
op|':'
newline|'\n'
indent|'        '
name|'expl'
op|'='
name|'_'
op|'('
string|'"Image metadata limit exceeded"'
op|')'
newline|'\n'
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPForbidden'
op|'('
name|'explanation'
op|'='
name|'expl'
op|')'
newline|'\n'
nl|'\n'
comment|'#  check the key length.'
nl|'\n'
dedent|''
name|'if'
name|'isinstance'
op|'('
name|'metadata'
op|','
name|'dict'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'for'
name|'key'
op|','
name|'value'
name|'in'
name|'six'
op|'.'
name|'iteritems'
op|'('
name|'metadata'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'len'
op|'('
name|'key'
op|')'
op|'=='
number|'0'
op|':'
newline|'\n'
indent|'                '
name|'expl'
op|'='
name|'_'
op|'('
string|'"Image metadata key cannot be blank"'
op|')'
newline|'\n'
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|'('
name|'explanation'
op|'='
name|'expl'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'len'
op|'('
name|'key'
op|')'
op|'>'
number|'255'
op|':'
newline|'\n'
indent|'                '
name|'expl'
op|'='
name|'_'
op|'('
string|'"Image metadata key too long"'
op|')'
newline|'\n'
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|'('
name|'explanation'
op|'='
name|'expl'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'        '
name|'expl'
op|'='
name|'_'
op|'('
string|'"Invalid image metadata"'
op|')'
newline|'\n'
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|'('
name|'explanation'
op|'='
name|'expl'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|get_networks_for_instance_from_nw_info
dedent|''
dedent|''
name|'def'
name|'get_networks_for_instance_from_nw_info'
op|'('
name|'nw_info'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'networks'
op|'='
name|'collections'
op|'.'
name|'OrderedDict'
op|'('
op|')'
newline|'\n'
name|'for'
name|'vif'
name|'in'
name|'nw_info'
op|':'
newline|'\n'
indent|'        '
name|'ips'
op|'='
name|'vif'
op|'.'
name|'fixed_ips'
op|'('
op|')'
newline|'\n'
name|'floaters'
op|'='
name|'vif'
op|'.'
name|'floating_ips'
op|'('
op|')'
newline|'\n'
name|'label'
op|'='
name|'vif'
op|'['
string|"'network'"
op|']'
op|'['
string|"'label'"
op|']'
newline|'\n'
name|'if'
name|'label'
name|'not'
name|'in'
name|'networks'
op|':'
newline|'\n'
indent|'            '
name|'networks'
op|'['
name|'label'
op|']'
op|'='
op|'{'
string|"'ips'"
op|':'
op|'['
op|']'
op|','
string|"'floating_ips'"
op|':'
op|'['
op|']'
op|'}'
newline|'\n'
dedent|''
name|'for'
name|'ip'
name|'in'
name|'itertools'
op|'.'
name|'chain'
op|'('
name|'ips'
op|','
name|'floaters'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'ip'
op|'['
string|"'mac_address'"
op|']'
op|'='
name|'vif'
op|'['
string|"'address'"
op|']'
newline|'\n'
dedent|''
name|'networks'
op|'['
name|'label'
op|']'
op|'['
string|"'ips'"
op|']'
op|'.'
name|'extend'
op|'('
name|'ips'
op|')'
newline|'\n'
name|'networks'
op|'['
name|'label'
op|']'
op|'['
string|"'floating_ips'"
op|']'
op|'.'
name|'extend'
op|'('
name|'floaters'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'networks'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|get_networks_for_instance
dedent|''
name|'def'
name|'get_networks_for_instance'
op|'('
name|'context'
op|','
name|'instance'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Returns a prepared nw_info list for passing into the view builders\n\n    We end up with a data structure like::\n\n        {\'public\': {\'ips\': [{\'address\': \'10.0.0.1\',\n                             \'version\': 4,\n                             \'mac_address\': \'aa:aa:aa:aa:aa:aa\'},\n                            {\'address\': \'2001::1\',\n                             \'version\': 6,\n                             \'mac_address\': \'aa:aa:aa:aa:aa:aa\'}],\n                    \'floating_ips\': [{\'address\': \'172.16.0.1\',\n                                      \'version\': 4,\n                                      \'mac_address\': \'aa:aa:aa:aa:aa:aa\'},\n                                     {\'address\': \'172.16.2.1\',\n                                      \'version\': 4,\n                                      \'mac_address\': \'aa:aa:aa:aa:aa:aa\'}]},\n         ...}\n    """'
newline|'\n'
name|'nw_info'
op|'='
name|'compute_utils'
op|'.'
name|'get_nw_info_for_instance'
op|'('
name|'instance'
op|')'
newline|'\n'
name|'return'
name|'get_networks_for_instance_from_nw_info'
op|'('
name|'nw_info'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|raise_http_conflict_for_instance_invalid_state
dedent|''
name|'def'
name|'raise_http_conflict_for_instance_invalid_state'
op|'('
name|'exc'
op|','
name|'action'
op|','
name|'server_id'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Raises a webob.exc.HTTPConflict instance containing a message\n    appropriate to return via the API based on the original\n    InstanceInvalidState exception.\n    """'
newline|'\n'
name|'attr'
op|'='
name|'exc'
op|'.'
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'attr'"
op|')'
newline|'\n'
name|'state'
op|'='
name|'exc'
op|'.'
name|'kwargs'
op|'.'
name|'get'
op|'('
string|"'state'"
op|')'
newline|'\n'
name|'if'
name|'attr'
name|'is'
name|'not'
name|'None'
name|'and'
name|'state'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'        '
name|'msg'
op|'='
name|'_'
op|'('
string|'"Cannot \'%(action)s\' instance %(server_id)s while it is in "'
nl|'\n'
string|'"%(attr)s %(state)s"'
op|')'
op|'%'
op|'{'
string|"'action'"
op|':'
name|'action'
op|','
string|"'attr'"
op|':'
name|'attr'
op|','
nl|'\n'
string|"'state'"
op|':'
name|'state'
op|','
nl|'\n'
string|"'server_id'"
op|':'
name|'server_id'
op|'}'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
comment|'# At least give some meaningful message'
nl|'\n'
indent|'        '
name|'msg'
op|'='
name|'_'
op|'('
string|'"Instance %(server_id)s is in an invalid state for "'
nl|'\n'
string|'"\'%(action)s\'"'
op|')'
op|'%'
op|'{'
string|"'action'"
op|':'
name|'action'
op|','
string|"'server_id'"
op|':'
name|'server_id'
op|'}'
newline|'\n'
dedent|''
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPConflict'
op|'('
name|'explanation'
op|'='
name|'msg'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|check_snapshots_enabled
dedent|''
name|'def'
name|'check_snapshots_enabled'
op|'('
name|'f'
op|')'
op|':'
newline|'\n'
indent|'    '
op|'@'
name|'functools'
op|'.'
name|'wraps'
op|'('
name|'f'
op|')'
newline|'\n'
DECL|function|inner
name|'def'
name|'inner'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'CONF'
op|'.'
name|'allow_instance_snapshots'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'warning'
op|'('
name|'_LW'
op|'('
string|"'Rejecting snapshot request, snapshots currently'"
nl|'\n'
string|"' disabled'"
op|')'
op|')'
newline|'\n'
name|'msg'
op|'='
name|'_'
op|'('
string|'"Instance snapshots are not permitted at this time."'
op|')'
newline|'\n'
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|'('
name|'explanation'
op|'='
name|'msg'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'f'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'inner'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|url_join
dedent|''
name|'def'
name|'url_join'
op|'('
op|'*'
name|'parts'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Convenience method for joining parts of a URL\n\n    Any leading and trailing \'/\' characters are removed, and the parts joined\n    together with \'/\' as a separator. If last element of \'parts\' is an empty\n    string, the returned URL will have a trailing slash.\n    """'
newline|'\n'
name|'parts'
op|'='
name|'parts'
name|'or'
op|'['
string|'""'
op|']'
newline|'\n'
name|'clean_parts'
op|'='
op|'['
name|'part'
op|'.'
name|'strip'
op|'('
string|'"/"'
op|')'
name|'for'
name|'part'
name|'in'
name|'parts'
name|'if'
name|'part'
op|']'
newline|'\n'
name|'if'
name|'not'
name|'parts'
op|'['
op|'-'
number|'1'
op|']'
op|':'
newline|'\n'
comment|'# Empty last element should add a trailing slash'
nl|'\n'
indent|'        '
name|'clean_parts'
op|'.'
name|'append'
op|'('
string|'""'
op|')'
newline|'\n'
dedent|''
name|'return'
string|'"/"'
op|'.'
name|'join'
op|'('
name|'clean_parts'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|ViewBuilder
dedent|''
name|'class'
name|'ViewBuilder'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Model API responses as dictionaries."""'
newline|'\n'
nl|'\n'
DECL|member|_get_project_id
name|'def'
name|'_get_project_id'
op|'('
name|'self'
op|','
name|'request'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Get project id from request url if present or empty string\n        otherwise\n        """'
newline|'\n'
name|'project_id'
op|'='
name|'request'
op|'.'
name|'environ'
op|'['
string|'"nova.context"'
op|']'
op|'.'
name|'project_id'
newline|'\n'
name|'if'
name|'project_id'
name|'in'
name|'request'
op|'.'
name|'url'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'project_id'
newline|'\n'
dedent|''
name|'return'
string|"''"
newline|'\n'
nl|'\n'
DECL|member|_get_links
dedent|''
name|'def'
name|'_get_links'
op|'('
name|'self'
op|','
name|'request'
op|','
name|'identifier'
op|','
name|'collection_name'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
op|'['
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"self"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'self'
op|'.'
name|'_get_href_link'
op|'('
name|'request'
op|','
name|'identifier'
op|','
name|'collection_name'
op|')'
op|','
nl|'\n'
op|'}'
op|','
nl|'\n'
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"bookmark"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'self'
op|'.'
name|'_get_bookmark_link'
op|'('
name|'request'
op|','
nl|'\n'
name|'identifier'
op|','
nl|'\n'
name|'collection_name'
op|')'
op|','
nl|'\n'
op|'}'
op|']'
newline|'\n'
nl|'\n'
DECL|member|_get_next_link
dedent|''
name|'def'
name|'_get_next_link'
op|'('
name|'self'
op|','
name|'request'
op|','
name|'identifier'
op|','
name|'collection_name'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Return href string with proper limit and marker params."""'
newline|'\n'
name|'params'
op|'='
name|'request'
op|'.'
name|'params'
op|'.'
name|'copy'
op|'('
op|')'
newline|'\n'
name|'params'
op|'['
string|'"marker"'
op|']'
op|'='
name|'identifier'
newline|'\n'
name|'prefix'
op|'='
name|'self'
op|'.'
name|'_update_compute_link_prefix'
op|'('
name|'request'
op|'.'
name|'application_url'
op|')'
newline|'\n'
name|'url'
op|'='
name|'url_join'
op|'('
name|'prefix'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_get_project_id'
op|'('
name|'request'
op|')'
op|','
nl|'\n'
name|'collection_name'
op|')'
newline|'\n'
name|'return'
string|'"%s?%s"'
op|'%'
op|'('
name|'url'
op|','
name|'urlparse'
op|'.'
name|'urlencode'
op|'('
name|'params'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_get_href_link
dedent|''
name|'def'
name|'_get_href_link'
op|'('
name|'self'
op|','
name|'request'
op|','
name|'identifier'
op|','
name|'collection_name'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Return an href string pointing to this object."""'
newline|'\n'
name|'prefix'
op|'='
name|'self'
op|'.'
name|'_update_compute_link_prefix'
op|'('
name|'request'
op|'.'
name|'application_url'
op|')'
newline|'\n'
name|'return'
name|'url_join'
op|'('
name|'prefix'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_get_project_id'
op|'('
name|'request'
op|')'
op|','
nl|'\n'
name|'collection_name'
op|','
nl|'\n'
name|'str'
op|'('
name|'identifier'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_get_bookmark_link
dedent|''
name|'def'
name|'_get_bookmark_link'
op|'('
name|'self'
op|','
name|'request'
op|','
name|'identifier'
op|','
name|'collection_name'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Create a URL that refers to a specific resource."""'
newline|'\n'
name|'base_url'
op|'='
name|'remove_trailing_version_from_href'
op|'('
name|'request'
op|'.'
name|'application_url'
op|')'
newline|'\n'
name|'base_url'
op|'='
name|'self'
op|'.'
name|'_update_compute_link_prefix'
op|'('
name|'base_url'
op|')'
newline|'\n'
name|'return'
name|'url_join'
op|'('
name|'base_url'
op|','
nl|'\n'
name|'self'
op|'.'
name|'_get_project_id'
op|'('
name|'request'
op|')'
op|','
nl|'\n'
name|'collection_name'
op|','
nl|'\n'
name|'str'
op|'('
name|'identifier'
op|')'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_get_collection_links
dedent|''
name|'def'
name|'_get_collection_links'
op|'('
name|'self'
op|','
nl|'\n'
name|'request'
op|','
nl|'\n'
name|'items'
op|','
nl|'\n'
name|'collection_name'
op|','
nl|'\n'
name|'id_key'
op|'='
string|'"uuid"'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Retrieve \'next\' link, if applicable. This is included if:\n        1) \'limit\' param is specified and equals the number of items.\n        2) \'limit\' param is specified but it exceeds CONF.osapi_max_limit,\n        in this case the number of items is CONF.osapi_max_limit.\n        3) \'limit\' param is NOT specified but the number of items is\n        CONF.osapi_max_limit.\n        """'
newline|'\n'
name|'links'
op|'='
op|'['
op|']'
newline|'\n'
name|'max_items'
op|'='
name|'min'
op|'('
nl|'\n'
name|'int'
op|'('
name|'request'
op|'.'
name|'params'
op|'.'
name|'get'
op|'('
string|'"limit"'
op|','
name|'CONF'
op|'.'
name|'osapi_max_limit'
op|')'
op|')'
op|','
nl|'\n'
name|'CONF'
op|'.'
name|'osapi_max_limit'
op|')'
newline|'\n'
name|'if'
name|'max_items'
name|'and'
name|'max_items'
op|'=='
name|'len'
op|'('
name|'items'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'last_item'
op|'='
name|'items'
op|'['
op|'-'
number|'1'
op|']'
newline|'\n'
name|'if'
name|'id_key'
name|'in'
name|'last_item'
op|':'
newline|'\n'
indent|'                '
name|'last_item_id'
op|'='
name|'last_item'
op|'['
name|'id_key'
op|']'
newline|'\n'
dedent|''
name|'elif'
string|"'id'"
name|'in'
name|'last_item'
op|':'
newline|'\n'
indent|'                '
name|'last_item_id'
op|'='
name|'last_item'
op|'['
string|'"id"'
op|']'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'last_item_id'
op|'='
name|'last_item'
op|'['
string|'"flavorid"'
op|']'
newline|'\n'
dedent|''
name|'links'
op|'.'
name|'append'
op|'('
op|'{'
nl|'\n'
string|'"rel"'
op|':'
string|'"next"'
op|','
nl|'\n'
string|'"href"'
op|':'
name|'self'
op|'.'
name|'_get_next_link'
op|'('
name|'request'
op|','
nl|'\n'
name|'last_item_id'
op|','
nl|'\n'
name|'collection_name'
op|')'
op|','
nl|'\n'
op|'}'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'links'
newline|'\n'
nl|'\n'
DECL|member|_update_link_prefix
dedent|''
name|'def'
name|'_update_link_prefix'
op|'('
name|'self'
op|','
name|'orig_url'
op|','
name|'prefix'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'prefix'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'orig_url'
newline|'\n'
dedent|''
name|'url_parts'
op|'='
name|'list'
op|'('
name|'urlparse'
op|'.'
name|'urlsplit'
op|'('
name|'orig_url'
op|')'
op|')'
newline|'\n'
name|'prefix_parts'
op|'='
name|'list'
op|'('
name|'urlparse'
op|'.'
name|'urlsplit'
op|'('
name|'prefix'
op|')'
op|')'
newline|'\n'
name|'url_parts'
op|'['
number|'0'
op|':'
number|'2'
op|']'
op|'='
name|'prefix_parts'
op|'['
number|'0'
op|':'
number|'2'
op|']'
newline|'\n'
name|'url_parts'
op|'['
number|'2'
op|']'
op|'='
name|'prefix_parts'
op|'['
number|'2'
op|']'
op|'+'
name|'url_parts'
op|'['
number|'2'
op|']'
newline|'\n'
name|'return'
name|'urlparse'
op|'.'
name|'urlunsplit'
op|'('
name|'url_parts'
op|')'
op|'.'
name|'rstrip'
op|'('
string|"'/'"
op|')'
newline|'\n'
nl|'\n'
DECL|member|_update_glance_link_prefix
dedent|''
name|'def'
name|'_update_glance_link_prefix'
op|'('
name|'self'
op|','
name|'orig_url'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'self'
op|'.'
name|'_update_link_prefix'
op|'('
name|'orig_url'
op|','
nl|'\n'
name|'CONF'
op|'.'
name|'osapi_glance_link_prefix'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_update_compute_link_prefix
dedent|''
name|'def'
name|'_update_compute_link_prefix'
op|'('
name|'self'
op|','
name|'orig_url'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'self'
op|'.'
name|'_update_link_prefix'
op|'('
name|'orig_url'
op|','
nl|'\n'
name|'CONF'
op|'.'
name|'osapi_compute_link_prefix'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|get_instance
dedent|''
dedent|''
name|'def'
name|'get_instance'
op|'('
name|'compute_api'
op|','
name|'context'
op|','
name|'instance_id'
op|','
name|'expected_attrs'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Fetch an instance from the compute API, handling error checking."""'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'compute_api'
op|'.'
name|'get'
op|'('
name|'context'
op|','
name|'instance_id'
op|','
nl|'\n'
name|'want_objects'
op|'='
name|'True'
op|','
nl|'\n'
name|'expected_attrs'
op|'='
name|'expected_attrs'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'exception'
op|'.'
name|'InstanceNotFound'
name|'as'
name|'e'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|'('
name|'explanation'
op|'='
name|'e'
op|'.'
name|'format_message'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|normalize_name
dedent|''
dedent|''
name|'def'
name|'normalize_name'
op|'('
name|'name'
op|')'
op|':'
newline|'\n'
comment|'# NOTE(alex_xu): This method is used by v2.1 legacy v2 compat mode.'
nl|'\n'
comment|'# In the legacy v2 API, some of APIs strip the spaces and some of APIs not.'
nl|'\n'
comment|'# The v2.1 disallow leading/trailing, for compatible v2 API and consistent,'
nl|'\n'
comment|'# we enable leading/trailing spaces and strip spaces in legacy v2 compat'
nl|'\n'
comment|"# mode. Althrough in legacy v2 API there are some APIs didn't strip spaces,"
nl|'\n'
comment|'# but actually leading/trailing spaces(that means user depend on leading/'
nl|'\n'
comment|'# trailing spaces distinguish different instance) is pointless usecase.'
nl|'\n'
indent|'    '
name|'return'
name|'name'
op|'.'
name|'strip'
op|'('
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|raise_feature_not_supported
dedent|''
name|'def'
name|'raise_feature_not_supported'
op|'('
name|'msg'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'if'
name|'msg'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'        '
name|'msg'
op|'='
name|'_'
op|'('
string|'"The requested functionality is not supported."'
op|')'
newline|'\n'
dedent|''
name|'raise'
name|'webob'
op|'.'
name|'exc'
op|'.'
name|'HTTPNotImplemented'
op|'('
name|'explanation'
op|'='
name|'msg'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|get_flavor
dedent|''
name|'def'
name|'get_flavor'
op|'('
name|'context'
op|','
name|'flavor_id'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'try'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'objects'
op|'.'
name|'Flavor'
op|'.'
name|'get_by_flavor_id'
op|'('
name|'context'
op|','
name|'flavor_id'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'exception'
op|'.'
name|'FlavorNotFound'
name|'as'
name|'error'
op|':'
newline|'\n'
indent|'        '
name|'raise'
name|'exc'
op|'.'
name|'HTTPNotFound'
op|'('
name|'explanation'
op|'='
name|'error'
op|'.'
name|'format_message'
op|'('
op|')'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|check_cells_enabled
dedent|''
dedent|''
name|'def'
name|'check_cells_enabled'
op|'('
name|'function'
op|')'
op|':'
newline|'\n'
indent|'    '
op|'@'
name|'functools'
op|'.'
name|'wraps'
op|'('
name|'function'
op|')'
newline|'\n'
DECL|function|inner
name|'def'
name|'inner'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'if'
name|'not'
name|'CONF'
op|'.'
name|'cells'
op|'.'
name|'enable'
op|':'
newline|'\n'
indent|'            '
name|'raise_feature_not_supported'
op|'('
op|')'
newline|'\n'
dedent|''
name|'return'
name|'function'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'inner'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|is_all_tenants
dedent|''
name|'def'
name|'is_all_tenants'
op|'('
name|'search_opts'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Checks to see if the all_tenants flag is in search_opts\n\n    :param dict search_opts: The search options for a request\n    :returns: boolean indicating if all_tenants are being requested or not\n    """'
newline|'\n'
name|'all_tenants'
op|'='
name|'search_opts'
op|'.'
name|'get'
op|'('
string|"'all_tenants'"
op|')'
newline|'\n'
name|'if'
name|'all_tenants'
op|':'
newline|'\n'
indent|'        '
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'all_tenants'
op|'='
name|'strutils'
op|'.'
name|'bool_from_string'
op|'('
name|'all_tenants'
op|','
name|'True'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'ValueError'
name|'as'
name|'err'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'InvalidInput'
op|'('
name|'six'
op|'.'
name|'text_type'
op|'('
name|'err'
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
comment|'# The empty string is considered enabling all_tenants'
nl|'\n'
indent|'        '
name|'all_tenants'
op|'='
string|"'all_tenants'"
name|'in'
name|'search_opts'
newline|'\n'
dedent|''
name|'return'
name|'all_tenants'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|image_uuid_from_href
dedent|''
name|'def'
name|'image_uuid_from_href'
op|'('
name|'image_href'
op|','
name|'key'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""If the image href was generated by nova api, strip image_href down to an\n    id and use the default glance connection params\n\n    :param image_href: URL or UUID for an image.\n    :param key: The request body key for the image_href value. This is used in\n        error messages.\n    :returns: The parsed image UUID.\n    :raises: webob.exc.HTTPBadRequest if the image_href does not have a valid\n        image UUID in it.\n    """'
newline|'\n'
name|'if'
name|'not'
name|'image_href'
op|':'
newline|'\n'
comment|'# NOTE(mriedem): This error message is copied from how our jsonschema'
nl|'\n'
comment|'# validation error looks.'
nl|'\n'
indent|'        '
name|'msg'
op|'='
name|'_'
op|'('
string|'"Invalid input for field/attribute %(path)s. "'
nl|'\n'
string|'"Value: %(value)s. %(message)s"'
op|')'
op|'%'
op|'{'
nl|'\n'
string|"'path'"
op|':'
name|'key'
op|','
string|"'value'"
op|':'
name|'image_href'
op|','
nl|'\n'
string|"'message'"
op|':'
op|'('
nl|'\n'
string|"'Invalid image reference format. Specify the image '"
nl|'\n'
string|"'reference by UUID or full URL.'"
op|')'
nl|'\n'
op|'}'
newline|'\n'
name|'raise'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|'('
name|'explanation'
op|'='
name|'msg'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'image_uuid'
op|'='
name|'image_href'
op|'.'
name|'split'
op|'('
string|"'/'"
op|')'
op|'.'
name|'pop'
op|'('
op|')'
newline|'\n'
nl|'\n'
name|'if'
name|'not'
name|'uuidutils'
op|'.'
name|'is_uuid_like'
op|'('
name|'image_uuid'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'msg'
op|'='
name|'_'
op|'('
string|'"Invalid input for field/attribute %(path)s. "'
nl|'\n'
string|'"Value: %(value)s. %(message)s"'
op|')'
op|'%'
op|'{'
nl|'\n'
string|"'path'"
op|':'
name|'key'
op|','
string|"'value'"
op|':'
name|'image_href'
op|','
nl|'\n'
string|"'message'"
op|':'
op|'('
nl|'\n'
string|"'Invalid image reference format. Specify the image '"
nl|'\n'
string|"'reference by UUID or full URL.'"
op|')'
nl|'\n'
op|'}'
newline|'\n'
name|'raise'
name|'exc'
op|'.'
name|'HTTPBadRequest'
op|'('
name|'explanation'
op|'='
name|'msg'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'return'
name|'image_uuid'
newline|'\n'
dedent|''
endmarker|''
end_unit
