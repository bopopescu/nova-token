...
name|'self'
op|'.'
name|'neutron_attempted'
op|'='
name|'False'
newline|'\n'
nl|'\n'
comment|'# Look for implmentation of extension point of server creation'
nl|'\n'
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# Look for implmentation of extension point of server create'
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# Look for implmentation of extension point of server rebuild'
nl|'\n'
dedent|''
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# Look for implmentation of extension point of server resize'
nl|'\n'
dedent|''
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|'# Look for implmentation of extension point of server update'
nl|'\n'
dedent|''
name|'self'
op|'.'
...
op|':'
newline|'\n'
comment|'# NOTE(dprince): neutron is not a requirement'
nl|'\n'
indent|'        '
name|'if'
name|'self'
...
nl|'\n'
comment|'# Replace with an extension point when the security groups'
nl|'\n'
comment|'# extension is ported'
nl|'\n'
dedent|''
name|'sg_names'
op|'='
...
op|']'
newline|'\n'
comment|"#if self.ext_mgr.is_loaded('os-security-groups'):"
nl|'\n'
comment|"#    security_groups = server_dict.get('security_groups')"
nl|'\n'
comment|'#    if security_groups is not None:'
nl|'\n'
comment|"#        sg_names = [sg['name'] for sg in security_groups"
nl|'\n'
comment|"#                    if sg.get('name')]"
nl|'\n'
name|'if'
name|'not'
name|'sg_names'
...
nl|'\n'
comment|'# to take into account _is_neutron_v2'
nl|'\n'
comment|"#if (self.ext_mgr.is_loaded('os-networks')"
nl|'\n'
comment|'#        or self._is_neutron_v2()):'
nl|'\n'
...
comment|'# TODO(cyeoh): bp v3-api-core-as-extensions'
nl|'\n'
comment|'# Replace with an extension point when the os-volumes'
nl|'\n'
comment|'# extension is ported'
nl|'\n'
comment|"#if self.ext_mgr.is_loaded('os-volumes'):"
nl|'\n'
comment|"#    block_device_mapping = server_dict.get('block_device_mapping', [])"
nl|'\n'
comment|'#    for bdm in block_device_mapping:'
nl|'\n'
comment|'#        self._validate_device_name(bdm["device_name"])'
nl|'\n'
comment|"#        if 'delete_on_termination' in bdm:"
nl|'\n'
comment|"#            bdm['delete_on_termination'] = strutils.bool_from_string("
nl|'\n'
comment|"#                bdm['delete_on_termination'])"
nl|'\n'
nl|'\n'
name|'ret_resv_id'
op|'='
...
comment|'# TODO(cyeoh): bp v3-api-core-as-extensions'
nl|'\n'
comment|'# Replace with an extension point when the os-multiple-create'
nl|'\n'
comment|'# extension is ported'
nl|'\n'
comment|"#if self.ext_mgr.is_loaded('os-multiple-create'):"
nl|'\n'
comment|"#    ret_resv_id = server_dict.get('return_reservation_id', False)"
nl|'\n'
comment|"#    min_count = server_dict.get('min_count', 1)"
nl|'\n'
comment|"#    max_count = server_dict.get('max_count', min_count)"
nl|'\n'
nl|'\n'
name|'try'
op|':'
...
name|'_'
op|'('
string|"'min_count must be an integer value'"
op|')'
newline|'\n'
name|'raise'
name|'exc'
...
name|'_'
op|'('
string|"'max_count must be an integer value'"
op|')'
newline|'\n'
name|'raise'
name|'exc'
...
comment|'# TODO(cyeoh): bp v3-api-core-as-extensions'
nl|'\n'
comment|'# Replace with extension point. For the moment'
nl|'\n'
comment|'# with no ep, assume os-volumes is not present'
nl|'\n'
comment|"#if not image_ref and bdm and self.ext_mgr.is_loaded('os-volumes'):"
nl|'\n'
comment|"#    return ''"
nl|'\n'
comment|'#else:'
nl|'\n'
comment|'#    image_href = self._image_ref_from_req_data(data)'
nl|'\n'
comment|'#    image_uuid = self._image_uuid_from_href(image_href)'
nl|'\n'
comment|'#    return image_uuid'
nl|'\n'
name|'image_href'
op|'='
name|'self'
...
newline|'\n'
nl|'\n'
comment|"# Add on the admin_pass attribute since the view doesn't do it"
nl|'\n'
...
nl|'\n'
comment|'# _get_volume_image_metadata only works, when boot'
nl|'\n'
