...
name|'_LW'
op|'('
string|"'Attempted to remove chain %s which does not '"
nl|'\n'
string|"'exist'"
op|')'
op|','
...
nl|'\n'
string|'"%(rule)r already in %(rules)r"'
op|','
nl|'\n'
op|'{'
string|"'rule'"
...
nl|'\n'
nl|'\n'
DECL|function|is_pid_cmdline_correct
dedent|''
dedent|''
name|'def'
...
indent|'        '
name|'return'
string|"'-m addrtype --dst-type LOCAL'"
newline|'\n'
dedent|''
name|'else'
op|':'
...
op|'='
op|'('
string|"'-p tcp -m tcp --dport %s %s -j ACCEPT'"
op|'%'
nl|'\n'
op|'('
name|'CONF'
...
op|']'
newline|'\n'
comment|'# NOTE(vish): if we are in multi-host mode and we are not sharing'
nl|'\n'
comment|'#             addresses, then we actually need to hand out the'
nl|'\n'
comment|'#             dhcp server address as the gateway.'
nl|'\n'
name|'if'
name|'network_ref'
op|'['
...
newline|'\n'
comment|'# NOTE(cfb): dnsmasq on linux only supports 64 characters in the hostname'
nl|'\n'
comment|'#            field (LP #1238910). Since the . counts as a character we need'
nl|'\n'
comment|'#            to truncate the hostname to only 63 characters.'
nl|'\n'
name|'hostname'
op|'='
name|'fixedip'
...
name|'_LW'
op|'('
string|"'hostname %s too long, truncating.'"
op|')'
op|','
name|'hostname'
op|')'
...
op|')'
newline|'\n'
comment|'# NOTE(vish): 3 is the dhcp option for gateway.'
nl|'\n'
dedent|''
name|'values'
op|'.'
...
nl|'\n'
nl|'\n'
DECL|function|_create_ovs_vif_cmd
dedent|''
dedent|''
name|'def'
...
op|')'
newline|'\n'
comment|'# Note at present there is no support for setting the'
nl|'\n'
comment|'# mtu for vhost-user type ports.'
nl|'\n'
name|'if'
name|'interface_type'
op|'!='
...
name|'debug'
op|'('
string|'"MTU not set on %(interface_name)s interface "'
nl|'\n'
string|'"of type %(interface_type)s."'
op|','
nl|'\n'
op|'{'
...
nl|'\n'
nl|'\n'
DECL|function|delete_bridge_dev
dedent|''
dedent|''
dedent|''
dedent|''
name|'def'
name|'delete_bridge_dev'
op|'('
name|'dev'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Delete a network bridge."""'
newline|'\n'
name|'if'
name|'device_exists'
op|'('
...
name|'_LE'
op|'('
string|'"Failed removing bridge device: \'%s\'"'
op|')'
op|','
name|'dev'
op|')'
...
name|'err'
op|'!='
string|'"device %s already exists; can\'t create "'
nl|'\n'
string|'"bridge with the same name\\n"'
op|'%'
op|'('
name|'bridge'
op|')'
...
nl|'\n'
string|'"can\'t ensubordinate it to bridge %s.\\n"'
op|'%'
op|'('
name|'interface'
op|','
...
newline|'\n'
nl|'\n'
comment|"# NOTE(apmelton): Linux bridge's default behavior is to use the"
nl|'\n'
comment|"# lowest mac of all plugged interfaces. This isn't a problem when"
nl|'\n'
comment|'# it is first created and the only interface is the bridged'
nl|'\n'
comment|'# interface. But, as instance interfaces are plugged, there is a'
nl|'\n'
comment|"# chance for the mac to change. So, set it here so that it won't"
nl|'\n'
comment|'# change in the future.'
nl|'\n'
dedent|''
name|'if'
name|'not'
...
nl|'\n'
nl|'\n'
comment|'# NOTE(cfb): Fix for LP #1316621, #1501366.'
nl|'\n'
comment|'#            We call ebtables with --concurrent which causes ebtables to'
nl|'\n'
comment|"#            use a lock file to deal with concurrent calls. Since we can't"
nl|'\n'
comment|'#            be sure the libvirt also uses --concurrent we retry in a loop'
nl|'\n'
comment|'#            to be sure.'
nl|'\n'
comment|'#'
nl|'\n'
comment|"#            ebtables doesn't implement a timeout and doesn't gracefully"
nl|'\n'
comment|'#            handle cleaning up a lock file if someone sends a SIGKILL to'
nl|'\n'
comment|'#            ebtables while its holding a lock. As a result we want to add'
nl|'\n'
comment|'#            a timeout to the ebtables calls but we first need to teach'
nl|'\n'
comment|'#            oslo_concurrency how to do that.'
nl|'\n'
DECL|function|_exec_ebtables
dedent|''
dedent|''
dedent|''
name|'def'
...
newline|'\n'
nl|'\n'
comment|'# List of error strings to re-try.'
nl|'\n'
name|'retry_strings'
op|'='
op|'('
nl|'\n'
string|"'Multiple ebtables programs'"
op|','
nl|'\n'
op|')'
newline|'\n'
nl|'\n'
comment|'# We always try at least once'
nl|'\n'
name|'attempts'
op|'='
name|'CONF'
...
op|':'
newline|'\n'
comment|'# Updated our counters if needed'
nl|'\n'
indent|'        '
name|'sleep'
op|'='
...
number|'1'
newline|'\n'
comment|'# NOTE(cfb): ebtables reports all errors with a return code of 255.'
nl|'\n'
comment|"#            As such we can't know if we hit a locking error, or some"
nl|'\n'
comment|"#            other error (like a rule doesn't exist) so we have to"
nl|'\n'
comment|'#            to parse stderr.'
nl|'\n'
name|'try'
op|':'
newline|'\n'
...
op|':'
newline|'\n'
comment|'# See if we can retry the error.'
nl|'\n'
indent|'            '
name|'if'
name|'any'
...
op|':'
newline|'\n'
comment|'# We need to sleep a bit before retrying'
nl|'\n'
indent|'                    '
name|'LOG'
op|'.'
...
name|'_LW'
op|'('
string|'"%(cmd)s failed. Sleeping %(time)s "'
nl|'\n'
string|'"seconds before retry."'
op|')'
op|','
nl|'\n'
op|'{'
...
op|':'
newline|'\n'
comment|'# Not eligible for retry'
nl|'\n'
indent|'                '
name|'if'
name|'check_exit_code'
...
nl|'\n'
nl|'\n'
DECL|function|set_vf_interface_vlan
name|'def'
...
newline|'\n'
nl|'\n'
comment|"# Set the VF's mac address and vlan"
nl|'\n'
name|'exit_code'
op|'='
op|'['
...
op|')'
newline|'\n'
comment|"# Bring up/down the VF's interface"
nl|'\n'
name|'utils'
op|'.'
name|'execute'
