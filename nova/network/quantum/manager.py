...
op|':'
newline|'\n'
comment|"# Don't update host information for network that does not"
nl|'\n'
comment|'# belong to you'
nl|'\n'
indent|'            '
name|'if'
name|'net'
...
newline|'\n'
nl|'\n'
comment|'# Similar to FlatDHCPMananger, except we check for quantum_use_dhcp flag'
nl|'\n'
comment|'# before we try to update_dhcp'
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_update_network_host
dedent|''
dedent|''
dedent|''
name|'def'
...
nl|'\n'
string|'"network for tenant \'%(q_tenant_id)s\' "'
nl|'\n'
string|'"with net-id \'%(quantum_net_id)s\'"'
op|')'
op|'%'
nl|'\n'
name|'locals'
...
newline|'\n'
nl|'\n'
comment|'# only delete gw ports if we are going to finish deleting network'
nl|'\n'
dedent|''
name|'if'
name|'gw_port_uuid'
...
name|'debug'
op|'('
string|'"Deleting network %s for tenant: %s"'
op|'%'
nl|'\n'
op|'('
name|'net_uuid'
...
op|':'
newline|'\n'
comment|'# need to figure out if a requested network is owned'
nl|'\n'
comment|'# by the tenant, or by the provider'
nl|'\n'
comment|'# Note: these are the only possible options, as the compute'
nl|'\n'
comment|'# API already validated networks using validate_network()'
nl|'\n'
indent|'            '
name|'proj_net_ids'
op|'='
...
op|':'
newline|'\n'
comment|'# This is not returning the instance IDs like the method name'
nl|'\n'
comment|'# would make you think; it is matching the return format of'
nl|'\n'
comment|"# the method it's overriding."
nl|'\n'
indent|'        '
name|'instance_ids'
op|'='
...
newline|'\n'
nl|'\n'
DECL|member|deallocate_ip_address
dedent|''
dedent|''
name|'def'
name|'deallocate_ip_address'
...
newline|'\n'
nl|'\n'
comment|'# enable_dhcp() could also use this'
nl|'\n'
comment|'#'
nl|'\n'
comment|'# Use semaphore to :'
nl|'\n'
comment|'# 1) avoid race between restart_dhcps'
nl|'\n'
comment|'# 2) avoid race between update_dhcp_hostfile_with_texts'
nl|'\n'
dedent|''
op|'@'
name|'utils'
...
name|'synchronized'
op|'('
string|"'quantum-restart-dhcp'"
op|')'
newline|'\n'
DECL|member|_atomic_restart_dhcp
name|'def'
name|'_atomic_restart_dhcp'
op|'('
name|'self'
op|','
name|'context'
...
op|')'
newline|'\n'
comment|"# Since we only update hosts file, explict kill_dhcp() isn't needed."
nl|'\n'
comment|'# restart_dhcp() will reload hostfile if dnsmasq is already running,'
nl|'\n'
comment|'# or start new dnsmasq'
nl|'\n'
name|'self'
op|'.'
name|'driver'
...
op|':'
newline|'\n'
comment|'# no host specific setup is needed in quantum manager'
nl|'\n'
indent|'        '
name|'pass'
newline|'\n'
