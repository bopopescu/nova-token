...
newline|'\n'
nl|'\n'
comment|'# NOTE(s0mik) : If DHCP is not in use, we need to timeout IPs'
nl|'\n'
comment|'# periodically.  See comment in deallocate_ips_by_vif for more'
nl|'\n'
name|'self'
op|'.'
name|'net_manager'
...
op|')'
newline|'\n'
comment|'# NOTE(s0mik): Sets fixed-ip to deallocated, but leaves the entry'
nl|'\n'
comment|'# associated with the instance-id.  This prevents us from handing it'
nl|'\n'
comment|'# out again immediately, as allocating it to a new instance before'
nl|'\n'
comment|'# a DHCP lease has timed-out is bad.  Instead, the fixed-ip will'
nl|'\n'
comment|'# be disassociated with the instance-id by a call to one of two'
nl|'\n'
comment|'# methods inherited from FlatManager:'
nl|'\n'
comment|'# - if DHCP is in use, a lease expiring in dnsmasq triggers'
nl|'\n'
comment|'#   a call to release_fixed_ip in the network manager.'
nl|'\n'
comment|'# - otherwise, _disassociate_stale_fixed_ips is called periodically'
nl|'\n'
comment|'#   to disassociate all fixed ips that are unallocated'
nl|'\n'
comment|'#   but still associated with an instance-id.'
nl|'\n'
nl|'\n'
