...
name|'self'
op|'.'
name|'_get_flat_networks'
op|'('
name|'context'
op|')'
newline|'\n'
nl|'\n'
DECL|member|_get_flat_networks
dedent|''
name|'def'
name|'_get_flat_networks'
op|'('
...
newline|'\n'
indent|'        '
string|'"""returns all networks for which vlan is None"""'
newline|'\n'
name|'networks'
op|'='
name|'self'
...
newline|'\n'
nl|'\n'
comment|'# deallocate mac addresses'
nl|'\n'
name|'self'
op|'.'
name|'db'
...
op|')'
newline|'\n'
comment|'# NOTE(tr3buchet) from vish, may need to check to make sure'
nl|'\n'
comment|'#                 disassocate worked in the future'
nl|'\n'
name|'args'
op|'='
op|'{'
...
op|')'
newline|'\n'
comment|'# then deallocate fixed_ip'
nl|'\n'
dedent|''
name|'self'
op|'.'
...
newline|'\n'
indent|'        '
string|'"""unpickles instance and calls _create_nw_info\n           then pickles its return for passing through rpc\n\n        rpc.called by network_api\n        instance is expected to be pickled\n        return value is also pickled\n        """'
newline|'\n'
name|'instance'
op|'='
name|'pickle'
...
name|'_'
op|'('
string|'"getting network info for instance |%s|"'
op|')'
op|','
name|'instance'
op|'['
...
op|']'
newline|'\n'
comment|'# a mac_address contains address, instance_id, network_id'
nl|'\n'
...
newline|'\n'
indent|'        '
string|'"""generates and stores mac addresses"""'
newline|'\n'
name|'for'
name|'network'
name|'in'
...
op|'}'
newline|'\n'
comment|'# try 5 times to create a unique mac_address'
nl|'\n'
name|'for'
name|'i'
name|'in'
...
name|'_'
op|'('
string|'"5 attempts at create failed"'
op|')'
op|')'
newline|'\n'
nl|'\n'
...
newline|'\n'
nl|'\n'
DECL|member|_get_mac_addr_for_fixed_ip
dedent|''
name|'def'
name|'_get_mac_addr_for_fixed_ip'
op|'('
name|'self'
op|','
name|'context'
...
op|')'
newline|'\n'
comment|'# determine the mac_address the instance has for the network'
nl|'\n'
comment|'# that the fixed ip address belongs to'
nl|'\n'
name|'for'
name|'mac_addr'
name|'in'
...
op|':'
newline|'\n'
comment|'# return it since there can be only one'
nl|'\n'
indent|'                '
name|'return'
name|'mac_addr'
...
newline|'\n'
nl|'\n'
comment|"# instance doesn't have a mac address for the fixed_ip's network"
nl|'\n'
dedent|''
dedent|''
name|'return'
...
nl|'\n'
string|'" mac %(mac_address)s vs %(mac)s"'
op|')'
op|'%'
name|'locals'
op|'('
...
nl|'\n'
string|'" bad mac %(mac_address)s vs %(mac)s"'
op|')'
op|'%'
name|'locals'
op|'('
...
newline|'\n'
indent|'        '
string|'"""Set the network hosts for the flat networks, if they are unset"""'
newline|'\n'
name|'networks'
op|'='
name|'self'
...
string|'"""determine which networks an instance should connect to"""'
newline|'\n'
comment|'# get network associated with project'
nl|'\n'
comment|'# TODO(tr3buchet): currently there can be only one, but this should'
nl|'\n'
comment|'# change. when it does this should be project_get_networks'
nl|'\n'
name|'return'
name|'self'
op|'.'
