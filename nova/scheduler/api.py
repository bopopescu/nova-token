...
op|':'
newline|'\n'
comment|"# NOTE(sirp): since we're not re-routing in this case, and we"
nl|'\n'
comment|'# we were passed a UUID, we need to replace that UUID with an'
nl|'\n'
comment|'# integer ID in the argument list so that the zone-local code'
nl|'\n'
comment|'# can continue to use integer IDs.'
nl|'\n'
indent|'                    '
name|'item_id'
op|'='
...
op|':'
newline|'\n'
comment|'# NOTE(sirp): args comes in as a tuple, so we need to convert it'
nl|'\n'
comment|'# to a list to mutate it, and then convert it back to a tuple'
nl|'\n'
indent|'            '
name|'args'
op|'.'
