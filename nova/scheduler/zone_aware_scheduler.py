...
name|'self'
op|'.'
name|'_provision_resource_in_child_zone'
op|'('
name|'context'
op|','
name|'item'
...
newline|'\n'
nl|'\n'
DECL|member|_provision_resource_in_child_zone
dedent|''
name|'def'
name|'_provision_resource_in_child_zone'
op|'('
...
newline|'\n'
indent|'        '
string|'"""Create the requested resource in a child zone."""'
newline|'\n'
comment|'# Start by attempting to decrypt the blob to see if this'
nl|'\n'
comment|'# request is:'
nl|'\n'
comment|'# 1. valid, '
nl|'\n'
comment|'# 2. intended for this zone or a child zone.'
nl|'\n'
comment|'# if 2 ... forward call to child zone.'
nl|'\n'
comment|'# Note: If we have "blob" that means the request was passed'
nl|'\n'
comment|'# into us. If we have "child_blob" that means we just asked'
nl|'\n'
comment|'# the child zone for the weight info.'
nl|'\n'
name|'LOG'
op|'.'
name|'debug'
...
name|'_'
op|'('
string|'"****** PROVISION IN CHILD %(item)s"'
op|')'
op|'%'
name|'locals'
op|'('
...
name|'_'
op|'('
string|'"Ill-formed or incorrectly "'
nl|'\n'
string|'"routed \'blob\' data sent "'
nl|'\n'
...
newline|'\n'
nl|'\n'
comment|'# Note: we have to reverse engineer from our args to get back the'
nl|'\n'
comment|'# image, flavor, ipgroup, etc. since the original call could have'
nl|'\n'
comment|"# come in from EC2 (which doesn't use these things)."
nl|'\n'
indent|'        '
name|'LOG'
op|'.'
...
name|'_'
op|'('
string|'"****** ASK CHILD %(zone_info)s ** %(request_spec)s"'
op|')'
op|'%'
name|'locals'
op|'('
...
name|'_'
op|'('
string|'"XXXXXXX - SELECT %(request_spec)s"'
op|')'
op|'%'
name|'locals'
op|'('
...
name|'_'
op|'('
string|'"XXXXXXX - DEFAULT SCHEDULE %(request_spec)s"'
op|')'
op|'%'
name|'locals'
op|'('
...
name|'_'
op|'('
string|'"XXXXXXX - 4 -  _SCHEDULE - CHILD RESULTS %(child_results)s"'
op|')'
op|'%'
name|'locals'
op|'('
