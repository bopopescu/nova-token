...
op|'}'
newline|'\n'
DECL|variable|_MAIN_FACADE
name|'_MAIN_FACADE'
op|'='
string|"'main'"
newline|'\n'
DECL|variable|_API_FACADE
name|'_API_FACADE'
op|'='
string|"'api'"
newline|'\n'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(dheeraj): This fragment is copied from oslo.db'
nl|'\n'
indent|'    '
name|'return'
name|'db_session'
...
nl|'\n'
nl|'\n'
DECL|function|_check_instance_exists
dedent|''
name|'def'
...
newline|'\n'
nl|'\n'
comment|"# Extract 'expected' values, which aren't updates"
nl|'\n'
dedent|''
name|'expected'
op|'='
...
nl|'\n'
nl|'\n'
comment|'# If we were called from instance_update_and_get_original, in by far'
nl|'\n'
comment|'# the most probable race scenario the original object we fetched was'
nl|'\n'
comment|'# already out of date. This means we can check it here to work out why'
nl|'\n'
comment|'# and raise the most specific exception possible. However, note that'
nl|'\n'
comment|'# there is also a much less likely race between _instance_get_by_uuid'
nl|'\n'
comment|'# called by instance_update_and_get_original and update_on_match called'
nl|'\n'
comment|'# by this function. In this case, though, the original call to'
nl|'\n'
comment|'# _instance_get_by_uuid will have created a MySQL read view, which'
nl|'\n'
comment|'# means that fetching it again here will continue to return old data'
nl|'\n'
comment|"# despite update_on_match having failed due to new data we can't see."
nl|'\n'
comment|'# Consequently, fetching it again here in the same db session is'
nl|'\n'
comment|'# pointless. We could fetch it in a separate db transaction, but as'
nl|'\n'
comment|'# that would also have the possibility of racing we would just be'
nl|'\n'
comment|'# chasing our tail. Instead, we handle the less likely race by falling'
nl|'\n'
comment|'# through below.'
nl|'\n'
nl|'\n'
comment|'# If we were called from instance_update, again the most likely race is'
nl|'\n'
comment|'# that the object was already out of date. There will be no existing'
nl|'\n'
comment|'# MySQL read view as there is no prior select, so _instance_get_by_uuid'
nl|'\n'
comment|'# will always return current data, and we can raise a specific'
nl|'\n'
comment|'# exception. However, there is the exceptionally unlikely possibility'
nl|'\n'
comment|'# of a race between update_on_match and _instance_get_by_uuid below,'
nl|'\n'
comment|'# where additionally the intervening update creates the expected'
nl|'\n'
comment|'# conditions which previously failed. Again, we handle this safely by'
nl|'\n'
comment|'# falling through below.'
nl|'\n'
indent|'        '
name|'if'
name|'original'
...
newline|'\n'
nl|'\n'
comment|'# We got here because we raced between fetching and updating the'
nl|'\n'
comment|'# instance in this transaction as described above. Consequently we'
nl|'\n'
comment|"# don't know what the conflict was, only that there was one."
nl|'\n'
dedent|''
dedent|''
name|'raise'
...
op|':'
newline|'\n'
comment|"# We can't go over-quota if we're not reserving anything or if"
nl|'\n'
comment|'# we have unlimited quotas.'
nl|'\n'
indent|'        '
name|'if'
name|'user_quotas'
...
nl|'\n'
nl|'\n'
comment|'# NOTE(snikitin): We have one table (tags) which is not'
nl|'\n'
comment|'# subclass of NovaBase. That is way this table does not contain'
nl|'\n'
comment|"# column 'deleted'"
nl|'\n'
indent|'    '
name|'if'
string|"'deleted'"
...
name|'_'
op|'('
string|'"IntegrityError detected when archiving table %s"'
op|')'
op|'%'
name|'tablename'
newline|'\n'
...
nl|'\n'
nl|'\n'
DECL|function|_augment_flavor_to_migrate
dedent|''
name|'def'
name|'_augment_flavor_to_migrate'
op|'('
name|'flavor_to_migrate'
op|','
name|'full_flavor'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Make sure that extra_specs on the flavor to migrate is updated."""'
newline|'\n'
name|'if'
name|'not'
name|'flavor_to_migrate'
...
nl|'\n'
nl|'\n'
DECL|function|_augment_flavors_to_migrate
dedent|''
dedent|''
dedent|''
name|'def'
...
nl|'\n'
nl|'\n'
DECL|function|_load_missing_flavor
dedent|''
dedent|''
name|'def'
name|'_load_missing_flavor'
op|'('
name|'instance'
op|','
name|'flavor_cache'
...
name|'require_admin_context'
newline|'\n'
DECL|function|migrate_flavor_data
name|'def'
...
op|':'
newline|'\n'
comment|'# NOTE(danms): This is only ever run in nova-manage, and we need to avoid'
nl|'\n'
comment|'# a circular import'
nl|'\n'
indent|'    '
name|'from'
name|'nova'
...
op|')'
newline|'\n'
comment|"# NOTE(danms): Don't touch instances that are likely in the"
nl|'\n'
comment|'# middle of some other operation. This is just a guess and not'
nl|'\n'
comment|"# a lock. There is still a race here, although it's the same"
nl|'\n'
comment|'# race as the normal code, since we use expected_task_state below.'
nl|'\n'
name|'if'
name|'not'
name|'force'
...
newline|'\n'
nl|'\n'
comment|'# NOTE(danms): If we have a really old instance with no flavor'
nl|'\n'
comment|'# information at all, flavor will not have been set during load.'
nl|'\n'
comment|"# If that's the case, look up the flavor by id (which implies that"
nl|'\n'
comment|'# old_ and new_flavor are None). No need to augment with extra_specs'
nl|'\n'
comment|"# since we're doing the lookup from scratch."
nl|'\n'
dedent|''
dedent|''
name|'if'
...
name|'_LE'
op|'('
string|"'Unable to lookup flavor for legacy instance; '"
nl|'\n'
string|"'migration is not possible without manual '"
nl|'\n'
string|"'intervention'"
op|')'
op|','
