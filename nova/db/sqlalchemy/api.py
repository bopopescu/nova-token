...
op|')'
newline|'\n'
comment|'# NOTE(klmitch): for some reason, using the SQLAlchemy code'
nl|'\n'
comment|'# here instead of the direct SQL update above causes the'
nl|'\n'
comment|'# test_run_terminate_timestamps test (and only that one) to'
nl|'\n'
comment|'# fail with an obscure TypeError exception from deep within'
nl|'\n'
comment|'# SQLAlchemy; the nearest nova function in the traceback is'
nl|'\n'
comment|'# instance_get()'
nl|'\n'
comment|'# session.query(models.Instance).\\'
nl|'\n'
comment|'#              filter_by(id=instance_id).\\'
nl|'\n'
comment|"#              update({'deleted': 1,"
nl|'\n'
comment|"#                      'deleted_at': datetime.datetime.utcnow(),"
nl|'\n'
comment|"#                      'updated_at': models.Instance.updated_at + 0})"
nl|'\n'
name|'session'
op|'.'
name|'query'
