...
op|':'
newline|'\n'
comment|'# NOTE(dprince): The dump_ tables were originally created from an'
nl|'\n'
comment|"# earlier schema version so we don't want to add the pci_stats"
nl|'\n'
comment|'# column so that schema diffs are exactly the same.'
nl|'\n'
indent|'            '
name|'if'
name|'column'
...
newline|'\n'
nl|'\n'
comment|"# NOTE(mriedem): DB2 can't create the FK since we don't have the unique"
nl|'\n'
comment|"# constraint on instances.uuid because it's nullable (so a unique"
nl|'\n'
comment|"# constraint isn't created for instances.uuid, only a unique index)."
nl|'\n'
name|'consoles_instance_uuid_column_args'
op|'='
op|'['
string|"'instance_uuid'"
op|','
...
newline|'\n'
nl|'\n'
comment|"# NOTE(mriedem): DB2 doesn't allow duplicate indexes either."
nl|'\n'
name|'DB2_INDEX_SKIPS'
op|'='
name|'POSTGRES_INDEX_SKIPS'
newline|'\n'
nl|'\n'
name|'MYSQL_INDEX_SKIPS'
op|'='
op|'['
nl|'\n'
comment|'# we create this one manually for MySQL above'
nl|'\n'
string|"'migrations_by_host_nodes_and_status_idx'"
nl|'\n'
op|']'
...
newline|'\n'
nl|'\n'
comment|"# NOTE(mriedem): DB2 doesn't support unique constraints on columns that"
nl|'\n'
comment|'# are nullable so we can only create foreign keys on unique constraints'
nl|'\n'
comment|'# that actually exist, which excludes any FK on instances.uuid.'
nl|'\n'
name|'if'
name|'migrate_engine'
op|'.'
...
nl|'\n'
indent|'        '
name|'secgroup_instance_association_instance_uuid_fkey'
op|'='
op|'('
nl|'\n'
...
op|':'
newline|'\n'
comment|'# For MySQL and DB2 we name our fkeys explicitly'
nl|'\n'
comment|'# so they match Havana'
nl|'\n'
indent|'            '
name|'fkey'
op|'='
