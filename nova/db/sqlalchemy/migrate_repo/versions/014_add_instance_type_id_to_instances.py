begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright 2010 OpenStack LLC.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
name|'from'
name|'sqlalchemy'
name|'import'
op|'*'
newline|'\n'
name|'from'
name|'sqlalchemy'
op|'.'
name|'sql'
name|'import'
name|'text'
newline|'\n'
name|'from'
name|'migrate'
name|'import'
op|'*'
newline|'\n'
nl|'\n'
comment|'#from nova import log as logging'
nl|'\n'
nl|'\n'
nl|'\n'
DECL|variable|meta
name|'meta'
op|'='
name|'MetaData'
op|'('
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|c_instance_type
name|'c_instance_type'
op|'='
name|'Column'
op|'('
string|"'instance_type'"
op|','
nl|'\n'
name|'String'
op|'('
name|'length'
op|'='
number|'255'
op|','
name|'convert_unicode'
op|'='
name|'False'
op|','
nl|'\n'
name|'assert_unicode'
op|'='
name|'None'
op|','
name|'unicode_error'
op|'='
name|'None'
op|','
nl|'\n'
DECL|variable|_warn_on_bytestring
name|'_warn_on_bytestring'
op|'='
name|'False'
op|')'
op|','
nl|'\n'
DECL|variable|nullable
name|'nullable'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|variable|c_instance_type_id
name|'c_instance_type_id'
op|'='
name|'Column'
op|'('
string|"'instance_type_id'"
op|','
nl|'\n'
name|'String'
op|'('
name|'length'
op|'='
number|'255'
op|','
name|'convert_unicode'
op|'='
name|'False'
op|','
nl|'\n'
name|'assert_unicode'
op|'='
name|'None'
op|','
name|'unicode_error'
op|'='
name|'None'
op|','
nl|'\n'
DECL|variable|_warn_on_bytestring
name|'_warn_on_bytestring'
op|'='
name|'False'
op|')'
op|','
nl|'\n'
DECL|variable|nullable
name|'nullable'
op|'='
name|'True'
op|')'
newline|'\n'
nl|'\n'
DECL|variable|instance_types
name|'instance_types'
op|'='
name|'Table'
op|'('
string|"'instance_types'"
op|','
name|'meta'
op|','
nl|'\n'
name|'Column'
op|'('
string|"'id'"
op|','
name|'Integer'
op|'('
op|')'
op|','
name|'primary_key'
op|'='
name|'True'
op|','
name|'nullable'
op|'='
name|'False'
op|')'
op|','
nl|'\n'
name|'Column'
op|'('
string|"'name'"
op|','
nl|'\n'
name|'String'
op|'('
name|'length'
op|'='
number|'255'
op|','
name|'convert_unicode'
op|'='
name|'False'
op|','
name|'assert_unicode'
op|'='
name|'None'
op|','
nl|'\n'
name|'unicode_error'
op|'='
name|'None'
op|','
name|'_warn_on_bytestring'
op|'='
name|'False'
op|')'
op|','
nl|'\n'
DECL|variable|unique
name|'unique'
op|'='
name|'True'
op|')'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|upgrade
name|'def'
name|'upgrade'
op|'('
name|'migrate_engine'
op|')'
op|':'
newline|'\n'
comment|"# Upgrade operations go here. Don't create your own engine;"
nl|'\n'
comment|'# bind migrate_engine to your metadata'
nl|'\n'
indent|'    '
name|'meta'
op|'.'
name|'bind'
op|'='
name|'migrate_engine'
newline|'\n'
nl|'\n'
name|'instances'
op|'='
name|'Table'
op|'('
string|"'instances'"
op|','
name|'meta'
op|','
name|'autoload'
op|'='
name|'True'
op|','
nl|'\n'
name|'autoload_with'
op|'='
name|'migrate_engine'
op|')'
newline|'\n'
nl|'\n'
name|'instances'
op|'.'
name|'create_column'
op|'('
name|'c_instance_type_id'
op|')'
newline|'\n'
nl|'\n'
name|'recs'
op|'='
name|'migrate_engine'
op|'.'
name|'execute'
op|'('
name|'instance_types'
op|'.'
name|'select'
op|'('
op|')'
op|')'
newline|'\n'
name|'for'
name|'row'
name|'in'
name|'recs'
op|':'
newline|'\n'
indent|'        '
name|'type_id'
op|'='
name|'row'
op|'['
number|'0'
op|']'
newline|'\n'
name|'type_name'
op|'='
name|'row'
op|'['
number|'1'
op|']'
newline|'\n'
name|'migrate_engine'
op|'.'
name|'execute'
op|'('
name|'instances'
op|'.'
name|'update'
op|'('
op|')'
op|'.'
name|'where'
op|'('
name|'instances'
op|'.'
name|'c'
op|'.'
name|'instance_type'
op|'=='
name|'type_name'
op|')'
op|'.'
name|'values'
op|'('
name|'instance_type_id'
op|'='
name|'type_id'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'instances'
op|'.'
name|'c'
op|'.'
name|'instance_type'
op|'.'
name|'drop'
op|'('
op|')'
newline|'\n'
comment|'#instances.c.instance_type_id.alter(nullable=False)'
nl|'\n'
nl|'\n'
nl|'\n'
DECL|function|downgrade
dedent|''
name|'def'
name|'downgrade'
op|'('
name|'migrate_engine'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'meta'
op|'.'
name|'bind'
op|'='
name|'migrate_engine'
newline|'\n'
nl|'\n'
name|'instances'
op|'='
name|'Table'
op|'('
string|"'instances'"
op|','
name|'meta'
op|','
name|'autoload'
op|'='
name|'True'
op|','
nl|'\n'
name|'autoload_with'
op|'='
name|'migrate_engine'
op|')'
newline|'\n'
nl|'\n'
name|'instances'
op|'.'
name|'create_column'
op|'('
name|'c_instance_type'
op|')'
newline|'\n'
nl|'\n'
name|'recs'
op|'='
name|'migrate_engine'
op|'.'
name|'execute'
op|'('
name|'instance_types'
op|'.'
name|'select'
op|'('
op|')'
op|')'
newline|'\n'
name|'for'
name|'row'
name|'in'
name|'recs'
op|':'
newline|'\n'
indent|'        '
name|'type_id'
op|'='
name|'row'
op|'['
number|'0'
op|']'
newline|'\n'
name|'type_name'
op|'='
name|'row'
op|'['
number|'1'
op|']'
newline|'\n'
name|'migrate_engine'
op|'.'
name|'execute'
op|'('
name|'instances'
op|'.'
name|'update'
op|'('
op|')'
op|'.'
name|'where'
op|'('
name|'instances'
op|'.'
name|'c'
op|'.'
name|'instance_type_id'
op|'=='
name|'type_id'
op|')'
op|'.'
name|'values'
op|'('
name|'instance_type'
op|'='
name|'type_name'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'instances'
op|'.'
name|'c'
op|'.'
name|'instance_type_id'
op|'.'
name|'drop'
op|'('
op|')'
newline|'\n'
comment|'#instances.c.instance_type.alter(nullable=False)'
nl|'\n'
dedent|''
endmarker|''
end_unit
