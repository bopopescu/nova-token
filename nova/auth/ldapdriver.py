begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright 2010 United States Government as represented by the'
nl|'\n'
comment|'# Administrator of the National Aeronautics and Space Administration.'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
string|'"""\nAuth driver for ldap.  Includes FakeLdapDriver.\n\nIt should be easy to create a replacement for this driver supporting\nother backends by creating another class that exposes the same\npublic methods.\n"""'
newline|'\n'
nl|'\n'
name|'import'
name|'functools'
newline|'\n'
name|'import'
name|'sys'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'flags'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'log'
name|'as'
name|'logging'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'openstack'
op|'.'
name|'common'
name|'import'
name|'cfg'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|ldap_opts
name|'ldap_opts'
op|'='
op|'['
nl|'\n'
name|'cfg'
op|'.'
name|'IntOpt'
op|'('
string|"'ldap_schema_version'"
op|','
nl|'\n'
DECL|variable|default
name|'default'
op|'='
number|'2'
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'Current version of the LDAP schema'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_url'"
op|','
nl|'\n'
DECL|variable|default
name|'default'
op|'='
string|"'ldap://localhost'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'Point this at your ldap server'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_password'"
op|','
nl|'\n'
DECL|variable|default
name|'default'
op|'='
string|"'changeme'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'LDAP password'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_user_dn'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'cn=Manager,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'DN of admin user'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_user_id_attribute'"
op|','
nl|'\n'
DECL|variable|default
name|'default'
op|'='
string|"'uid'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'Attribute to use as id'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_user_name_attribute'"
op|','
nl|'\n'
DECL|variable|default
name|'default'
op|'='
string|"'cn'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'Attribute to use as name'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_user_unit'"
op|','
nl|'\n'
DECL|variable|default
name|'default'
op|'='
string|"'Users'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'OID for Users'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_user_subtree'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'ou=Users,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'OU for Users'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'BoolOpt'
op|'('
string|"'ldap_user_modify_only'"
op|','
nl|'\n'
DECL|variable|default
name|'default'
op|'='
name|'False'
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'Modify user attributes instead of creating/deleting'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_project_subtree'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'ou=Groups,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'OU for Projects'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'role_project_subtree'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'ou=Groups,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'OU for Roles'"
op|')'
op|','
nl|'\n'
nl|'\n'
comment|"# NOTE(vish): mapping with these flags is necessary because we're going"
nl|'\n'
comment|'#             to tie in to an existing ldap schema'
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_cloudadmin'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'cn=cloudadmins,ou=Groups,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'cn for Cloud Admins'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_itsec'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'cn=itsec,ou=Groups,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'cn for ItSec'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_sysadmin'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'cn=sysadmins,ou=Groups,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'cn for Sysadmins'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_netadmin'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'cn=netadmins,ou=Groups,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'cn for NetAdmins'"
op|')'
op|','
nl|'\n'
name|'cfg'
op|'.'
name|'StrOpt'
op|'('
string|"'ldap_developer'"
op|','
nl|'\n'
name|'default'
op|'='
string|"'cn=developers,ou=Groups,dc=example,dc=com'"
op|','
nl|'\n'
DECL|variable|help
name|'help'
op|'='
string|"'cn for Developers'"
op|')'
op|','
nl|'\n'
op|']'
newline|'\n'
nl|'\n'
DECL|variable|FLAGS
name|'FLAGS'
op|'='
name|'flags'
op|'.'
name|'FLAGS'
newline|'\n'
name|'FLAGS'
op|'.'
name|'add_options'
op|'('
name|'ldap_opts'
op|')'
newline|'\n'
nl|'\n'
DECL|variable|LOG
name|'LOG'
op|'='
name|'logging'
op|'.'
name|'getLogger'
op|'('
string|'"nova.ldapdriver"'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
name|'if'
name|'FLAGS'
op|'.'
name|'memcached_servers'
op|':'
newline|'\n'
indent|'    '
name|'import'
name|'memcache'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'    '
name|'from'
name|'nova'
op|'.'
name|'testing'
op|'.'
name|'fake'
name|'import'
name|'memcache'
newline|'\n'
nl|'\n'
nl|'\n'
comment|'# TODO(vish): make an abstract base class with the same public methods'
nl|'\n'
comment|"#             to define a set interface for AuthDrivers. I'm delaying"
nl|'\n'
comment|"#             creating this now because I'm expecting an auth refactor"
nl|'\n'
comment|'#             in which we may want to change the interface a bit more.'
nl|'\n'
nl|'\n'
nl|'\n'
DECL|function|_clean
dedent|''
name|'def'
name|'_clean'
op|'('
name|'attr'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Clean attr for insertion into ldap"""'
newline|'\n'
name|'if'
name|'attr'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'None'
newline|'\n'
dedent|''
name|'if'
name|'isinstance'
op|'('
name|'attr'
op|','
name|'unicode'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'str'
op|'('
name|'attr'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'attr'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|sanitize
dedent|''
name|'def'
name|'sanitize'
op|'('
name|'fn'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Decorator to sanitize all args"""'
newline|'\n'
op|'@'
name|'functools'
op|'.'
name|'wraps'
op|'('
name|'fn'
op|')'
newline|'\n'
DECL|function|_wrapped
name|'def'
name|'_wrapped'
op|'('
name|'self'
op|','
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'args'
op|'='
op|'['
name|'_clean'
op|'('
name|'x'
op|')'
name|'for'
name|'x'
name|'in'
name|'args'
op|']'
newline|'\n'
name|'kwargs'
op|'='
name|'dict'
op|'('
op|'('
name|'k'
op|','
name|'_clean'
op|'('
name|'v'
op|')'
op|')'
name|'for'
op|'('
name|'k'
op|','
name|'v'
op|')'
name|'in'
name|'kwargs'
op|')'
newline|'\n'
name|'return'
name|'fn'
op|'('
name|'self'
op|','
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
dedent|''
name|'_wrapped'
op|'.'
name|'func_name'
op|'='
name|'fn'
op|'.'
name|'func_name'
newline|'\n'
name|'return'
name|'_wrapped'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|LDAPWrapper
dedent|''
name|'class'
name|'LDAPWrapper'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
DECL|member|__init__
indent|'    '
name|'def'
name|'__init__'
op|'('
name|'self'
op|','
name|'ldap'
op|','
name|'url'
op|','
name|'user'
op|','
name|'password'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'ldap'
op|'='
name|'ldap'
newline|'\n'
name|'self'
op|'.'
name|'url'
op|'='
name|'url'
newline|'\n'
name|'self'
op|'.'
name|'user'
op|'='
name|'user'
newline|'\n'
name|'self'
op|'.'
name|'password'
op|'='
name|'password'
newline|'\n'
name|'self'
op|'.'
name|'conn'
op|'='
name|'None'
newline|'\n'
nl|'\n'
DECL|member|__wrap_reconnect
dedent|''
name|'def'
name|'__wrap_reconnect'
op|'('
name|'f'
op|')'
op|':'
newline|'\n'
DECL|function|inner
indent|'        '
name|'def'
name|'inner'
op|'('
name|'self'
op|','
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'self'
op|'.'
name|'conn'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'                '
name|'self'
op|'.'
name|'connect'
op|'('
op|')'
newline|'\n'
name|'return'
name|'f'
op|'('
name|'self'
op|'.'
name|'conn'
op|')'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'try'
op|':'
newline|'\n'
indent|'                    '
name|'return'
name|'f'
op|'('
name|'self'
op|'.'
name|'conn'
op|')'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'SERVER_DOWN'
op|':'
newline|'\n'
indent|'                    '
name|'self'
op|'.'
name|'connect'
op|'('
op|')'
newline|'\n'
name|'return'
name|'f'
op|'('
name|'self'
op|'.'
name|'conn'
op|')'
op|'('
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'return'
name|'inner'
newline|'\n'
nl|'\n'
DECL|member|connect
dedent|''
name|'def'
name|'connect'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'conn'
op|'='
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'initialize'
op|'('
name|'self'
op|'.'
name|'url'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'conn'
op|'.'
name|'simple_bind_s'
op|'('
name|'self'
op|'.'
name|'user'
op|','
name|'self'
op|'.'
name|'password'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'SERVER_DOWN'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'conn'
op|'='
name|'None'
newline|'\n'
name|'raise'
newline|'\n'
nl|'\n'
DECL|variable|search_s
dedent|''
dedent|''
name|'search_s'
op|'='
name|'__wrap_reconnect'
op|'('
name|'lambda'
name|'conn'
op|':'
name|'conn'
op|'.'
name|'search_s'
op|')'
newline|'\n'
DECL|variable|add_s
name|'add_s'
op|'='
name|'__wrap_reconnect'
op|'('
name|'lambda'
name|'conn'
op|':'
name|'conn'
op|'.'
name|'add_s'
op|')'
newline|'\n'
DECL|variable|delete_s
name|'delete_s'
op|'='
name|'__wrap_reconnect'
op|'('
name|'lambda'
name|'conn'
op|':'
name|'conn'
op|'.'
name|'delete_s'
op|')'
newline|'\n'
DECL|variable|modify_s
name|'modify_s'
op|'='
name|'__wrap_reconnect'
op|'('
name|'lambda'
name|'conn'
op|':'
name|'conn'
op|'.'
name|'modify_s'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|LdapDriver
dedent|''
name|'class'
name|'LdapDriver'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Ldap Auth driver\n\n    Defines enter and exit and therefore supports the with/as syntax.\n    """'
newline|'\n'
nl|'\n'
DECL|variable|project_pattern
name|'project_pattern'
op|'='
string|"'(owner=*)'"
newline|'\n'
DECL|variable|isadmin_attribute
name|'isadmin_attribute'
op|'='
string|"'isNovaAdmin'"
newline|'\n'
DECL|variable|project_attribute
name|'project_attribute'
op|'='
string|"'owner'"
newline|'\n'
DECL|variable|project_objectclass
name|'project_objectclass'
op|'='
string|"'groupOfNames'"
newline|'\n'
DECL|variable|conn
name|'conn'
op|'='
name|'None'
newline|'\n'
DECL|variable|mc
name|'mc'
op|'='
name|'None'
newline|'\n'
nl|'\n'
DECL|member|__init__
name|'def'
name|'__init__'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Imports the LDAP module"""'
newline|'\n'
name|'self'
op|'.'
name|'ldap'
op|'='
name|'__import__'
op|'('
string|"'ldap'"
op|')'
newline|'\n'
name|'if'
name|'FLAGS'
op|'.'
name|'ldap_schema_version'
op|'=='
number|'1'
op|':'
newline|'\n'
indent|'            '
name|'LdapDriver'
op|'.'
name|'project_pattern'
op|'='
string|"'(objectclass=novaProject)'"
newline|'\n'
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
op|'='
string|"'isAdmin'"
newline|'\n'
name|'LdapDriver'
op|'.'
name|'project_attribute'
op|'='
string|"'projectManager'"
newline|'\n'
name|'LdapDriver'
op|'.'
name|'project_objectclass'
op|'='
string|"'novaProject'"
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'__cache'
op|'='
name|'None'
newline|'\n'
name|'if'
name|'LdapDriver'
op|'.'
name|'conn'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'LdapDriver'
op|'.'
name|'conn'
op|'='
name|'LDAPWrapper'
op|'('
name|'self'
op|'.'
name|'ldap'
op|','
name|'FLAGS'
op|'.'
name|'ldap_url'
op|','
nl|'\n'
name|'FLAGS'
op|'.'
name|'ldap_user_dn'
op|','
nl|'\n'
name|'FLAGS'
op|'.'
name|'ldap_password'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'LdapDriver'
op|'.'
name|'mc'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'LdapDriver'
op|'.'
name|'mc'
op|'='
name|'memcache'
op|'.'
name|'Client'
op|'('
name|'FLAGS'
op|'.'
name|'memcached_servers'
op|','
name|'debug'
op|'='
number|'0'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__enter__
dedent|''
dedent|''
name|'def'
name|'__enter__'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
comment|'# TODO(yorik-sar): Should be per-request cache, not per-driver-request'
nl|'\n'
indent|'        '
name|'self'
op|'.'
name|'__cache'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'return'
name|'self'
newline|'\n'
nl|'\n'
DECL|member|__exit__
dedent|''
name|'def'
name|'__exit__'
op|'('
name|'self'
op|','
name|'exc_type'
op|','
name|'exc_value'
op|','
name|'traceback'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'__cache'
op|'='
name|'None'
newline|'\n'
name|'return'
name|'False'
newline|'\n'
nl|'\n'
DECL|member|__local_cache
dedent|''
name|'def'
name|'__local_cache'
op|'('
name|'key_fmt'
op|')'
op|':'
comment|'# pylint: disable=E0213'
newline|'\n'
indent|'        '
string|'"""Wrap function to cache it\'s result in self.__cache.\n        Works only with functions with one fixed argument.\n        """'
newline|'\n'
DECL|function|do_wrap
name|'def'
name|'do_wrap'
op|'('
name|'fn'
op|')'
op|':'
newline|'\n'
indent|'            '
op|'@'
name|'functools'
op|'.'
name|'wraps'
op|'('
name|'fn'
op|')'
newline|'\n'
DECL|function|inner
name|'def'
name|'inner'
op|'('
name|'self'
op|','
name|'arg'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'cache_key'
op|'='
name|'key_fmt'
op|'%'
op|'('
name|'arg'
op|','
op|')'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'                    '
name|'res'
op|'='
name|'self'
op|'.'
name|'__cache'
op|'['
name|'cache_key'
op|']'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
string|"'Local cache hit for %s by key %s'"
op|'%'
nl|'\n'
op|'('
name|'fn'
op|'.'
name|'__name__'
op|','
name|'cache_key'
op|')'
op|')'
newline|'\n'
name|'return'
name|'res'
newline|'\n'
dedent|''
name|'except'
name|'KeyError'
op|':'
newline|'\n'
indent|'                    '
name|'res'
op|'='
name|'fn'
op|'('
name|'self'
op|','
name|'arg'
op|','
op|'**'
name|'kwargs'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'__cache'
op|'['
name|'cache_key'
op|']'
op|'='
name|'res'
newline|'\n'
name|'return'
name|'res'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'inner'
newline|'\n'
dedent|''
name|'return'
name|'do_wrap'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
op|'@'
name|'__local_cache'
op|'('
string|"'uid_user-%s'"
op|')'
newline|'\n'
DECL|member|get_user
name|'def'
name|'get_user'
op|'('
name|'self'
op|','
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Retrieve user by id"""'
newline|'\n'
name|'attr'
op|'='
name|'self'
op|'.'
name|'__get_ldap_user'
op|'('
name|'uid'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__to_user'
op|'('
name|'attr'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|get_user_from_access_key
name|'def'
name|'get_user_from_access_key'
op|'('
name|'self'
op|','
name|'access'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Retrieve user by access key"""'
newline|'\n'
name|'cache_key'
op|'='
string|"'uak_dn_%s'"
op|'%'
op|'('
name|'access'
op|','
op|')'
newline|'\n'
name|'user_dn'
op|'='
name|'self'
op|'.'
name|'mc'
op|'.'
name|'get'
op|'('
name|'cache_key'
op|')'
newline|'\n'
name|'if'
name|'user_dn'
op|':'
newline|'\n'
indent|'            '
name|'user'
op|'='
name|'self'
op|'.'
name|'__to_user'
op|'('
nl|'\n'
name|'self'
op|'.'
name|'__find_object'
op|'('
name|'user_dn'
op|','
name|'scope'
op|'='
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'SCOPE_BASE'
op|')'
op|')'
newline|'\n'
name|'if'
name|'user'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'user'
op|'['
string|"'access'"
op|']'
op|'=='
name|'access'
op|':'
newline|'\n'
indent|'                    '
name|'return'
name|'user'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                    '
name|'self'
op|'.'
name|'mc'
op|'.'
name|'set'
op|'('
name|'cache_key'
op|','
name|'None'
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'query'
op|'='
string|"'(accessKey=%s)'"
op|'%'
name|'access'
newline|'\n'
name|'dn'
op|'='
name|'FLAGS'
op|'.'
name|'ldap_user_subtree'
newline|'\n'
name|'user_obj'
op|'='
name|'self'
op|'.'
name|'__find_object'
op|'('
name|'dn'
op|','
name|'query'
op|')'
newline|'\n'
name|'user'
op|'='
name|'self'
op|'.'
name|'__to_user'
op|'('
name|'user_obj'
op|')'
newline|'\n'
name|'if'
name|'user'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'mc'
op|'.'
name|'set'
op|'('
name|'cache_key'
op|','
name|'user_obj'
op|'['
string|"'dn'"
op|']'
op|'['
number|'0'
op|']'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'user'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
op|'@'
name|'__local_cache'
op|'('
string|"'pid_project-%s'"
op|')'
newline|'\n'
DECL|member|get_project
name|'def'
name|'get_project'
op|'('
name|'self'
op|','
name|'pid'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Retrieve project by id"""'
newline|'\n'
name|'dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'pid'
op|','
name|'search'
op|'='
name|'False'
op|')'
newline|'\n'
name|'attr'
op|'='
name|'self'
op|'.'
name|'__find_object'
op|'('
name|'dn'
op|','
name|'LdapDriver'
op|'.'
name|'project_pattern'
op|','
nl|'\n'
name|'scope'
op|'='
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'SCOPE_BASE'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__to_project'
op|'('
name|'attr'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|get_users
name|'def'
name|'get_users'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Retrieve list of users"""'
newline|'\n'
name|'attrs'
op|'='
name|'self'
op|'.'
name|'__find_objects'
op|'('
name|'FLAGS'
op|'.'
name|'ldap_user_subtree'
op|','
nl|'\n'
string|"'(objectclass=novaUser)'"
op|')'
newline|'\n'
name|'users'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'attr'
name|'in'
name|'attrs'
op|':'
newline|'\n'
indent|'            '
name|'user'
op|'='
name|'self'
op|'.'
name|'__to_user'
op|'('
name|'attr'
op|')'
newline|'\n'
name|'if'
name|'user'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'                '
name|'users'
op|'.'
name|'append'
op|'('
name|'user'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'users'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|get_projects
name|'def'
name|'get_projects'
op|'('
name|'self'
op|','
name|'uid'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Retrieve list of projects"""'
newline|'\n'
name|'pattern'
op|'='
name|'LdapDriver'
op|'.'
name|'project_pattern'
newline|'\n'
name|'if'
name|'uid'
op|':'
newline|'\n'
indent|'            '
name|'pattern'
op|'='
string|'"(&%s(member=%s))"'
op|'%'
op|'('
name|'pattern'
op|','
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|')'
newline|'\n'
dedent|''
name|'attrs'
op|'='
name|'self'
op|'.'
name|'__find_objects'
op|'('
name|'FLAGS'
op|'.'
name|'ldap_project_subtree'
op|','
nl|'\n'
name|'pattern'
op|')'
newline|'\n'
name|'return'
op|'['
name|'self'
op|'.'
name|'__to_project'
op|'('
name|'attr'
op|')'
name|'for'
name|'attr'
name|'in'
name|'attrs'
op|']'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|create_user
name|'def'
name|'create_user'
op|'('
name|'self'
op|','
name|'name'
op|','
name|'access_key'
op|','
name|'secret_key'
op|','
name|'is_admin'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Create a user"""'
newline|'\n'
name|'if'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'name'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserExists'
op|'('
name|'user'
op|'='
name|'name'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'FLAGS'
op|'.'
name|'ldap_user_modify_only'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'self'
op|'.'
name|'__ldap_user_exists'
op|'('
name|'name'
op|')'
op|':'
newline|'\n'
comment|'# Retrieve user by name'
nl|'\n'
indent|'                '
name|'user'
op|'='
name|'self'
op|'.'
name|'__get_ldap_user'
op|'('
name|'name'
op|')'
newline|'\n'
comment|'# Entry could be malformed, test for missing attrs.'
nl|'\n'
comment|'# Malformed entries are useless, replace attributes found.'
nl|'\n'
name|'attr'
op|'='
op|'['
op|']'
newline|'\n'
name|'if'
string|"'secretKey'"
name|'in'
name|'user'
op|'.'
name|'keys'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_REPLACE'
op|','
string|"'secretKey'"
op|','
nl|'\n'
op|'['
name|'secret_key'
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                    '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_ADD'
op|','
string|"'secretKey'"
op|','
nl|'\n'
op|'['
name|'secret_key'
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
string|"'accessKey'"
name|'in'
name|'user'
op|'.'
name|'keys'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_REPLACE'
op|','
string|"'accessKey'"
op|','
nl|'\n'
op|'['
name|'access_key'
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                    '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_ADD'
op|','
string|"'accessKey'"
op|','
nl|'\n'
op|'['
name|'access_key'
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
name|'in'
name|'user'
op|'.'
name|'keys'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_REPLACE'
op|','
nl|'\n'
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
op|','
nl|'\n'
op|'['
name|'str'
op|'('
name|'is_admin'
op|')'
op|'.'
name|'upper'
op|'('
op|')'
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                    '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_ADD'
op|','
nl|'\n'
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
op|','
nl|'\n'
op|'['
name|'str'
op|'('
name|'is_admin'
op|')'
op|'.'
name|'upper'
op|'('
op|')'
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'conn'
op|'.'
name|'modify_s'
op|'('
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'name'
op|')'
op|','
name|'attr'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'get_user'
op|'('
name|'name'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'name'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'attr'
op|'='
op|'['
nl|'\n'
op|'('
string|"'objectclass'"
op|','
op|'['
string|"'person'"
op|','
nl|'\n'
string|"'organizationalPerson'"
op|','
nl|'\n'
string|"'inetOrgPerson'"
op|','
nl|'\n'
string|"'novaUser'"
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'ou'"
op|','
op|'['
name|'FLAGS'
op|'.'
name|'ldap_user_unit'
op|']'
op|')'
op|','
nl|'\n'
op|'('
name|'FLAGS'
op|'.'
name|'ldap_user_id_attribute'
op|','
op|'['
name|'name'
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'sn'"
op|','
op|'['
name|'name'
op|']'
op|')'
op|','
nl|'\n'
op|'('
name|'FLAGS'
op|'.'
name|'ldap_user_name_attribute'
op|','
op|'['
name|'name'
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'secretKey'"
op|','
op|'['
name|'secret_key'
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'accessKey'"
op|','
op|'['
name|'access_key'
op|']'
op|')'
op|','
nl|'\n'
op|'('
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
op|','
op|'['
name|'str'
op|'('
name|'is_admin'
op|')'
op|'.'
name|'upper'
op|'('
op|')'
op|']'
op|')'
op|','
nl|'\n'
op|']'
newline|'\n'
name|'self'
op|'.'
name|'conn'
op|'.'
name|'add_s'
op|'('
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'name'
op|')'
op|','
name|'attr'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__to_user'
op|'('
name|'dict'
op|'('
name|'attr'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|create_project
name|'def'
name|'create_project'
op|'('
name|'self'
op|','
name|'name'
op|','
name|'manager_uid'
op|','
nl|'\n'
name|'description'
op|'='
name|'None'
op|','
name|'member_uids'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Create a project"""'
newline|'\n'
name|'if'
name|'self'
op|'.'
name|'__project_exists'
op|'('
name|'name'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'ProjectExists'
op|'('
name|'project'
op|'='
name|'name'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'manager_uid'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'manager_uid'
op|')'
newline|'\n'
dedent|''
name|'manager_dn'
op|'='
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'manager_uid'
op|')'
newline|'\n'
comment|'# description is a required attribute'
nl|'\n'
name|'if'
name|'description'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'description'
op|'='
name|'name'
newline|'\n'
dedent|''
name|'members'
op|'='
op|'['
op|']'
newline|'\n'
name|'if'
name|'member_uids'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'for'
name|'member_uid'
name|'in'
name|'member_uids'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'member_uid'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'member_uid'
op|')'
newline|'\n'
dedent|''
name|'members'
op|'.'
name|'append'
op|'('
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'member_uid'
op|')'
op|')'
newline|'\n'
comment|'# always add the manager as a member because members is required'
nl|'\n'
dedent|''
dedent|''
name|'if'
name|'not'
name|'manager_dn'
name|'in'
name|'members'
op|':'
newline|'\n'
indent|'            '
name|'members'
op|'.'
name|'append'
op|'('
name|'manager_dn'
op|')'
newline|'\n'
dedent|''
name|'attr'
op|'='
op|'['
nl|'\n'
op|'('
string|"'objectclass'"
op|','
op|'['
name|'LdapDriver'
op|'.'
name|'project_objectclass'
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'cn'"
op|','
op|'['
name|'name'
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'description'"
op|','
op|'['
name|'description'
op|']'
op|')'
op|','
nl|'\n'
op|'('
name|'LdapDriver'
op|'.'
name|'project_attribute'
op|','
op|'['
name|'manager_dn'
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'member'"
op|','
name|'members'
op|')'
op|']'
newline|'\n'
name|'dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'name'
op|','
name|'search'
op|'='
name|'False'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'conn'
op|'.'
name|'add_s'
op|'('
name|'dn'
op|','
name|'attr'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__to_project'
op|'('
name|'dict'
op|'('
name|'attr'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|modify_project
name|'def'
name|'modify_project'
op|'('
name|'self'
op|','
name|'project_id'
op|','
name|'manager_uid'
op|'='
name|'None'
op|','
name|'description'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Modify an existing project"""'
newline|'\n'
name|'if'
name|'not'
name|'manager_uid'
name|'and'
name|'not'
name|'description'
op|':'
newline|'\n'
indent|'            '
name|'return'
newline|'\n'
dedent|''
name|'attr'
op|'='
op|'['
op|']'
newline|'\n'
name|'if'
name|'manager_uid'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'manager_uid'
op|')'
op|':'
newline|'\n'
indent|'                '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'manager_uid'
op|')'
newline|'\n'
dedent|''
name|'manager_dn'
op|'='
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'manager_uid'
op|')'
newline|'\n'
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_REPLACE'
op|','
name|'LdapDriver'
op|'.'
name|'project_attribute'
op|','
nl|'\n'
name|'manager_dn'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'description'
op|':'
newline|'\n'
indent|'            '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_REPLACE'
op|','
string|"'description'"
op|','
name|'description'
op|')'
op|')'
newline|'\n'
dedent|''
name|'dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'project_id'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'conn'
op|'.'
name|'modify_s'
op|'('
name|'dn'
op|','
name|'attr'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'is_in_project'
op|'('
name|'manager_uid'
op|','
name|'project_id'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'add_to_project'
op|'('
name|'manager_uid'
op|','
name|'project_id'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|add_to_project
name|'def'
name|'add_to_project'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'project_id'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Add user to project"""'
newline|'\n'
name|'dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'project_id'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__add_to_group'
op|'('
name|'uid'
op|','
name|'dn'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|remove_from_project
name|'def'
name|'remove_from_project'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'project_id'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Remove user from project"""'
newline|'\n'
name|'dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'project_id'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__remove_from_group'
op|'('
name|'uid'
op|','
name|'dn'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|is_in_project
name|'def'
name|'is_in_project'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'project_id'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Check if user is in project"""'
newline|'\n'
name|'dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'project_id'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__is_in_group'
op|'('
name|'uid'
op|','
name|'dn'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|has_role
name|'def'
name|'has_role'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'role'
op|','
name|'project_id'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Check if user has role\n\n        If project is specified, it checks for local role, otherwise it\n        checks for global role\n        """'
newline|'\n'
name|'role_dn'
op|'='
name|'self'
op|'.'
name|'__role_to_dn'
op|'('
name|'role'
op|','
name|'project_id'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__is_in_group'
op|'('
name|'uid'
op|','
name|'role_dn'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|add_role
name|'def'
name|'add_role'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'role'
op|','
name|'project_id'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Add role for user (or user and project)"""'
newline|'\n'
name|'role_dn'
op|'='
name|'self'
op|'.'
name|'__role_to_dn'
op|'('
name|'role'
op|','
name|'project_id'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'__group_exists'
op|'('
name|'role_dn'
op|')'
op|':'
newline|'\n'
comment|"# create the role if it doesn't exist"
nl|'\n'
indent|'            '
name|'description'
op|'='
string|"'%s role for %s'"
op|'%'
op|'('
name|'role'
op|','
name|'project_id'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'__create_group'
op|'('
name|'role_dn'
op|','
name|'role'
op|','
name|'uid'
op|','
name|'description'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'self'
op|'.'
name|'__add_to_group'
op|'('
name|'uid'
op|','
name|'role_dn'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|remove_role
name|'def'
name|'remove_role'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'role'
op|','
name|'project_id'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Remove role for user (or user and project)"""'
newline|'\n'
name|'role_dn'
op|'='
name|'self'
op|'.'
name|'__role_to_dn'
op|'('
name|'role'
op|','
name|'project_id'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__remove_from_group'
op|'('
name|'uid'
op|','
name|'role_dn'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|get_user_roles
name|'def'
name|'get_user_roles'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'project_id'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Retrieve list of roles for user (or user and project)"""'
newline|'\n'
name|'if'
name|'project_id'
name|'is'
name|'None'
op|':'
newline|'\n'
comment|"# NOTE(vish): This is unneccesarily slow, but since we can't"
nl|'\n'
comment|'#             guarantee that the global roles are located'
nl|'\n'
comment|"#             together in the ldap tree, we're doing this version."
nl|'\n'
indent|'            '
name|'roles'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'role'
name|'in'
name|'FLAGS'
op|'.'
name|'allowed_roles'
op|':'
newline|'\n'
indent|'                '
name|'role_dn'
op|'='
name|'self'
op|'.'
name|'__role_to_dn'
op|'('
name|'role'
op|')'
newline|'\n'
name|'if'
name|'self'
op|'.'
name|'__is_in_group'
op|'('
name|'uid'
op|','
name|'role_dn'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'roles'
op|'.'
name|'append'
op|'('
name|'role'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'roles'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'project_dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'project_id'
op|')'
newline|'\n'
name|'query'
op|'='
op|'('
string|"'(&(&(objectclass=groupOfNames)(!%s))(member=%s))'"
op|'%'
nl|'\n'
op|'('
name|'LdapDriver'
op|'.'
name|'project_pattern'
op|','
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|')'
op|')'
newline|'\n'
name|'roles'
op|'='
name|'self'
op|'.'
name|'__find_objects'
op|'('
name|'project_dn'
op|','
name|'query'
op|')'
newline|'\n'
name|'return'
op|'['
name|'role'
op|'['
string|"'cn'"
op|']'
op|'['
number|'0'
op|']'
name|'for'
name|'role'
name|'in'
name|'roles'
op|']'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|delete_user
name|'def'
name|'delete_user'
op|'('
name|'self'
op|','
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Delete a user"""'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'uid'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'__remove_from_all'
op|'('
name|'uid'
op|')'
newline|'\n'
name|'if'
name|'FLAGS'
op|'.'
name|'ldap_user_modify_only'
op|':'
newline|'\n'
comment|'# Delete attributes'
nl|'\n'
indent|'            '
name|'attr'
op|'='
op|'['
op|']'
newline|'\n'
comment|'# Retrieve user by name'
nl|'\n'
name|'user'
op|'='
name|'self'
op|'.'
name|'__get_ldap_user'
op|'('
name|'uid'
op|')'
newline|'\n'
name|'if'
string|"'secretKey'"
name|'in'
name|'user'
op|'.'
name|'keys'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_DELETE'
op|','
string|"'secretKey'"
op|','
nl|'\n'
name|'user'
op|'['
string|"'secretKey'"
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
string|"'accessKey'"
name|'in'
name|'user'
op|'.'
name|'keys'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_DELETE'
op|','
string|"'accessKey'"
op|','
nl|'\n'
name|'user'
op|'['
string|"'accessKey'"
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
name|'in'
name|'user'
op|'.'
name|'keys'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_DELETE'
op|','
nl|'\n'
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
op|','
nl|'\n'
name|'user'
op|'['
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
op|']'
op|')'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'conn'
op|'.'
name|'modify_s'
op|'('
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|','
name|'attr'
op|')'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
comment|'# Delete entry'
nl|'\n'
indent|'            '
name|'self'
op|'.'
name|'conn'
op|'.'
name|'delete_s'
op|'('
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|')'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|delete_project
name|'def'
name|'delete_project'
op|'('
name|'self'
op|','
name|'project_id'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Delete a project"""'
newline|'\n'
name|'project_dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'project_id'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'__delete_roles'
op|'('
name|'project_dn'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'__delete_group'
op|'('
name|'project_dn'
op|')'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'sanitize'
newline|'\n'
DECL|member|modify_user
name|'def'
name|'modify_user'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'access_key'
op|'='
name|'None'
op|','
name|'secret_key'
op|'='
name|'None'
op|','
name|'admin'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Modify an existing user"""'
newline|'\n'
name|'if'
name|'not'
name|'access_key'
name|'and'
name|'not'
name|'secret_key'
name|'and'
name|'admin'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'return'
newline|'\n'
dedent|''
name|'attr'
op|'='
op|'['
op|']'
newline|'\n'
name|'if'
name|'access_key'
op|':'
newline|'\n'
indent|'            '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_REPLACE'
op|','
string|"'accessKey'"
op|','
name|'access_key'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'secret_key'
op|':'
newline|'\n'
indent|'            '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_REPLACE'
op|','
string|"'secretKey'"
op|','
name|'secret_key'
op|')'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'admin'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'attr'
op|'.'
name|'append'
op|'('
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_REPLACE'
op|','
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
op|','
nl|'\n'
name|'str'
op|'('
name|'admin'
op|')'
op|'.'
name|'upper'
op|'('
op|')'
op|')'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'conn'
op|'.'
name|'modify_s'
op|'('
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|','
name|'attr'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__user_exists
dedent|''
name|'def'
name|'__user_exists'
op|'('
name|'self'
op|','
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Check if user exists"""'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'get_user'
op|'('
name|'uid'
op|')'
name|'is'
name|'not'
name|'None'
newline|'\n'
nl|'\n'
DECL|member|__ldap_user_exists
dedent|''
name|'def'
name|'__ldap_user_exists'
op|'('
name|'self'
op|','
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Check if the user exists in ldap"""'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__get_ldap_user'
op|'('
name|'uid'
op|')'
name|'is'
name|'not'
name|'None'
newline|'\n'
nl|'\n'
DECL|member|__project_exists
dedent|''
name|'def'
name|'__project_exists'
op|'('
name|'self'
op|','
name|'project_id'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Check if project exists"""'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'get_project'
op|'('
name|'project_id'
op|')'
name|'is'
name|'not'
name|'None'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'__local_cache'
op|'('
string|"'uid_attrs-%s'"
op|')'
newline|'\n'
DECL|member|__get_ldap_user
name|'def'
name|'__get_ldap_user'
op|'('
name|'self'
op|','
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Retrieve LDAP user entry by id"""'
newline|'\n'
name|'dn'
op|'='
name|'FLAGS'
op|'.'
name|'ldap_user_subtree'
newline|'\n'
name|'query'
op|'='
op|'('
string|"'(&(%s=%s)(objectclass=novaUser))'"
op|'%'
nl|'\n'
op|'('
name|'FLAGS'
op|'.'
name|'ldap_user_id_attribute'
op|','
name|'uid'
op|')'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__find_object'
op|'('
name|'dn'
op|','
name|'query'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__find_object
dedent|''
name|'def'
name|'__find_object'
op|'('
name|'self'
op|','
name|'dn'
op|','
name|'query'
op|'='
name|'None'
op|','
name|'scope'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Find an object by dn and query"""'
newline|'\n'
name|'objects'
op|'='
name|'self'
op|'.'
name|'__find_objects'
op|'('
name|'dn'
op|','
name|'query'
op|','
name|'scope'
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'objects'
op|')'
op|'=='
number|'0'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'None'
newline|'\n'
dedent|''
name|'return'
name|'objects'
op|'['
number|'0'
op|']'
newline|'\n'
nl|'\n'
DECL|member|__find_dns
dedent|''
name|'def'
name|'__find_dns'
op|'('
name|'self'
op|','
name|'dn'
op|','
name|'query'
op|'='
name|'None'
op|','
name|'scope'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Find dns by query"""'
newline|'\n'
name|'if'
name|'scope'
name|'is'
name|'None'
op|':'
newline|'\n'
comment|'# One of the flags is 0!'
nl|'\n'
indent|'            '
name|'scope'
op|'='
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'SCOPE_SUBTREE'
newline|'\n'
dedent|''
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'res'
op|'='
name|'self'
op|'.'
name|'conn'
op|'.'
name|'search_s'
op|'('
name|'dn'
op|','
name|'scope'
op|','
name|'query'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'NO_SUCH_OBJECT'
op|':'
newline|'\n'
indent|'            '
name|'return'
op|'['
op|']'
newline|'\n'
comment|'# Just return the DNs'
nl|'\n'
dedent|''
name|'return'
op|'['
name|'dn'
name|'for'
name|'dn'
op|','
name|'_attributes'
name|'in'
name|'res'
op|']'
newline|'\n'
nl|'\n'
DECL|member|__find_objects
dedent|''
name|'def'
name|'__find_objects'
op|'('
name|'self'
op|','
name|'dn'
op|','
name|'query'
op|'='
name|'None'
op|','
name|'scope'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Find objects by query"""'
newline|'\n'
name|'if'
name|'scope'
name|'is'
name|'None'
op|':'
newline|'\n'
comment|'# One of the flags is 0!'
nl|'\n'
indent|'            '
name|'scope'
op|'='
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'SCOPE_SUBTREE'
newline|'\n'
dedent|''
name|'if'
name|'query'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'query'
op|'='
string|'"(objectClass=*)"'
newline|'\n'
dedent|''
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'res'
op|'='
name|'self'
op|'.'
name|'conn'
op|'.'
name|'search_s'
op|'('
name|'dn'
op|','
name|'scope'
op|','
name|'query'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'NO_SUCH_OBJECT'
op|':'
newline|'\n'
indent|'            '
name|'return'
op|'['
op|']'
newline|'\n'
comment|'# Just return the attributes'
nl|'\n'
comment|'# FIXME(yorik-sar): Whole driver should be refactored to'
nl|'\n'
comment|'#                   prevent this hack'
nl|'\n'
dedent|''
name|'res1'
op|'='
op|'['
op|']'
newline|'\n'
name|'for'
name|'dn'
op|','
name|'attrs'
name|'in'
name|'res'
op|':'
newline|'\n'
indent|'            '
name|'attrs'
op|'['
string|"'dn'"
op|']'
op|'='
op|'['
name|'dn'
op|']'
newline|'\n'
name|'res1'
op|'.'
name|'append'
op|'('
name|'attrs'
op|')'
newline|'\n'
dedent|''
name|'return'
name|'res1'
newline|'\n'
nl|'\n'
DECL|member|__find_role_dns
dedent|''
name|'def'
name|'__find_role_dns'
op|'('
name|'self'
op|','
name|'tree'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Find dns of role objects in given tree"""'
newline|'\n'
name|'query'
op|'='
op|'('
string|"'(&(objectclass=groupOfNames)(!%s))'"
op|'%'
nl|'\n'
name|'LdapDriver'
op|'.'
name|'project_pattern'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__find_dns'
op|'('
name|'tree'
op|','
name|'query'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__find_group_dns_with_member
dedent|''
name|'def'
name|'__find_group_dns_with_member'
op|'('
name|'self'
op|','
name|'tree'
op|','
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Find dns of group objects in a given tree that contain member"""'
newline|'\n'
name|'query'
op|'='
op|'('
string|"'(&(objectclass=groupOfNames)(member=%s))'"
op|'%'
nl|'\n'
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|')'
newline|'\n'
name|'dns'
op|'='
name|'self'
op|'.'
name|'__find_dns'
op|'('
name|'tree'
op|','
name|'query'
op|')'
newline|'\n'
name|'return'
name|'dns'
newline|'\n'
nl|'\n'
DECL|member|__group_exists
dedent|''
name|'def'
name|'__group_exists'
op|'('
name|'self'
op|','
name|'dn'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Check if group exists"""'
newline|'\n'
name|'query'
op|'='
string|"'(objectclass=groupOfNames)'"
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'__find_object'
op|'('
name|'dn'
op|','
name|'query'
op|')'
name|'is'
name|'not'
name|'None'
newline|'\n'
nl|'\n'
DECL|member|__role_to_dn
dedent|''
name|'def'
name|'__role_to_dn'
op|'('
name|'self'
op|','
name|'role'
op|','
name|'project_id'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Convert role to corresponding dn"""'
newline|'\n'
name|'if'
name|'project_id'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'FLAGS'
op|'.'
name|'__getitem__'
op|'('
string|'"ldap_%s"'
op|'%'
name|'role'
op|')'
op|'.'
name|'value'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'project_dn'
op|'='
name|'self'
op|'.'
name|'__project_to_dn'
op|'('
name|'project_id'
op|')'
newline|'\n'
name|'return'
string|"'cn=%s,%s'"
op|'%'
op|'('
name|'role'
op|','
name|'project_dn'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__create_group
dedent|''
dedent|''
name|'def'
name|'__create_group'
op|'('
name|'self'
op|','
name|'group_dn'
op|','
name|'name'
op|','
name|'uid'
op|','
nl|'\n'
name|'description'
op|','
name|'member_uids'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Create a group"""'
newline|'\n'
name|'if'
name|'self'
op|'.'
name|'__group_exists'
op|'('
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPGroupExists'
op|'('
name|'group'
op|'='
name|'name'
op|')'
newline|'\n'
dedent|''
name|'members'
op|'='
op|'['
op|']'
newline|'\n'
name|'if'
name|'member_uids'
name|'is'
name|'not'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'for'
name|'member_uid'
name|'in'
name|'member_uids'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'member_uid'
op|')'
op|':'
newline|'\n'
indent|'                    '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'member_uid'
op|')'
newline|'\n'
dedent|''
name|'members'
op|'.'
name|'append'
op|'('
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'member_uid'
op|')'
op|')'
newline|'\n'
dedent|''
dedent|''
name|'dn'
op|'='
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
newline|'\n'
name|'if'
name|'not'
name|'dn'
name|'in'
name|'members'
op|':'
newline|'\n'
indent|'            '
name|'members'
op|'.'
name|'append'
op|'('
name|'dn'
op|')'
newline|'\n'
dedent|''
name|'attr'
op|'='
op|'['
nl|'\n'
op|'('
string|"'objectclass'"
op|','
op|'['
string|"'groupOfNames'"
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'cn'"
op|','
op|'['
name|'name'
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'description'"
op|','
op|'['
name|'description'
op|']'
op|')'
op|','
nl|'\n'
op|'('
string|"'member'"
op|','
name|'members'
op|')'
op|']'
newline|'\n'
name|'self'
op|'.'
name|'conn'
op|'.'
name|'add_s'
op|'('
name|'group_dn'
op|','
name|'attr'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__is_in_group
dedent|''
name|'def'
name|'__is_in_group'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Check if user is in group"""'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'uid'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'not'
name|'self'
op|'.'
name|'__group_exists'
op|'('
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'False'
newline|'\n'
dedent|''
name|'res'
op|'='
name|'self'
op|'.'
name|'__find_object'
op|'('
name|'group_dn'
op|','
nl|'\n'
string|"'(member=%s)'"
op|'%'
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|','
nl|'\n'
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'SCOPE_BASE'
op|')'
newline|'\n'
name|'return'
name|'res'
name|'is'
name|'not'
name|'None'
newline|'\n'
nl|'\n'
DECL|member|__add_to_group
dedent|''
name|'def'
name|'__add_to_group'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Add user to group"""'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'uid'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'not'
name|'self'
op|'.'
name|'__group_exists'
op|'('
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPGroupNotFound'
op|'('
name|'group_id'
op|'='
name|'group_dn'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'self'
op|'.'
name|'__is_in_group'
op|'('
name|'uid'
op|','
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPMembershipExists'
op|'('
name|'uid'
op|'='
name|'uid'
op|','
name|'group_dn'
op|'='
name|'group_dn'
op|')'
newline|'\n'
dedent|''
name|'attr'
op|'='
op|'['
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_ADD'
op|','
string|"'member'"
op|','
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|')'
op|']'
newline|'\n'
name|'self'
op|'.'
name|'conn'
op|'.'
name|'modify_s'
op|'('
name|'group_dn'
op|','
name|'attr'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__remove_from_group
dedent|''
name|'def'
name|'__remove_from_group'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Remove user from group"""'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'__group_exists'
op|'('
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPGroupNotFound'
op|'('
name|'group_id'
op|'='
name|'group_dn'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'uid'
op|')'
newline|'\n'
dedent|''
name|'if'
name|'not'
name|'self'
op|'.'
name|'__is_in_group'
op|'('
name|'uid'
op|','
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPGroupMembershipNotFound'
op|'('
name|'user_id'
op|'='
name|'uid'
op|','
nl|'\n'
name|'group_id'
op|'='
name|'group_dn'
op|')'
newline|'\n'
comment|'# NOTE(vish): remove user from group and any sub_groups'
nl|'\n'
dedent|''
name|'sub_dns'
op|'='
name|'self'
op|'.'
name|'__find_group_dns_with_member'
op|'('
name|'group_dn'
op|','
name|'uid'
op|')'
newline|'\n'
name|'for'
name|'sub_dn'
name|'in'
name|'sub_dns'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'__safe_remove_from_group'
op|'('
name|'uid'
op|','
name|'sub_dn'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__safe_remove_from_group
dedent|''
dedent|''
name|'def'
name|'__safe_remove_from_group'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Remove user from group, deleting group if user is last member"""'
newline|'\n'
comment|'# FIXME(vish): what if deleted user is a project manager?'
nl|'\n'
name|'attr'
op|'='
op|'['
op|'('
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'MOD_DELETE'
op|','
string|"'member'"
op|','
name|'self'
op|'.'
name|'__uid_to_dn'
op|'('
name|'uid'
op|')'
op|')'
op|']'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'conn'
op|'.'
name|'modify_s'
op|'('
name|'group_dn'
op|','
name|'attr'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'OBJECT_CLASS_VIOLATION'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|'"Attempted to remove the last member of a group. "'
nl|'\n'
string|'"Deleting the group at %s instead."'
op|')'
op|','
name|'group_dn'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'__delete_group'
op|'('
name|'group_dn'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__remove_from_all
dedent|''
dedent|''
name|'def'
name|'__remove_from_all'
op|'('
name|'self'
op|','
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Remove user from all roles and projects"""'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'__user_exists'
op|'('
name|'uid'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPUserNotFound'
op|'('
name|'user_id'
op|'='
name|'uid'
op|')'
newline|'\n'
dedent|''
name|'role_dns'
op|'='
name|'self'
op|'.'
name|'__find_group_dns_with_member'
op|'('
nl|'\n'
name|'FLAGS'
op|'.'
name|'role_project_subtree'
op|','
name|'uid'
op|')'
newline|'\n'
name|'for'
name|'role_dn'
name|'in'
name|'role_dns'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'__safe_remove_from_group'
op|'('
name|'uid'
op|','
name|'role_dn'
op|')'
newline|'\n'
dedent|''
name|'project_dns'
op|'='
name|'self'
op|'.'
name|'__find_group_dns_with_member'
op|'('
nl|'\n'
name|'FLAGS'
op|'.'
name|'ldap_project_subtree'
op|','
name|'uid'
op|')'
newline|'\n'
name|'for'
name|'project_dn'
name|'in'
name|'project_dns'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'__safe_remove_from_group'
op|'('
name|'uid'
op|','
name|'project_dn'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__delete_group
dedent|''
dedent|''
name|'def'
name|'__delete_group'
op|'('
name|'self'
op|','
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Delete Group"""'
newline|'\n'
name|'if'
name|'not'
name|'self'
op|'.'
name|'__group_exists'
op|'('
name|'group_dn'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'LDAPGroupNotFound'
op|'('
name|'group_id'
op|'='
name|'group_dn'
op|')'
newline|'\n'
dedent|''
name|'self'
op|'.'
name|'conn'
op|'.'
name|'delete_s'
op|'('
name|'group_dn'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__delete_roles
dedent|''
name|'def'
name|'__delete_roles'
op|'('
name|'self'
op|','
name|'project_dn'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Delete all roles for project"""'
newline|'\n'
name|'for'
name|'role_dn'
name|'in'
name|'self'
op|'.'
name|'__find_role_dns'
op|'('
name|'project_dn'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'self'
op|'.'
name|'__delete_group'
op|'('
name|'role_dn'
op|')'
newline|'\n'
nl|'\n'
DECL|member|__to_project
dedent|''
dedent|''
name|'def'
name|'__to_project'
op|'('
name|'self'
op|','
name|'attr'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Convert ldap attributes to Project object"""'
newline|'\n'
name|'if'
name|'attr'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'None'
newline|'\n'
dedent|''
name|'member_dns'
op|'='
name|'attr'
op|'.'
name|'get'
op|'('
string|"'member'"
op|','
op|'['
op|']'
op|')'
newline|'\n'
name|'return'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'attr'
op|'['
string|"'cn'"
op|']'
op|'['
number|'0'
op|']'
op|','
nl|'\n'
string|"'name'"
op|':'
name|'attr'
op|'['
string|"'cn'"
op|']'
op|'['
number|'0'
op|']'
op|','
nl|'\n'
string|"'project_manager_id'"
op|':'
nl|'\n'
name|'self'
op|'.'
name|'__dn_to_uid'
op|'('
name|'attr'
op|'['
name|'LdapDriver'
op|'.'
name|'project_attribute'
op|']'
op|'['
number|'0'
op|']'
op|')'
op|','
nl|'\n'
string|"'description'"
op|':'
name|'attr'
op|'.'
name|'get'
op|'('
string|"'description'"
op|','
op|'['
name|'None'
op|']'
op|')'
op|'['
number|'0'
op|']'
op|','
nl|'\n'
string|"'member_ids'"
op|':'
op|'['
name|'self'
op|'.'
name|'__dn_to_uid'
op|'('
name|'x'
op|')'
name|'for'
name|'x'
name|'in'
name|'member_dns'
op|']'
op|'}'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'__local_cache'
op|'('
string|"'uid_dn-%s'"
op|')'
newline|'\n'
DECL|member|__uid_to_dn
name|'def'
name|'__uid_to_dn'
op|'('
name|'self'
op|','
name|'uid'
op|','
name|'search'
op|'='
name|'True'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Convert uid to dn"""'
newline|'\n'
comment|'# By default return a generated DN'
nl|'\n'
name|'userdn'
op|'='
op|'('
name|'FLAGS'
op|'.'
name|'ldap_user_id_attribute'
op|'+'
string|"'=%s,%s'"
nl|'\n'
op|'%'
op|'('
name|'uid'
op|','
name|'FLAGS'
op|'.'
name|'ldap_user_subtree'
op|')'
op|')'
newline|'\n'
name|'if'
name|'search'
op|':'
newline|'\n'
indent|'            '
name|'query'
op|'='
op|'('
string|"'%s=%s'"
op|'%'
op|'('
name|'FLAGS'
op|'.'
name|'ldap_user_id_attribute'
op|','
name|'uid'
op|')'
op|')'
newline|'\n'
name|'user'
op|'='
name|'self'
op|'.'
name|'__find_dns'
op|'('
name|'FLAGS'
op|'.'
name|'ldap_user_subtree'
op|','
name|'query'
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'user'
op|')'
op|'>'
number|'0'
op|':'
newline|'\n'
indent|'                '
name|'userdn'
op|'='
name|'user'
op|'['
number|'0'
op|']'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'userdn'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'__local_cache'
op|'('
string|"'pid_dn-%s'"
op|')'
newline|'\n'
DECL|member|__project_to_dn
name|'def'
name|'__project_to_dn'
op|'('
name|'self'
op|','
name|'pid'
op|','
name|'search'
op|'='
name|'True'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Convert pid to dn"""'
newline|'\n'
comment|'# By default return a generated DN'
nl|'\n'
name|'projectdn'
op|'='
op|'('
string|"'cn=%s,%s'"
op|'%'
op|'('
name|'pid'
op|','
name|'FLAGS'
op|'.'
name|'ldap_project_subtree'
op|')'
op|')'
newline|'\n'
name|'if'
name|'search'
op|':'
newline|'\n'
indent|'            '
name|'query'
op|'='
op|'('
string|"'(&(cn=%s)%s)'"
op|'%'
op|'('
name|'pid'
op|','
name|'LdapDriver'
op|'.'
name|'project_pattern'
op|')'
op|')'
newline|'\n'
name|'project'
op|'='
name|'self'
op|'.'
name|'__find_dns'
op|'('
name|'FLAGS'
op|'.'
name|'ldap_project_subtree'
op|','
name|'query'
op|')'
newline|'\n'
name|'if'
name|'len'
op|'('
name|'project'
op|')'
op|'>'
number|'0'
op|':'
newline|'\n'
indent|'                '
name|'projectdn'
op|'='
name|'project'
op|'['
number|'0'
op|']'
newline|'\n'
dedent|''
dedent|''
name|'return'
name|'projectdn'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'staticmethod'
newline|'\n'
DECL|member|__to_user
name|'def'
name|'__to_user'
op|'('
name|'attr'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Convert ldap attributes to User object"""'
newline|'\n'
name|'if'
name|'attr'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'None'
newline|'\n'
dedent|''
name|'if'
op|'('
string|"'accessKey'"
name|'in'
name|'attr'
op|'.'
name|'keys'
op|'('
op|')'
name|'and'
string|"'secretKey'"
name|'in'
name|'attr'
op|'.'
name|'keys'
op|'('
op|')'
name|'and'
nl|'\n'
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
name|'in'
name|'attr'
op|'.'
name|'keys'
op|'('
op|')'
op|')'
op|':'
newline|'\n'
indent|'            '
name|'return'
op|'{'
nl|'\n'
string|"'id'"
op|':'
name|'attr'
op|'['
name|'FLAGS'
op|'.'
name|'ldap_user_id_attribute'
op|']'
op|'['
number|'0'
op|']'
op|','
nl|'\n'
string|"'name'"
op|':'
name|'attr'
op|'['
name|'FLAGS'
op|'.'
name|'ldap_user_name_attribute'
op|']'
op|'['
number|'0'
op|']'
op|','
nl|'\n'
string|"'access'"
op|':'
name|'attr'
op|'['
string|"'accessKey'"
op|']'
op|'['
number|'0'
op|']'
op|','
nl|'\n'
string|"'secret'"
op|':'
name|'attr'
op|'['
string|"'secretKey'"
op|']'
op|'['
number|'0'
op|']'
op|','
nl|'\n'
string|"'admin'"
op|':'
op|'('
name|'attr'
op|'['
name|'LdapDriver'
op|'.'
name|'isadmin_attribute'
op|']'
op|'['
number|'0'
op|']'
op|'=='
string|"'TRUE'"
op|')'
op|'}'
newline|'\n'
dedent|''
name|'else'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'None'
newline|'\n'
nl|'\n'
dedent|''
dedent|''
op|'@'
name|'__local_cache'
op|'('
string|"'dn_uid-%s'"
op|')'
newline|'\n'
DECL|member|__dn_to_uid
name|'def'
name|'__dn_to_uid'
op|'('
name|'self'
op|','
name|'dn'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Convert user dn to uid"""'
newline|'\n'
name|'query'
op|'='
string|"'(objectclass=novaUser)'"
newline|'\n'
name|'user'
op|'='
name|'self'
op|'.'
name|'__find_object'
op|'('
name|'dn'
op|','
name|'query'
op|','
name|'scope'
op|'='
name|'self'
op|'.'
name|'ldap'
op|'.'
name|'SCOPE_BASE'
op|')'
newline|'\n'
name|'return'
name|'user'
op|'['
name|'FLAGS'
op|'.'
name|'ldap_user_id_attribute'
op|']'
op|'['
number|'0'
op|']'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|FakeLdapDriver
dedent|''
dedent|''
name|'class'
name|'FakeLdapDriver'
op|'('
name|'LdapDriver'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Fake Ldap Auth driver"""'
newline|'\n'
nl|'\n'
DECL|member|__init__
name|'def'
name|'__init__'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'import'
name|'nova'
op|'.'
name|'auth'
op|'.'
name|'fakeldap'
newline|'\n'
name|'sys'
op|'.'
name|'modules'
op|'['
string|"'ldap'"
op|']'
op|'='
name|'nova'
op|'.'
name|'auth'
op|'.'
name|'fakeldap'
newline|'\n'
name|'super'
op|'('
name|'FakeLdapDriver'
op|','
name|'self'
op|')'
op|'.'
name|'__init__'
op|'('
op|')'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
