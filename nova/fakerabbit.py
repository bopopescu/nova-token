begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright 2010 United States Government as represented by the'
nl|'\n'
comment|'# Administrator of the National Aeronautics and Space Administration.'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
nl|'\n'
string|'"""Based a bit on the carrot.backends.queue backend... but a lot better."""'
newline|'\n'
nl|'\n'
name|'import'
name|'Queue'
name|'as'
name|'queue'
newline|'\n'
nl|'\n'
name|'from'
name|'carrot'
op|'.'
name|'backends'
name|'import'
name|'base'
newline|'\n'
name|'from'
name|'eventlet'
name|'import'
name|'greenthread'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
name|'import'
name|'log'
name|'as'
name|'logging'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|LOG
name|'LOG'
op|'='
name|'logging'
op|'.'
name|'getLogger'
op|'('
string|'"nova.fakerabbit"'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|EXCHANGES
name|'EXCHANGES'
op|'='
op|'{'
op|'}'
newline|'\n'
DECL|variable|QUEUES
name|'QUEUES'
op|'='
op|'{'
op|'}'
newline|'\n'
DECL|variable|CONSUMERS
name|'CONSUMERS'
op|'='
op|'{'
op|'}'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|Message
name|'class'
name|'Message'
op|'('
name|'base'
op|'.'
name|'BaseMessage'
op|')'
op|':'
newline|'\n'
indent|'    '
name|'pass'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|Exchange
dedent|''
name|'class'
name|'Exchange'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
DECL|member|__init__
indent|'    '
name|'def'
name|'__init__'
op|'('
name|'self'
op|','
name|'name'
op|','
name|'exchange_type'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'name'
op|'='
name|'name'
newline|'\n'
name|'self'
op|'.'
name|'exchange_type'
op|'='
name|'exchange_type'
newline|'\n'
name|'self'
op|'.'
name|'_queue'
op|'='
name|'queue'
op|'.'
name|'Queue'
op|'('
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_routes'
op|'='
op|'{'
op|'}'
newline|'\n'
nl|'\n'
DECL|member|publish
dedent|''
name|'def'
name|'publish'
op|'('
name|'self'
op|','
name|'message'
op|','
name|'routing_key'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'nm'
op|'='
name|'self'
op|'.'
name|'name'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|"'(%(nm)s) publish (key: %(routing_key)s)'"
nl|'\n'
string|"' %(message)s'"
op|')'
op|'%'
name|'locals'
op|'('
op|')'
op|')'
newline|'\n'
name|'if'
name|'routing_key'
name|'in'
name|'self'
op|'.'
name|'_routes'
op|':'
newline|'\n'
indent|'            '
name|'for'
name|'f'
name|'in'
name|'self'
op|'.'
name|'_routes'
op|'['
name|'routing_key'
op|']'
op|':'
newline|'\n'
indent|'                '
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|"'Publishing to route %s'"
op|')'
op|','
name|'f'
op|')'
newline|'\n'
name|'f'
op|'('
name|'message'
op|','
name|'routing_key'
op|'='
name|'routing_key'
op|')'
newline|'\n'
nl|'\n'
DECL|member|bind
dedent|''
dedent|''
dedent|''
name|'def'
name|'bind'
op|'('
name|'self'
op|','
name|'callback'
op|','
name|'routing_key'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_routes'
op|'.'
name|'setdefault'
op|'('
name|'routing_key'
op|','
op|'['
op|']'
op|')'
newline|'\n'
name|'self'
op|'.'
name|'_routes'
op|'['
name|'routing_key'
op|']'
op|'.'
name|'append'
op|'('
name|'callback'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|Queue
dedent|''
dedent|''
name|'class'
name|'Queue'
op|'('
name|'object'
op|')'
op|':'
newline|'\n'
DECL|member|__init__
indent|'    '
name|'def'
name|'__init__'
op|'('
name|'self'
op|','
name|'name'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'name'
op|'='
name|'name'
newline|'\n'
name|'self'
op|'.'
name|'_queue'
op|'='
name|'queue'
op|'.'
name|'Queue'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|__repr__
dedent|''
name|'def'
name|'__repr__'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
string|"'<Queue: %s>'"
op|'%'
name|'self'
op|'.'
name|'name'
newline|'\n'
nl|'\n'
DECL|member|push
dedent|''
name|'def'
name|'push'
op|'('
name|'self'
op|','
name|'message'
op|','
name|'routing_key'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'_queue'
op|'.'
name|'put'
op|'('
name|'message'
op|')'
newline|'\n'
nl|'\n'
DECL|member|size
dedent|''
name|'def'
name|'size'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'self'
op|'.'
name|'_queue'
op|'.'
name|'qsize'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|pop
dedent|''
name|'def'
name|'pop'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'return'
name|'self'
op|'.'
name|'_queue'
op|'.'
name|'get'
op|'('
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|Backend
dedent|''
dedent|''
name|'class'
name|'Backend'
op|'('
name|'base'
op|'.'
name|'BaseBackend'
op|')'
op|':'
newline|'\n'
DECL|member|queue_declare
indent|'    '
name|'def'
name|'queue_declare'
op|'('
name|'self'
op|','
name|'queue'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'global'
name|'QUEUES'
newline|'\n'
name|'if'
name|'queue'
name|'not'
name|'in'
name|'QUEUES'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|"'Declaring queue %s'"
op|')'
op|','
name|'queue'
op|')'
newline|'\n'
name|'QUEUES'
op|'['
name|'queue'
op|']'
op|'='
name|'Queue'
op|'('
name|'queue'
op|')'
newline|'\n'
nl|'\n'
DECL|member|exchange_declare
dedent|''
dedent|''
name|'def'
name|'exchange_declare'
op|'('
name|'self'
op|','
name|'exchange'
op|','
name|'type'
op|','
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'global'
name|'EXCHANGES'
newline|'\n'
name|'if'
name|'exchange'
name|'not'
name|'in'
name|'EXCHANGES'
op|':'
newline|'\n'
indent|'            '
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|"'Declaring exchange %s'"
op|')'
op|','
name|'exchange'
op|')'
newline|'\n'
name|'EXCHANGES'
op|'['
name|'exchange'
op|']'
op|'='
name|'Exchange'
op|'('
name|'exchange'
op|','
name|'type'
op|')'
newline|'\n'
nl|'\n'
DECL|member|queue_bind
dedent|''
dedent|''
name|'def'
name|'queue_bind'
op|'('
name|'self'
op|','
name|'queue'
op|','
name|'exchange'
op|','
name|'routing_key'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'global'
name|'EXCHANGES'
newline|'\n'
name|'global'
name|'QUEUES'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|"'Binding %(queue)s to %(exchange)s with'"
nl|'\n'
string|"' key %(routing_key)s'"
op|')'
op|'%'
name|'locals'
op|'('
op|')'
op|')'
newline|'\n'
name|'EXCHANGES'
op|'['
name|'exchange'
op|']'
op|'.'
name|'bind'
op|'('
name|'QUEUES'
op|'['
name|'queue'
op|']'
op|'.'
name|'push'
op|','
name|'routing_key'
op|')'
newline|'\n'
nl|'\n'
DECL|member|declare_consumer
dedent|''
name|'def'
name|'declare_consumer'
op|'('
name|'self'
op|','
name|'queue'
op|','
name|'callback'
op|','
name|'consumer_tag'
op|','
op|'*'
name|'args'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'global'
name|'CONSUMERS'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
string|'"Adding consumer %s"'
op|','
name|'consumer_tag'
op|')'
newline|'\n'
name|'CONSUMERS'
op|'['
name|'consumer_tag'
op|']'
op|'='
op|'('
name|'queue'
op|','
name|'callback'
op|')'
newline|'\n'
nl|'\n'
DECL|member|cancel
dedent|''
name|'def'
name|'cancel'
op|'('
name|'self'
op|','
name|'consumer_tag'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'global'
name|'CONSUMERS'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
string|'"Removing consumer %s"'
op|','
name|'consumer_tag'
op|')'
newline|'\n'
name|'del'
name|'CONSUMERS'
op|'['
name|'consumer_tag'
op|']'
newline|'\n'
nl|'\n'
DECL|member|consume
dedent|''
name|'def'
name|'consume'
op|'('
name|'self'
op|','
name|'limit'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'global'
name|'CONSUMERS'
newline|'\n'
name|'num'
op|'='
number|'0'
newline|'\n'
name|'while'
name|'True'
op|':'
newline|'\n'
indent|'            '
name|'for'
op|'('
name|'queue'
op|','
name|'callback'
op|')'
name|'in'
name|'CONSUMERS'
op|'.'
name|'itervalues'
op|'('
op|')'
op|':'
newline|'\n'
indent|'                '
name|'item'
op|'='
name|'self'
op|'.'
name|'get'
op|'('
name|'queue'
op|')'
newline|'\n'
name|'if'
name|'item'
op|':'
newline|'\n'
indent|'                    '
name|'callback'
op|'('
name|'item'
op|')'
newline|'\n'
name|'num'
op|'+='
number|'1'
newline|'\n'
name|'yield'
newline|'\n'
name|'if'
name|'limit'
name|'and'
name|'num'
op|'=='
name|'limit'
op|':'
newline|'\n'
indent|'                        '
name|'raise'
name|'StopIteration'
op|'('
op|')'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'greenthread'
op|'.'
name|'sleep'
op|'('
number|'0.1'
op|')'
newline|'\n'
nl|'\n'
DECL|member|get
dedent|''
dedent|''
name|'def'
name|'get'
op|'('
name|'self'
op|','
name|'queue'
op|','
name|'no_ack'
op|'='
name|'False'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'global'
name|'QUEUES'
newline|'\n'
name|'if'
name|'not'
name|'queue'
name|'in'
name|'QUEUES'
name|'or'
name|'not'
name|'QUEUES'
op|'['
name|'queue'
op|']'
op|'.'
name|'size'
op|'('
op|')'
op|':'
newline|'\n'
indent|'            '
name|'return'
name|'None'
newline|'\n'
dedent|''
op|'('
name|'message_data'
op|','
name|'content_type'
op|','
name|'content_encoding'
op|')'
op|'='
name|'QUEUES'
op|'['
name|'queue'
op|']'
op|'.'
name|'pop'
op|'('
op|')'
newline|'\n'
name|'message'
op|'='
name|'Message'
op|'('
name|'backend'
op|'='
name|'self'
op|','
name|'body'
op|'='
name|'message_data'
op|','
nl|'\n'
name|'content_type'
op|'='
name|'content_type'
op|','
nl|'\n'
name|'content_encoding'
op|'='
name|'content_encoding'
op|')'
newline|'\n'
name|'message'
op|'.'
name|'result'
op|'='
name|'True'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|"'Getting from %(queue)s: %(message)s'"
op|')'
op|'%'
name|'locals'
op|'('
op|')'
op|')'
newline|'\n'
name|'return'
name|'message'
newline|'\n'
nl|'\n'
DECL|member|prepare_message
dedent|''
name|'def'
name|'prepare_message'
op|'('
name|'self'
op|','
name|'message_data'
op|','
name|'delivery_mode'
op|','
nl|'\n'
name|'content_type'
op|','
name|'content_encoding'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Prepare message for sending."""'
newline|'\n'
name|'return'
op|'('
name|'message_data'
op|','
name|'content_type'
op|','
name|'content_encoding'
op|')'
newline|'\n'
nl|'\n'
DECL|member|publish
dedent|''
name|'def'
name|'publish'
op|'('
name|'self'
op|','
name|'message'
op|','
name|'exchange'
op|','
name|'routing_key'
op|','
op|'**'
name|'kwargs'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'global'
name|'EXCHANGES'
newline|'\n'
name|'if'
name|'exchange'
name|'in'
name|'EXCHANGES'
op|':'
newline|'\n'
indent|'            '
name|'EXCHANGES'
op|'['
name|'exchange'
op|']'
op|'.'
name|'publish'
op|'('
name|'message'
op|','
name|'routing_key'
op|'='
name|'routing_key'
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|function|reset_all
dedent|''
dedent|''
dedent|''
name|'def'
name|'reset_all'
op|'('
op|')'
op|':'
newline|'\n'
indent|'    '
name|'global'
name|'EXCHANGES'
newline|'\n'
name|'global'
name|'QUEUES'
newline|'\n'
name|'global'
name|'CONSUMERS'
newline|'\n'
name|'EXCHANGES'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'QUEUES'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'CONSUMERS'
op|'='
op|'{'
op|'}'
newline|'\n'
dedent|''
endmarker|''
end_unit
