begin_unit
comment|'# vim: tabstop=4 shiftwidth=4 softtabstop=4'
nl|'\n'
nl|'\n'
comment|'# Copyright 2010 OpenStack LLC.'
nl|'\n'
comment|'# All Rights Reserved.'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Licensed under the Apache License, Version 2.0 (the "License"); you may'
nl|'\n'
comment|'#    not use this file except in compliance with the License. You may obtain'
nl|'\n'
comment|'#    a copy of the License at'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#         http://www.apache.org/licenses/LICENSE-2.0'
nl|'\n'
comment|'#'
nl|'\n'
comment|'#    Unless required by applicable law or agreed to in writing, software'
nl|'\n'
comment|'#    distributed under the License is distributed on an "AS IS" BASIS, WITHOUT'
nl|'\n'
comment|'#    WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the'
nl|'\n'
comment|'#    License for the specific language governing permissions and limitations'
nl|'\n'
comment|'#    under the License.'
nl|'\n'
string|'"""Implementation of an image service that uses Glance as the backend"""'
newline|'\n'
nl|'\n'
name|'from'
name|'__future__'
name|'import'
name|'absolute_import'
newline|'\n'
nl|'\n'
name|'from'
name|'glance'
op|'.'
name|'common'
name|'import'
name|'exception'
name|'as'
name|'glance_exception'
newline|'\n'
nl|'\n'
name|'from'
name|'nova'
name|'import'
name|'exception'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'flags'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'log'
newline|'\n'
name|'from'
name|'nova'
name|'import'
name|'utils'
newline|'\n'
name|'from'
name|'nova'
op|'.'
name|'image'
name|'import'
name|'service'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|variable|LOG
name|'LOG'
op|'='
name|'log'
op|'.'
name|'getLogger'
op|'('
string|"'nova.image.glance'"
op|')'
newline|'\n'
nl|'\n'
DECL|variable|FLAGS
name|'FLAGS'
op|'='
name|'flags'
op|'.'
name|'FLAGS'
newline|'\n'
nl|'\n'
DECL|variable|GlanceClient
name|'GlanceClient'
op|'='
name|'utils'
op|'.'
name|'import_class'
op|'('
string|"'glance.client.Client'"
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
DECL|class|GlanceImageService
name|'class'
name|'GlanceImageService'
op|'('
name|'service'
op|'.'
name|'BaseImageService'
op|')'
op|':'
newline|'\n'
indent|'    '
string|'"""Provides storage and retrieval of disk image objects within Glance."""'
newline|'\n'
DECL|variable|IMAGE_PROPERTIES
name|'IMAGE_PROPERTIES'
op|'='
op|'['
string|"'instance_id'"
op|','
string|"'os_type'"
op|']'
newline|'\n'
nl|'\n'
DECL|member|__init__
name|'def'
name|'__init__'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
name|'self'
op|'.'
name|'client'
op|'='
name|'GlanceClient'
op|'('
name|'FLAGS'
op|'.'
name|'glance_host'
op|','
name|'FLAGS'
op|'.'
name|'glance_port'
op|')'
newline|'\n'
nl|'\n'
DECL|member|index
dedent|''
name|'def'
name|'index'
op|'('
name|'self'
op|','
name|'context'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Calls out to Glance for a list of images available\n        """'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'client'
op|'.'
name|'get_images'
op|'('
op|')'
newline|'\n'
nl|'\n'
DECL|member|detail
dedent|''
name|'def'
name|'detail'
op|'('
name|'self'
op|','
name|'context'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Calls out to Glance for a list of detailed image information\n        """'
newline|'\n'
name|'image_metas'
op|'='
name|'self'
op|'.'
name|'client'
op|'.'
name|'get_images_detailed'
op|'('
op|')'
newline|'\n'
name|'return'
name|'image_metas'
newline|'\n'
name|'return'
op|'['
name|'self'
op|'.'
name|'_depropertify_metadata_from_glance'
op|'('
name|'image_meta'
op|')'
nl|'\n'
name|'for'
name|'image_meta'
name|'in'
name|'image_metas'
op|']'
newline|'\n'
nl|'\n'
DECL|member|show
dedent|''
name|'def'
name|'show'
op|'('
name|'self'
op|','
name|'context'
op|','
name|'image_id'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns a dict containing image data for the given opaque image id.\n        """'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'metadata'
op|'='
name|'self'
op|'.'
name|'client'
op|'.'
name|'get_image_meta'
op|'('
name|'image_id'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'glance_exception'
op|'.'
name|'NotFound'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'NotFound'
newline|'\n'
nl|'\n'
dedent|''
name|'meta'
op|'='
name|'self'
op|'.'
name|'_depropertify_metadata_from_glance'
op|'('
name|'metadata'
op|')'
newline|'\n'
name|'return'
name|'meta'
newline|'\n'
nl|'\n'
DECL|member|show_by_name
dedent|''
name|'def'
name|'show_by_name'
op|'('
name|'self'
op|','
name|'context'
op|','
name|'name'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Returns a dict containing image data for the given name.\n        """'
newline|'\n'
comment|'# TODO(vish): replace this with more efficient call when glance'
nl|'\n'
comment|'#             supports it.'
nl|'\n'
name|'images'
op|'='
name|'self'
op|'.'
name|'detail'
op|'('
name|'context'
op|')'
newline|'\n'
name|'image'
op|'='
name|'None'
newline|'\n'
name|'for'
name|'cantidate'
name|'in'
name|'images'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'name'
op|'=='
name|'cantidate'
op|'.'
name|'get'
op|'('
string|"'name'"
op|')'
op|':'
newline|'\n'
indent|'                '
name|'image'
op|'='
name|'cantidate'
newline|'\n'
name|'break'
newline|'\n'
dedent|''
dedent|''
name|'if'
name|'image'
name|'is'
name|'None'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'NotFound'
newline|'\n'
dedent|''
name|'return'
name|'image'
newline|'\n'
nl|'\n'
DECL|member|get
dedent|''
name|'def'
name|'get'
op|'('
name|'self'
op|','
name|'context'
op|','
name|'image_id'
op|','
name|'data'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Calls out to Glance for metadata and data and writes data.\n        """'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'metadata'
op|','
name|'image_chunks'
op|'='
name|'self'
op|'.'
name|'client'
op|'.'
name|'get_image'
op|'('
name|'image_id'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'glance_exception'
op|'.'
name|'NotFound'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'NotFound'
newline|'\n'
dedent|''
name|'for'
name|'chunk'
name|'in'
name|'image_chunks'
op|':'
newline|'\n'
indent|'            '
name|'data'
op|'.'
name|'write'
op|'('
name|'chunk'
op|')'
newline|'\n'
nl|'\n'
dedent|''
name|'meta'
op|'='
name|'self'
op|'.'
name|'_depropertify_metadata_from_glance'
op|'('
name|'metadata'
op|')'
newline|'\n'
name|'return'
name|'meta'
newline|'\n'
nl|'\n'
DECL|member|create
dedent|''
name|'def'
name|'create'
op|'('
name|'self'
op|','
name|'context'
op|','
name|'metadata'
op|','
name|'data'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Store the image data and return the new image id.\n\n        :raises AlreadyExists if the image already exist.\n\n        """'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|'"Creating image in Glance. Metdata passed in %s"'
op|')'
op|','
nl|'\n'
name|'metadata'
op|')'
newline|'\n'
nl|'\n'
name|'meta'
op|'='
name|'self'
op|'.'
name|'_propertify_metadata_for_glance'
op|'('
name|'metadata'
op|')'
newline|'\n'
name|'LOG'
op|'.'
name|'debug'
op|'('
name|'_'
op|'('
string|'"Metadata after formatting for Glance %s"'
op|')'
op|','
name|'meta'
op|')'
newline|'\n'
name|'return'
name|'self'
op|'.'
name|'client'
op|'.'
name|'add_image'
op|'('
name|'meta'
op|','
name|'data'
op|')'
newline|'\n'
nl|'\n'
DECL|member|update
dedent|''
name|'def'
name|'update'
op|'('
name|'self'
op|','
name|'context'
op|','
name|'image_id'
op|','
name|'metadata'
op|','
name|'data'
op|'='
name|'None'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Replace the contents of the given image with the new data.\n\n        :raises NotFound if the image does not exist.\n\n        """'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'result'
op|'='
name|'self'
op|'.'
name|'client'
op|'.'
name|'update_image'
op|'('
name|'image_id'
op|','
name|'metadata'
op|','
name|'data'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'glance_exception'
op|'.'
name|'NotFound'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'NotFound'
newline|'\n'
dedent|''
name|'return'
name|'result'
newline|'\n'
nl|'\n'
DECL|member|delete
dedent|''
name|'def'
name|'delete'
op|'('
name|'self'
op|','
name|'context'
op|','
name|'image_id'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Delete the given image.\n\n        :raises NotFound if the image does not exist.\n\n        """'
newline|'\n'
name|'try'
op|':'
newline|'\n'
indent|'            '
name|'result'
op|'='
name|'self'
op|'.'
name|'client'
op|'.'
name|'delete_image'
op|'('
name|'image_id'
op|')'
newline|'\n'
dedent|''
name|'except'
name|'glance_exception'
op|'.'
name|'NotFound'
op|':'
newline|'\n'
indent|'            '
name|'raise'
name|'exception'
op|'.'
name|'NotFound'
newline|'\n'
dedent|''
name|'return'
name|'result'
newline|'\n'
nl|'\n'
DECL|member|delete_all
dedent|''
name|'def'
name|'delete_all'
op|'('
name|'self'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""\n        Clears out all images\n        """'
newline|'\n'
name|'pass'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'classmethod'
newline|'\n'
DECL|member|_propertify_metadata_for_glance
name|'def'
name|'_propertify_metadata_for_glance'
op|'('
name|'cls'
op|','
name|'metadata'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Return a metadata dict suitable for passing to Glance.\n\n        The ImageService exposes metadata as a flat-dict; however, Glance\n        distinguishes between two different types of metadata:\n\n            1. First-class attributes: These are columns on the image table\n               and represent metadata that is common to all images on all IAAS\n               providers.\n\n            2. Properties: These are entries in the image_properties table and\n               represent image/IAAS-provider specific metadata.\n\n        To reconcile this difference, this function accepts a flat-dict of\n        metadata, figures out which attributes are stored as image properties\n        in Glance, and then adds those to a `properties` dict nested within\n        the metadata.\n        """'
newline|'\n'
name|'new_metadata'
op|'='
name|'metadata'
op|'.'
name|'copy'
op|'('
op|')'
newline|'\n'
name|'properties'
op|'='
op|'{'
op|'}'
newline|'\n'
name|'for'
name|'property_'
name|'in'
name|'cls'
op|'.'
name|'IMAGE_PROPERTIES'
op|':'
newline|'\n'
indent|'            '
name|'if'
name|'property_'
name|'in'
name|'new_metadata'
op|':'
newline|'\n'
indent|'                '
name|'value'
op|'='
name|'new_metadata'
op|'.'
name|'pop'
op|'('
name|'property_'
op|')'
newline|'\n'
name|'properties'
op|'['
name|'property_'
op|']'
op|'='
name|'value'
newline|'\n'
dedent|''
dedent|''
name|'new_metadata'
op|'['
string|"'properties'"
op|']'
op|'='
name|'properties'
newline|'\n'
name|'return'
name|'new_metadata'
newline|'\n'
nl|'\n'
dedent|''
op|'@'
name|'classmethod'
newline|'\n'
DECL|member|_depropertify_metadata_from_glance
name|'def'
name|'_depropertify_metadata_from_glance'
op|'('
name|'cls'
op|','
name|'metadata'
op|')'
op|':'
newline|'\n'
indent|'        '
string|'"""Return a metadata dict suitable for returning from ImageService\n        """'
newline|'\n'
name|'new_metadata'
op|'='
name|'metadata'
op|'.'
name|'copy'
op|'('
op|')'
newline|'\n'
name|'if'
string|"'properties'"
name|'in'
name|'new_metadata'
op|':'
newline|'\n'
indent|'            '
name|'properties'
op|'='
name|'new_metadata'
op|'.'
name|'pop'
op|'('
string|"'properties'"
op|')'
newline|'\n'
name|'for'
name|'property_'
name|'in'
name|'cls'
op|'.'
name|'IMAGE_PROPERTIES'
op|':'
newline|'\n'
indent|'                '
name|'if'
name|'property_'
name|'in'
name|'properties'
name|'and'
name|'property_'
name|'not'
name|'in'
name|'new_metadata'
op|':'
newline|'\n'
indent|'                    '
name|'value'
op|'='
name|'properties'
op|'['
name|'property_'
op|']'
newline|'\n'
name|'new_metadata'
op|'['
name|'property_'
op|']'
op|'='
name|'value'
newline|'\n'
dedent|''
dedent|''
dedent|''
name|'return'
name|'new_metadata'
newline|'\n'
dedent|''
dedent|''
endmarker|''
end_unit
