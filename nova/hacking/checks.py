...
op|')'
newline|'\n'
comment|'# Excludes oslo.config OptGroup objects'
nl|'\n'
DECL|variable|cfg_opt_re
name|'cfg_opt_re'
op|'='
name|'re'
op|'.'
...
op|')'
newline|'\n'
DECL|variable|doubled_words_re
name|'doubled_words_re'
op|'='
name|'re'
op|'.'
name|'compile'
op|'('
nl|'\n'
string|'r"\\b(then?|[iao]n|i[fst]|but|f?or|at|and|[dt]o)\\s+\\1\\b"'
op|')'
newline|'\n'
nl|'\n'
DECL|variable|opt_help_text_min_char_count
name|'opt_help_text_min_char_count'
op|'='
number|'10'
newline|'\n'
nl|'\n'
...
nl|'\n'
nl|'\n'
DECL|class|_FindVariableReferences
dedent|''
dedent|''
name|'class'
name|'_FindVariableReferences'
op|'('
name|'ast'
op|'.'
name|'NodeVisitor'
...
op|':'
newline|'\n'
comment|'# This means the value of a variable was loaded. For example a'
nl|'\n'
comment|"# variable 'foo' was used like:"
nl|'\n'
comment|'# mocked_thing.bar = foo'
nl|'\n'
comment|'# foo()'
nl|'\n'
comment|'# self.assertRaises(excepion, foo)'
nl|'\n'
indent|'            '
name|'self'
op|'.'
...
nl|'\n'
nl|'\n'
DECL|class|CheckForUncalledTestClosure
dedent|''
dedent|''
name|'class'
...
op|':'
newline|'\n'
comment|"# self._filename is 'stdin' in the unit test for this check."
nl|'\n'
indent|'        '
name|'if'
op|'('
...
op|']'
newline|'\n'
comment|'# Walk just the direct nodes of the test method'
nl|'\n'
name|'for'
name|'child_node'
name|'in'
...
newline|'\n'
nl|'\n'
comment|'# Walk all nodes to find references'
nl|'\n'
dedent|''
dedent|''
...
name|'node'
op|','
string|"'N349: Test closures not called: %s'"
nl|'\n'
op|'%'
string|"','"
op|'.'
...
nl|'\n'
nl|'\n'
DECL|function|check_config_option_in_central_place
dedent|''
dedent|''
name|'def'
...
op|'='
op|'('
string|'"N342: Config options should be in the central location "'
nl|'\n'
string|'"\'/nova/conf/*\'. Do not declare new config options outside "'
nl|'\n'
string|'"of that folder."'
op|')'
newline|'\n'
comment|"# That's the correct location"
nl|'\n'
name|'if'
string|'"nova/conf/"'
name|'in'
...
name|'return'
newline|'\n'
comment|'# TODO(markus_z) This is just temporary until all config options are'
nl|'\n'
comment|'# moved to the central place. To avoid that a once cleaned up place'
nl|'\n'
comment|'# introduces new config options, we do a check here. This array will'
nl|'\n'
comment|'# get quite huge over the time, but will be removed at the end of the'
nl|'\n'
comment|'# reorganization.'
nl|'\n'
comment|"# You can add the full path to a module or folder. It's just a substring"
nl|'\n'
comment|'# check, which makes it flexible enough.'
nl|'\n'
dedent|''
name|'cleaned_up'
op|'='
op|'['
string|'"nova/console/serial.py"'
op|','
nl|'\n'
...
nl|'\n'
nl|'\n'
DECL|function|check_doubled_words
dedent|''
dedent|''
name|'def'
...
newline|'\n'
indent|'    '
string|'"""Check for the common doubled-word typos\n\n    N343\n    """'
newline|'\n'
name|'msg'
op|'='
op|'('
string|'"N343: Doubled word \'%(word)s\' typo found"'
op|')'
newline|'\n'
nl|'\n'
name|'match'
...
nl|'\n'
nl|'\n'
DECL|function|check_python3_no_iteritems
dedent|''
dedent|''
name|'def'
...
op|'='
op|'('
string|'"N344: Use six.iteritems() instead of dict.iteritems()."'
op|')'
newline|'\n'
nl|'\n'
name|'if'
...
nl|'\n'
nl|'\n'
DECL|function|check_python3_no_iterkeys
dedent|''
dedent|''
name|'def'
...
op|'='
op|'('
string|'"N345: Use six.iterkeys() instead of dict.iterkeys()."'
op|')'
newline|'\n'
nl|'\n'
name|'if'
...
nl|'\n'
nl|'\n'
DECL|function|check_python3_no_itervalues
dedent|''
dedent|''
name|'def'
...
op|'='
op|'('
string|'"N346: Use six.itervalues() instead of dict.itervalues()."'
op|')'
newline|'\n'
nl|'\n'
name|'if'
...
nl|'\n'
nl|'\n'
DECL|function|cfg_help_with_enough_text
dedent|''
dedent|''
name|'def'
...
op|':'
newline|'\n'
comment|'# TODO(markus_z): The count of 10 chars is the *highest* number I could'
nl|'\n'
comment|'# use to introduce this new check without breaking the gate. IOW, if I'
nl|'\n'
comment|'# use a value of 15 for example, the gate checks will fail because we have'
nl|'\n'
comment|'# a few config options which use fewer chars than 15 to explain their'
nl|'\n'
comment|'# usage (for example the options "ca_file" and "cert").'
nl|'\n'
comment|'# As soon as the implementation of bp centralize-config-options is'
nl|'\n'
comment|'# finished, I wanted to increase that magic number to a higher (to be'
nl|'\n'
comment|'# defined) value.'
nl|'\n'
comment|'# This check is an attempt to programmatically check a part of the review'
nl|'\n'
comment|'#  guidelines http://docs.openstack.org/developer/nova/code-review.html'
nl|'\n'
nl|'\n'
indent|'    '
name|'msg'
op|'='
op|'('
string|'"N347: A config option is a public interface to the cloud admins "'
nl|'\n'
string|'"and should be properly documented. A part of that is to provide "'
nl|'\n'
string|'"enough help text to describe this option. Use at least %s chars "'
nl|'\n'
string|'"for that description. Is is likely that this minimum will be "'
nl|'\n'
string|'"increased in the future."'
op|'%'
name|'opt_help_text_min_char_count'
op|')'
newline|'\n'
...
newline|'\n'
nl|'\n'
comment|'# ignore DeprecatedOpt objects. They get mentioned in the release notes'
nl|'\n'
comment|"# and don't need a lengthy help text anymore"
nl|'\n'
dedent|''
name|'if'
string|'"DeprecatedOpt"'
name|'in'
name|'logical_line'
op|':'
newline|'\n'
...
newline|'\n'
nl|'\n'
DECL|function|get_token_values
dedent|''
name|'def'
name|'get_token_values'
op|'('
name|'start_index'
op|','
name|'length'
...
newline|'\n'
nl|'\n'
DECL|function|get_help_token_index
dedent|''
name|'def'
name|'get_help_token_index'
op|'('
op|')'
op|':'
newline|'\n'
...
newline|'\n'
nl|'\n'
DECL|function|get_trimmed_help_text
dedent|''
name|'def'
name|'get_trimmed_help_text'
op|'('
name|'t'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
DECL|function|has_enough_help_text
dedent|''
name|'def'
name|'has_enough_help_text'
op|'('
name|'txt'
op|')'
op|':'
...
number|'0'
op|','
string|"'N348 Deprecated library function os.popen(). '"
nl|'\n'
string|"'Replace it using subprocess module. '"
op|')'
newline|'\n'
nl|'\n'
nl|'\n'
