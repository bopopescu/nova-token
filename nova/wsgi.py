...
op|':'
newline|'\n'
comment|'# single process mode, useful for profiling, test, debug etc.'
nl|'\n'
indent|'            '
name|'self'
op|'.'
...
newline|'\n'
nl|'\n'
comment|"# master_worker doesn't actually do work (i.e. handle API request)"
nl|'\n'
comment|"# but it's a managing process to handle signal/termination for"
nl|'\n'
comment|'# this type of API service, only needed if workers > 1'
nl|'\n'
dedent|''
name|'self'
op|'.'
...
name|'_'
op|'('
string|'"Started %(name)s on %(host)s:%(port)s in process"'
op|')'
nl|'\n'
op|'%'
name|'self'
...
newline|'\n'
nl|'\n'
DECL|member|run_server_in_process
dedent|''
name|'def'
...
newline|'\n'
indent|'        '
string|'"""Start workers and wait for them to join"""'
newline|'\n'
...
newline|'\n'
comment|'#TODO(zhiteng) Gracefully kill all eventlet greenthread'
nl|'\n'
name|'self'
op|'.'
name|'_logger'
...
name|'_'
op|'('
string|"'SIGTERM or SIGINT received'"
op|')'
op|')'
newline|'\n'
name|'signal'
...
name|'_'
op|'('
string|'"Started %(children_count)d worker for %(name)s"'
op|')'
nl|'\n'
op|'%'
op|'{'
...
op|')'
newline|'\n'
comment|'# to avoid race condition that child receive signal before'
nl|'\n'
comment|'# parent and is respawned'
nl|'\n'
name|'signal'
op|'.'
name|'signal'
...
name|'_'
op|'('
string|"'[%(name)s] Started worker (pid: %(pid)s)'"
op|')'
op|'%'
nl|'\n'
op|'{'
...
name|'_'
op|'('
string|"'Caught keyboard interrupt. Exiting.'"
op|')'
op|')'
newline|'\n'
name|'self'
...
op|':'
newline|'\n'
comment|'# set running state to false and kill all workers'
nl|'\n'
indent|'            '
name|'self'
op|'.'
...
op|')'
newline|'\n'
comment|'# now terminate master_worker'
nl|'\n'
dedent|''
name|'if'
name|'self'
...
op|':'
newline|'\n'
comment|'# Resize Pool to stop accepting new connection'
nl|'\n'
indent|'            '
name|'self'
op|'.'
...
op|':'
newline|'\n'
comment|'# for services enabled multi-process,a separate master_worker'
nl|'\n'
comment|'# is already waiting'
nl|'\n'
indent|'                '
name|'pass'
newline|'\n'
