...
nl|'\n'
nl|'\n'
DECL|member|test_systemCallUninterruptedByChildExit
dedent|''
name|'def'
name|'test_systemCallUninterruptedByChildExit'
op|'('
name|'self'
op|')'
op|':'
...
newline|'\n'
nl|'\n'
comment|'# XXX Since pygobject/pygtk wants to use signal.set_wakeup_fd,'
nl|'\n'
comment|"# we aren't actually providing this functionality on the glib2"
nl|'\n'
comment|'# or gtk2 reactors yet.  See #4286 for the possibility of'
nl|'\n'
comment|'# improving this.'
nl|'\n'
...
op|'('
nl|'\n'
string|'"%s is not supported without siginterrupt"'
op|'%'
name|'reactorClassName'
op|')'
newline|'\n'
...
name|'popen'
op|'('
string|'\'%s -c "import time; time.sleep(0.1)"\''
op|'%'
nl|'\n'
op|'('
name|'sys'
...
name|'popen'
op|'('
string|'\'%s -c "import time; time.sleep(0.5); print \\\'Foo\\\'"\''
op|'%'
nl|'\n'
op|'('
name|'sys'
...
op|')'
newline|'\n'
comment|'# The read call below will blow up with an EINTR from the'
nl|'\n'
comment|'# SIGCHLD from the first process exiting if we install a'
nl|'\n'
comment|'# SIGCHLD handler without SA_RESTART.  (which we used to do)'
nl|'\n'
name|'result'
op|'.'
name|'append'
